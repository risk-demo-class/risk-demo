...
按顺序执行所有 SQL 脚本: 创建数据库 -> 业务表 -> 业务数据 -> 风控表 -> 风控规则 -> ENUM迁移
...
# Windows: fix aiomysql "Event loop is closed"
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_edu_business_tables.sql", "创建 7 张教育业务表"),
    ("init_edu_business_data.sql", "导入教育业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("migration_edu.sql", "扩展风险表 ENUM 到教育领域 + 同步初始黑名单"),
    ("init_edu_risk_data.sql", "导入 8 条预置教育风控规则"),
]
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 7 张业务表 + 9 张风控表 + 8 条教育风控规则全部就绪")
    else:
