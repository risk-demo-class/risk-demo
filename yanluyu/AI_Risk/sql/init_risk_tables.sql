...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('订单欺诈','支付风险','账户风险','售后滥用','地址风险','物流风险','报名欺诈','退费滥用','身份异常','行为异常') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('下单','支付','售后申请','物流投诉','报名','退费申请','打赏','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('下单','支付','售后申请','物流投诉','报名','退费申请','打赏') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','订单','地址','课程','设备') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('下单','支付','售后申请','物流投诉','报名','退费申请','打赏') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','地址','手机号','学号','身份证号','设备指纹') NOT NULL COMMENT '黑名单类型',
