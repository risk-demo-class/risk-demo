...
class TestRiskCheckRequest:
    ...
    def test_all_3_event_types(self):
        """3 个合法 event_type 都能过"""
        for et in ["报名", "退费申请", "打赏"]:
    ...
    def test_missing_required_field(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RiskCheckRequest(event_type="报名", user_id="u")  # 没传 source_id
...
class TestRuleCreate:
    ...
    def test_rule_score_out_of_range(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RuleCreate(
                rule_id="R1", rule_name="t", rule_category="报名欺诈", event_type="通用",
