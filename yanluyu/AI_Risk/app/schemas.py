...
class RiskCheckRequest(BaseModel):
    """风险检查请求"""
    event_type: Literal["报名", "退费申请", "打赏"]
    source_id: str = Field(description="关联业务ID (order_id / refund_id / donation_id 等)")
    user_id: str
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["订单欺诈", "支付风险", "账户风险", "售后滥用", "地址风险", "物流风险", "报名欺诈", "退费滥用", "身份异常", "行为异常"]
    event_type: Literal["下单", "支付", "售后申请", "物流投诉", "报名", "退费申请", "打赏", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["订单欺诈", "支付风险", "账户风险", "售后滥用", "地址风险", "物流风险", "报名欺诈", "退费滥用", "身份异常", "行为异常"]] = None
    event_type: Optional[Literal["下单", "支付", "售后申请", "物流投诉", "报名", "退费申请", "打赏", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["用户", "地址", "手机号", "学号", "身份证号", "设备指纹"]
    blacklist_value: str
