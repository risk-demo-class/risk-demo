...
# Education biz query functions
async def _biz_user_courses(db, user_id, order_id, limit):
    stmt = select(Course.course_id, Course.name, Course.category, Course.price, Course.teacher_id).select_from(Course).join(OrderInfo, Course.course_id == OrderInfo.course_id).where(OrderInfo.user_id == user_id).distinct().limit(limit)
    return list((await db.execute(stmt)).all())


async def _biz_user_refunds(db, user_id, order_id, limit):
    from app.models import RefundRequest as RR
    stmt = select(RR.refund_id, RR.order_id, RR.reason, RR.study_minutes_before_refund, RR.refund_amount, RR.refund_status, RR.create_time).where(RR.user_id == user_id).order_by(RR.create_time.desc()).limit(limit)
    return list((await db.execute(stmt)).all())


async def _biz_course_detail(db, user_id, order_id, limit):
    stmt = select(Course.course_id, Course.name, Course.category, Course.price, Course.teacher_id, Course.total_hours, Course.target_audience, Course.is_live).where(Course.course_id == order_id).limit(limit)
    return list((await db.execute(stmt)).all())
...
_BIZ_QUERY_HANDLERS = {
    ...
    "user_courses": _biz_user_courses,
    "user_refunds": _biz_user_refunds,
    "course_detail": _biz_course_detail,
