...
class TestCaseDeduplication:
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="大额转账", source_id="TX001", user_id="1001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="大额转账", source_id="TX002", user_id="1001",
        )
        ...
        assert db.added[0].source_id == "TX002"
        assert db.added[0].event_type == "大额转账"

    @pytest.mark.asyncio
    async def test_existing_rejected_case_skips_new_one(self):
        """已有自动拒绝的终态 case → 仍然 skip，防止重试重复建案。"""
        from app.engine import decision
        from app.schemas import RiskCheckRequest
        from types import SimpleNamespace

        request = RiskCheckRequest(
            event_type="大额转账", source_id="TX003", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "交易反欺诈", "极高", 95, "拒绝"))

        class _FakeResult:
            def scalar_one_or_none(self):
                return "cas_rejected_xxx"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="大额转账", source_id="TX004", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "交易反欺诈", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        ...
        request = RiskCheckRequest(
            event_type="大额转账", source_id="TX005", user_id="U001",
        )
