...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    """
    # 银行版：source_id 统一承载 txn_id / loan_id / login_id
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    ...
    # 银行信用卡申请可通过 event_data 传入申请额度，兼容无独立申请流水号的场景。
    event_data = ctx.request.event_data or {}
    if isinstance(event_data.get("amount"), (int, float)):
        features["order_total_amount"] = float(event_data["amount"])
    return features
...
async def _maybe_create_case(
    ...
    # 幂等: 同一业务事件只能对应一个案件。
    # 自动拒绝案件会直接进入“已拒绝”终态；若只查未结案，前端重试会反复
    # 创建终态案件，导致同一笔交易/申请在案件工作台中重复出现。
    ...
    if existing:
        logger.info(
            "跳过建案: 已有案件 case=%s (source_id=%s, event_type=%s)",
