...
async def _stats_trend_7d(db: AsyncSession) -> list[dict]:
    """最近 7 个自然日的评估趋势，缺少数据的日期补 0，保证图表始终有 7 个点。"""
    ...
    rows = (await db.execute(
        ...
        .where(func.date(RiskAssessment.create_time).between(start_date, end_date))
        .group_by(func.date(RiskAssessment.create_time))
    )).all()
    by_date = {
        r.dt: {"count": int(r.cnt), "high_risk_count": int(r.high_cnt or 0)}
        for r in rows
    }
    return [
        {
            "date": str(start_date + timedelta(days=offset)),
            "count": by_date.get(start_date + timedelta(days=offset), {}).get("count", 0),
            "high_risk_count": by_date.get(start_date + timedelta(days=offset), {}).get("high_risk_count", 0),
        }
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (用户ID, 如 '1001')、event_type (事件类型, 可选值: 信用卡申请/贷款申请/大额转账/异常登录)、source_id (关联业务ID, 如订单号)。"
