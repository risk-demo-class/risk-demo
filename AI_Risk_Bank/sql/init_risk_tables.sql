...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('账户安全','交易反欺诈','信贷风险','设备风险','渠道风险','黑名单') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('信用卡申请','贷款申请','大额转账','异常登录','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('信用卡申请','贷款申请','大额转账','异常登录') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('信用卡申请','贷款申请','大额转账','异常登录') DEFAULT NULL COMMENT '触发案件的事件类型',
    ...
    UNIQUE INDEX `uq_risk_case_source_event` (`source_id`, `event_type`)
