...
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
-- 7.1 案件幂等唯一约束: 同一业务事件只能创建一个案件。
-- 若老库已有重复数据，请先人工确认保留案件后再执行本迁移，避免静默删除审计记录。
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
     WHERE TABLE_SCHEMA = 'ecs' AND TABLE_NAME = 'risk_case' AND INDEX_NAME = 'uq_risk_case_source_event') = 0,
    'ALTER TABLE risk_case ADD UNIQUE INDEX uq_risk_case_source_event (source_id, event_type)',
    'SELECT 1'
));
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
