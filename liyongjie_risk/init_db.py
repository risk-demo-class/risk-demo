"""
银行风控系统 - 一键数据库初始化脚本 (异步)
按顺序执行所有 SQL 脚本: 创建数据库 → 建表 → 灌规则数据

总表数: 12 张 (覆盖登录/转账/贷款/信用卡四大场景)
预置规则: 15 条 (R001-R035, PRD 第 6 节)
"""
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_bank_risk_tables.sql", "创建 12 张银行风控表"),
    ("init_risk_rules.sql",     "导入 15 条预置风控规则 (R001-R035)"),
]
    ...
    async with conn.cursor() as cur:
        for stmt in statements:
            try:
                ...
                code = e.args[0]
                # 1051 = Unknown table (DROP TABLE IF EXISTS 的正常提示)
                # 1062 = Duplicate entry (数据已存在, 重复跑 init_db.py 时正常)
                # 1050 = Table already exists (CREATE TABLE IF NOT EXISTS)
                if code in (1050, 1051, 1062):
...
async def main():
    parser = argparse.ArgumentParser(
        description="银行风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
用法:
  python init_db.py                           # 默认: --reset (删库重建, 适合教学/演示)
  python init_db.py --keep-data              # 保留: 只补表结构 (数据不重置)
  python init_db.py --reset --yes            # 重置 + 自动确认 (CI/脚本用)
  python init_db.py --db bank_risk_test      # 初始化测试库
        """,
    )
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 12 张表重建 + 15 条规则全部就绪")
    else:
