...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('退费滥用','账户风险','设备风险','认证风险','交易异常','学习异常') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('课程报名','退费申请','学历认证','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('课程报名','退费申请','学历认证') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','订单','设备') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `assignee_id` BIGINT DEFAULT NULL COMMENT '当前审核员用户ID',
    `assign_time` DATETIME DEFAULT NULL COMMENT '最近分派时间',
    ...
    `source_id` VARCHAR(50) DEFAULT NULL COMMENT '原始业务ID(订单/退费/认证ID), 重做检查时用',
    `event_type` ENUM('课程报名','退费申请','学历认证') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','学号','身份证','设备指纹') NOT NULL COMMENT '黑名单类型',
...
CREATE TABLE IF NOT EXISTS `risk_user_profile` (
    ...
    `address_count` INT DEFAULT 0 COMMENT '关联设备数(兼容字段名)',
    `complaint_count` INT DEFAULT 0 COMMENT '认证失败次数(兼容字段名)',
...
CREATE TABLE IF NOT EXISTS `risk_action_log` (
    ...
    `action_type` ENUM('CREATE_RULE','UPDATE_RULE','TOGGLE_RULE','DELETE_RULE','REVIEW_CASE','AUTO_REJECT_CASE','AUTO_CLOSE_CASE','ADD_BLACKLIST','REMOVE_BLACKLIST','CREATE_USER','UPDATE_USER','TOGGLE_USER','RESET_PASSWORD','ASSIGN_CASE') NOT NULL COMMENT '操作类型',
