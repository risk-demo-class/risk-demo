...
def _check_env_file() -> tuple[str, str]:
    ...
    if not os.path.exists(".env"):
        return "WARN", (
            ".env 不存在, 请复制 .env.example 为 .env，"
            "并填写本机 MySQL 配置；AI 对话需要时再填写 LLM_API_KEY"
        )
    return "OK", ".env 存在"
...
def _check_mysql_alive() -> tuple[str, str]:
    ...
    try:
        ...
        return "WARN", (
            f"MySQL 连不上 ({settings.DB_HOST}:{settings.DB_PORT} "
            f"{settings.DB_USER}@{settings.DB_NAME}): {type(e).__name__}\n"
            "         请检查 MySQL 服务、账号密码、端口和数据库名称"
        )
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT COUNT(*) FROM sys_user WHERE is_active = 1")
                user_count = cur.fetchone()[0]
            return "OK", f"数据库已初始化, risk_rule {rule_count} 条 / 可登录用户 {user_count} 个"
        finally:
            conn.close()
    except Exception as e:
        err = str(e)
...
def _check_port_available(port: int = 8000) -> tuple[str, str]:
    ...
    try:
        ...
        return "FAIL", (
            f"端口 {port} 已被占用，已取消重复启动。"
            "请先在旧服务终端按 Ctrl+C，再重新运行 python run_app.py"
        )
    finally:
        sock.close()
...
def _check_xgb_model() -> tuple[str, str]:
    ...
    try:
        ...
        return "WARN", (
            ...
            "业务仍能跑（纯规则模式），重新训练教育模型请运行: "
            "python scripts/train_education_model.py"
        )
    except Exception as e:
