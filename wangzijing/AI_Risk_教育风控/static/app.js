...
function shortId(value, max = 18) {
    ...
    const left = Math.max(6, Math.ceil((max - 1) / 2));
    const right = Math.max(4, Math.floor((max - 1) / 2));
    return `${escapeHtml(text.slice(0, left))}…${escapeHtml(text.slice(-right))}`;
}

function riskBadgeHtml(level) {
    return `<span class="badge ${getRiskBadgeClass(level)}">${escapeHtml(level || '-')}</span>`;
}

function decisionBadgeHtml(decision) {
    const cls = {
        '通过': 'bg-success', '标记': 'bg-info text-dark',
        '人工审核': 'bg-warning text-dark', '拒绝': 'bg-danger',
    }[decision] || 'bg-secondary';
    return `<span class="badge ${cls}">${escapeHtml(decision || '-')}</span>`;
}

function tableStateHtml(colspan, state = 'loading', message = '') {
    const config = {
        loading: ['spinner-border spinner-border-sm', message || '正在加载数据…'],
        empty: ['bi bi-inbox', message || '暂无符合条件的数据'],
        error: ['bi bi-wifi-off', message || '数据加载失败，请稍后重试'],
    }[state] || ['bi bi-info-circle', message || '暂无数据'];
    const icon = state === 'loading'
        ? `<span class="${config[0]}" aria-hidden="true"></span>`
        : `<i class="${config[0]}" aria-hidden="true"></i>`;
    return `<tr class="table-state"><td colspan="${Number(colspan)}"><div class="table-state-box">${icon}<span>${escapeHtml(config[1])}</span></div></td></tr>`;
}
...
function setButtonBusy(button, busy, busyText = '处理中…') {
    ...
    if (busy) {
        ...
        button.innerHTML = `<span class="spinner-border spinner-border-sm" aria-hidden="true"></span> ${escapeHtml(busyText)}`;
    } else {
...
function showToast(message, type = 'success') {
    ...
    const palette = {
        success: ['bi-check-circle-fill', '#e8f7ed', '#236b3d'],
        danger: ['bi-exclamation-octagon-fill', '#ffeaea', '#963535'],
        warning: ['bi-exclamation-triangle-fill', '#fff4dc', '#8a600f'],
        info: ['bi-info-circle-fill', '#e8f5f6', '#246e74'],
    }[type] || ['bi-info-circle-fill', '#eef2f1', '#4e5b57'];
    const toast = document.createElement('div');
    toast.className = 'toast risk-toast';
    toast.setAttribute('role', 'status');
    toast.style.background = palette[1];
    toast.style.color = palette[2];
    toast.innerHTML = `<div class="toast-body"><i class="bi ${palette[0]}"></i><span class="flex-grow-1">${escapeHtml(message)}</span><button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="关闭"></button></div>`;
...
function renderMLScoreBlock(mlScore, mlDecision) {
    ...
    const decision = mlDecision
        ? `<span class="badge bg-info text-dark">${escapeHtml(mlDecision)}</span>`
