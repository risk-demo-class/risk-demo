"""教育风控系统的一键数据库初始化脚本（异步）。

执行顺序：创建数据库 → 七张教育业务表 → 业务基础数据 →
九张风控核心表 → 八条教育行业规则 → 后台用户与两角色。
"""
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 7 张教育业务表"),
    ("init_business_data.sql", "导入 593 条教育业务基础数据"),
    ("init_risk_tables.sql", "创建 9 张风控核心表"),
    ("init_risk_data.sql", "导入 EDU001-EDU008 教育行业规则"),
    ("init_auth.sql", "创建后台用户表并初始化两角色演示账号"),
]
...
用法:
  ...
  python scripts/init_db.py --db ai_risk_education_test  # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 7 张业务表 + 9 张风控表 + 1 张用户表 + 593 条业务数据 + 8 条教育规则已就绪")
    else:
