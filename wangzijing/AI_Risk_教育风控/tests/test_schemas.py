...
class TestRiskCheckRequest:
    ...
    def test_valid_request(self):
        ...
        assert req.event_type == "课程报名"
        assert req.user_id == "EDU001"
        # 可选字段默认 None
        assert req.event_data is None

    def test_all_3_education_event_types(self):
        """3 个教育行业 event_type 都能通过。"""
        for et in ["课程报名", "退费申请", "学历认证"]:
    ...
    def test_missing_required_field(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RiskCheckRequest(event_type="课程报名", user_id="u")  # 没传 source_id
...
class TestRuleCreate:
    ...
    def test_rule_score_out_of_range(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RuleCreate(
                rule_id="R1", rule_name="t", rule_category="交易异常", event_type="通用",
