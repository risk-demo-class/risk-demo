...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("退费滥用", "账户风险", "设备风险", "认证风险", "交易异常", "学习异常",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("课程报名", "退费申请", "学历认证", "通用", name="rule_event_type_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("课程报名", "退费申请", "学历认证", name="event_type_enum"),
...
class RiskCase(Base):
    ...
    assignee_id: Mapped[Optional[int]] = mapped_column(BigInteger, nullable=True, comment="当前审核员用户ID")
    assign_time: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, comment="最近分派时间")
    ...
    # --- 业务回溯字段 (重做检查时使用) ---
    source_id: Mapped[Optional[str]] = mapped_column(String(50), comment="原始业务ID(订单/退费/认证ID)")
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("课程报名", "退费申请", "学历认证", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
    ...
    __table_args__ = (
        ...
        Index("idx_risk_case_assignee_id", "assignee_id"),
    )
...
class RiskBlacklist(Base):
    ...
    blacklist_id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True,
        autoincrement=True, comment="黑名单ID",
    )
    blacklist_type: Mapped[str] = mapped_column(
        Enum("用户", "学号", "身份证", "设备指纹", name="blacklist_type_enum"),
...
class RiskUserProfile(Base):
    ...
    address_count: Mapped[int] = mapped_column(Integer, default=0, comment="关联设备数(兼容字段名)")
    complaint_count: Mapped[int] = mapped_column(Integer, default=0, comment="认证失败次数(兼容字段名)")
...
class RiskActionLog(Base):
    ...
    log_id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True,
        autoincrement=True, comment="日志ID",
    )
    ...
    action_type: Mapped[str] = mapped_column(
        Enum(
            ...
            "ADD_BLACKLIST", "REMOVE_BLACKLIST", "CREATE_USER", "UPDATE_USER", "TOGGLE_USER",
            "RESET_PASSWORD", "ASSIGN_CASE",
...
class RiskAlert(Base):
    ...
    alert_id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True,
        autoincrement=True, comment="告警ID",
    )
