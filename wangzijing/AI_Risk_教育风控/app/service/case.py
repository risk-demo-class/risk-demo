...
from app.models import RiskAssessment, RiskBlacklist, RiskCase, RiskEvent, RiskUserProfile, SysUser
...
async def add_blacklist(
    ...
    if existing:
        ...
        await record_action(
            db, operator=operator, action_type="ADD_BLACKLIST",
...
async def remove_blacklist(db: AsyncSession, blacklist_id: int, operator: str = "admin") -> bool:
    bl = (await db.execute(
        select(RiskBlacklist).where(
    ...
    await record_action(
        db, operator=operator, action_type="REMOVE_BLACKLIST",
        target_type="blacklist", target_id=bl.blacklist_id,
        before_value={
            "blacklist_type": bl.blacklist_type,
            "blacklist_value": bl.blacklist_value,
...
async def get_case_list(
    ...
    active_only=True: 只查未结案 (待审核 + 审核中), 用于案件工作台默认视图.
    history_only=True: 只查已结案 (已通过/已拒绝/已关闭), 用于个人审核记录页.
    ...
    if status:
        ...
        filters.append(RiskCase.case_status.in_(("已通过", "已拒绝", "已关闭")))
    elif active_only:
        filters.append(RiskCase.case_status.in_(("待审核", "审核中")))
    if category:
        filters.append(RiskCase.case_category == category)
    if assignee_id is not None:
        filters.append(RiskCase.assignee_id == assignee_id)
    ...
    paged_stmt = (
        select(RiskCase, RiskAssessment.final_score, RiskAssessment.risk_level, SysUser.display_name)
        .outerjoin(RiskAssessment, RiskCase.assessment_id == RiskAssessment.assessment_id)
        .outerjoin(SysUser, RiskCase.assignee_id == SysUser.user_id)
        .where(*filters)
    ...
    items = [
        CaseItem(
            ...
            assignee_id=getattr(r[0], "assignee_id", None),
            assignee_name=getattr(r, "display_name", None),
            assign_time=getattr(r[0], "assign_time", None),
        )
        for r in rows
    ]
...
async def get_case_detail(db: AsyncSession, case_id: str) -> CaseDetailResponse | None:
    row = (await db.execute(
        select(RiskCase, SysUser.display_name)
        .outerjoin(SysUser, RiskCase.assignee_id == SysUser.user_id)
        .where(RiskCase.case_id == case_id)
    )).one_or_none()
    case = row[0] if row else None
    if not case:
        return None
    ...
    return CaseDetailResponse(
        ...
        assignee_name=row.display_name if row else None,
        assign_time=case.assign_time,
    )
...
async def assign_case(
    ...
    if not case:
        return None
    if case.case_status in {"已通过", "已拒绝", "已关闭"}:
        raise HTTPException(status_code=400, detail="已结案案件不能重新分配")
    reviewer = (await db.execute(
        select(SysUser).where(
            SysUser.user_id == request.assignee_id,
            SysUser.role == "REVIEWER",
            SysUser.is_active == 1,
        )
    )).scalar_one_or_none()
    if not reviewer:
        raise HTTPException(status_code=400, detail="请选择一名启用中的审核员")
    previous = case.assignee_id
    previous_status = case.case_status
    case.assignee_id = reviewer.user_id
    case.assign_time = datetime.now()
    if case.case_status == "待审核":
        case.case_status = "审核中"
    await record_action(
        db, operator=operator, action_type="ASSIGN_CASE", target_type="case", target_id=case.case_id,
        before_value={"assignee_id": previous, "case_status": previous_status},
        after_value={
            "assignee_id": reviewer.user_id,
            "assignee": reviewer.username,
            "case_status": case.case_status,
        },
        remark="案件分配" if previous is None else "案件重新分配",
    )
    await db.commit()
...
async def get_case_statistics(db: AsyncSession, assignee_id: int | None = None) -> CaseStatistics:
    assignee_filter = RiskCase.assignee_id == assignee_id if assignee_id is not None else None
    total_stmt = select(func.count(RiskCase.case_id))
    if assignee_filter is not None:
        total_stmt = total_stmt.where(assignee_filter)
    total = (await db.execute(total_stmt)).scalar() or 0

    status_stmt = select(RiskCase.case_status, func.count(RiskCase.case_id))
    if assignee_filter is not None:
        status_stmt = status_stmt.where(assignee_filter)
    status_rows = (await db.execute(status_stmt.group_by(RiskCase.case_status))).all()
    status_map = {row[0]: row[1] for row in status_rows}

    category_stmt = select(RiskCase.case_category, func.count(RiskCase.case_id))
    if assignee_filter is not None:
        category_stmt = category_stmt.where(assignee_filter)
    category_rows = (await db.execute(category_stmt.group_by(RiskCase.case_category))).all()
...
async def list_assessments(
    ...
    筛选维度 (可选):
      ...
      - risk_level: 低 / 中 / 高 / 极高
      - event_type: 课程报名 / 退费申请 / 学历认证 (JOIN risk_event)
      - user_id: 精确匹配
