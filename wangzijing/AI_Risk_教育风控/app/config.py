...
class Settings(BaseSettings):
    ...
    DB_USER: str = "root"
    DB_PASSWORD: str = ""
    DB_NAME: str = "ai_risk_education"
    TEST_DB_NAME: str = "ai_risk_education_test"

    # ---- 登录与会话配置 ----
    # 正式部署时必须在 .env 中改成长度足够的随机字符串。
    AUTH_SECRET_KEY: str = "edu-risk-dev-secret-change-me"
    AUTH_COOKIE_NAME: str = "edu_guard_session"
    AUTH_SESSION_HOURS: int = 8
    AUTH_COOKIE_SECURE: bool = False

    # ---- 教育行业业务枚举 ----
    EDUCATION_EVENT_TYPES: tuple[str, ...] = ("课程报名", "退费申请", "学历认证")
    EDUCATION_BLACKLIST_TYPES: tuple[str, ...] = ("用户", "学号", "身份证", "设备指纹")
    ...
    # 教育版模型由 scripts/train_education_model.py 生成；文件缺失时仍自动降级为纯规则。
