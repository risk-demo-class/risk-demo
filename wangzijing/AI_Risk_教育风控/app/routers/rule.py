...
from app.auth import AuthUser, get_current_user, operator_name
...
async def api_create_rule(
    ...
    db.add(rule)
    await record_action(
        db, operator=operator_name(current_user), action_type="CREATE_RULE",
...
async def api_update_rule(
    ...
    await record_action(
        db, operator=operator_name(current_user), action_type="UPDATE_RULE",
        target_type="rule", target_id=rule.rule_id,
...
async def api_toggle_rule(
    ...
    await record_action(
        db, operator=operator_name(current_user), action_type="TOGGLE_RULE",
        target_type="rule", target_id=rule.rule_id,
...
async def api_delete_rule(
    ...
    if not rule:
        raise HTTPException(status_code=404, detail="规则不存在")
    await record_action(
        db, operator=operator_name(current_user), action_type="DELETE_RULE",
        target_type="rule", target_id=rule.rule_id,
