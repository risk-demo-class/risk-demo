...
async def api_add_blacklist(
    ...
    result = await add_blacklist(db, data, operator=operator_name(current_user))
...
async def api_remove_blacklist(
    ...
    if not await remove_blacklist(db, blacklist_id, operator=operator_name(current_user)):
