...
from app.auth import AuthUser, get_current_user, operator_name, require_admin
from app.schemas import CaseAssignRequest, CaseDetailResponse, CaseListResponse, CaseReviewRequest, CaseStatistics
from app.service.case import assign_case, get_case_detail, get_case_list, get_case_statistics, review_case
...
async def api_list_cases(
    ...
    history_only: bool = Query(False, description="只看已通过、已拒绝和已关闭案件"),
    db: AsyncSession = Depends(get_db_async),
    current_user: AuthUser = Depends(get_current_user),
):
    return await get_case_list(
        db, status=status, category=category, page=page, page_size=page_size,
        active_only=active_only, history_only=history_only,
        assignee_id=None if current_user.is_admin else current_user.user_id,
    )


@case_router.get("/statistics", response_model=CaseStatistics)
async def api_case_statistics(
    db: AsyncSession = Depends(get_db_async), current_user: AuthUser = Depends(get_current_user),
):
    return await get_case_statistics(
        db, assignee_id=None if current_user.is_admin else current_user.user_id,
    )


@case_router.get("/{case_id}", response_model=CaseDetailResponse)
async def api_get_case(
    case_id: str, db: AsyncSession = Depends(get_db_async), current_user: AuthUser = Depends(get_current_user),
):
    detail = await get_case_detail(db, case_id)
    if not detail:
        raise HTTPException(status_code=404, detail="案件不存在")
    if not current_user.is_admin and detail.assignee_id != current_user.user_id:
        raise HTTPException(status_code=403, detail="只能查看分配给自己的案件")
    return detail


@case_router.post("/{case_id}/assign", response_model=CaseDetailResponse)
async def api_assign_case(
    ...
    result = await assign_case(db, case_id, data, operator_name(current_user))
    if not result:
        raise HTTPException(status_code=404, detail="案件不存在")
    return result
...
async def api_review_case(
    ...
    if not current_user.is_admin and data.add_to_blacklist:
        raise HTTPException(status_code=403, detail="审核员可以审核案件，但不能修改黑名单")
    detail = await get_case_detail(db, case_id)
    if not detail:
        raise HTTPException(status_code=404, detail="案件不存在")
    if not current_user.is_admin and detail.assignee_id != current_user.user_id:
        raise HTTPException(status_code=403, detail="只能审核分配给自己的案件")
    data = data.model_copy(update={"reviewer": operator_name(current_user)})
