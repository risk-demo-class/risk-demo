...
from app.engine.feature import EducationEventContext, compute_all_features, resolve_event_context
...
def check_veto(hits: list[RuleHitResult]) -> bool:
    ...
    教育版极高规则为 EDU003（极高退费率）、EDU004（同设备多账号）和
    EDU008（超高金额报名）。
    """
    return any(h.risk_level == "极高" for h in hits)


_ACTION_RANK = {"通过": 0, "标记": 1, "人工审核": 2, "拒绝": 3}
_ACTION_SCORE_FLOOR = {
    "通过": 0,
    "标记": settings.RISK_PASS_THRESHOLD,
    "人工审核": settings.RISK_MARK_THRESHOLD,
    "拒绝": settings.RISK_REVIEW_THRESHOLD,
}


def _strongest_rule_action(hits: list[RuleHitResult]) -> str:
    """返回规则要求的最严格动作；没有命中时返回通过。"""
    return max((hit.action for hit in hits), key=lambda action: _ACTION_RANK.get(action, 0), default="通过")
...
class _RiskCheckContext:
    ...
    business: Optional[EducationEventContext] = None
    event_id: str = ""
...
def _build_context(
    ...
    return _RiskCheckContext(
        ...
        order_id=event_context.order_id if event_context else None,
        device_id_hash=event_context.device_id_hash if event_context else None,
        operation_time=event_context.operation_time if event_context else None,
        business=event_context,
    )


async def _enrich_business_context(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c：解析订单、设备和操作时间；process_event 已解析时直接复用。"""
    if ctx.business is None:
        ctx.business = await resolve_event_context(
            db,
            event_type=ctx.request.event_type,
            source_id=ctx.request.source_id,
            user_id=ctx.user_id,
        )
    ctx.order_id = ctx.business.order_id
    ctx.device_id_hash = ctx.business.device_id_hash
    ctx.operation_time = ctx.business.operation_time
...
def _save_feature_snapshot(
    ...
    entity_id 填充规则 (按 entity_type):
      ...
      - 设备特征 → ctx.device_id_hash（缺失用 user_id 顶替）
    """
    for fname, fval in features.items():
        ...
        if entity_type == "用户":
            ...
            # 核心表 risk_feature.entity_id 保持 VARCHAR(50) 不变；
            # 完整 64 位设备哈希仍保存在业务表和黑名单中，特征快照只存前 50 位定位标识。
            entity_id = (ctx.device_id_hash or ctx.user_id)[:50]
        db.add(RiskFeature(
            event_id=ctx.event_id,
...
def _calculate_decision(
    ...
    # 规则动作是业务保护下限：模型可以抬高风险，但不能把明确规则要求降级。
    rule_action = _strongest_rule_action(rules)
    if _ACTION_RANK.get(rule_action, 0) > _ACTION_RANK.get(decision, 0):
        decision = rule_action
        final_score = max(final_score, _ACTION_SCORE_FLOOR[rule_action])
        risk_level = _score_to_level(final_score)
...
async def _update_user_profile(
    ...
    # 风控核心表字段名保持不变；教育语义分别为关联设备数、认证失败次数。
...
async def run_risk_check(
    ...
    ctx = _build_context(request, event_context)
    await _enrich_business_context(db, ctx)
