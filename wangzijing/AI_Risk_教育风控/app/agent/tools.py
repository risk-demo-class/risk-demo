...
async def _biz_user_refunds(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            ...
            RefundRequest.apply_time,
            RefundRequest.processed_time,
        )
        .where(RefundRequest.user_id == user_id)
        .order_by(RefundRequest.apply_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_user_devices(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            DeviceBinding.binding_id,
            DeviceBinding.device_id_hash,
            DeviceBinding.ip_hash,
            DeviceBinding.first_seen,
            DeviceBinding.last_seen,
            DeviceBinding.is_current,
        )
...
async def _biz_order_learning(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """报名订单对应的学习进度。"""
    stmt = (
        select(
        ...
        .where(LearningProgress.order_id == order_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_high_value_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        ...
        .where(OrderInfo.final_amount >= 10000)
        .order_by(OrderInfo.final_amount.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_verifications(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户的实名、学籍和学历认证记录。"""
    stmt = (
        select(
            IdentityVerification.verify_id,
            IdentityVerification.verify_type,
            IdentityVerification.verify_result,
            IdentityVerification.fail_reason,
            IdentityVerification.submit_time,
            IdentityVerification.review_time,
        )
        .where(IdentityVerification.user_id == user_id)
        .order_by(IdentityVerification.submit_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
_BIZ_QUERY_HANDLERS = {
    ...
    "user_verifications": _biz_user_verifications,
    "order_learning": _biz_order_learning,
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id（用户ID）、event_type（课程报名/退费申请/学历认证）、source_id（订单/退费/认证ID）。"
...
@tool(
    description=(
        ...
        "返回: 用户风险评分、报名与退费统计、关联设备数和认证失败次数；没画像时实时计算14个用户特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action（add/remove/check/list）、blacklist_type（用户/学号/身份证/设备指纹）、value（ID或哈希值）、reason（加黑原因）。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type（user_orders/user_refunds/user_devices/user_verifications/order_learning/recent_orders/high_value_orders）、user_id、order_id、limit。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
