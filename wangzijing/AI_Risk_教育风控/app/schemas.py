...
class RiskCheckRequest(BaseModel):
    ...
    source_id: str = Field(description="关联业务ID (order_id / refund_id / verify_id)")
    user_id: str
...
class RiskCheckResponse(BaseModel):
    ...
    # 命中的行业黑名单类型: 用户/学号/身份证/设备指纹；None 表示未命中。
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["退费滥用", "账户风险", "设备风险", "认证风险", "交易异常", "学习异常"]
    event_type: Literal["课程报名", "退费申请", "学历认证", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["退费滥用", "账户风险", "设备风险", "认证风险", "交易异常", "学习异常"]] = None
    event_type: Optional[Literal["课程报名", "退费申请", "学历认证", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["用户", "学号", "身份证", "设备指纹"]
    blacklist_value: str
...
if __name__ == "__main__":
    ...
    hits = [
        RuleHitInfo(
            rule_id="EDU008", rule_name="超高金额报名",
            rule_category="交易异常", risk_level="极高",
            risk_score=95, action="拒绝",
            description="单笔订单实付金额≥10000元, 一票否决",
        ),
        RuleHitInfo(
            rule_id="EDU005", rule_name="新设备大额报名",
            rule_category="设备风险", risk_level="高",
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="EDU001", event_type="课程报名", event_source_id="ORD00001",
