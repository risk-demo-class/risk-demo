...
【工作流 6 步】
  1. 重置专用教学数据库 (init_db.py --reset --yes, 含 7 条规则 + 黑卡)
  2. 造 500 个含银行风险模式的虚构用户
  3. 造 2000 条真实 pipeline 训练数据 (25 维完整快照)
...
【用法】
  ...
  python scripts/one_command.py --skip-init     # 跳过 1+2, 假设数据库与银行业务 source 已就绪
...
def step_2_risk_users() -> None:
    """步骤 2: 造含银行风险模式的虚构业务 source。"""
    _run_subprocess(
        "2/6 造 500 个银行风险模式用户",
        [sys.executable, os.path.join(SCRIPTS, "gen_risky_users.py"), "--count", "500"],
    )


def step_3_train_dataset() -> None:
    """步骤 3: 运行四类 source，生成 2000 条真实 pipeline 训练数据。"""
    _run_subprocess(
        "3/6 造 2000 条银行训练数据",
        [sys.executable, os.path.join(SCRIPTS, "gen_train_dataset.py"), "--count", "2000", "--reset"],
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="银行风控系统 - 一条龙命令 (6 步: 重置 → 业务数据 → 训练数据 → 训练 → 回填 → 演示)",
