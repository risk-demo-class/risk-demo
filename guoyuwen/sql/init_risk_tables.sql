-- ============================================
-- 风控核心表 DDL 初始化脚本（9 张表）
-- 由当前连接选择 bank_risk 或 bank_risk_test，本文件不切换数据库。
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('订单欺诈','支付风险','账户风险','售后滥用','地址风险','物流风险','账户接管','交易欺诈','信贷风险','卡片风险','设备风险','名单风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('下单','支付','售后申请','物流投诉','登录','转账','贷款申请','绑卡','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('下单','支付','售后申请','物流投诉','登录','转账','贷款申请','绑卡') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('下单','支付','售后申请','物流投诉','登录','转账','贷款申请','绑卡') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','地址','手机号','设备指纹','IP','银行卡号','身份证号') NOT NULL COMMENT '黑名单类型',
