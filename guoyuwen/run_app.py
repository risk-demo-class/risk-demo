...
def _check_mysql_alive() -> tuple[str, str]:
    """检查 MySQL 是否可连. 用 socket 探测, 不依赖 pymysql 之外的服务."""
    import pymysql
    ...
    try:
        ...
        return "WARN", (
            ...
            f"         业务功能会失败. 检查: MySQL 启动了? 密码对了? 端口 {port} 通? 库 {db} 存在?"
        )
...
def _check_db_initialized() -> tuple[str, str]:
    ...
    try:
        ...
        conn = pymysql.connect(
            ...
            password=settings.DB_PASSWORD, database=settings.DB_NAME, connect_timeout=3,
        )
        try:
