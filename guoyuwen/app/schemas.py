...
BankEventType = Literal["登录", "转账", "贷款申请", "绑卡"]
BankRuleCategory = Literal[
    "账户接管", "交易欺诈", "信贷风险", "卡片风险", "设备风险", "名单风险"
]
BankBlacklistType = Literal["用户", "设备指纹", "IP", "银行卡号", "身份证号"]
...
class RiskCheckRequest(BaseModel):
    ...
    event_type: BankEventType = Field(description="银行事件：登录/转账/贷款申请/绑卡")
    source_id: str = Field(description="事件源ID，含义由 event_type 决定")
    user_id: str = Field(description="发起风险检查的银行用户ID")
    # 仅保留给尚未进入 Goal 3 的内部 pipeline 兼容；银行调用方不需要传递。
    order_id: Optional[str] = None
    receive_id: Optional[str] = None
    event_data: Optional[dict] = Field(
        default=None, description="可选事件上下文，不替代 source_id 校验"
    )
    # 【2026-08-12】评估落库时间覆盖：仅数据生成脚本使用（按 source 业务时间 /
    # 按最近 N 天铺开），银行调用方通常不传；为空则默认当前时间。
    create_time: Optional[datetime] = Field(
        default=None, description="评估 create_time 覆盖值（数据生成脚本用）"
    )
...
class RuleCreate(BaseModel):
    ...
    rule_category: BankRuleCategory
    event_type: Literal["登录", "转账", "贷款申请", "绑卡", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[BankRuleCategory] = None
    event_type: Optional[Literal["登录", "转账", "贷款申请", "绑卡", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    ...
    blacklist_type: BankBlacklistType
    blacklist_value: str = Field(description="虚构标识、掩码值或不可逆摘要")
...
if __name__ == "__main__":
    ...
    hits = [
        RuleHitInfo(
            ...
            risk_score=95,
            action="拒绝",
            description="仅用于展示银行规则响应结构，规则实现属于 Goal 3",
        ),
        RuleHitInfo(
            rule_id="R_DEMO_005",
            rule_name="教学代理网络登录",
    ...
    resp = RiskCheckResponse(
        assessment_id="ast_demo_xxx",
        event_id="evt_demo_xxx",
        user_id="DEMO_USER_001",
        final_score=95,
        risk_level="极高",
        decision="拒绝",
        ...
        features={"user_txn_count_30d": 3, "order_event_amount": 60000},
    ...
    print(
        f"  triggered_rules (前 1 条): {resp.triggered_rules[0].rule_name} ({resp.triggered_rules[0].risk_level})"
    )
    ...
    print("\n" + "=" * 60)
    print(
        "总结: 19 个 schema 覆盖 风控检查 / 规则管理 / 黑名单 / 案件 / 用户画像 / Agent / 评估历史"
    )
