...
async def add_blacklist(db: AsyncSession, request: BlacklistCreate) -> BlacklistResponse:
    ...
    # 【P1 修复 2026-08-12】已软删的同值记录 → 复活同一行 (保留 ID 与审计,
    # 不撞唯一索引 (blacklist_type, blacklist_value)); 软删行不含 deleted_at 会触发 500。
    soft_deleted = (await db.execute(
        select(RiskBlacklist).where(
            RiskBlacklist.blacklist_type == request.blacklist_type,
            RiskBlacklist.blacklist_value == request.blacklist_value,
            RiskBlacklist.deleted_at.is_not(None),
        ).limit(1)
    )).scalars().first()
    if soft_deleted is not None:
        return await _resurrect_blacklist(db, soft_deleted, request)
    ...
    try:
        ...
        # 并发兜底: 两个请求同时复活/新增同值会撞唯一索引, 回滚后交给路由返回 409, 避免裸 500
        await db.rollback()
        raise
    await record_action(
        db, operator="admin", action_type="ADD_BLACKLIST",
...
async def _resurrect_blacklist(
    db: AsyncSession, row: RiskBlacklist, request: BlacklistCreate
) -> BlacklistResponse:
    """把已软删的同值黑名单行复活为当前记录 (P1 修复 2026-08-12)。

    软删行仍占用唯一索引 (blacklist_type, blacklist_value)；直接 INSERT 会触发
    IntegrityError → 500。复活同一行 (deleted_at 清空 + 更新 reason/expire) 既
    保留原始 ID 与 create_time，也避免索引冲突，且审计可追溯。
    """
    before_value = {
        "reason": row.reason,
    ...
    row.expire_time = request.expire_time
    await record_action(
        db, operator="admin", action_type="ADD_BLACKLIST",
        target_type="blacklist", target_id=row.blacklist_id,
        before_value=before_value,
        after_value={
            "reason": request.reason,
            "expire_time": str(request.expire_time) if request.expire_time else None,
        },
        remark=f"恢复黑名单 {request.blacklist_type}={request.blacklist_value}",
    )
    return BlacklistResponse(
...
async def get_blacklist(
    ...
    P1/P6 修复 2026-08-12: 新增 blacklist_type 筛选, 替代前端过滤跨页错乱.
    """
...
async def get_case_detail(db: AsyncSession, case_id: str) -> CaseDetailResponse | None:
    ...
    if profile:
        try:
            bank_profile = json.loads(profile.profile_data) if profile.profile_data else {}
        except (json.JSONDecodeError, TypeError):
            bank_profile = {}
        # 兼容本目标开发期间生成过的首版中文 JSON 键；对外只返回稳定银行字段。
        bank_profile = {
            "txn_count_30d": bank_profile.get("txn_count_30d", bank_profile.get("近30天交易数", profile.total_orders)),
            "avg_txn_amount": bank_profile.get("avg_txn_amount", float(profile.avg_order_amount)),
            "bound_card_count": bank_profile.get("bound_card_count", 0),
            "failed_login_count_30d": bank_profile.get("failed_login_count_30d", bank_profile.get("近30天失败登录", profile.total_refunds)),
            "device_count": bank_profile.get("device_count", bank_profile.get("关联设备数", profile.address_count)),
            "loan_apply_count_30d": bank_profile.get("loan_apply_count_30d", bank_profile.get("近30天贷款申请数", profile.complaint_count)),
            "max_debt_ratio": bank_profile.get("max_debt_ratio", bank_profile.get("最高负债率", 0)),
        }
        user_profile = {
...
async def list_assessments(
    ...
    筛选维度 (可选):
      ...
      - risk_level: 低 / 中 / 高 / 极高
      - event_type: 登录 / 转账 / 贷款申请 / 绑卡 (JOIN risk_event)
      - user_id: 精确匹配
    ...
    class _StatDB:
        ...
        async def execute(self, stmt):
            ...
            if self.call_count == 1:
                ...
                return _R([("交易欺诈", 12), ("账户接管", 8), ("设备风险", 5)])
            else:
                return _R([])
