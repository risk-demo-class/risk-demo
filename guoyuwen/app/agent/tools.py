...
async def _blacklist_add_impl(
    *, db: AsyncSession, blacklist_type: Literal["用户", "设备指纹", "IP", "银行卡号", "身份证号"], value: str, reason: str,
) -> str:
...
async def _manage_blacklist_impl(
    *, db: AsyncSession, action: str, blacklist_type: Literal["用户", "设备指纹", "IP", "银行卡号", "身份证号"], value: str, reason: str,
) -> str:
...
async def _stats_trend(db: AsyncSession, days: int = 7) -> list[dict]:
    """近 days 天每日评估 + 高风险数. GROUP BY 表达式函数 MySQL 5.7+ 也支持."""
    # 近 N 日 = 今天 + 之前 N-1 天, 正好 N 个自然日
...
async def _query_dashboard_stats_impl(*, db: AsyncSession, days: int = 7) -> str:
    """仪表盘统计统一实现: 今日 + 待审 + 近 days 天趋势 + 规则 TOP5."""
    today = await _stats_today(db)
    pending = await _stats_pending_cases(db)
    trend = await _stats_trend(db, days=days)
...
async def _biz_user_transactions(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """客户最近转账。"""
    payer = BankCard
    stmt = (
        select(
        ...
        .join(payer, Transaction.from_card == payer.card_id)
        .where(payer.user_id == user_id)
...
async def _biz_user_loans(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """客户贷款申请。"""
    stmt = (
        select(
            ...
            LoanApplication.institution_code,
            LoanApplication.status,
            LoanApplication.apply_at,
        )
        .where(LoanApplication.user_id == user_id)
        .order_by(LoanApplication.apply_at.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_cards(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """客户银行卡关系，仅返回摘要标识。"""
    stmt = (
        select(
            BankCard.card_id,
            BankCard.card_no_hash,
            BankCard.bank_code,
            BankCard.card_type,
            BankCard.credit_limit,
            BankCard.bind_at,
            BankCard.is_active,
        )
        .where(BankCard.user_id == user_id)
        .order_by(BankCard.bind_at.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_login_history(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """客户登录环境历史。"""
    stmt = (
        select(
            LoginLog.login_id,
...
async def _biz_recent_transactions(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """运营视角最近转账。"""
    stmt = (
        select(
            Transaction.txn_id,
...
async def _biz_shared_devices(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """关联多客户的设备指纹摘要。"""
    stmt = (
        select(
        ...
        .group_by(DeviceFingerprint.device_id, DeviceFingerprint.fingerprint_hash)
        .having(func.count(func.distinct(DeviceFingerprint.user_id)) >= 2)
...
_BIZ_QUERY_HANDLERS = {
    ...
    "login_history": _biz_login_history,
    "recent_transactions": _biz_recent_transactions,
    "shared_devices": _biz_shared_devices,
}
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id、event_type (登录/转账/贷款申请/绑卡)、source_id (对应业务表主键)。"
...
@tool(
    description=(
        ...
        "返回: 客户风险评分、交易频率、失败登录、设备、贷款申请等银行画像。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (add/remove/check/list)、blacklist_type (用户/设备指纹/IP/银行卡号/身份证号)、value、reason。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        "查询风控仪表盘统计数据, 包含今日评估数、高风险数、待审案件数、通过率、近 N 天趋势、规则TOP5。"
        "参数: days (趋势窗口天数, 默认7)。"
        "返回: JSON 字符串 (4 段数据)。"
    )
)
async def query_dashboard_stats(days: int = 7) -> str:
    return await _safe_call("统计查询", _query_dashboard_stats_impl, days=days)
...
@tool(
    description=(
        ...
        "参数: query_type (user_transactions/user_loans/user_cards/login_history/recent_transactions/shared_devices)、user_id、source_id、limit。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
...
if __name__ == "__main__":
    ...
    tools_list = [
        ...
        ("query_dashboard_stats", "仪表盘统计 (今日+待审+近 N 天趋势+TOP5 规则)"),
    ...
    impls = [
        ...
        _manage_blacklist_impl, _stats_today, _stats_pending_cases, _stats_trend,
        ...
        _biz_user_transactions, _biz_user_loans, _biz_user_cards,
        _biz_login_history, _biz_recent_transactions, _biz_shared_devices,
