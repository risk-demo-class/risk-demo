...
from app.models import RiskAssessment, RiskCase, RiskEvent, RiskFeature, RiskUserProfile
...
def _score_to_level(score: int, event_type: str = "通用") -> str:
    ...
    四类银行事件当前统一使用 30/60/85；保留按事件配置能力以便审计。
    """
...
class _RiskCheckContext:
    ...
    event_id: str = ""
    # 【2026-08-12】评估/事件/案件统一的"业务时间锚点": 数据生成脚本可传
    # request.create_time 覆盖, 否则默认当前时间. 让 create_time 与事件业务时间一致.
    as_of: datetime = field(default_factory=datetime.now)


def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    """步骤 1b: 请求 → Context；兼容字段由 process_event 统一补全。"""
    ...
    return _RiskCheckContext(
        ...
        as_of=request.create_time or datetime.now(),
    )


def _ctx_as_of(ctx: _RiskCheckContext) -> datetime:
    """取 ctx 的业务时间锚点；兼容测试用 SimpleNamespace 构造的轻量 ctx。"""
    return getattr(ctx, "as_of", None) or datetime.now()


async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 保留七步流水线钩子；银行环境已由 process_event 补全。"""
    return None
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 计算 25 个银行特征（客户/本次事件/设备网络环境）。"""
...
def _classify_feature_entity(feature_name: str) -> tuple[str, str]:
    """将银行特征映射到核心表冻结的三种实体枚举。

    ``订单``/``地址``仅是核心表不可变的兼容枚举，分别承载本次银行事件和
    设备/IP/地理环境；这里的枚举文字不改变银行业务语义。
    """
...
def _save_feature_snapshot(
    ...
    entity_id 填充规则 (按 entity_type):
      - 用户特征 → ctx.user_id
      - 本次事件特征（冻结枚举“订单”）→ source_id
      - 环境特征（冻结枚举“地址”）→ 设备或 IP 兼容标识
    """
...
async def _update_user_profile(
    ...
    # 核心画像表字段冻结，Goal 3 不改列；兼容列承载银行运营计数，完整银行画像
    # 同时写入 profile_data，前端只读取稳定的银行画像字段。
    profile.total_orders = int(features.get("user_txn_count_30d", 0))
    profile.total_refunds = int(features.get("user_failed_login_count_30d", 0))
    # 【P7 修复 2026-08-12】refund_rate 是冻结核心表的电商兼容列, 银行语义下没有诚实的值
    # (原实现 failed_logins/txn_count 是错误指标); 置 0 表示"未计算", 真实银行指标存 profile_data.
    profile.refund_rate = 0
    profile.avg_order_amount = features.get("user_avg_txn_amount", 0)
    profile.address_count = int(features.get("user_device_count", 0))
    profile.complaint_count = int(features.get("user_loan_apply_count_30d", 0))
    profile.profile_data = json.dumps(
        {
            "bank_semantic_version": 1,
            "txn_count_30d": features.get("user_txn_count_30d", 0),
            "avg_txn_amount": features.get("user_avg_txn_amount", 0),
            "bound_card_count": features.get("user_bound_card_count", 0),
            "failed_login_count_30d": features.get("user_failed_login_count_30d", 0),
            "device_count": features.get("user_device_count", 0),
            "loan_apply_count_30d": features.get("user_loan_apply_count_30d", 0),
            "max_debt_ratio": features.get("user_max_debt_ratio", 0),
        },
        ensure_ascii=False,
    )
    # 累计评估次数 +1 (or 0 兜底, 防止新建时 None)
    profile.assessment_count = (profile.assessment_count or 0) + 1
    profile.last_assessment_time = _ctx_as_of(ctx)
