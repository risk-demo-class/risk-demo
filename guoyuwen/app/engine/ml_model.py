...
_LOADED: bool = False
# 生成训练数据集期间置 True：强制走纯规则决策，防止旧模型推理污染标签（自举）。
_generation_mode: bool = False


@contextmanager
def disabled():
    """生成训练数据时禁用 ML 推理：process_event 走纯规则，避免旧模型污染标签。

    同时阻止后台 ``load_model`` 在此窗口内把模型重新装上（_generation_mode 会让
    ``load_model`` 提前返回、``is_model_loaded`` 恒为 False），消除竞态。
    """
    global _generation_mode
    saved = _generation_mode
    _generation_mode = True
    try:
        yield
    finally:
        _generation_mode = saved
...
def predict(features: dict) -> MlResult:
    ...
    try:
        ...
        # 【P10 修复 2026-08-12】推理失败应按"未加载"走纯规则 (is_loaded=False),
        # 否则 decision 会按 is_loaded=True 用 score=0 参与融合, 静默拉低规则决策.
        return MlResult(score=0.0, decision="通过", is_loaded=False)
...
def train_and_save(
    ...
    # 3. 验证集可由调用方提供（Goal 3 使用按用户分组切分）；未提供时保留
    #    通用的分层随机切分兼容能力。
    ...
    if validation_data is not None:
        ...
        X_tr, X_val, y_tr, y_val = train_test_split(
            X, y, test_size=settings.XGB_TEST_SIZE, stratify=y, random_state=seed,
        )
        dtrain = xgb.DMatrix(X_tr, label=y_tr, feature_names=FEATURE_COLUMNS)
        dval = xgb.DMatrix(X_val, label=y_val, feature_names=FEATURE_COLUMNS)
        eval_set = [(dtrain, "train"), (dval, "val")]
    ...
    if len(eval_set) > 1:
        ...
        # 固定 0.5 阈值，避免用同一验证集搜索阈值后再报告乐观 F1。
        best_thr = 0.5
        ...
        val_tp = int(np.sum((y_val_pred == 1) & (y_val == 1)))
        val_fp = int(np.sum((y_val_pred == 1) & (y_val == 0)))
        val_fn = int(np.sum((y_val_pred == 0) & (y_val == 1)))
        val_tn = int(np.sum((y_val_pred == 0) & (y_val == 0)))
        val_metrics = {
            "n_val": int(len(y_val)),
            "val_accuracy": round(val_acc, 4),
            "val_f1": round(val_f1, 4),
            "val_auc": round(float(val_auc), 4),
            "val_precision": round(val_tp / (val_tp + val_fp), 4) if val_tp + val_fp else 0.0,
            "val_recall": round(val_tp / (val_tp + val_fn), 4) if val_tp + val_fn else 0.0,
            "val_confusion_matrix": [[val_tn, val_fp], [val_fn, val_tp]],
...
if __name__ == "__main__":
    ...
    sample.update({
        "user_txn_count_7d": 1,
        "user_txn_count_30d": 3,
        "user_total_txn_amount": 5000,
        "user_avg_txn_amount": 1666.67,
        "user_max_txn_amount": 3000,
        "user_bound_card_count": 2,
        "user_device_count": 1,
        "user_account_age_days": 720,
        "order_event_amount": 1500,
    ...
    # 让近 7 天交易次数 (FEATURE_COLUMNS[0]) 强相关 y，便于冒烟观察重要性。
