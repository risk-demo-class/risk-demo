...
def evaluate_condition(condition: dict, features: dict[str, float]) -> bool:
    ...
    条件格式:
      {"field": "order_event_amount", "op": ">", "value": 50000} ← 单条件
      {"and": [条件1, 条件2]}                                      ← 逻辑组合
      {"or": [条件1, 条件2]}                                       ← 逻辑组合
    """
    if "and" in condition:
...
if __name__ == "__main__":
    ...
    test_cases = [
        # (描述, 条件, 特征, 期望)
        ("基础 >=",         {"field": "order_event_amount", "op": ">=", "value": 5000}, {"order_event_amount": 8000}, True),
        ("基础 ==",         {"field": "addr_is_unusual", "op": "==", "value": 1},      {"addr_is_unusual": 1},      True),
        ("基础 !=",         {"field": "user_txn_count_30d", "op": "!=", "value": 0},   {"user_txn_count_30d": 5},   True),
        ("AND 嵌套", {"and": [
            {"field": "order_event_amount", "op": ">", "value": 50000},
            {"field": "addr_is_unusual", "op": "==", "value": 1},
        ]}, {"order_event_amount": 60000, "addr_is_unusual": 1}, True),
        ("OR 嵌套",         {"or": [
            {"field": "user_txn_count_7d", "op": ">=", "value": 100},
            {"field": "user_txn_count_7d", "op": "<", "value": 1},
        ]}, {"user_txn_count_7d": 0}, True),
        ("AND 失败",        {"and": [
            {"field": "x", "op": ">", "value": 5},
            {"field": "y", "op": ">", "value": 5},
        ]}, {"x": 10, "y": 1}, False),
    ...
    rules = [
        _MockRule("BANK_R001", {"field": "order_event_amount", "op": ">", "value": 50000}, 95, "极高", "拒绝", 100),
        _MockRule("BANK_R002", {"field": "order_txn_count_1h", "op": ">=", "value": 3}, 75, "高", "人工审核", 90),
        _MockRule("BANK_R007", {"field": "addr_is_proxy", "op": "==", "value": 1}, 45, "中", "标记", 70),
    ]
    feats = {"order_event_amount": 60000, "order_txn_count_1h": 4, "addr_is_proxy": 1}
    ...
    print("\n" + "=" * 60)
    print("结论: 比较操作符与递归 and/or 均可支撑数据库配置的银行规则")
