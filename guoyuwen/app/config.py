...
class Settings(BaseSettings):
    ...
    # ---- 银行业务枚举与风控决策阈值 ----
    BANK_EVENT_TYPES: tuple[str, ...] = ("登录", "转账", "贷款申请", "绑卡")
    BANK_RULE_CATEGORIES: tuple[str, ...] = (
        "账户接管",
        "交易欺诈",
        "信贷风险",
        "卡片风险",
    ...
    # 按 event_type 拆，不同银行控制点使用独立教学阈值。
    ...
    # ---- XGBoost 双轨融合配置 ----
    XGB_MODEL_PATH: str = (
        "app/engine/xgb_model.json"  # 相对路径, ml_model.py resolve 成绝对
    )
    ...
    XGB_TRAIN_DATA_LIMIT: int = (
        2500  # 训练时从 risk_assessment 取最近 N 条 (按 create_time DESC)
    )
    # 训练参数 (P4-L3 2026-08-08 优化正负样本比 + 早停)
    XGB_TEST_SIZE: float = 0.2  # 验证集比例 (stratify 拆分, 保持正负比)
    XGB_EARLY_STOPPING_ROUNDS: int = (
        10  # 早停: 验证集指标连续 N 轮不升就停 (监控指标见下)
    )
    XGB_MIN_SAMPLES: int = 1250  # 最小样本数 = 25 维 × 50 倍 (教学经验值)
    XGB_MAX_SCALE_POS_WEIGHT: float = (
        10.0  # scale_pos_weight 上限, 防过拟合 (不平衡太严重时)
    )
...
if __name__ == "__main__":
    ...
    sections = [
        ...
        (
            "告警 (P4-L2)",
            [
                "ALERT_PENDING_CASE_THRESHOLD",
                "ALERT_RULE_HIT_RATE_MIN",
                "ALERT_BLACKLIST_HIT_RATE_MAX",
                "ALERT_CHECK_WINDOW_HOURS",
            ],
        ),
