...
async def api_list_rules(
    ...
    P5 修复 2026-08-12: 列表过滤软删规则, 与 load_enabled_rules 一致."""
    # 1 次 COUNT
...
async def api_get_rule(rule_id: str, db: AsyncSession = Depends(get_db_async)):
    rule = (await db.execute(
        select(RiskRule).where(
            RiskRule.rule_id == rule_id, RiskRule.deleted_at.is_(None),
        )
    )).scalar_one_or_none()
    if not rule:
...
async def api_create_rule(data: RuleCreate, db: AsyncSession = Depends(get_db_async)):
    ...
    # P5 修复 2026-08-12: 规则软删后允许用同一 rule_id 重建 (复活同一行, 保留历史引用)
    if existing is not None:
        before_value = orm_to_dict(existing, [
            "rule_name", "rule_category", "event_type", "risk_level",
            "risk_score", "action", "is_enabled", "priority",
        ])
        rule = existing
        rule.rule_name = data.rule_name
        rule.rule_category = data.rule_category
        rule.event_type = data.event_type
        rule.rule_condition = json.dumps(data.rule_condition, ensure_ascii=False)
        ...
        await record_action(
            ...
            after_value=orm_to_dict(rule, [
                "rule_name", "rule_category", "event_type", "risk_level",
                "risk_score", "action", "is_enabled", "priority",
            ]),
            remark="复活已删除的规则 (软删后重建)",
        )
        await db.commit()
        return _rule_to_response(rule)
...
async def api_delete_rule(rule_id: str, db: AsyncSession = Depends(get_db_async)):
    ...
    # 【P5 修复 2026-08-12】硬删改为软删 (deleted_at), 与 P3-M9 规范及 load_enabled_rules 一致,
    # 保留审计与历史 rule_results 引用; 列表/详情/启停均已过滤软删.
...
if __name__ == "__main__":
    ...
    example = {
        ...
        "rule_category": "交易欺诈",
        "event_type": "转账",
        "rule_condition": {"field": "order_event_amount", "op": ">", "value": 50000},
        "risk_level": "高",
        "risk_score": 50,
        "action": "人工审核",
        "priority": 70,
