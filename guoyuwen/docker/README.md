...
# 初始化数据库（幂等；--reset --yes 仅限 bank_risk/bank_risk_test 正式重建）
docker compose exec app python scripts/init_db.py --db bank_risk_test
```

### 4. 造数据 + 训练 XGBoost

```bash
# 业务数据 (可重复、幂等; 四类事件 source 合计)
docker compose exec app python scripts/gen_business_data.py --count 2400 --seed 20260812

# 训练数据 + 训练 (P4-L3 第二轮: 早停 + AUC/F1)
docker compose exec app python scripts/gen_train_dataset.py --count 2000 --seed 20260812 --clean
...
services:
  mysql:
    volumes:
      - ../sql/init_business_tables.sql:/docker-entrypoint-initdb.d/01_business_tables.sql:ro
      - ../sql/init_risk_tables.sql:/docker-entrypoint-initdb.d/02_risk_tables.sql:ro
      - ../sql/init_business_data.sql:/docker-entrypoint-initdb.d/03_business_data.sql:ro
      - ../sql/init_risk_data.sql:/docker-entrypoint-initdb.d/04_risk_data.sql:ro
...
### 1. MySQL 8.0（`mysql`）
- 端口 `3306`（主机映射）
- 首次启动自动执行 `sql/init_business_tables.sql` + `sql/init_risk_tables.sql` + `sql/init_business_data.sql` + `sql/init_risk_data.sql`
...
**Q: 我不想用 Docker，能直接本地跑吗？**

A: 当然。Docker 是容器化部署方案，本机开发建议直接用 uvicorn 跑本地 MySQL：
```powershell
