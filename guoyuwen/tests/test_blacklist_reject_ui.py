...
class TestBlacklistRejectResponse:
    ...
    def test_blacklist_reject_has_message(self):
        ...
        req = RiskCheckRequest(
            event_type="登录", source_id="DEMO_LOGIN_001", user_id="DEMO_USR_001",
        )
    ...
    def test_blacklist_reject_has_blocked_by(self):
        ...
        resp = event_module._blacklist_reject(req, "设备指纹")
    ...
    def test_blacklist_reject_ml_score_none(self):
        ...
        resp = event_module._blacklist_reject(req, "IP")
...
class TestRiskCheckPageBlacklistHint:
    ...
    @pytest.fixture
    def html(self):
        path = Path(__file__).resolve().parents[1] / "templates" / "risk_check.html"
    ...
    def test_risk_check_html_explains_ml_skip_on_blacklist(self, html):
        """撞黑分支明确显示未调用模型，避免误报“模型未加载”。"""
        assert "const blocked =" in html
        assert "黑名单短路，未调用模型" in html
        assert "result.blocked_by" in html
