...
class TestRuleRouterHook:
    ...
    async def test_create_rule_writes_log(self):
        ...
        data = RuleCreate(
            ...
            rule_category="交易欺诈", event_type="转账",
    ...
    @pytest.mark.asyncio
    async def test_delete_rule_is_soft_delete_not_hard(self):
        """【P5 修复 2026-08-12】规则删除应为软删 (deleted_at), 不再 db.delete 硬删."""
        from app.routers.rule import api_delete_rule

        rule = SimpleNamespace(
            rule_id="R_SOFT", rule_name="要软删的规则",
            risk_level="高", risk_score=70, action="人工审核",
            is_enabled=1, priority=10,
        )
        db = MagicMock()
        ...
        await api_delete_rule(rule_id="R_SOFT", db=db)

        db.delete.assert_not_called(), "应软删, 不应调用 db.delete 硬删"
        assert getattr(rule, "deleted_at", None) is not None, "应设置 deleted_at"
...
class TestBlacklistHook:
    ...
    async def test_add_blacklist_writes_log(self):
        ...
        added = []
        db.add = lambda obj: added.append(obj)
        # 第 1 次查 (active 查重) = None, 第 2 次查 (软删行复活检查) = None, 第 3 次 flush 完 = bl_new
