...
class TestFeatureColumns:
    ...
    def test_features_to_array(self):
        ...
        features = {
            "user_txn_count_7d": 10,         # 0
            "user_txn_count_30d": 3,         # 1
            # intentionally omit 22 middle keys -> 0
            "addr_is_unusual": 0,            # 24 (last in FEATURE_COLUMNS)
        }
...
class TestModelLoad:
    ...
    def test_load_real_model(self, capsys):
        ...
        normal_features = {name: 0.0 for name in ml_model.FEATURE_COLUMNS}
        normal_features.update({
            "user_txn_count_7d": 1,
            "user_txn_count_30d": 5,
            "user_total_txn_amount": 2000.0,
            "user_avg_txn_amount": 400.0,
            "user_max_txn_amount": 800.0,
            "user_bound_card_count": 2,
            "user_device_count": 1,
            "user_account_age_days": 600,
            "order_event_amount": 200.0,
            "order_txn_count_1h": 1,
            "order_new_device_days": 90,
            "order_device_user_count": 1,
        })
        # 3. 准备一组"高风险用户"特征 (强信号, 期望 → 人工审核/拒绝)
        risky_features = dict(normal_features)
        risky_features.update({
            "user_txn_count_7d": 80,
            "user_txn_count_30d": 200,
            "user_total_txn_amount": 800000.0,
            "order_event_amount": 80000.0,
            "order_txn_count_1h": 5,
            "order_is_night": 1,
            "addr_is_unusual": 1,
        })
...
class TestPredict:
    ...
    def test_predict_risky_user(self):
        ...
        # 高风险用户: 25 维全部填, 叠加异地大额/凌晨密集/新设备信号。
        risky = {
            "user_txn_count_7d": 50,
            "user_txn_count_30d": 100,
            "user_total_txn_amount": 800000,
            "user_avg_txn_amount": 8000,
            "user_max_txn_amount": 120000,
            "user_bound_card_count": 8,
            "user_failed_login_count_30d": 12,
            "user_device_count": 9,
            "user_loan_apply_count_30d": 6,
            "user_loan_institution_count_30d": 5,
            "user_max_debt_ratio": 0.95,
            "user_night_operation_count_7d": 20,
            "user_incoming_card_count_1h": 6,
            "user_account_age_days": 30,
            "order_event_amount": 80000,
            "order_txn_count_1h": 5,
            "order_is_night": 1,
            "order_new_device_days": 1,
            "order_device_user_count": 8,
            "order_payee_card_count_1h": 5,
            "order_loan_institution_count_30d": 5,
            "order_debt_ratio": 0.95,
            "addr_is_proxy": 1,
            "addr_is_tor": 0,
            "addr_is_unusual": 1,
        }
    ...
    def test_predict_fallback_when_not_loaded(self):
        ...
        try:
            ...
            result = ml_model.predict({"user_txn_count_7d": 1})
...
class TestTrainQuality:
    ...
    def test_train_reports_f1_at_fixed_validation_threshold(self, tmp_path):
        """验证集使用预先固定的 0.5 阈值，避免扫描验证标签美化 F1。"""
        ...
        assert "best_f1_threshold" not in metrics
        assert "val_f1" in metrics
