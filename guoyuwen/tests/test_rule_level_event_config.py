...
5. 前端 app.js 包含 25 特征 + 构建器函数 (静态扫描)
...
class TestRiskEventThresholds:
    ...
    def test_get_event_thresholds_with_fallback(self):
        ...
        th = settings.get_event_thresholds("转账")
        assert th == {"pass": 30, "mark": 60, "review": 85}
        # fallback: 通用阈值
    ...
    def test_转账阈值最敏感(self):
        """资金转移控制点应不宽于登录、贷款申请和绑卡。"""
        transfer = settings.RISK_EVENT_THRESHOLDS["转账"]
        for event_type in ("登录", "贷款申请", "绑卡"):
            current = settings.RISK_EVENT_THRESHOLDS[event_type]
            assert transfer["pass"] <= current["pass"]
            assert transfer["mark"] <= current["mark"]
            assert transfer["review"] <= current["review"]
...
class TestScoreToLevelWithEventType:
    ...
    def test_score_to_level_default_event(self):
        ...
        # 不传 event_type 用银行统一阈值 (PASS=30, MARK=60, REVIEW=85)
    ...
    def test_score_to_level_with_event_type(self):
        ...
        assert _score_to_level(29, "登录") == "低"
        assert _score_to_level(30, "登录") == "中"
        assert _score_to_level(59, "登录") == "中"
        assert _score_to_level(60, "登录") == "高"
        assert _score_to_level(85, "登录") == "极高"

    def test_score_to_decision_with_event_type(self):
        from app.engine.decision import _score_to_decision
        assert _score_to_decision(29, "转账") == "通过"
        assert _score_to_decision(30, "转账") == "标记"
        assert _score_to_decision(85, "转账") == "拒绝"
    ...
    def test_score_to_level_fallback_unknown_event(self):
        ...
        assert _score_to_level(84, "不存在的") == "高"
        assert _score_to_level(85, "不存在的") == "极高"
...
class TestFrontendRuleBuilder:
    """app.js 必须包含: 25 特征 + 构建器函数 + 风险等级映射 + 阈值映射."""

    @pytest.fixture
    def appjs(self):
        p = Path(__file__).resolve().parents[1] / "static" / "app.js"
...
class TestFrontendRulesPage:
    ...
    @pytest.fixture
    def html(self):
        p = Path(__file__).resolve().parents[1] / "templates" / "rules.html"
        return p.read_text(encoding="utf-8")
