...
class TestDeadCodeRemoved:
    ...
    def test_bank_ownership_validators_kept(self):
        """银行 source 归属和转账卡关系校验必须保留。"""
        assert hasattr(validator_module, "_ensure_owned_source")
        assert hasattr(validator_module, "_ensure_transfer_source")
...
class TestValidatorPublicApi:
    ...
    def test_validator_demo_describes_bank_dispatch(self):
        """__main__ 只展示银行事件派发表，不保留电商校验说明。"""
        src = inspect.getsource(validator_module)
        assert "银行事件 source_id 派发表" in src
