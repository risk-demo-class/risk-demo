...
class TestScoreCalculation:
    ...
    def test_single_hit(self):
        hit = RuleHitResult(_MockRule("R1", "测试", "交易欺诈", "高", 70, "人工审核"))
        assert calculate_final_score([hit]) == 70

    def test_multiple_hits_bonus(self):
        """多规则命中: max + BONUS × extra_count"""
        hits = [
            RuleHitResult(_MockRule("R1", "测试1", "交易欺诈", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R2", "测试2", "账户接管", "中", 40, "标记")),
...
class TestScoreMapping:
    ...
    def test_level_mapping(self):
        ...
        assert _score_to_level(84) == "高"
        assert _score_to_level(85) == "极高"
        assert _score_to_level(100) == "极高"

    def test_decision_mapping(self):
        # 银行四事件统一使用 30/60/85 决策边界。
        ...
        assert _score_to_decision(84) == "人工审核"
        assert _score_to_decision(85) == "拒绝"
...
class TestCaseDeduplication:
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="DEMO_TXN_001", user_id="DEMO_USR_001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="DEMO_TXN_002", user_id="DEMO_USR_001",
        )
        ...
        assert db.added[0].source_id == "DEMO_TXN_002"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="DEMO_TXN_003", user_id="DEMO_USR_001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "交易欺诈", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="DEMO_TXN_REJECT_001", user_id="DEMO_USR_001",
        )
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="DEMO_TXN_REVIEW_001", user_id="DEMO_USR_002",
        )
...
class TestCaseCategoryAggregation:
    ...
    async def test_majority_category_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="登录", source_id="DEMO_LOGIN_X", user_id="DEMO_USR_001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # priority 高的 R007 (支付风险) 排第一
        rules = [
            RuleHitResult(_MockRule("R007", "异常登录", "账户接管", "极高", 90, "拒绝")),
            RuleHitResult(_MockRule("R006", "新设备登录", "账户接管", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R008", "代理网络", "账户接管", "高", 65, "人工审核")),
            RuleHitResult(_MockRule("R001", "大额转账", "交易欺诈", "高", 70, "人工审核")),
        ]
        ...
        assert case.case_category == "账户接管", (
            f"应取命中数最多的 '账户接管', 实际 {case.case_category}"
        )
    ...
    async def test_tie_first_one_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="DEMO_TXN_Y", user_id="DEMO_USR_001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # 1 条订单欺诈 + 1 条地址风险 → 平局
        rules = [
            RuleHitResult(_MockRule("R1", "a", "交易欺诈", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R2", "b", "设备风险", "高", 70, "人工审核")),
        ]
        ...
        assert db.added[0].case_category in ("交易欺诈", "设备风险")
