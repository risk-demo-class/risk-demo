...
class TestCaseServiceBlacklistSignatures:
    ...
    def test_get_blacklist_blacklist_type_optional(self):
        """get_blacklist 支持可选 blacklist_type (P6: 后端筛选替代前端过滤); 不传则查全表."""
        from app.service.case import get_blacklist
        import inspect
        sig = inspect.signature(get_blacklist)
        params = list(sig.parameters.keys())
        assert "blacklist_type" in params, "get_blacklist 应有 blacklist_type 参数"
        assert sig.parameters["blacklist_type"].default is None, (
            "blacklist_type 应默认为 None (不筛选)"
        )
