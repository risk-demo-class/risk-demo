...
def _get_db_settings() -> tuple[str, int, str, str, str]:
    """从 .env (app.config settings) 统一取数据库连接参数."""
    from app.config import settings  # 加载 .env 配置
    return (settings.DB_HOST, settings.DB_PORT, settings.DB_USER,
            settings.DB_PASSWORD, settings.DB_NAME)


def _check_mysql_alive() -> tuple[str, str]:
    """检查 MySQL 是否在 settings.DB_HOST:DB_PORT 跑."""
    import pymysql
    host, port, user, password, db = _get_db_settings()
    try:
        ...
        conn.close()
        return "OK", f"MySQL {user}@{host}:{port}/{db} 可连"
    except Exception as e:
        return "WARN", (
            f"MySQL 连不上 ({host}:{port} {user}@{db}): {type(e).__name__}\n"
            f"         业务功能会失败. 检查: MySQL 启动了? 密码对了? 端口 {port} 通?"
        )
