"""
制造业风控系统 - 一键数据库初始化脚本 (异步)
按顺序执行所有 SQL 脚本: 创建数据库 → 业务表 → 风控表 → 风控规则

总表数: 7 业务 + 8 风控 = 15 张
造数: 建完库再跑 python scripts/gen_data.py (业务演示数据, 日期动态生成)
"""
import argparse
import asyncio
import os
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 7 张业务表"),
    ("init_risk_tables.sql", "创建 8 张风控表"),
    ("init_risk_data.sql", "导入 8 条预置风控规则 (R001/R002/R005/R008/R012/R018/R025/R030)"),
]
...
async def main():
    parser = argparse.ArgumentParser(
        description="制造业风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""用法:
  python scripts/init_db.py                       # 默认: --reset (删库重建)
  python scripts/init_db.py --keep-data          # 保留: 只补表结构
  python scripts/init_db.py --reset --yes        # 重置 + 自动确认 (CI/脚本用)
    ...
    group.add_argument("--reset", dest="reset", action="store_true", default=True,
                       help="默认行为: 先 DROP DATABASE 再 CREATE")
    group.add_argument("--keep-data", dest="reset", action="store_false",
                       help="保留数据, 只补表结构")
    parser.add_argument("--yes", "-y", action="store_true", help="跳过重置确认")
    args = parser.parse_args()

    print("=" * 60)
    print("制造业风控系统 - 数据库初始化 (异步)")
    ...
    try:
        ...
        print("  检查: MySQL 启动了吗? 密码对吗? (默认 root/Aa666666)")
        sys.exit(1)
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 15 张表重建 + 8 条规则就绪")
        print("下一步: python scripts/gen_data.py   (生成业务演示数据)")
    else:
