...
async def _biz_user_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            ...
            OrderInfo.status,
            (OrderInfo.total_amount - OrderInfo.discount_amount).label("total_amount"),
        )
...
async def _biz_order_detail(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        ...
        .outerjoin(LearningProgress, LearningProgress.order_id == OrderInfo.order_id)
...
async def _biz_high_value_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        ...
        .where((OrderInfo.total_amount - OrderInfo.discount_amount) >= 5000)
        .order_by((OrderInfo.total_amount - OrderInfo.discount_amount).desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (教育用户ID)、event_type (课程报名/退费申请/直播打赏/学习行为)、source_id (对应教育业务ID)。"
...
@tool(
    description=(
        ...
        "返回: 用户风险评分、课程报名、退费率和关联设备等画像数据；没画像时实时计算14个用户特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (add/remove/check/list)、blacklist_type (用户/学号/身份证哈希/设备指纹/直播账号)、value、reason。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
