...
from app.models import BlacklistExtra, RiskAssessment, RiskBlacklist, RiskCase, RiskEvent, RiskUserProfile
...
async def check_blacklist(db: AsyncSession, blacklist_type: str, value: str) -> bool:
    if not value:
        return False
    if blacklist_type != "用户":
        bl = (await db.execute(
            select(BlacklistExtra).where(
                BlacklistExtra.entry_type == blacklist_type,
                BlacklistExtra.entry_value == value,
                BlacklistExtra.is_active.is_(True),
            )
        )).scalar_one_or_none()
        if not bl:
            return False
        return not bl.expire_at or bl.expire_at >= datetime.now()
...
async def add_blacklist(db: AsyncSession, request: BlacklistCreate) -> BlacklistResponse:
    if request.blacklist_type != "用户":
        existing = (await db.execute(
            select(BlacklistExtra).where(
                BlacklistExtra.entry_type == request.blacklist_type,
                BlacklistExtra.entry_value == request.blacklist_value,
            )
        )).scalar_one_or_none()
        if existing:
            existing.reason = request.reason or "管理员手动加入"
            existing.expire_at = request.expire_time
            existing.is_active = True
            entry = existing
        else:
            entry = BlacklistExtra(
                entry_type=request.blacklist_type,
                entry_value=request.blacklist_value,
                reason=request.reason or "管理员手动加入",
                expire_at=request.expire_time,
                is_active=True,
        ...
        await record_action(
            ...
            target_type="blacklist", target_id=f"extra:{entry.entry_id}",
            after_value={
                "blacklist_type": entry.entry_type,
                "blacklist_value": entry.entry_value,
                "reason": entry.reason,
            },
            remark=f"新增或更新教育黑名单 {entry.entry_type}",
        )
        return BlacklistResponse(
            blacklist_id=f"extra:{entry.entry_id}",
            blacklist_type=entry.entry_type,
            blacklist_value=entry.entry_value,
            reason=entry.reason,
            expire_time=entry.expire_at,
...
async def get_blacklist(
    ...
    responses.extend(
        BlacklistResponse(
            blacklist_id=f"extra:{b.entry_id}",
            blacklist_type=b.entry_type,
            blacklist_value=b.entry_value,
            reason=b.reason,
            expire_time=b.expire_at,
            create_time=b.create_time,
        )
        for b in extra_items
    )
    responses.sort(key=lambda item: item.create_time or datetime.min, reverse=True)
    start = (page - 1) * page_size
    return total, responses[start:start + page_size]


# 软删 (P3-M9): UPDATE deleted_at = NOW() 而非硬删, 保留 audit
async def remove_blacklist(db: AsyncSession, blacklist_id: int | str) -> bool:
    raw_id = str(blacklist_id)
    if raw_id.startswith("extra:"):
        try:
            entry_id = int(raw_id.removeprefix("extra:"))
        except ValueError:
            return False
        entry = (await db.execute(
            select(BlacklistExtra).where(
                BlacklistExtra.entry_id == entry_id,
                BlacklistExtra.is_active.is_(True),
            )
        )).scalar_one_or_none()
        if not entry:
        ...
        await record_action(
            ...
            target_type="blacklist", target_id=raw_id,
            before_value={"blacklist_type": entry.entry_type, "blacklist_value": entry.entry_value},
            remark=f"移除教育黑名单 {entry.entry_type}",
        )
    ...
    bl = (await db.execute(
        select(RiskBlacklist).where(
            RiskBlacklist.blacklist_id == core_id,
            RiskBlacklist.deleted_at.is_(None),
        )
    )).scalar_one_or_none()
    if not bl:
...
async def get_case_list(
    ...
    if category:
        filters.append(RiskCase.case_category == to_core_category(category))
    ...
    items = [
        CaseItem(
            ...
            case_category=to_education_category(r[0].case_category),
            ...
            event_type=to_education_event(r[0].event_type),
        )
        for r in rows
    ]
...
async def get_case_detail(db: AsyncSession, case_id: str) -> CaseDetailResponse | None:
    ...
    if assessment and assessment.rule_results:
        try:
            ...
            for rule in triggered_rules:
                rule.rule_category = to_education_category(rule.rule_category)
    ...
    return CaseDetailResponse(
        ...
        case_category=to_education_category(case.case_category),
        ...
        event_type=to_education_event(case.event_type),
    )
...
async def get_case_statistics(db: AsyncSession) -> CaseStatistics:
    ...
    by_category = {
        to_education_category(cat): cnt for cat, cnt in category_rows if cat
    }  # 过滤 None，并还原教育行业分类
...
async def list_assessments(
    ...
    筛选维度 (可选):
      ...
      - risk_level: 低 / 中 / 高 / 极高
      - event_type: 课程报名 / 退费申请 / 直播打赏 / 学习行为 (JOIN risk_event)
      - user_id: 精确匹配
    ...
    if event_type:
        count_stmt = count_stmt.where(RiskEvent.event_type == to_core_event(event_type))
    for f in a_filters:
        count_stmt = count_stmt.where(f)
    total = int((await db.execute(count_stmt)).scalar() or 0)
    ...
    if event_type:
        paged_stmt = paged_stmt.where(RiskEvent.event_type == to_core_event(event_type))
    paged_stmt = (
    ...
    items = [
        AssessmentItem(
            ...
            user_id=r[0].user_id,
            event_type=to_education_event(r[1]) or "未知",
...
async def get_assessment_detail(
    ...
    return AssessmentDetailResponse(
        ...
        event_id=a.event_id,
        user_id=a.user_id,
        event_type=to_education_event(ev.event_type) if ev else "未知",
