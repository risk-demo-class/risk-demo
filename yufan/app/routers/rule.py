...
def _core_category(value: str) -> str:
    return to_core_category(value)
...
async def api_list_rules(
    ...
    if category:
        count_stmt = count_stmt.where(RiskRule.rule_category == _core_category(category))
    total = int((await db.execute(count_stmt)).scalar() or 0)

    # 1 次 paged SELECT
    stmt = select(RiskRule)
    if category:
        stmt = stmt.where(RiskRule.rule_category == _core_category(category))
    stmt = (
...
async def api_create_rule(data: RuleCreate, db: AsyncSession = Depends(get_db_async)):
    ...
    rule = RiskRule(
        ...
        rule_category=_core_category(data.rule_category),
        event_type="通用",
...
def _rule_to_response(rule: RiskRule) -> RuleResponse:
    return RuleResponse(
        ...
        rule_category=CORE_TO_EDUCATION_CATEGORY.get(rule.rule_category, rule.rule_category),
