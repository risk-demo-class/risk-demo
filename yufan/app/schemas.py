...
class RiskCheckRequest(BaseModel):
    """风险检查请求"""
    event_type: Literal["课程报名", "退费申请", "直播打赏", "学习行为"]
    source_id: str = Field(min_length=1, description="关联教育业务实体ID")
    user_id: str
    order_id: Optional[str] = None
    receive_id: Optional[str] = Field(default=None, description="决策流水线兼容字段，教育版承载设备关联ID")
...
class RiskCheckResponse(BaseModel):
    ...
    # 命中黑名单时不走 7 步决策，用该字段区分业务拒绝与黑名单拒绝。
    blocked_by: Optional[str] = Field(default=None, description="命中的黑名单类型")
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["报名异常", "退费滥用", "账号风险", "学习异常", "打赏风险", "设备风险"]
    event_type: Literal["课程报名", "退费申请", "直播打赏", "学习行为", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["报名异常", "退费滥用", "账号风险", "学习异常", "打赏风险", "设备风险"]] = None
    event_type: Optional[Literal["课程报名", "退费申请", "直播打赏", "学习行为", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["用户", "学号", "身份证哈希", "设备指纹", "直播账号"]
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        ...
        order_id="ORD_N0001", device_id="DEV_NORMAL_001",
        event_data={"amount": 1999, "course_id": "COURSE_001"},
    )
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="U0001", event_type="课程报名", event_source_id="ORD_N0001",
