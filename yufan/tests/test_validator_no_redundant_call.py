...
class TestProcessEventCallsValidatorOnce:
    ...
    async def test_process_event_calls_validate_risk_check_request_once(self, monkeypatch):
        ...
        request = RiskCheckRequest(
            event_type="课程报名", source_id="ORD001", user_id="U001", order_id="ORD001"
        )
...
class TestValidatorPublicApi:
    ...
    def test_validator_has_four_education_event_mappings(self):
        assert set(validator_module._EVENT_SOURCE_VALIDATORS) == {
            "课程报名", "退费申请", "直播打赏", "学习行为",
        }
