"""
教育行业风控系统 - 一键数据库初始化脚本 (异步)
按顺序执行 SQL 脚本: 创建数据库 → 业务表 → 业务数据 → 风控表 → 风控规则
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 7 张教育业务表", "schema"),
    ("init_business_data.sql", "导入教育业务测试数据", "data"),
    ("init_risk_tables.sql", "创建 9 张风控表", "schema"),
    ("init_risk_data.sql", "导入教育行业风控规则", "data"),
]
...
async def execute_sql_file(conn, filepath, description):
    ...
    # 按分号拆分语句，过滤空语句和注释
    statements = connection_scoped_statements(sql_content)
    # 老师的风控 SQL 兼容历史手动执行方式，文件中写有 ``USE ecs``。
    # Python 初始化器必须尊重 --db 指定的连接库，因此忽略 SQL 内的 USE，
    # 防止测试/演示时把表误建到固定 ecs 库。
...
def connection_scoped_statements(sql_text):
    """拆分 SQL，并移除会绕过 ``--db`` 目标连接库的 USE 语句。"""
    return [
        stmt for stmt in split_sql_statements(sql_text)
        if not stmt.lstrip().upper().startswith("USE ")
    ]


async def main():
    parser = argparse.ArgumentParser(
        description="教育行业风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
...
用法:
  ...
  python scripts/init_db.py --schema-only --yes  # 只建 7+9 张表, 不导入数据/规则
  python scripts/init_db.py --db ai_risk_education_test  # 初始化测试库
        """,
    )
    ...
    parser.add_argument(
        ...
        help="只执行业务表和风控表 DDL，不导入业务数据和规则",
    )
    ...
    print("=" * 60)
    selected_files = [
        item for item in SQL_FILES if not args.schema_only or item[2] == "schema"
    ]
    total_steps = 1 + len(selected_files)

    print("教育行业风控系统 - 数据库初始化 (异步)")
    print(f"目标: {args.user}@{args.host}:{args.port}/{args.db}")
    print(f"模式: {'[RESET] 先删后建' if args.reset else '[KEEP-DATA] 保留数据, 只补表'}")
    print(f"内容: {'仅表结构' if args.schema_only else '表结构 + 数据 + 规则'}")
    print("=" * 60)
    ...
    # 后续步骤: 按顺序执行选中的 SQL 脚本
    conn = await get_connection(args.host, args.port, args.user, args.password, db=args.db)
    total_errors = 0

    try:
        for idx, (filename, desc, _kind) in enumerate(selected_files, start=2):
            filepath = os.path.join(SQL_DIR, filename)
            if not os.path.exists(filepath):
                print(f"\n[步骤 {idx}/{total_steps}] 跳过: {filename} 不存在")
                continue
            print(f"\n[步骤 {idx}/{total_steps}]", end="")
    ...
    if total_errors == 0:
        ...
        if args.reset:
            if args.schema_only:
                print("数据库已重置: 7 张教育业务表 + 9 张风控表已创建")
            else:
                print("数据库已重置: 16 张表 + 教育业务数据 + 基线规则全部就绪")
    else:
