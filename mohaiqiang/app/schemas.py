...
class RiskCheckRequest(BaseModel):
    ...
    event_type: Literal[
        ...
        "投诉", "理赔申请", "异常上报", "报关清关", "海外仓", "COD结算", "结汇退税", "实名认证",
    ]
    source_id: str = Field(description="关联业务ID (运单号/投诉ID/理赔ID/COD单号等)")
    user_id: str = Field(description="寄件人ID (风控主主体)")
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["实名风险", "危险品风险", "跨境风险", "代收货款风险", "地址风险", "寄件行为风险"]
    event_type: Literal[
        "寄件下单", "揽收", "中转", "派送", "签收", "拒收", "退回",
        "投诉", "理赔申请", "异常上报", "报关清关", "海外仓", "COD结算", "结汇退税",
        "实名认证", "通用",
    ] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["实名风险", "危险品风险", "跨境风险", "代收货款风险", "地址风险", "寄件行为风险"]] = None
