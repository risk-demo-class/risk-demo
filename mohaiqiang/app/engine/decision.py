...
7 步流水线:
  1. 校验 + 准备上下文 (Context)
  2. 创建事件记录 (risk_event)
  3-4. 计算 + 保存特征快照 (30 维, risk_feature)
  5. 加载 + 匹配规则
...
from app.models import (
    LogisticsWaybill, RiskAssessment, RiskCase, RiskEvent, RiskFeature, RiskUserProfile,
)
...
def check_veto(hits: list[RuleHitResult]) -> bool:
    ...
    当前物流项目"极高"级别规则示例: 危险品瞒报 / 极端高频寄件 / 极端 COD 拒收.
    """
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    """步骤 1b: 请求 → Context, 补全 waybill_no.

    运单类事件 source_id 就是运单号; 投诉/理赔/COD 事件由 event.py 已把
    waybill_no 放进 event_data.
    """
    waybill_no = request.event_data.get("waybill_no") if request.event_data else None
    if not waybill_no and request.event_type in (
...
async def _enrich_address_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 从运单补全地址 ID."""
    if ctx.address_id or not ctx.waybill_no:
        return
    row = (await db.execute(
        select(LogisticsWaybill.address_id).where(
            LogisticsWaybill.waybill_no == ctx.waybill_no
        )
    )).first()
    if row:
        ctx.address_id = row.address_id
...
def _save_feature_snapshot(
    ...
    """步骤 4: 把 30 个特征落库到 risk_feature (审计回溯).

    entity_id 填充规则 (按 entity_type):
      - 寄件人特征 → ctx.user_id
      - 运单特征 → ctx.waybill_no (缺失用 source_id 顶替)
      - 地址特征 → ctx.address_id (缺失用 user_id 顶替)
    """
    for fname, fval in features.items():
        ...
        if entity_type == "寄件人":
            ...
            entity_id = ctx.waybill_no or ctx.request.source_id
        else:  # 地址
            entity_id = ctx.address_id or ctx.user_id
        db.add(RiskFeature(
            event_id=ctx.event_id,
...
async def _update_user_profile(
    ...
    """步骤 7c: 更新寄件人风险画像 (upsert 模式: 有则更新, 无则插入)."""
    ...
    profile.total_orders = int(features.get("sender_total_waybills", 0))
    profile.total_refunds = int(features.get("sender_claim_count", 0))
    profile.refund_rate = features.get("sender_cod_reject_rate", 0)
    profile.avg_order_amount = features.get("sender_avg_declared_value", 0)
    profile.address_count = int(
        features.get("sender_remote_address_count", 0)
        + features.get("sender_temp_address_count", 0)
    )
    profile.complaint_count = int(features.get("sender_complaint_count", 0))
...
def _build_response(
    ...
    features: 30 特征全返回, 但 RISK_FEATURES_FULL_RETURN=False 时脱敏 (P3-M6).
    真生产要按角色判断, 留 TODO.
    """
    triggered_rules = [
        RuleHitInfo(
...
async def run_risk_check(
    ...
    await _enrich_address_id(db, ctx)
