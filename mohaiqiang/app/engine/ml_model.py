...
【重要约定】
  - 特征顺序固定 (FEATURE_COLUMNS), 跟 feature.py 算出的 30 个 key 一一对应
...
# 改这个列表前必须同步: feature.py::compute_sender_features
# / tests/test_xgboost.py::test_feature_columns_align
# 对不齐会让 XGBoost 训练/推理退化成 0 填充, 业务上相当于模型没工作
FEATURE_COLUMNS: list[str] = [
    # --- 20 寄件人特征 (compute_sender_features) ---
    "sender_total_waybills",
    "sender_waybills_30d",
    "sender_waybills_7d",
    "sender_night_pickup_count",
    "sender_dangerous_item_count",
    "sender_cod_total_amount",
    "sender_cod_reject_rate",
    "sender_realname_verified",
    "sender_verify_fail_count",
    "sender_complaint_count",
    "sender_claim_count",
    "sender_abnormal_count",
    "sender_cross_border_count",
    "sender_remote_address_count",
    "sender_temp_address_count",
    "sender_shared_address_count",
    "sender_insured_ratio",
    "sender_high_value_ratio",
    "sender_value_weight_ratio",
    "sender_account_risk",
    # --- 6 运单特征 (compute_waybill_features) ---
    "waybill_declared_value",
    "waybill_weight_kg",
    "waybill_value_weight_ratio",
    "waybill_is_dangerous",
    "waybill_is_cross_border",
    "waybill_status_risk",
    # --- 4 地址特征 (compute_address_features) ---
    "address_is_remote",
    "address_is_temp",
...
def _features_to_array(features: dict) -> np.ndarray:
    """30 维 dict → numpy 数组 (1×30). 缺失用 0.0 兜底; None/字符串统一 float."""
    row = []
...
def predict(features: dict) -> MlResult:
    """推理入口: 30 维特征 → P(拒绝) + ML 维度决策. 模型没加载 → 兜底 score=0 通过."""
...
def train_and_save(
    ...
    # 1. 样本量预警: 30 维特征, 推荐最小 1250 (教学经验值)
    if n < settings.XGB_MIN_SAMPLES:
        logger.warning(
            "样本量 %d < 推荐最小 %d (30 维特征经验值), 模型可能欠拟合/过拟合, "
    ...
    if val_metrics and val_metrics.get("val_auc", 0) < settings.XGB_MIN_VAL_AUC:
        logger.warning(
            ...
            "建议: --balance-pos 重造数据 / 检查 feature.py 30 维是否真的有区分度",
...
if __name__ == "__main__":
    ...
    # 2. 特征向量化 (30 维 dict → numpy 数组)
    print("\n[2] 30 维特征 → numpy 数组 (_features_to_array):")
    print(f"  FEATURE_COLUMNS 长度 = {len(FEATURE_COLUMNS)}  (必须是 30, 跟 feature.py 对齐)")
    sample = {
        "sender_total_waybills": 20, "sender_waybills_30d": 10, "sender_waybills_7d": 4,
        "sender_night_pickup_count": 2, "sender_dangerous_item_count": 1,
        "sender_cod_total_amount": 8000, "sender_cod_reject_rate": 0.2,
        "sender_realname_verified": 1, "sender_verify_fail_count": 0,
        "sender_complaint_count": 2, "sender_claim_count": 1,
        "sender_abnormal_count": 1, "sender_cross_border_count": 3,
        "sender_remote_address_count": 1, "sender_temp_address_count": 2,
        "sender_shared_address_count": 1, "sender_insured_ratio": 0.3,
        "sender_high_value_ratio": 0.4, "sender_value_weight_ratio": 1000,
        "sender_account_risk": 0,
        "waybill_declared_value": 1500, "waybill_weight_kg": 2.0,
        "waybill_value_weight_ratio": 750, "waybill_is_dangerous": 0,
        "waybill_is_cross_border": 1, "waybill_status_risk": 0,
        "address_is_remote": 0, "address_is_temp": 0,
        "address_shared_count": 1, "address_use_count": 5,
    }
    arr = _features_to_array(sample)
    print(f"  shape           = {arr.shape}  (期望 (1, 30))")
    ...
    X_demo = np.random.uniform(0, 1, size=(300, 30)).astype(np.float32)
    y_demo = (np.random.rand(300) > 0.66).astype(np.int32)  # 约 100 正
    # 让 sender_total_waybills (FEATURE_COLUMNS[0]) 强相关 y, 方便看特征重要性
    ...
    print("\n" + "=" * 60)
    print("总结: 30 维特征对齐 / 模型加载 / predict 兜底 / 概率阈值, 4 个核心能力演示完成")
    print("训练数据准备: python scripts/gen_logistics_risk_data_dates.py --days 30 --per-day 50")
