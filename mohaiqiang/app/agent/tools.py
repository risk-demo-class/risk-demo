...
from app.engine.feature import compute_sender_features
...
# ============================================================
# 业务数据查询 (物流 11 种 query_type)
# ============================================================

async def _biz_sender_waybills(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """寄件人的运单列表."""
    stmt = (
        select(
            LogisticsWaybill.waybill_no,
            LogisticsWaybill.item_name,
            LogisticsWaybill.item_category,
            LogisticsWaybill.declared_value,
            LogisticsWaybill.weight_kg,
            LogisticsWaybill.cod_amount,
            LogisticsWaybill.is_cross_border,
            LogisticsWaybill.status,
            LogisticsWaybill.create_time,
        )
        .where(LogisticsWaybill.sender_id == user_id)
        .order_by(LogisticsWaybill.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_waybill_detail(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """运单详情 (含寄件人/收件人/地址)."""
    stmt = (
        select(
            LogisticsWaybill.waybill_no,
            LogisticsWaybill.sender_id,
            LogisticsSender.sender_name,
            LogisticsSender.sender_phone,
            LogisticsRecipient.recipient_name,
            LogisticsRecipient.recipient_phone,
            LogisticsAddress.province,
            LogisticsAddress.city,
            LogisticsAddress.district,
            LogisticsAddress.street_address,
            LogisticsWaybill.item_name,
            LogisticsWaybill.item_category,
            LogisticsWaybill.declared_value,
            LogisticsWaybill.weight_kg,
        ...
        .join(LogisticsSender, LogisticsSender.sender_id == LogisticsWaybill.sender_id)
        .join(LogisticsRecipient, LogisticsRecipient.recipient_id == LogisticsWaybill.recipient_id)
        .join(LogisticsAddress, LogisticsAddress.address_id == LogisticsWaybill.address_id)
        .where(
            LogisticsWaybill.waybill_no == waybill_no
            if waybill_no else LogisticsWaybill.sender_id == user_id
        )
        .order_by(LogisticsWaybill.create_time.desc())
        .limit(limit)
    )
...
async def _biz_sender_addresses(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """寄件人使用过的收件地址."""
    stmt = (
        select(
            LogisticsAddress.address_id,
            LogisticsAddress.province,
            LogisticsAddress.city,
            LogisticsAddress.district,
            LogisticsAddress.street_address,
            LogisticsAddress.is_remote,
            LogisticsAddress.is_temp,
            LogisticsAddress.use_count,
        )
        .select_from(LogisticsWaybill)
        .join(LogisticsAddress, LogisticsAddress.address_id == LogisticsWaybill.address_id)
        .where(LogisticsWaybill.sender_id == user_id)
...
async def _biz_customs_info(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """跨境报关信息."""
    stmt = (
        select(
            LogisticsCustomsInfo.customs_id,
            LogisticsCustomsInfo.waybill_no,
            LogisticsCustomsInfo.trade_mode,
            LogisticsCustomsInfo.hs_code,
            LogisticsCustomsInfo.declared_value,
            LogisticsCustomsInfo.currency,
            LogisticsCustomsInfo.destination_country,
            LogisticsCustomsInfo.customs_status,
            LogisticsCustomsInfo.clear_time,
        )
        .select_from(LogisticsCustomsInfo)
        .join(LogisticsWaybill, LogisticsWaybill.waybill_no == LogisticsCustomsInfo.waybill_no)
        .where(
            LogisticsWaybill.waybill_no == waybill_no
            if waybill_no else LogisticsWaybill.sender_id == user_id
        )
...
async def _biz_cod_settlements(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """代收货款结算记录."""
    stmt = (
        select(
            LogisticsCodSettlement.cod_id,
            LogisticsCodSettlement.waybill_no,
            LogisticsCodSettlement.cod_amount,
            LogisticsCodSettlement.collect_status,
            LogisticsCodSettlement.collect_time,
            LogisticsCodSettlement.reject_time,
            LogisticsCodSettlement.settle_time,
        )
        ...
        .order_by(LogisticsCodSettlement.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_claims(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """理赔记录."""
    stmt = (
        select(
            LogisticsClaim.claim_id,
            LogisticsClaim.waybill_no,
            LogisticsClaim.claim_type,
            LogisticsClaim.claim_amount,
            LogisticsClaim.claim_status,
            LogisticsClaim.apply_time,
        )
        .select_from(LogisticsClaim)
        .join(LogisticsWaybill, LogisticsWaybill.waybill_no == LogisticsClaim.waybill_no)
        .where(
            LogisticsWaybill.waybill_no == waybill_no
            if waybill_no else LogisticsWaybill.sender_id == user_id
        )
        .order_by(LogisticsClaim.apply_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_complaints(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """投诉记录."""
    stmt = (
        select(
            LogisticsComplaint.complaint_id,
            LogisticsComplaint.waybill_no,
            LogisticsComplaint.complaint_type,
            LogisticsComplaint.complaint_content,
            LogisticsComplaint.complaint_status,
            LogisticsComplaint.create_time,
        )
        .select_from(LogisticsComplaint)
        .join(LogisticsWaybill, LogisticsWaybill.waybill_no == LogisticsComplaint.waybill_no)
        .where(
            LogisticsWaybill.waybill_no == waybill_no
            if waybill_no else LogisticsWaybill.sender_id == user_id
        )
        .order_by(LogisticsComplaint.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_abnormal_records(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """异常件记录."""
    stmt = (
        select(
            LogisticsAbnormalRecord.abnormal_id,
            LogisticsAbnormalRecord.waybill_no,
            LogisticsAbnormalRecord.abnormal_type,
            LogisticsAbnormalRecord.abnormal_time,
            LogisticsAbnormalRecord.description,
            LogisticsAbnormalRecord.handle_status,
        )
        .select_from(LogisticsAbnormalRecord)
        .join(LogisticsWaybill, LogisticsWaybill.waybill_no == LogisticsAbnormalRecord.waybill_no)
        .where(
            LogisticsWaybill.waybill_no == waybill_no
            if waybill_no else LogisticsWaybill.sender_id == user_id
        )
        .order_by(LogisticsAbnormalRecord.abnormal_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_high_value_waybills(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """高申报价值运单 (>= 1000)."""
    stmt = (
        select(
            LogisticsWaybill.waybill_no,
        ...
        .where(
            LogisticsWaybill.declared_value >= 1000,
            LogisticsWaybill.sender_id == user_id if user_id else True,
        )
        .order_by(LogisticsWaybill.declared_value.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_recent_waybills(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """系统最近运单 (管理员视角)."""
    stmt = (
        select(
            LogisticsWaybill.waybill_no,
            LogisticsWaybill.sender_id,
            LogisticsWaybill.recipient_id,
            LogisticsWaybill.declared_value,
            LogisticsWaybill.status,
            LogisticsWaybill.create_time,
        )
...
async def _biz_recipient_info(db: AsyncSession, user_id: str, waybill_no: str, limit: int) -> list[Any]:
    """收件人信息 (waybill_no 传收件人ID)."""
    stmt = select(
        LogisticsRecipient.recipient_id,
        LogisticsRecipient.recipient_name,
        LogisticsRecipient.recipient_phone,
        LogisticsRecipient.is_real_name_verified,
        LogisticsRecipient.verify_fail_count,
    ).where(LogisticsRecipient.recipient_id == waybill_no).limit(limit)
    return list((await db.execute(stmt)).all())


# 业务查询 dispatch table
_BIZ_QUERY_HANDLERS = {
    "sender_waybills": _biz_sender_waybills,
    "waybill_detail": _biz_waybill_detail,
    "sender_addresses": _biz_sender_addresses,
    "customs_info": _biz_customs_info,
    "cod_settlements": _biz_cod_settlements,
    "claims": _biz_claims,
    "complaints": _biz_complaints,
    "abnormal_records": _biz_abnormal_records,
    "high_value_waybills": _biz_high_value_waybills,
    "recent_waybills": _biz_recent_waybills,
    "recipient_info": _biz_recipient_info,
}


async def _query_business_data_impl(
    *, db: AsyncSession, query_type: str, user_id: str, waybill_no: str, limit: int,
) -> str:
    """业务数据查询统一入口 (字典派发 11 种 query_type)."""
    handler = _BIZ_QUERY_HANDLERS.get(query_type)
    if not handler:
        return f"不支持的查询类型: {query_type}, 可选: {', '.join(_BIZ_QUERY_HANDLERS)}"
    rows = await handler(db, user_id, waybill_no, limit)
    return json.dumps(
...
@tool(
    description=(
        "对指定寄件人和物流事件执行实时风险检查。"
        "参数: user_id (寄件人ID, 如 'SND001')、event_type (事件类型, 可选值: 寄件下单/揽收/中转/派送/签收/拒收/退回/投诉/理赔申请/异常上报/报关清关/海外仓/COD结算/结汇退税/实名认证)、source_id (关联业务ID, 如运单号/投诉ID/理赔ID/COD单号)。"
...
@tool(
    description=(
        ...
        "返回: 寄件人的风险评分、寄件量、COD拒收率、地址异常、实名信息等画像数据; 没画像时实时算 25 维物流特征。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: sender_waybills/waybill_detail/sender_addresses/customs_info/cod_settlements/claims/complaints/abnormal_records/high_value_waybills/recent_waybills/recipient_info)、user_id (寄件人ID, 部分需要)、waybill_no (运单号, 部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
async def query_business_data(
    query_type: str, user_id: str = "", waybill_no: str = "", limit: int = 10,
) -> str:
    return await _safe_call("业务数据查询", _query_business_data_impl,
                            query_type=query_type, user_id=user_id, waybill_no=waybill_no, limit=limit)
...
if __name__ == "__main__":
    ...
    tools_list = [
        ("risk_check",            "对寄件人和物流事件执行实时风控检查"),
        ("query_cases",           "查询案件列表 (按 status)"),
        ("query_user_profile",    "查询寄件人风险画像"),
        ...
        ("query_business_data",   "物流业务数据查询 (11 query_type)"),
    ]
    ...
    print(f"  _BIZ_QUERY_HANDLERS = {list(_BIZ_QUERY_HANDLERS.keys())}  (11 个 query_type)")
    ...
    impls = [
        ...
        _biz_sender_waybills, _biz_waybill_detail, _biz_sender_addresses,
        _biz_customs_info, _biz_cod_settlements, _biz_claims,
        _biz_complaints, _biz_abnormal_records, _biz_high_value_waybills,
        _biz_recent_waybills, _biz_recipient_info, _query_business_data_impl,
    ]
    print(f"  共 {len(impls)} 个 _impl (其中 4 个黑名单 + 11 个业务查询 + 4 个统计)")
