...
#   原因: gen_logistics_train_dataset.py 造训练数据时强制 ml_score=NULL (避免"未训练模型"垃圾值),
        ...
        with conn.cursor() as cur:
            ...
            if len(rows) < settings.XGB_TRAIN_DATA_LIMIT * 0.8:
                logger.warning(
                    ...
                    "建议先 init_db.py --reset --yes 清空, 再 gen_logistics_data.py + gen_logistics_risk_data.py 重新造",
            ...
            if span_days is not None and span_days < 1 and db_total > 100:
                logger.warning(
                    ...
                    "建议: gen_logistics_data.py --count 200 --clean + gen_logistics_risk_data.py --count 500 --clean-ml",
            ...
            logger.info("拉取特征: %d 个 event 各 30 维", len(event_to_features))
            # 2. 拼训练矩阵 (只保留 30 维特征齐全的样本)
            X_list, y_list = [], []
            missing = 0
            for eid, label in event_to_label.items():
                feats = event_to_features.get(eid)
                if not feats or len(feats) < 30:
                    missing += 1
                    continue
...
def main() -> None:
    ...
    if "val_accuracy" in metrics:
        ...
        if warnings:
            ...
            logger.warning("  建议: 1) python scripts/gen_logistics_risk_data.py --count 500 --clean-ml 重造评估数据")
            logger.warning("        2) 检查 feature.py 30 维是否真的有区分度 (y=1 跟 y=0 特征分布差多少)")
