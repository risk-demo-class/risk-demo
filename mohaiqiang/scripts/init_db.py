...
按顺序执行所有 SQL 脚本: 创建数据库 → 风控表 → 风控规则 → 物流业务表

总表数: 12 物流业务 + 9 风控 = 21 张
...
SQL_FILES = [
    ...
    ("init_risk_data.sql", "导入 24 条物流风控规则 (R001-R024)"),
    ("init_logistics_tables.sql", "创建 12 张物流风控业务表"),
]
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 21 张表重建 (9 风控 + 12 物流业务) + 24 条物流规则全部就绪")
    else:
