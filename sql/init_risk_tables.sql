...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('串货风险','订货欺诈','保修滥用','维修欺诈','经销商资质','区域风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('经销商订货','采购订单','保修申请','售后维修','串货举报','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('经销商订货','采购订单','保修申请','售后维修','串货举报') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('经销商订货','采购订单','保修申请','售后维修','串货举报') DEFAULT NULL COMMENT '触发案件的事件类型',
