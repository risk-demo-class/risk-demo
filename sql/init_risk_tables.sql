...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('寄件欺诈','实名风险','危险品瞒报','跨境异常','到付拒收','地址风险','投诉滥用','综合风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('寄件下单','到付签收','跨境申报','投诉申诉','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('寄件下单','到付签收','跨境申报','投诉申诉') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('寄件下单','到付签收','跨境申报','投诉申诉') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','身份证号','手机号','地址','运单号','设备指纹') NOT NULL COMMENT '黑名单类型',
