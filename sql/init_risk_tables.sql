...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('信贷欺诈','账户风险','支付风险','设备风险','关联风险','贷后风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('贷款申请','放款','还款','转账','登录','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
    ...
    `priority` INT NOT NULL DEFAULT 0 COMMENT '优先级(越高越先执行)',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('贷款申请','放款','还款','转账','登录') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('贷款申请','放款','还款','转账','登录') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','身份证号','手机号','银行卡号','设备指纹','IP','对公账户','统一社会信用代码') NOT NULL COMMENT '黑名单类型',
...
CREATE TABLE IF NOT EXISTS `risk_user_profile` (
    ...
    `risk_level` ENUM('低','中','高','极高') NOT NULL DEFAULT '低' COMMENT '风险等级',
    `total_orders` INT NOT NULL DEFAULT 0 COMMENT '贷款申请笔数(银行语义)',
    `total_refunds` INT NOT NULL DEFAULT 0 COMMENT '当前逾期期数(银行语义)',
    `refund_rate` DECIMAL(5,4) NOT NULL DEFAULT 0 COMMENT '逾期风险度(银行语义, overdue/(overdue+1))',
    `avg_order_amount` DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '平均申请金额(银行语义)',
    `address_count` INT NOT NULL DEFAULT 0 COMMENT '关联关系数(银行语义)',
    `complaint_count` INT NOT NULL DEFAULT 0 COMMENT '征信近24月逾期次数(银行语义)',
...
CREATE TABLE IF NOT EXISTS `risk_alert` (
    ...
    `status` ENUM('PENDING','HANDLING','RESOLVED','IGNORED') NOT NULL DEFAULT 'PENDING' COMMENT '处理状态',
