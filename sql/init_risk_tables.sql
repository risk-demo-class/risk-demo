...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('欺诈风险','信用风险','反洗钱','账户风险','贷后风险','合规风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('贷款申请','放款','还款','客户投诉','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('贷款申请','放款','还款','客户投诉') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('客户','申请','设备') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('客户','手机号','地址','设备') NOT NULL COMMENT '黑名单类型',
...
CREATE TABLE IF NOT EXISTS `risk_user_profile` (
    ...
    `total_loans` INT DEFAULT 0 COMMENT '总申请数',
    `overdue_count` INT DEFAULT 0 COMMENT '逾期次数',
    `overdue_rate` DECIMAL(5,4) DEFAULT 0 COMMENT '逾期率',
    `avg_loan_amount` DECIMAL(10,2) DEFAULT 0 COMMENT '平均申请金额',
    `contact_count` INT DEFAULT 0 COMMENT '联系信息数',
