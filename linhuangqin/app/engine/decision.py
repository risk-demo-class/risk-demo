...
from app.models import (
    RiskAssessment, RiskCase, RiskEvent, RiskFeature, RiskUserProfile, WaybillInfo,
)
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    """步骤 1b: 请求 → Context, 顺便补全 waybill_id.

    物流三段式规则:
      - 卖家下单寄件 / 揽收入仓 / 出仓发货 / 运输中 / 派送中 / 买家签收 / 买家拒收 / 买家退回寄件:
          source_id 就是 waybill_id
      - 验货完成 / 退货入仓验货: source_id 是 inspection_record.record_id, waybill_id 要从 request 传
    """
...
async def _enrich_consignee(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 从运单里补全 consignee_id (买家 ID)."""
    if ctx.consignee_id or not ctx.waybill_id:
        return
    row = (await db.execute(
        select(WaybillInfo.consignee_id).where(WaybillInfo.waybill_id == ctx.waybill_id)
    )).first()
    if row:
        ctx.consignee_id = row.consignee_id
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算物流风控特征 (卖家/运单/买家/验货/轨迹/逆向)"""
    return await compute_all_features(
        db,
        shipper_id=ctx.user_id,
        consignee_id=ctx.consignee_id,
        waybill_id=ctx.waybill_id,
    )


def _classify_feature_entity(feature_name: str) -> tuple[str, str]:
    """特征名前缀 → 实体类型. shipper_* → 卖家, buyer_* → 买家, 其余 → 运单"""
    if feature_name.startswith("shipper_"):
        return "卖家", feature_name
    if feature_name.startswith("buyer_"):
        return "买家", feature_name
...
def _save_feature_snapshot(
    ...
    entity_id 填充规则 (按 entity_type):
      - 卖家特征 → ctx.user_id (shipper_id)
      - 买家特征 → ctx.consignee_id (缺失用 source_id 顶替)
      - 运单特征 → ctx.waybill_id (缺失用 source_id 顶替)
    """
    for fname, fval in features.items():
        ...
        if entity_type == "卖家":
            ...
            entity_id = ctx.consignee_id or ctx.request.source_id
        else:  # 运单
            entity_id = ctx.waybill_id or ctx.request.source_id
        db.add(RiskFeature(
            event_id=ctx.event_id,
...
def _calculate_decision(
    ...
    # 模型未加载 → ml_score=None (落库 NULL, 训练 SQL 用 ml_score IS NULL 过滤干净数据)
    # 返回 rule_score 用于前端显示各部分拆解
    return final_score, risk_level, decision, ml_result.score if ml_result else None, ml_decision, rule_score
...
async def _update_user_profile(
    ...
    profile.total_refunds = int(features.get("shipper_dispute_count", 0))
    # 空包率 (替代电商退款率)
    profile.refund_rate = float(features.get("shipper_empty_pkg_rate", 0))
    profile.avg_order_amount = float(features.get("declared_value", 0))
    profile.address_count = int(features.get("shipper_imei_dup_count", 0))
    profile.complaint_count = int(features.get("shipper_complaint_count", 0))
...
def _build_response(
    ...
    return RiskCheckResponse(
        ...
        ml_weight_rule=ml_weight_rule,
        ml_weight_xgb=ml_weight_xgb,
    )
...
async def run_risk_check(
    ...
    await _enrich_consignee(db, ctx)
    ...
    # 6. 算决策 (含一票否决 + 双轨融合)
    final_score, risk_level, decision, ml_score, ml_decision, rule_score = _calculate_decision(rules, features)
