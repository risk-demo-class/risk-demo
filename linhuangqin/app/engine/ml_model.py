...
【重要约定】
  - 特征顺序固定 (FEATURE_COLUMNS), 跟 feature.py 算出的 38 个 key 一一对应
...
# 固定 38 维物流特征顺序 (训练和推理都用这个顺序, 防 dict 顺序不一致导致特征错位)
# 改这个列表前必须同步: feature.py::compute_*_features / tests/test_xgboost.py
# 对不齐会让 XGBoost 训练/推理退化成 0 填充, 业务上相当于模型没工作
FEATURE_COLUMNS: list[str] = [
    # 卖家维度 (9)
    "shipper_waybills_7d",
    "shipper_waybills_30d",
    "shipper_waybills_total",
    "shipper_empty_pkg_rate",
    "shipper_night_rate",
    "shipper_imei_dup_count",
    "shipper_complaint_count",
    "shipper_dispute_count",
    "shipper_is_verified",
    # 运单/货物维度 (13)
    "cargo_weight",
    "cargo_volume",
    ...
    # 买家维度 (3)
    "buyer_return_rate",
    "buyer_high_value_return_rate",
    "buyer_addr_dispute_rate",
    # 验货维度 (5)
    "inspect_duration_min",
    "inspect_grade_result",
    "inspect_functional_abnormal",
    "grade_diff",
    "inspector_grade_diff_rate",
    # 轨迹/逆向维度 (8)
    "track_completeness",
    "track_time_anomaly",
    "track_weight_deviation",
    "return_weight_diff",
    "return_imei_match",
    "return_grade_drop",
...
assert len(FEATURE_COLUMNS) == 38, f"物流特征数量必须是 38, 当前 {len(FEATURE_COLUMNS)}"
