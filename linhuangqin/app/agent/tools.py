...
async def _query_user_profile_impl(*, db: AsyncSession, user_id: str) -> str:
    """查询卖家风险画像. 没画像时实时算卖家特征 (不写库)."""
    profile = await get_user_profile(db, user_id)
    if not profile:
        features = await compute_all_features(db, shipper_id=user_id)
        return json.dumps({
            "shipper_id": user_id,
            "has_profile": False,
...
async def _blacklist_add_impl(
    *, db: AsyncSession, blacklist_type: Literal["卖家","手机号","地址"], value: str, reason: str,
) -> str:
...
async def _manage_blacklist_impl(
    *, db: AsyncSession, action: str, blacklist_type: Literal["卖家","手机号","地址"], value: str, reason: str,
) -> str:
...
async def _biz_user_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """卖家近期的运单 (含声明价值/保价/COD)."""
    stmt = (
        select(
            WaybillInfo.waybill_id,
            WaybillInfo.create_time,
            WaybillInfo.waybill_status,
            WaybillInfo.declared_value.label("total_amount"),
            WaybillInfo.cargo_category,
            WaybillInfo.cargo_weight,
        )
        .where(WaybillInfo.shipper_id == user_id)
        .order_by(WaybillInfo.create_time.desc())
        .limit(limit)
    )
...
async def _biz_user_postsales(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            ComplaintClaim.claim_id,
            ComplaintClaim.create_time,
            ComplaintClaim.claim_type,
            ComplaintClaim.claim_reason,
            ComplaintClaim.claim_status,
            ComplaintClaim.claim_amount,
        )
        .select_from(ComplaintClaim)
        .join(WaybillInfo, ComplaintClaim.waybill_id == WaybillInfo.waybill_id)
        .where(WaybillInfo.shipper_id == user_id)
        .order_by(ComplaintClaim.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_user_addresses(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            ShipperInfo.shipper_id,
            ShipperInfo.shipper_name,
            ShipperInfo.shipper_phone,
            ShipperInfo.shipper_province,
            ShipperInfo.shipper_city,
            ShipperInfo.shipper_district,
        )
        .where(ShipperInfo.shipper_id == user_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_order_detail(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            WaybillDetail.detail_id,
            WaybillDetail.item_imei,
            WaybillDetail.cargo_name,
            WaybillDetail.item_brand,
            WaybillDetail.item_model,
            WaybillDetail.item_grade,
            WaybillDetail.declared_value,
        )
        .where(WaybillDetail.waybill_id == order_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_high_value_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        ...
        .where(WaybillInfo.declared_value >= 5000)
        .order_by(WaybillInfo.declared_value.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (卖家ID, 如 'SHP0001')、event_type (事件类型, 可选值: 卖家下单寄件/揽收入仓/验货完成/出仓发货/运输中/派送中/买家签收/买家拒收/买家退回寄件/退货入仓验货)、source_id (关联业务ID, 如运单号)。"
...
@tool(
    description=(
        "查询卖家的风险画像信息。"
"参数: user_id (卖家ID, 如 'SHP0001')。"
"返回: 卖家的风险评分、运单统计、空包率、纠纷数等画像数据; 没画像时实时算卖家特征。"
    )
)
...
async def manage_blacklist(
    action: str, blacklist_type: str = "卖家", value: str = "", reason: str = "",
) -> str:
