"""
二手交易平台物流侧风控系统 - 一键数据库初始化脚本 (异步)
按顺序执行所有 SQL 脚本: 创建数据库 → 业务表 → 业务数据 → 风控表 → 风控规则

总表数: 11 业务 + 9 风控 = 20 张 (2026-08-11 含 P4 2 张: risk_action_log + risk_alert)
...
async def main():
    parser = argparse.ArgumentParser(
        description="二手交易平台物流侧风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 20 张表重建 + 业务数据 + 30 条规则全部就绪")
    else:
