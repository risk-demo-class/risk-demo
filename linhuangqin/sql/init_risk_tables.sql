-- ============================================
-- 二手交易平台物流侧风控系统 - 风控表 DDL 初始化脚本
-- 创建 9 张风控表 (7 原 + 2 P4 系统管理表)
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('空包虚假发货','物流调包','验货质检','退回假货','恶意退货','轨迹造假','签收风险','刷单刷信誉','寄件信息异常','费用争议') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('卖家下单寄件','揽收入仓','验货完成','出仓发货','运输中','派送中','买家签收','买家拒收','买家退回寄件','退货入仓验货','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('卖家下单寄件','揽收入仓','验货完成','出仓发货','运输中','派送中','买家签收','买家拒收','买家退回寄件','退货入仓验货') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('卖家','买家','运单') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('卖家下单寄件','揽收入仓','验货完成','出仓发货','运输中','派送中','买家签收','买家拒收','买家退回寄件','退货入仓验货') DEFAULT NULL COMMENT '触发案件的事件类型',
