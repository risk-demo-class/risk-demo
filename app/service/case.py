...
from app.config import CORE_BLACKLIST_TYPE, EXTRA_BLACKLIST_TYPES
...
async def check_blacklist_extra(db: AsyncSession, blacklist_type: str, value: str) -> bool:
    """行业黑名单扩展表检查 (经销商/设备SN/维修工), 过期自动失效."""
    bl = (await db.execute(
    ...
    if not bl:
        return False
    if bl.expire_at and bl.expire_at < datetime.now():
        return False
    return True


def _extra_to_response(b: BlacklistExtra) -> BlacklistResponse:
    """行业黑名单 ORM → 统一响应模型."""
    return BlacklistResponse(
        blacklist_id=b.entry_id,
        blacklist_type=b.type,
...
async def add_blacklist(db: AsyncSession, request: BlacklistCreate) -> BlacklistResponse:
    # 行业黑名单类型 (经销商/设备SN/维修工) → 业务扩展表 blacklist_extra
    if request.blacklist_type in EXTRA_BLACKLIST_TYPES:
        existing = (await db.execute(
            select(BlacklistExtra).where(
                BlacklistExtra.type == request.blacklist_type,
                BlacklistExtra.value == request.blacklist_value,
            )
        )).scalar_one_or_none()
        if existing:
            existing.reason = request.reason
            existing.expire_at = request.expire_time
            return _extra_to_response(existing)
        bl = BlacklistExtra(
            type=request.blacklist_type,
            value=request.blacklist_value,
        ...
        await record_action(
            ...
            target_type="blacklist", target_id=str(bl.entry_id),
            before_value=None,
            after_value={
                "blacklist_type": bl.type,
                "blacklist_value": bl.value,
                "reason": bl.reason,
                "expire_time": str(bl.expire_at) if bl.expire_at else None,
            },
            remark=f"新增行业黑名单 {bl.type}={bl.value}",
        )
        return _extra_to_response(bl)

    # 用户黑名单 → 核心表 risk_blacklist (完全复用基线逻辑)
...
async def get_blacklist(
    ...
    """分页查询黑名单 (核心表 用户 + 行业扩展表 经销商/设备SN/维修工 合并)."""
    ...
    extra_total = int((await db.execute(
        select(func.count()).select_from(BlacklistExtra)
    )).scalar() or 0)
    extra_items = (await db.execute(
        select(BlacklistExtra).order_by(BlacklistExtra.create_time.desc())
    )).scalars().all()
    ...
    merged = [
        ...
        for b in core_items
    ] + [_extra_to_response(b) for b in extra_items]
    merged.sort(key=lambda x: x.create_time or datetime.min, reverse=True)
...
# 移除: 用户黑名单软删 (核心表), 行业黑名单硬删 (扩展表无审计字段, 直接删)
async def remove_blacklist(db: AsyncSession, blacklist_id: int) -> bool:
    ...
    if bl:
        await record_action(
            ...
            target_type="blacklist", target_id=str(bl.blacklist_id),
            before_value={
                "blacklist_type": bl.blacklist_type,
                "blacklist_value": bl.blacklist_value,
                "reason": bl.reason,
    ...
    extra = (await db.execute(
        select(BlacklistExtra).where(BlacklistExtra.entry_id == blacklist_id)
    )).scalar_one_or_none()
    if extra:
        await record_action(
            db, operator="admin", action_type="REMOVE_BLACKLIST",
            target_type="blacklist", target_id=str(extra.entry_id),
            before_value={
                "blacklist_type": extra.type,
                "blacklist_value": extra.value,
                "reason": extra.reason,
            },
            after_value=None,
            remark=f"移除行业黑名单 {extra.type}={extra.value}",
        )
        await db.execute(
            BlacklistExtra.__table__.delete().where(BlacklistExtra.entry_id == blacklist_id)
        )
        await db.commit()
        return True
