...
def evaluate_condition(condition: dict, features: dict[str, float]) -> bool:
    ...
    条件格式:
      {"field": "user_overdue_plan_count", "op": ">=", "value": 1}  ← 单条件
      {"and": [条件1, 条件2]}                                      ← 逻辑组合
      {"or": [条件1, 条件2]}                                       ← 逻辑组合
    """
    if "and" in condition:
...
if __name__ == "__main__":
    ...
    test_cases = [
        # (描述, 条件, 特征, 期望)
        ("基础 >=",         {"field": "order_amount", "op": ">=", "value": 100000}, {"order_amount": 150000}, True),
        ("基础 ==",         {"field": "order_is_night", "op": "==", "value": 1},    {"order_is_night": 1},    True),
        ("基础 !=",         {"field": "user_total_applications", "op": "!=", "value": 0}, {"user_total_applications": 5}, True),
        ("AND 嵌套 (多头+征信)", {"and": [
            {"field": "user_applications_60d", "op": ">=", "value": 3},
            {"field": "order_hard_query_6m", "op": ">=", "value": 6},
            {"field": "order_five_level_risk", "op": ">=", "value": 0},
        ]}, {"user_applications_60d": 4, "order_hard_query_6m": 8, "order_five_level_risk": 0}, True),
        ("OR 嵌套",         {"or": [
            {"field": "user_applications_60d", "op": ">=", "value": 100},
            {"field": "user_applications_60d", "op": "<", "value": 1},
        ]}, {"user_applications_60d": 0}, True),
        ("AND 失败",        {"and": [
            {"field": "x", "op": ">", "value": 5},
            {"field": "y", "op": ">", "value": 5},
        ]}, {"x": 10, "y": 1}, False),
    ...
    rules = [
        _MockRule("R001", {"field": "order_amount", "op": ">=", "value": 100000}, 70, "高", "人工审核", 80),
        _MockRule("R002", {"field": "user_applications_60d", "op": ">=", "value": 3}, 95, "极高", "拒绝", 100),
        _MockRule("R003", {"field": "user_overdue_plan_count", "op": ">=", "value": 1}, 60, "高", "人工审核", 70),
    ]
    # 高风险用户特征: 大额申请 + 多头 + 有逾期
    feats = {"order_amount": 150000, "user_applications_60d": 4, "user_overdue_plan_count": 2}
