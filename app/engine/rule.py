...
def evaluate_condition(condition: dict, features: dict[str, float]) -> bool:
    ...
    条件格式:
      {"field": "user_cancel_appt_rate", "op": ">", "value": 0.5}    ← 单条件
      {"and": [条件1, 条件2]}                                      ← 逻辑组合
      {"or": [条件1, 条件2]}                                       ← 逻辑组合
    """
    if "and" in condition:
...
if __name__ == "__main__":
    ...
    test_cases = [
        # (描述, 条件, 特征, 期望)
        ("基础 >=",         {"field": "order_claim_amount", "op": ">=", "value": 5000}, {"order_claim_amount": 8000}, True),
        ("基础 ==",         {"field": "order_is_night", "op": "==", "value": 1},            {"order_is_night": 1},            True),
        ("基础 !=",         {"field": "user_total_visits", "op": "!=", "value": 0},     {"user_total_visits": 5},      True),
        ("AND 嵌套 (R007)", {"and": [
            {"field": "user_cancel_appt_rate", "op": ">=", "value": 0.5},
            {"field": "user_avg_claim_amount", "op": ">=", "value": 2000},
            {"field": "user_cross_hospital_count", "op": ">=", "value": 3},
        ]}, {"user_cancel_appt_rate": 0.6, "user_avg_claim_amount": 3000, "user_cross_hospital_count": 5}, True),
        ("OR 嵌套",         {"or": [
            {"field": "user_total_visits", "op": ">=", "value": 100},
            {"field": "user_total_visits", "op": "<", "value": 1},
        ]}, {"user_total_visits": 0}, True),
        ("AND 失败",        {"and": [
            {"field": "x", "op": ">", "value": 5},
            {"field": "y", "op": ">", "value": 5},
        ]}, {"x": 10, "y": 1}, False),
    ...
    rules = [
        _MockRule("R001", {"field": "order_claim_amount", "op": ">=", "value": 5000}, 70, "高", "人工审核", 80),
        _MockRule("R002", {"field": "order_claim_amount", "op": ">=", "value": 10000}, 95, "极高", "拒绝", 100),
        _MockRule("R003", {"field": "user_cancel_appt_rate", "op": ">=", "value": 0.5}, 60, "高", "人工审核", 70),
    ]
    # 高风险用户特征: 大额 + 高退款率
    feats = {"order_claim_amount": 15000, "user_cancel_appt_rate": 0.6, "user_total_visits": 5}
