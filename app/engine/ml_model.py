...
# 固定 25 维特征顺序 (制造业: 12 用户 + 10 订单 + 3 区域, 训练和推理都用这个顺序)
...
def load_model(model_path: Optional[str] = None) -> bool:
    ...
    try:
        ...
        # 特征名校验: 模型训练时的 25 维特征名必须跟 FEATURE_COLUMNS 一致,
        # 否则是旧行业/旧版本模型, 直接判定未加载, 避免每条推理都刷异常日志
        if _MODEL.feature_names and list(_MODEL.feature_names) != FEATURE_COLUMNS:
            logger.warning(
                "模型特征名与当前 FEATURE_COLUMNS 不一致 (旧行业模型?), 忽略: %s", path,
            )
            _MODEL = None
            _LOADED = False
            return False
