...
def check_veto(hits: list[RuleHitResult]) -> bool:
    ...
    当前项目"极高"级别规则: R001 (串货举报>=2) / R007 (30 天 30 单) /
    R008 (SN 90 天 2 次维修) / R015 (保修率 80%+).
    """
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则:
      - "经销商订货" / "采购订单" 事件: source_id 就是 order_id
      - "保修申请" / "售后维修" 事件: source_id 是 warranty_id, order_id 从工单反查
      - "串货举报" 事件: source_id 是 report_id, order_id 从举报单反查
    """
    order_id = request.order_id
    if not order_id and request.event_type in ("经销商订货", "采购订单"):
...
async def _enrich_order_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 保修/维修/串货事件从业务表反查 order_id, 供订单特征计算."""
    if ctx.order_id:
        return
    if ctx.request.event_type in ("保修申请", "售后维修"):
        row = (await db.execute(
            select(WarrantyRecord.order_id)
            .where(WarrantyRecord.warranty_id == ctx.request.source_id)
            .limit(1)
        )).first()
        if row:
            ctx.order_id = row.order_id
    elif ctx.request.event_type == "串货举报":
        row = (await db.execute(
            select(CrossRegionReport.order_id)
            .where(CrossRegionReport.report_id == int(ctx.request.source_id))
            .limit(1)
        )).first()
        if row:
            ctx.order_id = row.order_id
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算 25 个风控特征 (用户/订单/区域 3 类)"""
...
def _save_feature_snapshot(
    ...
    entity_id 填充规则 (按 entity_type):
      ...
      - 区域特征 → ctx.receive_id (缺失用 user_id 顶替, 制造业无 receive_id)
    """
...
async def _update_user_profile(
    ...
    # 制造业语义映射: 退款次数→保修次数, 退款率→保修率, 地址→发货区域, 投诉→被举报串货
    profile.total_refunds = int(features.get("user_warranty_count", 0))
    total_orders = features.get("user_total_orders", 0)
    profile.refund_rate = (
        round(features.get("user_warranty_rate", 0), 4)
        if total_orders > 0 else 0
    )
    profile.avg_order_amount = features.get("user_avg_order_amount", 0)
    profile.address_count = int(features.get("addr_region_total", 0))
    profile.complaint_count = int(features.get("user_report_count", 0))
