...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    教育业务约定:
      - "课程报名" 事件: source_id 就是 enrollment_id
      - "退费申请" 事件: service/event.py 已补全 enrollment_id
      - "直播打赏" 事件: 没有报名单，仅计算用户维度特征
    """
    order_id = request.order_id
    if not order_id and request.event_type == "课程报名":
