"""
风控决策引擎 (医疗版): 串联 7 步流水线, 事件 → 特征 → 规则 → 评分 → 决策 → 持久化.
流程与电商版完全一致 (不许改流程), 仅业务耦合点适配医疗:
  - order_id 语义 = 诊疗单据 ID (挂号/处方/结算/药品订单, 即 source_id)
  - receive_id 语义 = 医院编码 (hospital_id)
...
from app.models import (
    Appointment, DrugOrder, InsuranceClaim, Prescription,
...
def check_veto(hits: list[RuleHitResult]) -> bool:
    ...
    医疗版"极高"级别规则: R001 (医保卡盗刷) / R002 (医生统方) /
    R008 (处方超量) / R030 (黑医保卡).
    """
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则 (医疗版): 4 种事件 (挂号/处方开具/医保结算/药品下单) 的 source_id
    都是诊疗单据 ID, 直接作为 order_id (单据维度特征用);
    receive_id 语义为医院编码, 从 request.hospital_id 取.
    """
    return _RiskCheckContext(
        request=request,
        user_id=request.user_id,
        order_id=request.source_id,
        receive_id=request.hospital_id,
    )


async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 从诊疗单据里补全 receive_id (医院编码).

    退出条件: 已有 receive_id. 依次查 挂号→处方→结算; 药品订单走关联处方间接查.
    """
    ...
    for model, pk_col in (
        ...
        row = (await db.execute(
            select(model.hospital_id).where(pk_col == ctx.order_id)
        )).first()
        if row:
            ctx.receive_id = row.hospital_id
            return
    ...
    row = (await db.execute(
        ...
        .where(DrugOrder.drug_order_id == ctx.order_id)
    )).first()
    if row:
        ctx.receive_id = row.hospital_id
...
def _save_feature_snapshot(
    ...
    entity_id 填充规则 (按 entity_type, 医疗语义):
      - 用户特征 (患者) → ctx.user_id
      - 订单特征 (诊疗单据) → ctx.order_id (缺失用 source_id 顶替)
      - 地址特征 (医疗机构) → ctx.receive_id (缺失用 user_id 顶替)
    """
...
def _calculate_decision(
    ...
    # 场景: R001 (医保卡盗刷) 命中 → rule_score=95, ML 给出 0.1 → 融合后 52.5
...
async def _update_user_profile(
    ...
    """步骤 7c: 更新患者风险画像 (upsert 模式: 有则更新, 无则插入).

    从 features 取 (复用刚才算好的 25 特征, 不再查 DB). 医疗语义映射:
      total_orders=总挂号数, total_refunds=取消挂号数, refund_rate=取消率,
      avg_order_amount=平均结算金额, address_count=就诊医院数,
      complaint_count=医保结算次数.
    """
    ...
    profile.total_orders = int(features.get("user_total_visits", 0))
    profile.total_refunds = int(features.get("user_cancel_count", 0))
    # 挂号取消率 = 取消次数 / 总挂号数, 防 0 除
    total_visits = features.get("user_total_visits", 0)
    profile.refund_rate = (
        round(features.get("user_cancel_count", 0) / total_visits, 4)
        if total_visits > 0 else 0
    )
    # 平均结算金额 = 结算总金额 / 结算次数, 防 0 除
    claim_count = features.get("user_claim_count", 0)
    profile.avg_order_amount = (
        round(features.get("user_claim_amount", 0) / claim_count, 2)
        if claim_count > 0 else 0
    )
    profile.address_count = int(features.get("user_hospital_count", 0))
    profile.complaint_count = int(features.get("user_claim_count", 0))
...
if __name__ == "__main__":
    ...
    cases3 = [
        ("R012 虚假病历 (无 veto)", [_mock_hit("R012", 70, "高")],     0.20, None),
        ("R001 医保卡盗刷 (有 veto)", [_mock_hit("R001", 95, "极高")], 0.05, None),
        ("R001 + ML 拉低 (有 veto)", [_mock_hit("R001", 95, "极高")], 0.05, None),
    ]
