...
def check_veto(hits: list[RuleHitResult]) -> bool:
    ...
    银行版"极高"级别规则: R003 (收入申报显著虚高) / R006 (设备多人共用) / R009 (征信五级分类异常).
    """
...
class _RiskCheckContext:
    ...
    user_id: str
    entity_id: Optional[str] = None     # 事件源实体: application_id / contract_id / txn_id / login_id
    device_id: Optional[str] = None     # 事件设备 (特征/黑名单用)
    ip: Optional[str] = None            # 事件 IP
    event_id: str = ""


def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    """步骤 1b: 请求 → Context. 银行版: source_id 就是事件源实体 ID."""
...
async def _enrich_context(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 按 event_type 补全事件设备/IP (特征与黑名单检查用)."""
    if ctx.request.event_type == "贷款申请":
        row = (await db.execute(
            select(LoanApplication.device_id, LoanApplication.ip)
            .where(LoanApplication.application_id == ctx.entity_id).limit(1)
        )).first()
        if row:
            ctx.device_id, ctx.ip = row.device_id, row.ip
    elif ctx.request.event_type == "放款":
        row = (await db.execute(
            select(LoanContract.user_id).where(LoanContract.contract_id == ctx.entity_id).limit(1)
        )).first()
        # 放款无设备/IP, 特征层兜底用借款人最近设备
        _ = row
    elif ctx.request.event_type == "转账":
        row = (await db.execute(
            select(Transaction.device_id, Transaction.ip)
            .where(Transaction.txn_id == ctx.entity_id).limit(1)
        )).first()
        if row:
            ctx.device_id, ctx.ip = row.device_id, row.ip
    elif ctx.request.event_type == "登录":
        row = (await db.execute(
            select(LoginLog.device_id, LoginLog.ip)
            .where(LoginLog.login_id == ctx.entity_id).limit(1)
        )).first()
        if row:
            ctx.device_id, ctx.ip = row.device_id, row.ip
    # "还款" 事件无设备/IP, 特征层兜底
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算 25 个风控特征 (借款人/事件/设备 3 类)"""
...
def _classify_feature_entity(feature_name: str) -> tuple[str, str]:
    """特征名前缀 → 实体类型. user_* → 用户, order_* → 订单(事件), 其他 → 地址(设备/IP)"""
...
def _save_feature_snapshot(
    ...
    entity_id 填充规则 (按 entity_type):
      - 用户特征 → ctx.user_id
      - 订单特征 → ctx.entity_id (事件源实体, 缺失用 source_id 顶替)
      - 地址特征 → ctx.device_id (缺失用 user_id 顶替)
    """
    for fname, fval in features.items():
        ...
        if entity_type == "用户":
            ...
            entity_id = ctx.entity_id or ctx.request.source_id
        else:  # 地址
            entity_id = ctx.device_id or ctx.user_id
        db.add(RiskFeature(
            event_id=ctx.event_id,
...
async def _update_user_profile(
    ...
    # 银行版映射 (risk_user_profile 表结构不动, 语义按行业复用)
    total_apps = features.get("user_total_applications", 0)
    overdue_cnt = features.get("user_overdue_plan_count", 0)
    profile.total_orders = int(total_apps)
    profile.total_refunds = int(overdue_cnt)
    profile.refund_rate = round(overdue_cnt / max(overdue_cnt + 1, 1), 4)
    profile.avg_order_amount = features.get("user_avg_apply_amount", 0)
    profile.address_count = int(features.get("user_relation_count", 0))
    profile.complaint_count = int(features.get("user_overdue_24m_count", 0))
    profile.profile_data = json.dumps({
        "user_applications_60d": features.get("user_applications_60d", 0),
        "user_avg_debt_ratio": features.get("user_avg_debt_ratio", 0),
        "user_income_gap_ratio": features.get("user_income_gap_ratio", 0),
        "user_hard_query_6m": features.get("user_hard_query_6m", 0),
        "user_credit_score": features.get("user_credit_score", 0),
        "user_account_age_days": features.get("user_account_age_days", 0),
        "user_relation_count": features.get("user_relation_count", 0),
    }, ensure_ascii=False)
...
if __name__ == "__main__":
    import sys
    # GBK 控制台无法打印 ✓/✗ 等字符, 显式切 UTF-8, 保证 demo 在 Windows 可跑
