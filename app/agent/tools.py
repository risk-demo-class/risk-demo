...
async def _blacklist_check_impl(
    *, db: AsyncSession, blacklist_type: str, value: str,
    ...
    if blacklist_type == "用户":
        ...
        is_blocked = await check_blacklist_extra(db, blacklist_type, value)
...
async def _blacklist_remove_impl(
    *, db: AsyncSession, blacklist_type: str, value: str,
    ...
    if extra:
        ...
        await db.commit()
        return f"已从行业黑名单移除: {blacklist_type}={value}"
    return f"未找到行业黑名单记录: {blacklist_type}={value}"
...
async def _biz_user_warranties(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """经销商的保修/维修工单 (JOIN order_info 反查 dealer)."""
    stmt = (
        select(
...
async def _biz_order_warranties(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """某订货单关联的保修/维修工单 (含产品信息)."""
    stmt = (
        select(
            WarrantyRecord.warranty_id,
            WarrantyRecord.product_sn,
...
async def _biz_high_value_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        ...
        .select_from(OrderInfo)
        .join(Product, OrderInfo.product_id == Product.product_id)
        .where(OrderInfo.total_amount >= 500000)
...
_BIZ_QUERY_HANDLERS = {
    ...
    "order_warranties": _biz_order_warranties,
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (经销商ID, 如 'D001'/'RISK001')、event_type (事件类型, 可选值: 经销商订货/采购订单/保修申请/售后维修/串货举报)、source_id (关联业务ID: 订货单号/保修工单号/举报单号)。"
...
@tool(
    description=(
        "查询用户的风险画像信息。"
        "参数: user_id (经销商ID, 如 'D001')。"
        "返回: 经销商的风险评分、订货统计、保修率、覆盖区域、被举报次数等画像数据; 没画像时实时算 user 特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/经销商/设备SN/维修工)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: user_orders 经销商订货单 / user_warranties 经销商保修工单 / dealer_regions 经销商发货区域 / order_warranties 订货单关联工单 / recent_orders 最近订货单 / high_value_orders 大额订货单)、user_id (部分需要)、order_id (部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
