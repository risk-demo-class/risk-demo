"""
AI Agent 工具集 (医疗版, 8 个 LangChain @tool).
...
async def _query_user_profile_impl(*, db: AsyncSession, user_id: str) -> str:
    """查询患者风险画像. 没画像时实时算 14 个 user 特征 (不写库)."""
...
# ============================================================
# 业务数据查询 (字典派发 6 种 query_type, 医疗版)
# ============================================================

async def _biz_user_appointments(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """患者的挂号记录 (LEFT JOIN 医院拿医院名)."""
    stmt = (
        select(
            Appointment.appt_id,
            ...
            Appointment.pay_amount,
            Appointment.appt_status,
            Appointment.create_time,
        )
        .select_from(Appointment)
        .outerjoin(Hospital, Appointment.hospital_id == Hospital.hospital_id)
        .where(Appointment.user_id == user_id)
        .order_by(Appointment.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_prescriptions(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """患者的处方记录 (LEFT JOIN 医生拿开方医生姓名)."""
    stmt = (
        select(
            Prescription.rx_id,
...
async def _biz_user_claims(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            ...
            InsuranceClaim.hospital_id,
            InsuranceClaim.total_amount,
            InsuranceClaim.insured_amount,
            InsuranceClaim.claim_status,
            InsuranceClaim.submit_at,
            InsuranceClaim.create_time,
        )
        .where(InsuranceClaim.user_id == user_id)
        .order_by(InsuranceClaim.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_drug_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """患者的药品订单 (代购识别: receiver_name ≠ 患者本人)."""
    stmt = (
        select(
            DrugOrder.drug_order_id,
            DrugOrder.rx_id,
            DrugOrder.drug_name,
            DrugOrder.quantity,
            DrugOrder.drug_category,
            DrugOrder.receiver_name,
            DrugOrder.total_amount,
            DrugOrder.create_time,
        )
        .where(DrugOrder.user_id == user_id)
        .order_by(DrugOrder.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_prescription_detail(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """处方详情 (order_id 传 rx_id, 含药品明细 JSON)."""
    stmt = (
        select(
            Prescription.rx_id,
            Prescription.doctor_id,
        ...
        .where(Prescription.rx_id == order_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_high_amount_claims(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """高额医保结算 (单笔总费用 >= 10000, 不限患者, 管理员视角).

    HAVING 在 GROUP BY 之后筛选聚合结果, 区别于 WHERE (筛行);
    结算表本身就是单笔记录, 这里保留 GROUP BY + HAVING 写法演示 SQL 能力.
    """
    stmt = (
        select(
            InsuranceClaim.claim_id,
            InsuranceClaim.user_id,
            InsuranceClaim.hospital_id,
            InsuranceClaim.total_amount,
            InsuranceClaim.insured_amount,
            InsuranceClaim.create_time,
        ...
        .having(InsuranceClaim.total_amount >= 10000)
        .order_by(InsuranceClaim.total_amount.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
# 业务查询 dispatch table
_BIZ_QUERY_HANDLERS = {
    "user_appointments": _biz_user_appointments,
    "user_prescriptions": _biz_user_prescriptions,
    "user_claims": _biz_user_claims,
    "user_drug_orders": _biz_user_drug_orders,
    "prescription_detail": _biz_prescription_detail,
    "high_amount_claims": _biz_high_amount_claims,
}
...
@tool(
    description=(
        "对指定患者和事件执行实时风险检查。"
        "参数: user_id (患者ID, 如 'P0001')、event_type (事件类型, 可选值: 挂号/处方开具/医保结算/药品下单)、source_id (关联业务ID, 如挂号单号/处方ID/结算单ID/药品订单ID)。"
...
@tool(
    description=(
        "查询患者的风险画像信息。"
        "参数: user_id (患者ID, 如 'P0001')。"
        "返回: 患者的风险评分、就诊统计、挂号取消率、结算次数等画像数据; 没画像时实时算 14 个 user 特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/医保卡号/身份证号/医生执业证/医院编码)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: user_appointments/user_prescriptions/user_claims/user_drug_orders/prescription_detail/high_amount_claims)、user_id (部分需要)、order_id (处方详情时传 rx_id)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
...
if __name__ == "__main__":
    ...
    impls = [
        ...
        _biz_user_appointments, _biz_user_prescriptions, _biz_user_claims,
        _biz_user_drug_orders, _biz_prescription_detail, _biz_high_amount_claims,
