...
async def _biz_user_shipments(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """用户的所有运单 (关联聚合总金额/保价费/物品分类数). LEFT JOIN 保没明细的运单金额显 0."""
    shipment_total_subq = (
        select(
            ShipmentItem.shipment_id,
            func.coalesce(func.sum(ShipmentItem.declared_value), 0).label("total_declared_value"),
            func.coalesce(func.sum(ShipmentItem.insurance_amount), 0).label("total_insurance"),
            func.count(ShipmentItem.item_id).label("item_count"),
            func.count(func.distinct(ShipmentItem.category_code)).label("category_count"),
        )
    ...
    stmt = (
        select(
            ...
            func.coalesce(shipment_total_subq.c.total_declared_value, 0).label("total_declared_value"),
            func.coalesce(shipment_total_subq.c.total_insurance, 0).label("total_insurance"),
            func.coalesce(shipment_total_subq.c.item_count, 0).label("item_count"),
            func.coalesce(shipment_total_subq.c.category_count, 0).label("category_count"),
        )
        .select_from(Shipment)
        .outerjoin(shipment_total_subq, Shipment.shipment_id == shipment_total_subq.c.shipment_id)
        .where(Shipment.sender_id == user_id)
        .order_by(Shipment.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_declarations(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """用户的跨境申报记录 (3 表 JOIN: declaration → shipment → user_info)."""
    stmt = (
        select(
            CustomsDeclaration.declaration_id,
            CustomsDeclaration.declare_time,
            CustomsDeclaration.declare_status,
            CustomsDeclaration.declared_total_value,
            CustomsDeclaration.destination_country,
            CustomsDeclaration.inspection_flag,
        )
        .select_from(CustomsDeclaration)
        .join(Shipment, CustomsDeclaration.shipment_id == Shipment.shipment_id)
        .where(Shipment.sender_id == user_id)
        .order_by(CustomsDeclaration.declare_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_addresses(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """用户的寄/收地址 (Address 表直接查)."""
    stmt = (
        select(
            Address.address_id,
            Address.address_tag,
        ...
        .order_by(Address.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_shipment_items(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """运单的明细行 (按运单号 source_id 查询)."""
    stmt = (
        select(
            ShipmentItem.item_id,
            ShipmentItem.category_code,
            ItemCategory.category_name,
            ShipmentItem.item_name,
            ShipmentItem.quantity,
            ShipmentItem.weight_kg,
            ShipmentItem.declared_value,
            ShipmentItem.insurance_amount,
            ShipmentItem.is_battery,
            ShipmentItem.is_liquid,
        )
        .select_from(ShipmentItem)
        .join(ItemCategory, ShipmentItem.category_code == ItemCategory.category_code)
        .where(ShipmentItem.shipment_id == source_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_recent_shipments(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """系统最近的运单 (不限 user, 管理员视角)."""
    shipment_total_subq = (
        select(
            ShipmentItem.shipment_id,
            func.coalesce(func.sum(ShipmentItem.declared_value), 0).label("total_declared_value"),
...
async def _biz_user_complaints(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    """用户的投诉申诉记录."""
    stmt = (
        select(
            ComplaintRecord.record_id,
            ComplaintRecord.complaint_type,
            ComplaintRecord.complaint_reason,
            ComplaintRecord.claim_amount,
            ComplaintRecord.handle_status,
            ComplaintRecord.handle_result,
            ComplaintRecord.create_time,
        )
        .where(ComplaintRecord.user_id == user_id)
        .order_by(ComplaintRecord.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
# 业务查询 dispatch table
_BIZ_QUERY_HANDLERS = {
    "user_shipments": _biz_user_shipments,
    "user_declarations": _biz_user_declarations,
    "user_addresses": _biz_user_addresses,
    "shipment_items": _biz_shipment_items,
    "recent_shipments": _biz_recent_shipments,
    "user_complaints": _biz_user_complaints,
}
...
async def _query_business_data_impl(
    *, db: AsyncSession, query_type: str, user_id: str, source_id: str, limit: int,
    ...
    rows = await handler(db, user_id, source_id, limit)
    return json.dumps(
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (用户ID, 如 '1001')、event_type (事件类型, 可选值: 寄件下单/到付签收/跨境申报/投诉申诉)、source_id (关联业务ID, 如运单号/申报号/投诉号)。"
...
@tool(
    description=(
        ...
        "返回: 用户的风险评分、运单统计、拒收率、地址数等画像数据; 没画像时实时算 14 个 user 特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/身份证号/手机号/地址/运单号/设备指纹)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: user_shipments/user_declarations/user_addresses/shipment_items/recent_shipments/user_complaints)、user_id (部分需要)、source_id (运单号等, 部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
