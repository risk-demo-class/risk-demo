...
async def _biz_user_applications(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户的贷款申请列表."""
    stmt = (
        select(
            LoanApplication.application_id,
            LoanApplication.apply_time,
            LoanApplication.apply_amount,
...
async def _biz_user_repayments(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户的还款流水."""
    stmt = (
        select(
            RepaymentRecord.record_id,
            RepaymentRecord.contract_id,
            RepaymentRecord.repay_amount,
            RepaymentRecord.principal_part,
            RepaymentRecord.interest_part,
            RepaymentRecord.channel,
            RepaymentRecord.repay_time,
        )
        .where(RepaymentRecord.user_id == user_id)
        .order_by(RepaymentRecord.repay_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_accounts(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户的银行账户 (储蓄卡/信用卡/对公)."""
    stmt = (
        select(
            BankAccount.account_id,
            BankAccount.account_no_tail,
            BankAccount.bank_name,
            BankAccount.account_type,
            BankAccount.card_status,
            BankAccount.credit_limit,
...
async def _biz_application_detail(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """贷款申请详情 (申请 + 征信快照)."""
    stmt = (
        select(
            LoanApplication.application_id,
            ...
            LoanApplication.term_months,
            LoanApplication.purpose,
            LoanApplication.guarantee_type,
            LoanApplication.repay_type,
            LoanApplication.monthly_income,
            LoanApplication.debt_ratio,
            LoanApplication.status,
            CreditReport.credit_score,
            CreditReport.recent_6m_hard_query_count,
            CreditReport.five_level_class,
            CreditReport.overdue_24m_count,
        )
        .outerjoin(CreditReport, CreditReport.application_id == LoanApplication.application_id)
        .where(LoanApplication.application_id == order_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_recent_applications(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """系统最近的贷款申请 (不限 user, 管理员视角)."""
    stmt = (
        select(
            LoanApplication.application_id,
...
async def _biz_high_amount_applications(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """大额贷款申请 (单笔 >= 50 万, 管理员视角)."""
    stmt = (
        select(
            LoanApplication.application_id,
            LoanApplication.user_id,
        ...
        .where(LoanApplication.apply_amount >= 500000)
        .order_by(LoanApplication.apply_amount.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
# 业务查询 dispatch table
_BIZ_QUERY_HANDLERS = {
    "user_applications": _biz_user_applications,
    "user_repayments": _biz_user_repayments,
    "user_accounts": _biz_user_accounts,
    "application_detail": _biz_application_detail,
    "recent_applications": _biz_recent_applications,
    "high_amount_applications": _biz_high_amount_applications,
}
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (借款人ID, 如 'U0001')、event_type (事件类型, 可选值: 贷款申请/放款/还款/转账/登录)、source_id (关联业务ID, 如申请ID LA0001)。"
...
@tool(
    description=(
        "查询用户的风险画像信息。"
        "参数: user_id (借款人ID, 如 'U0001')。"
        "返回: 用户的风险评分、申请统计、逾期、关联数等画像数据; 没画像时实时算 13 个 user 特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/身份证号/手机号/银行卡号/设备指纹/IP/统一社会信用代码)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: user_applications/user_repayments/user_accounts/application_detail/recent_applications/high_amount_applications)、user_id (部分需要)、order_id (部分需要, 如申请ID)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
...
if __name__ == "__main__":
    ...
    tools_list = [
        ...
        ("query_business_data",   "业务数据查询 (6 query_type: 申请/还款/账户/详情/最新/大额)"),
    ]
    ...
    impls = [
        ...
        _biz_user_applications, _biz_user_repayments, _biz_user_accounts,
        _biz_application_detail, _biz_recent_applications, _biz_high_amount_applications,
