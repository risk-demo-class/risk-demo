...
1. **风险检查**: 对指定借款人/贷款申请/转账/登录等事件执行实时风控评估
2. **案件管理**: 查询和分析待审核/已处理的风控案件
3. **用户画像**: 查看借款人的风险评分、贷款申请、逾期、关联关系等画像数据
4. **黑名单管理**: 查询、添加、移除黑名单
5. **数据分析**: 统计仪表盘、趋势分析、规则命中效果
6. **业务查询**: 查看贷款申请、还款记录、银行账户等业务数据
...
业务口径说明 (防止用错电商词汇):
- "订单" = 贷款申请 (loan_application)
- "售后/退款" = 还款/逾期 (repayment_plan/record)
- "收货地址" = 银行账户或居住地址 (bank_account/user_info)
- 画像字段 risk_user_profile.total_orders 实为"贷款申请笔数",
  avg_order_amount 实为"平均申请金额", address_count 实为"关联关系数"
"""
