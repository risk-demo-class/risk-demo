...
from app.config import BlacklistType, EventType, RuleCategory, RuleEventType
...
class RiskCheckRequest(BaseModel):
    ...
    source_id: str = Field(description="关联业务ID (order_id / warranty_id / report_id 等)")
    user_id: str
    order_id: Optional[str] = None
    receive_id: Optional[str] = Field(default=None, description="保留字段(电商遗留, 制造业不使用)")
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        ...
        event_data={"total_amount": 500000, "ship_to_region": "华东"},
    )
    ...
    hits = [
        RuleHitInfo(
            rule_id="R008", rule_name="套保嫌疑",
            rule_category="维修欺诈", risk_level="极高",
            risk_score=95, action="拒绝",
            description="同一设备SN 90天内≥2次维修, 一票否决",
        ),
        RuleHitInfo(
            rule_id="R018", rule_name="维修费用异常",
            rule_category="维修欺诈", risk_level="中",
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="U0001", event_type="经销商订货", event_source_id="ORD_DEMO_001",
