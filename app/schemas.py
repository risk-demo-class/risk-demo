...
class RiskCheckRequest(BaseModel):
    """风险检查请求 (银行业务版, 2026-08-11 Task 2)"""
    # 业务事件类型枚举: 与 app/config.py BUSINESS_EVENT_TYPES 对齐
    event_type: Literal["贷款申请", "放款", "还款", "转账", "登录"]
    source_id: str = Field(description="关联业务ID (application_id / contract_id / txn_id / login_id 等)")
    user_id: str
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["信贷欺诈", "账户风险", "支付风险", "设备风险", "关联风险", "贷后风险"]
    event_type: Literal["贷款申请", "放款", "还款", "转账", "登录", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["信贷欺诈", "账户风险", "支付风险", "设备风险", "关联风险", "贷后风险"]] = None
    event_type: Optional[Literal["贷款申请", "放款", "还款", "转账", "登录", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["用户", "身份证号", "手机号", "银行卡号", "设备指纹", "IP", "对公账户", "统一社会信用代码"]
    blacklist_value: str
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        event_type="贷款申请", source_id="LA0001", user_id="U0001",
        event_data={"apply_amount": 500000, "purpose": "消费"},
    )
