...
class Settings(BaseSettings):
    ...
    # ---- LLM 配置 (阿里云百炼) ----
    LLM_API_KEY: str = "sk-ws-H.ERXPPME.HmR4.MEYCIQCIWYcjibXNMnQtf231_FAfnOUaax3feV0kR1xzb_NFZAIhAK4VT4mAReNaMqbaar2pZuv54Qz0WE0xoJaoWtkitHSd"
...
# ============================================================
# 行业业务配置 (银行信贷风控, 2026-08-11, Task 2 数据层)
# 任务书要求: 黑名单 type 集合 + 业务事件类型枚举按行业自定
# 消费方: app/service/validator.py 派发表 / app/engine/rule.py 规则定义
#         app/engine/feature.py 特征计算 (Task 3)
# ============================================================

# 银行业务事件类型 (风险检查入口 RiskCheckRequest.event_type 的候选值)
BUSINESS_EVENT_TYPES = (
    "贷款申请",   # 贷前: 进件/反欺诈核查, source_id = loan_application.application_id
    "放款",       # 贷中: 合同放款/受托支付, source_id = loan_contract.contract_id
    "还款",       # 贷后: 还款/逾期, source_id = repayment_record.record_id
    "转账",       # 贷中/贷后: 资金流向监控, source_id = transaction.txn_id
    "登录",       # 账户行为: 设备/IP 风险, source_id = login_log.login_id
)

# 银行业务黑名单类型 (risk_blacklist.blacklist_type / blacklist_extra.type)
# 敏感值按 PII 合规存 SHA-256 哈希 (身份证号/银行卡号/设备指纹等)
...
if __name__ == "__main__":
    ...
    sections = [
        ...
        ("银行业务事件类型", list(BUSINESS_EVENT_TYPES)),
        ("银行业务黑名单类型", list(BUSINESS_BLACKLIST_TYPES)),
    ]
