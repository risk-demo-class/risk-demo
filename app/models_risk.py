"""
医疗风控系统 - 风控表 ORM (9 张)
风控系统自建表, 跟业务表分开管理. 表结构跟电商版 AI_Risk 完全复用,
仅枚举取值按医疗行业重做 (事件类型 / 规则分类 / 黑名单类型).
- 规则配置 (RiskRule)
- 事件审计 (RiskEvent / RiskFeature / RiskAssessment)
- 案件管理 (RiskCase)
- 黑名单 (RiskBlacklist)
- 用户画像 (RiskUserProfile)
- 系统管理 (RiskActionLog / RiskAlert)
"""
import json
...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("医保欺诈", "处方违规", "挂号黄牛", "药品代购", "账户风险", "机构风险",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("挂号", "处方开具", "医保结算", "药品下单", "通用", name="rule_event_type_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("挂号", "处方开具", "医保结算", "药品下单", name="event_type_enum"),
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        # 医疗语义映射: 用户=患者, 订单=诊疗单据(挂号/处方/结算/药品订单), 地址=医疗机构
...
class RiskCase(Base):
    ...
    # --- 业务回溯字段 (重做检查时使用) ---
    source_id: Mapped[Optional[str]] = mapped_column(String(50), comment="原始业务ID(挂号/处方/结算/药品订单ID)")
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("挂号", "处方开具", "医保结算", "药品下单", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        # 医疗行业黑名单类型: 用户=患者ID, 医保卡号/身份证号/医生执业证/医院编码为行业专属
        Enum("用户", "医保卡号", "身份证号", "医生执业证", "医院编码", name="blacklist_type_enum"),
...
class RiskUserProfile(Base):
    """患者风险画像. 表结构跟电商版完全复用, 医疗语义映射:
    total_orders=总就诊(挂号)数, total_refunds=取消挂号数, refund_rate=取消率,
    avg_order_amount=平均结算金额, address_count=就诊医院数, complaint_count=医保结算次数.
    """
    ...
    total_orders: Mapped[int] = mapped_column(Integer, default=0, comment="总就诊(挂号)数")
    total_refunds: Mapped[int] = mapped_column(Integer, default=0, comment="取消挂号次数")
    refund_rate: Mapped[Decimal] = mapped_column(Numeric(5, 4), default=0, comment="挂号取消率")
    avg_order_amount: Mapped[Decimal] = mapped_column(Numeric(10, 2), default=0, comment="平均结算金额")
    address_count: Mapped[int] = mapped_column(Integer, default=0, comment="就诊医院数量")
    complaint_count: Mapped[int] = mapped_column(Integer, default=0, comment="医保结算次数")
