...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("信贷欺诈", "账户风险", "支付风险", "设备风险", "关联风险", "贷后风险",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("贷款申请", "放款", "还款", "转账", "登录", "通用", name="rule_event_type_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("贷款申请", "放款", "还款", "转账", "登录", name="event_type_enum"),
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        ...
        nullable=False,
        comment="实体类型(银行语义: 用户=借款人, 订单=事件实体, 地址=设备/IP; 枚举为基线约束未改)",
    )
...
class RiskCase(Base):
    ...
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("贷款申请", "放款", "还款", "转账", "登录", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        Enum("用户", "身份证号", "手机号", "银行卡号", "设备指纹", "IP", "对公账户", "统一社会信用代码",
...
class RiskUserProfile(Base):
    ...
    total_orders: Mapped[int] = mapped_column(Integer, default=0, comment="贷款申请笔数(银行语义, 表结构复用基线字段)")
    total_refunds: Mapped[int] = mapped_column(Integer, default=0, comment="当前逾期期数(银行语义)")
    refund_rate: Mapped[Decimal] = mapped_column(Numeric(5, 4), default=0, comment="逾期风险度(银行语义, overdue/(overdue+1))")
    avg_order_amount: Mapped[Decimal] = mapped_column(Numeric(10, 2), default=0, comment="平均申请金额(银行语义)")
    address_count: Mapped[int] = mapped_column(Integer, default=0, comment="关联关系数(银行语义)")
    complaint_count: Mapped[int] = mapped_column(Integer, default=0, comment="征信近24月逾期次数(银行语义)")
