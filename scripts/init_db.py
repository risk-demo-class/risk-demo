"""
银行信贷风控系统 - 一键数据库初始化脚本 (异步)
按顺序执行所有 SQL 脚本: 创建数据库 → 业务表 → 业务数据 → 风控表 → 风控规则

总表数: 17 银行业务 + 9 风控 = 26 张 (2026-08-11 Task 2 换成银行业务表)
...
# 从 .env 加载默认连接配置 (与 app/config.py 等其他脚本一致)
load_dotenv(os.path.join(BASE_DIR, ".env"))

# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 17 张银行业务表"),
    ("init_business_data.sql", "导入业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 11 条银行业务规则 (R001-R011) + 黑名单"),
]

# 默认连接配置 (从 .env 加载, 与 app/config.py 一致)
DEFAULT_HOST = os.getenv("DB_HOST", "localhost")
DEFAULT_PORT = int(os.getenv("DB_PORT", 3306))
DEFAULT_USER = os.getenv("DB_USER", "root")
DEFAULT_PASSWORD = os.getenv("DB_PASSWORD", "123321")
DEFAULT_DB = os.getenv("DB_NAME", "ecs")
...
async def main():
    parser = argparse.ArgumentParser(
        description="银行信贷风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 26 张表重建 (17 银行业务 + 9 风控) + 业务数据 + 11 条规则全部就绪")
    else:
