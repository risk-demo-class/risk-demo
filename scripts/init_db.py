...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 8 张医疗业务表"),
    ("init_business_data.sql", "导入医疗业务种子数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (含 2 张系统管理表)"),
    ("init_risk_data.sql", "导入 30 条医疗预置风控规则 (R001-R030)"),
]
...
DEFAULT_PORT = 3306
DEFAULT_USER = "root"
DEFAULT_PASSWORD = "4498112"
DEFAULT_DB = "medical_risk"
...
用法:
  ...
  python scripts/init_db.py --db medical_risk_test # 初始化测试库
        """,
    )
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 17 张表重建 + 业务数据 + 30 条规则全部就绪")
    else:
