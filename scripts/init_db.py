...
总表数: 7 行业业务 + 9 风控 = 16 张 (2026-08-07 含 P4 2 张: risk_action_log + risk_alert)
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 7 张行业业务表 (制造业)"),
    ("init_business_data.sql", "导入业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 13 条预置风控规则 (制造业 R001-R026)"),
]
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 16 张表重建 + 业务数据 + 13 条规则全部就绪")
    else:
