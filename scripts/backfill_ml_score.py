        ...
        for i, (aid, eid, uid, dec) in enumerate(rows, 1):
            try:
                async with SessionLocal() as db:
                    # 找该 event 的 source_id + event_type, 反推 order_id (制造业)
                    ...
                    if not ev_row:
                        continue
                    src_id, ev_type = ev_row.event_source_id, ev_row.event_type
                    order_id = src_id if ev_type in ("经销商订货", "采购订单") else None
                    if order_id is None and ev_type in ("保修申请", "售后维修"):
                        order_id = (await db.execute(
                            __import__("sqlalchemy").text(
                                "SELECT order_id FROM warranty_record WHERE warranty_id = :wid LIMIT 1"
                            ),
                            {"wid": src_id},
                        )).scalar_one_or_none()
                    if order_id is None and ev_type == "串货举报":
                        order_id = (await db.execute(
                            __import__("sqlalchemy").text(
                                "SELECT order_id FROM cross_region_report WHERE report_id = :rid LIMIT 1"
                            ),
                            {"rid": int(src_id)},
                        )).scalar_one_or_none()
                    # 算 25 维特征
                    features = await compute_all_features(
                        db, uid, order_id=order_id,
                    )
                    # 推理
                    ml = predict(features)
                    if ml is None:
