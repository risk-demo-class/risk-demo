"""
银行信贷风控系统 - XGBoost 训练脚本 (一次性, 跑完即可)
数据源: MySQL risk_event + risk_feature + risk_assessment
步骤:
  1. 拉历史评估 (decision 不为空的, 排除掉 None)
  2. 每个 event_id JOIN 出 25 个银行特征 (宽表: pivot risk_feature)
...
def _load_data_from_mysql() -> tuple[np.ndarray, np.ndarray]:
    ...
    try:
        with conn.cursor() as cur:
            ...
            logger.info("拉取特征: %d 个 event 各 %d 维", len(event_to_features), len(FEATURE_COLUMNS))
            ...
            if len(y) > 0 and pos_ratio < 0.15:
                logger.warning(
                    ...
                    "模型学不到 '正例模式', 会假收敛. "
                    "建议: python scripts/gen_bank_risk_events.py --all 重跑事件数据",
