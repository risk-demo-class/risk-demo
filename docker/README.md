...
### 1. MySQL 8.0（`mysql`）
- 端口 `3306`（主机映射）
- 首次启动自动执行 `sql/init_all.sql` (建表 + 种子数据 + 30 条规则)
- 数据持久化到 `medical_risk_mysql_data` volume
...
- 日志挂载到 `medical_risk_app_logs` volume
- 健康检查 `GET /docs`
...
# 应用文件 (挂载到 volume)
docker volume inspect medical_risk_app_logs
# 或
ls $(docker volume inspect medical_risk_app_logs -f '{{ .Mountpoint }}')
```
