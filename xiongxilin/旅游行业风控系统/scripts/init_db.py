...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 10 张旅游业务表"),
    ("init_business_data.sql", "导入业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 25 条旅游预置风控规则 (R001-R025)"),
]
...
async def main():
    parser = argparse.ArgumentParser(
        description="旅游行业风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 19 张表重建 + 业务数据 + 25 条规则全部就绪")
    else:
