"""
旅游行业风控系统 - XGBoost 训练脚本 (一次性, 跑完即可)
...
# 旅游版说明:
#   生成评估时如果 XGBoost 未加载，decision.py 会写入 ml_score=0.0。
#   因此这里不能再用 ml_score IS NULL 过滤，否则刚生成的评估数据会被全部排除。
...
def _load_data_from_mysql() -> tuple[np.ndarray, np.ndarray]:
    """
    从 MySQL 拉数据, 返回 (X, y).
    X: (N, len(FEATURE_COLUMNS)) float32
    y: (N,) int (0/1)
    """
        ...
        with conn.cursor() as cur:
            ...
            if len(rows) < settings.XGB_TRAIN_DATA_LIMIT * 0.8:
                logger.warning(
                    ...
                    "建议先 init_db.py --reset --yes 清空, 再 gen_risk_data.py --count 1000 重新造",
            ...
            if span_days is not None and span_days < 1 and db_total > 100:
                logger.warning(
                    ...
                    "建议: gen_risk_data.py --count 1000",
            ...
            logger.info("拉取特征: %d 个 event, 目标特征数=%d", len(event_to_features), len(FEATURE_COLUMNS))
            # 2. 拼训练矩阵 (只保留旅游特征齐全的样本)
            X_list, y_list = [], []
            missing = 0
            ...
            if len(y) > 0 and (pos_count == 0 or pos_count == len(y)):
                raise RuntimeError(
                    f"训练数据只有一个类别: 总数={len(y)}, 正例={pos_count}. "
                    "请运行: python scripts/gen_risk_data.py --count 500 --reset-risk-data"
                )
            return X, y
    finally:
        conn.close()
...
def main() -> None:
    ...
    if "val_accuracy" in metrics:
        ...
        if warnings:
            ...
            logger.warning("        2) 检查 feature.py 旅游特征是否真的有区分度 (y=1 跟 y=0 特征分布差多少)")
