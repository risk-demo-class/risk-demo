...
【重要约定】
  - 特征顺序固定 (FEATURE_COLUMNS), 跟 feature.py 算出的旅游特征 key 一一对应
...
# 固定旅游特征顺序 (训练和推理都用这个顺序, 防 dict 顺序不一致导致特征错位)
...
FEATURE_COLUMNS: list[str] = [
    ...
    "user_refund_rate_90d",
    "user_complaint_count_90d",
    "user_visa_reject_count_90d",
    "user_visa_country_count_30d",
    "user_passenger_count_30d",
    "user_device_account_count",
    "user_pay_account_order_count_1h",
    "order_total_amount",
    "order_passenger_count",
    "order_days_to_departure",
    "order_trip_days",
    "order_is_cross_border",
    "order_is_night",
    "order_destination_risk_score",
    "order_passenger_match_rate",
    "order_same_flight_ticket_count_1h",
    "order_high_value_hotel_near_departure",
    "order_material_change_count",
    "order_refund_amount_rate",
    "addr_destination_risk_score",
    "addr_destination_order_count_7d",
    "addr_distinct_country_count_30d",
    "addr_is_new_destination",
    "addr_hotel_city_order_count_7d",
    "addr_route_order_count_1h",
]

assert len(FEATURE_COLUMNS) == 34, f"特征数量必须是 34, 当前 {len(FEATURE_COLUMNS)}"
...
def _features_to_array(features: dict) -> np.ndarray:
    """旅游特征 dict → numpy 数组. 缺失用 0.0 兜底; None/字符串统一 float."""
    row = []
...
def predict(features: dict) -> MlResult:
    """推理入口: 旅游特征 → P(拒绝) + ML 维度决策. 模型没加载 → 兜底 score=0 通过."""
...
def train_and_save(
    ...
    # 1. 样本量预警: 经验值 特征数 × 50 倍 才是稳的最小训练集
    if n < settings.XGB_MIN_SAMPLES:
        logger.warning(
            "样本量 %d < 推荐最小 %d (特征数 × 50 倍经验值), 模型可能欠拟合/过拟合, "
    ...
    if val_metrics and val_metrics.get("val_auc", 0) < settings.XGB_MIN_VAL_AUC:
        logger.warning(
            ...
            "建议: 重造更多旅游风控评估数据 / 检查 feature.py 特征是否真的有区分度",
...
if __name__ == "__main__":
    ...
    # 2. 特征向量化
    print("\n[2] 旅游特征 → numpy 数组 (_features_to_array):")
    print(f"  FEATURE_COLUMNS 长度 = {len(FEATURE_COLUMNS)}  (必须跟 feature.py 对齐)")
    sample = {
    ...
    X_demo = np.random.uniform(0, 1, size=(300, len(FEATURE_COLUMNS))).astype(np.float32)
    ...
    print("\n" + "=" * 60)
    print("总结: 旅游特征对齐 / 模型加载 / predict 兜底 / 概率阈值, 4 个核心能力演示完成")
    print("训练数据准备: python scripts/gen_risk_data.py --count 300")
