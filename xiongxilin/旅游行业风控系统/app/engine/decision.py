...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则:
      - "旅游下单" / "支付" / "出行前核验" 事件: source_id 就是 order_id
      - 机票/酒店/签证/售后/投诉事件: order_id 由 service.event 补全
    """
    order_id = request.order_id
    if not order_id and request.event_type in ("旅游下单", "支付", "出行前核验"):
...
async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """旅游版没有收货地址 receive_id，目的地特征直接由 order_id 计算。"""
    return
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算旅游风控特征 (用户/订单/目的地 3 类)"""
...
def _save_feature_snapshot(
    ...
    """步骤 4: 把旅游特征落库到 risk_feature (审计回溯).

    entity_id 填充规则 (按 entity_type):
      - 用户特征 → ctx.user_id
      - 订单特征 → ctx.order_id (缺失用 source_id 顶替)
      - 目的地特征 → ctx.order_id (缺失用 user_id 顶替)
    """
...
def _ml_prob_to_risk_score(prob: float, k: float = 3.0) -> int:
    ...
    函数映射使用sigmoid？ --- 在初始增加较快，在高的分增长缓慢/? 
...
async def _update_user_profile(
    ...
    profile.refund_rate = (
        round(features.get("user_refund_count_90d", 0) / total_orders, 4)
        if total_orders > 0 else 0
    )
    profile.avg_order_amount = features.get("user_avg_order_amount", 0)
    profile.address_count = int(features.get("addr_distinct_country_count_30d", 0))
    profile.complaint_count = int(features.get("user_complaint_count_90d", 0))
