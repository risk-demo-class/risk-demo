...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('旅游订单风险','机票风险','酒店风险','签证风险','支付风险','售后滥用','投诉风险','账户风险','目的地风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('旅游下单','机票预订','酒店预订','签证申请','支付','售后申请','投诉','出行前核验','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('旅游下单','机票预订','酒店预订','签证申请','支付','售后申请','投诉','出行前核验') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('旅游下单','机票预订','酒店预订','签证申请','支付','售后申请','投诉','出行前核验') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','手机号','护照号','身份证号','支付账号','设备指纹','IP','订单') NOT NULL COMMENT '黑名单类型',
