        ...
        for i, (aid, eid, uid, dec) in enumerate(rows, 1):
            try:
                async with SessionLocal() as db:
                    # 找该 event 的 source_id 和 event_type (旅游版)
                    ...
                    if not ev_row:
                        continue
                    # 订单事件: source_id 即 order_id; 签证事件无订单
                    order_id = (
                        ev_row.event_source_id
                        if ev_row.event_type in ("机票预订", "酒店预订", "跟团游预订")
                        else None
                    )
                    features = await compute_all_features(db, uid, order_id=order_id)
                    # 推理
                    ml = predict(features)
                    if ml is None:
                        continue
