...
-- 9 张风控表结构与基线完全一致 (复用), 仅按行业扩展 ENUM 取值:
--   risk_rule.rule_category / risk_rule.event_type / risk_event.event_type
--   risk_case.event_type / risk_blacklist.blacklist_type
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('机票囤积','酒店滥用','签证欺诈','账户风险','支付风险','行程风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('机票预订','酒店预订','签证申请','跟团游预订','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('机票预订','酒店预订','签证申请','跟团游预订') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `source_id` VARCHAR(50) DEFAULT NULL COMMENT '原始业务ID(订单/签证ID), 重做检查时用',
    `event_type` ENUM('机票预订','酒店预订','签证申请','跟团游预订') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','护照号','设备指纹','支付账号') NOT NULL COMMENT '黑名单类型',
