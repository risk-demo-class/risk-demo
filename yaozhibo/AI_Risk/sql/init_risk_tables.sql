...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('课程报名','课程退费','身份异常','设备异常','综合风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('COURSE_PURCHASE','REFUND_REQUEST','LEARNING_ACTIVITY','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('COURSE_PURCHASE','REFUND_REQUEST','LEARNING_ACTIVITY') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','课程','订单','学习','退费','设备') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `source_id` VARCHAR(50) DEFAULT NULL COMMENT '原始业务ID(课程订单/退费/学习进度ID), 重做检查时用',
    `event_type` ENUM('COURSE_PURCHASE','REFUND_REQUEST','LEARNING_ACTIVITY') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','学号','身份证','设备','直播账号') NOT NULL COMMENT '黑名单类型',
