"""
在线教育风控系统 - 一键数据库初始化脚本 (异步)
按顺序执行所有 SQL 脚本: 创建数据库 → 业务表 → 业务数据 → 风控表 → 风控规则

总表数: 6 业务 + 9 风控 = 15 张。
旧电商/医疗混合库的 ENUM 与教育版不兼容，教学环境请使用 --reset --yes 重建。
"""
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 6 张教育业务表"),
    ("init_business_data.sql", "导入教育业务种子数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 7 条教育风控规则 (R001-R030)"),
]
...
async def execute_sql_file(conn, filepath, description):
    ...
    async with conn.cursor() as cur:
        for stmt in statements:
            # 连接本身已经选中 --db 指定的数据库；忽略旧脚本中可能残留的 USE ecs，
            # 防止初始化测试库时误写生产/演示库。
...
async def verify_business_layer(conn) -> tuple[int, int]:
    """返回已创建的业务表数和业务数据总行数。"""
    existing = 0
    total_rows = 0
    async with conn.cursor() as cur:
        for table_name in BUSINESS_TABLES:
            ...
            if await cur.fetchone() is None:
                print(f"  验收失败: 缺少业务表 {table_name}")
                continue
            existing += 1
            await cur.execute(f"SELECT COUNT(*) FROM `{table_name}`")
            row = await cur.fetchone()
            total_rows += int(row[0])
    print(f"  业务层验收: {existing}/{len(BUSINESS_TABLES)} 张表, {total_rows} 行种子数据")
    return existing, total_rows
...
async def main():
    parser = argparse.ArgumentParser(
        description="在线教育风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    try:
        for idx, (filename, desc) in enumerate(SQL_FILES, start=2):
            ...
            total_errors += errors
        business_table_count, business_row_count = await verify_business_layer(conn)
        if business_table_count != len(BUSINESS_TABLES) or business_row_count < 20:
            total_errors += 1
    finally:
        await conn.ensure_closed()
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 15 张表重建 + 教育业务数据 + 7 条规则全部就绪")
    else:
