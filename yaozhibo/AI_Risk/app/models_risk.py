...
_EVENT_VALUES = tuple(sorted(BUSINESS_EVENT_TYPES))
_BLACKLIST_VALUES = tuple(sorted(BLACKLIST_TYPES))
...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("课程报名", "课程退费", "身份异常", "设备异常", "综合风险",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum(*_EVENT_VALUES, "通用", name="rule_event_type_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum(*_EVENT_VALUES, name="event_type_enum"),
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("用户", "课程", "订单", "学习", "退费", "设备", name="feature_entity_type_enum"),
...
class RiskCase(Base):
    ...
    # --- 业务回溯字段 (重做检查时使用) ---
    source_id: Mapped[Optional[str]] = mapped_column(String(50), comment="原始业务ID(课程订单/退费/学习进度ID)")
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum(*_EVENT_VALUES, name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        Enum(*_BLACKLIST_VALUES, name="blacklist_type_enum"),
