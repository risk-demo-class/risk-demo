...
async def _biz_high_value_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        ...
        .where(OrderInfo.total_amount >= 8000)
...
_BIZ_QUERY_HANDLERS = {
    ...
    "user_refunds": _biz_user_postsales,
    "user_learning": _biz_user_addresses,
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id、event_type (可选: COURSE_PURCHASE/REFUND_REQUEST/LEARNING_ACTIVITY)、source_id。"
...
@tool(
    description=(
        ...
        "返回: 用户的风险评分、购课、退费和设备关联画像；没画像时实时计算教育特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (add/remove/check/list)、blacklist_type (用户/学号/身份证/设备/直播账号)、value、reason。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (可选: user_orders/user_refunds/user_learning/order_detail/recent_orders/high_value_orders)、user_id、order_id、limit。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
