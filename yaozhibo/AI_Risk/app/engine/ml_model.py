...
FEATURE_COLUMNS: list[str] = [
    ...
    "user_role_teacher_flag",
    "user_purchase_count_1h",
    "user_purchase_amount_1h",
    "user_purchase_count_7d",
    "user_purchase_amount_7d",
    "user_refund_count_90d",
    "user_refund_amount_90d",
    "user_refund_rate_90d",
    "user_learning_minutes_30d",
    "user_avg_completion_rate",
    "order_total_amount",
    "course_price",
    "course_is_student_only",
    "course_new_account_purchase_count_7d",
    "course_distinct_users_7d",
    "student_only_purchase_count_30d",
    "current_study_minutes",
    "current_completion_rate",
    "refund_study_minutes",
    "refund_amount",
    "device_distinct_users_30d",
    "student_id_blacklisted",
    "id_card_blacklisted",
    "device_id_blacklisted",
]
...
def load_model(model_path: Optional[str] = None) -> bool:
    ...
    global _MODEL, _LOADED
    # 每次加载都从干净状态开始，避免“新路径加载失败但旧模型仍继续推理”。
    _MODEL = None
    _LOADED = False
    ...
    try:
        ...
        model_features = list(_MODEL.feature_names or [])
        if model_features and model_features != FEATURE_COLUMNS:
            logger.warning(
                "XGBoost 模型特征与当前教育特征不一致, 忽略旧模型并走纯规则模式: %s",
                path,
            )
            _MODEL = None
            _LOADED = False
...
if __name__ == "__main__":
    ...
    sample.update({
        ...
        "user_purchase_count_7d": 2,
        "user_purchase_amount_7d": 6999,
        "user_learning_minutes_30d": 600,
        "user_avg_completion_rate": 0.45,
        "order_total_amount": 3999,
        "course_price": 3999,
        "current_study_minutes": 600,
        "current_completion_rate": 0.45,
        "device_distinct_users_30d": 1,
    })
