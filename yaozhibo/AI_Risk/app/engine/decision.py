...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    """步骤 1b: 请求转为流水线上下文，不携带领域专用字段。"""
    return _RiskCheckContext(request=request, user_id=request.user_id)
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算 25 个教育风控特征。"""
    return await compute_all_features(
        db,
        user_id=ctx.user_id,
        event_type=str(ctx.request.event_type),
...
def _classify_feature_entity(feature_name: str) -> tuple[str, str]:
    """特征名前缀 → 教育实体类型。"""
    if feature_name.startswith(("user_", "student_id_", "id_card_")):
        return "用户", feature_name
    if feature_name.startswith("order_"):
        return "订单", feature_name
    if feature_name.startswith("course_") or feature_name.startswith("student_only_"):
        return "课程", feature_name
    if feature_name.startswith("refund_"):
        return "退费", feature_name
...
def _save_feature_snapshot(
    ...
    用户特征以 user_id 定位，其他事件特征以 source_id 定位。
    """
    for fname, fval in features.items():
        entity_type, _ = _classify_feature_entity(fname)
        entity_id = ctx.user_id if entity_type == "用户" else ctx.request.source_id
        db.add(RiskFeature(
            event_id=ctx.event_id,
...
def _calculate_decision(
    ...
    ml_decision = ml_result.decision if ml_result else None
...
async def _update_user_profile(
    ...
    profile.total_orders = int(features.get("user_purchase_count_7d", 0))
    profile.total_refunds = int(features.get("user_refund_count_90d", 0))
    profile.refund_rate = features.get("user_refund_rate_90d", 0)
    purchase_count = features.get("user_purchase_count_7d", 0)
    profile.avg_order_amount = (
        round(features.get("user_purchase_amount_7d", 0) / purchase_count, 2)
        if purchase_count else 0
    )
    # 保留核心表字段，教育版映射：address_count=关联账号数，complaint_count=近90天退费数。
    profile.address_count = int(features.get("device_distinct_users_30d", 0))
    profile.complaint_count = int(features.get("user_refund_count_90d", 0))
