...
class RiskCheckRequest(BaseModel):
    """风险检查请求"""
    event_type: Literal["寄件下单", "安检申报", "跨境申报", "签收处理", "代收货款结算"]
    source_id: str = Field(description="关联业务ID（shipment/declaration/delivery id）")
    user_id: str
    order_id: Optional[str] = Field(default=None, exclude=True, description="内部兼容字段：shipment_id")
    receive_id: Optional[str] = Field(default=None, exclude=True, description="内部兼容字段：address_id")
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["实名风险", "危险品风险", "跨境申报风险", "代收货款风险", "地址风险", "寄件行为风险"]
    event_type: Literal["寄件下单", "安检申报", "跨境申报", "签收处理", "代收货款结算", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["实名风险", "危险品风险", "跨境申报风险", "代收货款风险", "地址风险", "寄件行为风险"]] = None
    event_type: Optional[Literal["寄件下单", "安检申报", "跨境申报", "签收处理", "代收货款结算", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["寄件人", "收件人", "手机号", "证件号", "地址", "设备指纹", "跨境收件方"]
    blacklist_value: str
