...
class TestCaseDeduplication:
    ...
    场景: 同一订单的"预订下单"事件被前端重试 3 次, 每次都触发"人工审核"
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="预订下单", source_id="order_001", user_id="1001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="预订下单", source_id="order_002", user_id="1001",
        )
        ...
        assert db.added[0].event_type == "预订下单"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="预订下单", source_id="order_003", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "预订欺诈", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        ...
        request = RiskCheckRequest(
            event_type="预订下单", source_id="order_reject_001", user_id="U001",
        )
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="预订下单", source_id="order_review_001", user_id="U002",
        )
...
class TestCaseCategoryAggregation:
    ...
    async def test_majority_category_wins(self):
        ...
        # 3 条账户风险 vs 1 条预订欺诈 → 取多的
        assert case.case_category == "账户风险", (
            f"应取命中数最多的 '账户风险', 实际 {case.case_category}"
        )
    ...
    async def test_tie_first_one_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="预订下单", source_id="order_y", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # 1 条预订欺诈 + 1 条设备风险 → 平局
        rules = [
        ...
        assert db.added[0].case_category in ("预订欺诈", "设备风险")
