...
class TestFeatureColumns:
    ...
    def test_feature_prefixes(self):
        """11 user_* + 8 order_* + 6 trip_* (align with feature.py naming)"""
        ...
        assert len(user) == 11, f"user features should be 11, got {len(user)}"
        assert len(order) == 8, f"order features should be 8, got {len(order)}"
        assert len(trip) == 6, f"trip features should be 6, got {len(trip)}"
    ...
    def test_features_to_array(self):
        ...
        # intentionally omit 22 middle keys -> 0
            "trip_same_hotel_1h": 0,         # 24 (last in FEATURE_COLUMNS)
        }
    ...
    def test_feature_columns_align_with_feature_module(self):
        ...
        """
        # 11 user + 8 order + 6 trip, aligned with feature.py's 3 compute_* 函数
        expected = [
            # --- 11 user features (compute_user_features) ---
            "user_total_orders",
            "user_orders_30d",
            "user_orders_7d",
            "user_total_amount",
            ...
            "user_visa_reject_90d",
            "user_visa_countries_30d",
            "user_account_age_days",
            "user_real_name_status",
            "user_refund_rate",
            ...
            "order_booking_count",
            "order_is_urgent",
            # --- 6 trip features (compute_address_features, Q5 命名) ---
            "trip_passenger_match_rate",
            "trip_blacklist_passport_count",
            "trip_same_flight_1h",
            "trip_distinct_passenger_count",
            "trip_visa_apply_30d",
            "trip_same_hotel_1h",
        ]
...
class TestModelLoad:
    ...
    def test_load_real_model(self, capsys):
        ...
        normal_features = {
            ...
            "user_avg_order_amount": 666.0,
            "user_max_order_amount": 800.0,
            "user_visa_reject_90d": 0,
            "user_visa_countries_30d": 1,
            "user_account_age_days": 400,
            "user_real_name_status": 1,
            "user_refund_rate": 0.0,
            "order_total_amount": 200.0,
            "order_is_night": 0,
            "order_trip_days": 7,
            "order_passenger_count": 1,
            "order_is_overseas": 0,
            "order_is_flight": 1,
            "order_booking_count": 1,
            "order_is_urgent": 0,
            "trip_passenger_match_rate": 1.0,
            "trip_blacklist_passport_count": 0,
            "trip_same_flight_1h": 1,
            "trip_distinct_passenger_count": 1,
            "trip_visa_apply_30d": 1,
            "trip_same_hotel_1h": 0,
        }
        # 3. 准备一组"高风险用户"特征 (强信号, 期望 → 人工审核/拒绝)
        risky_features = dict(normal_features)
        risky_features.update({
            "user_visa_reject_90d": 3,
            "user_visa_countries_30d": 4,
            "user_refund_rate": 0.9,
            "trip_blacklist_passport_count": 1,
            "trip_same_flight_1h": 6,
        })
...
class TestPredict:
    ...
    def test_predict_normal_user(self):
        """Normal user (full 25-dim low features) -> predict 接口健全.
        注: 不强 assert '正常必须通过' —— 那是模型质量范畴 (训练时 val_auc 把关),
        基线 test_load_real_model 同样明确不强 assert (P4-L3). 这里只校验接口契约:
        分数范围 [0,1] + 决策必须落在 4 档合法值内.
        """
        # 旅游版: 补全 25 维正常特征, 避免缺失特征 0 填充被模型误判为高风险
        result = ml_model.predict({
            "user_total_orders": 5,
            "user_orders_30d": 0,
            "user_orders_7d": 0,
            "user_total_amount": 2000,
            "user_avg_order_amount": 400,
            "user_max_order_amount": 800,
            "user_visa_reject_90d": 0,
            "user_visa_countries_30d": 1,
            "user_account_age_days": 400,
            "user_real_name_status": 1,
            ...
            "order_is_overseas": 0,
            "order_is_flight": 0,
            "order_booking_count": 0,
            "order_is_urgent": 0,
            "trip_passenger_match_rate": 1.0,
            "trip_blacklist_passport_count": 0,
            "trip_same_flight_1h": 0,
            "trip_distinct_passenger_count": 1,
            "trip_visa_apply_30d": 1,
            "trip_same_hotel_1h": 0,
        })
        assert 0.0 <= result.score <= 1.0
        assert result.decision in ("\u901a\u8fc7", "\u6807\u8bb0", "\u4eba\u5de5\u5ba1\u6838", "\u62d2\u7edd")
    ...
    def test_predict_risky_user(self):
        ...
        risky = {
            # 用户画像 (拒签史 + 短期多国 + 高频退改)
            "user_total_orders": 30,
            "user_orders_30d": 12,
            "user_orders_7d": 4,
            "user_total_amount": 80000,
            "user_avg_order_amount": 2600,
            "user_max_order_amount": 15000,
            "user_visa_reject_90d": 3,          # 拒签史
            "user_visa_countries_30d": 4,       # 短期多国
            "user_account_age_days": 5,         # 新账号
            "user_real_name_status": 0,
            "user_refund_rate": 0.7,            # 高频退改
            # 订单特征 (大额 + 凌晨 + 短行程 + 跨境)
            "order_total_amount": 60000,        # 大额跨境
            "order_is_night": 1,                # 0点突击
            "order_trip_days": 4,               # 行程 < 7 天
            "order_passenger_count": 6,
            "order_is_overseas": 1,
            "order_is_flight": 1,
            "order_booking_count": 6,
            "order_is_urgent": 1,
            # 行程/乘客特征 (黑护照 + 黄牛囤票 + 乘客不一致)
            "trip_passenger_match_rate": 0.2,
            "trip_blacklist_passport_count": 1,
            "trip_same_flight_1h": 6,
            "trip_distinct_passenger_count": 6,
            "trip_visa_apply_30d": 5,
            "trip_same_hotel_1h": 1,
        }
