...
【工作流 6 步】
  ...
  2. 造 30 个 RISK 高风险用户 (gen_risky_users.py --count 30, 12 模式轮换)
...
def step_2_risk_users() -> None:
    ...
    _run_subprocess(
        "2/6 造 30 个 RISK 用户 (gen_risky_users.py --count 30, 12 模式轮换)",
...
示例:
  ...
  python scripts/one_command.py --only-start   # 只跑步骤 6 (造今日业务数据)
    ...
    try:
        ...
        print("=" * 70)
        print("排查建议:")
        print("  1. 检查 MySQL 是否启动 (默认 localhost:3307, root/123456)")
