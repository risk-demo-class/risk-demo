...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 8 张旅游业务表"),
    ("init_business_data.sql", "导入旅游业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (ENUM 已扩展旅游值)"),
    ("init_risk_data.sql", "导入 12 条预置旅游风控规则 (R001-R034)"),
]

# 保证 import app.config 可用 (从项目根目录执行)
sys.path.insert(0, BASE_DIR)

from app.config import settings  # noqa: E402

# 默认连接配置: 全部来自 .env (旅游版 DB_NAME=tourism, 与电商基线 ecs 完全隔离)
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print(f"数据库已重置: {args.db} 库 17 张表重建 + 业务数据 + 12 条规则全部就绪")
    else:
