...
class Settings(BaseSettings):
    ...
    # 【旅游风控】独立数据库, 与电商基线 ecs 完全隔离, 绝不覆盖基线表
    DB_NAME: str = "tourism"
    TEST_DB_NAME: str = "tourism_test"
...
# ============================================================
# 业务枚举集中管理 (任务书要求: 黑名单类型 + 业务事件类型在 config.py)
# schemas.py 的 Pydantic Literal 是这些常量的"字面镜像" (Pydantic 限制不能引用),
# 一致性由 tests/test_business_enums.py 保证
# ============================================================

# 业务事件类型 (risk_event / risk_rule.event_type / risk_case.event_type)
BUSINESS_EVENT_TYPES: list[str] = ["预订下单", "支付", "退改申请", "签证申请"]

# "订单类"事件: source_id = order_id (预订下单 / 支付)
BUSINESS_ORDER_EVENTS: tuple[str, ...] = ("预订下单", "支付")
BUSINESS_REFUND_EVENT: str = "退改申请"
BUSINESS_VISA_EVENT: str = "签证申请"

# 业务黑名单类型 (risk_blacklist.blacklist_type)
BUSINESS_BLACKLIST_TYPES: list[str] = ["用户", "护照号", "签证号", "设备指纹"]

# 业务规则类别 (risk_rule.rule_category)
BUSINESS_RULE_CATEGORIES: list[str] = [
    "预订欺诈", "退改滥用", "签证风险", "账户风险", "设备风险", "黑名单风险",
]
