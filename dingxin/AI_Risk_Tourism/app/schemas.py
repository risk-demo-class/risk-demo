...
class RiskCheckRequest(BaseModel):
    """风险检查请求"""
    event_type: Literal["预订下单", "支付", "退改申请", "签证申请"]
    source_id: str = Field(description="关联业务ID (order_id / refund_id / visa_id 等)")
    user_id: str
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["预订欺诈", "退改滥用", "签证风险", "账户风险", "设备风险", "黑名单风险"]
    event_type: Literal["预订下单", "支付", "退改申请", "签证申请", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["预订欺诈", "退改滥用", "签证风险", "账户风险", "设备风险", "黑名单风险"]] = None
    event_type: Optional[Literal["预订下单", "支付", "退改申请", "签证申请", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["用户", "护照号", "签证号", "设备指纹"]
    blacklist_value: str
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        event_type="预订下单", source_id="ord_demo_001", user_id="U0001",
        order_id="ord_demo_001", receive_id="rec_001",
        event_data={"amount": 5000, "category": "跟团游"},
    )
    ...
    hits = [
        RuleHitInfo(
            rule_id="R002", rule_name="大额跨境订单",
            rule_category="预订欺诈", risk_level="极高",
            risk_score=95, action="拒绝",
            description="大额跨境订单, 一票否决",
        ),
        RuleHitInfo(
            rule_id="R005", rule_name="高频退改",
            rule_category="退改滥用", risk_level="高",
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="U0001", event_type="预订下单", event_source_id="ord_demo_001",
