...
async def _query_user_profile_impl(*, db: AsyncSession, user_id: str) -> str:
    """查询用户风险画像. 没画像时实时算 11 个 user 特征 (不写库)."""
...
async def _blacklist_remove_impl(
    *, db: AsyncSession, blacklist_type: str, value: str,
    ...
    # 用 (type, value) 联合定位, 再走统一软删接口
    # (软删 + P4-L1 审计 + blacklist_extra 业务台账同步, 替代原来的硬删)
    bl = (await db.execute(
    ...
    removed = await remove_blacklist(db, bl.blacklist_id)
    if not removed:
        return f"黑名单记录不存在或已移除: {blacklist_type}={value}"
...
# ============================================================
# 业务数据查询 (旅游版, 字典派发 6 种 query_type)
# ============================================================
...
async def _biz_user_passengers(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户的乘客记录 (JOIN order_info 关联用户)."""
    stmt = (
        select(
            PassengerInfo.passenger_id,
            PassengerInfo.order_id,
            PassengerInfo.name,
        ...
        .where(OrderInfo.user_id == user_id)
        .order_by(PassengerInfo.passenger_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_order_flights(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """订单的机票预订."""
    stmt = (
        select(
...
async def _biz_order_hotels(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """订单的酒店预订."""
    stmt = (
        select(
        ...
        .where(BookingHotel.order_id == order_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
_BIZ_QUERY_HANDLERS = {
    ...
    "order_flights": _biz_order_flights,
    "order_hotels": _biz_order_hotels,
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (用户ID, 如 'U1001' 或 'RISK001')、event_type (事件类型, 可选值: 预订下单/支付/退改申请/签证申请)、source_id (关联业务ID, 如订单号/退改单ID/签证ID)。"
...
@tool(
    description=(
        "查询用户的风险画像信息。"
        "参数: user_id (用户ID, 如 'U1001' 或 'RISK001')。"
        "返回: 用户的风险评分、订单统计、退改率、乘客数等画像数据; 没画像时实时算 11 个 user 特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/护照号/签证号/设备指纹)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        "查询旅游业务数据, 用于数据分析和风控辅助判断。"
        "参数: query_type (查询类型, 可选值: user_orders/user_passengers/user_visas/order_flights/order_hotels/recent_orders)、user_id (部分需要)、order_id (部分需要)、limit (返回数量, 默认10)。"
        "返回: 旅游业务数据 (JSON 字符串)。"
    )
)
...
if __name__ == "__main__":
    ...
    impls = [
        ...
        _biz_user_orders, _biz_user_passengers, _biz_user_visas,
        _biz_order_flights, _biz_order_hotels, _biz_recent_orders,
