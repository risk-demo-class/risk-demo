...
# blacklist_extra 业务台账支持的类型 (ENUM 限定; 用户/地址/手机号不入台账)
_MIRROR_TYPES = {"护照号", "签证号", "设备指纹"}


async def _sync_blacklist_extra(
    db: AsyncSession,
    blacklist_type: str,
    blacklist_value: str,
    reason: str | None,
    ...
    """同步业务台账 blacklist_extra (与 risk_blacklist 同事务).

    - 仅同步 护照号/签证号/设备指纹 (blacklist_extra 的 ENUM 不支持 用户/地址/手机号)
    - 已存在行 (含软删) → 更新 reason/expire_at/deleted_at (软删后可重新启用)
    - 不存在 → 新增
    """
    if blacklist_type not in _MIRROR_TYPES:
        return
    existing = (await db.execute(
        select(BlacklistExtra).where(
            BlacklistExtra.type == blacklist_type,
            BlacklistExtra.value == blacklist_value,
        )
    )).scalar_one_or_none()
    if existing:
        existing.reason = reason
        existing.expire_at = expire_time
...
async def add_blacklist(db: AsyncSession, request: BlacklistCreate) -> BlacklistResponse:
    ...
    if existing:
        ...
        await _sync_blacklist_extra(
            db, request.blacklist_type, request.blacklist_value,
            request.reason, request.expire_time, deleted_at=None,
        )
...
async def remove_blacklist(db: AsyncSession, blacklist_id: int) -> bool:
    ...
    await _sync_blacklist_extra(
        db, bl.blacklist_type, bl.blacklist_value,
        bl.reason, bl.expire_time, deleted_at=bl.deleted_at,
    )
    await db.commit()
    return True
...
async def list_assessments(
    ...
    筛选维度 (可选):
      ...
      - risk_level: 低 / 中 / 高 / 极高
      - event_type: 预订下单 / 支付 / 退改申请 / 签证申请 (JOIN risk_event)
      - user_id: 精确匹配
    ...
    class _StatDB:
        ...
        async def execute(self, stmt):
            ...
            if self.call_count == 1:
                ...
                return _R([("预订欺诈", 12), ("签证风险", 8), ("账户风险", 5)])
            else:
                return _R([])
