...
def calculate_final_score(hits: list[RuleHitResult]) -> int:
    ...
    return min(max_score + bonus, 100)#限制得分在100以内
    #hits代表命中的特征,也就是违规的特征
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则 (旅游版):
      - "预订下单" / "支付" 事件: source_id 就是 order_id
      - "退改申请" 事件: source_id 是 refund_id, order_id 由 event.py 补全
      - "签证申请" 事件: 无订单, 只算用户特征
    """
    order_id = request.order_id
    if not order_id and request.event_type in ("预订下单", "支付"):
...
async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 旅游版无收货地址概念, 保持 receive_id=None.
    (Q5 决策: trip_* 行程特征通过 compute_all_features 的 order_id 槽位计算,
     快照 entity_id 兜底为 user_id, 流程签名与 7 步结构不变)"""
    ctx.receive_id = None
...
async def _update_user_profile(
    ...
    # 旅游版: 无独立 refund_count 特征, 由退改率反推次数, 保证画像一致
    total_orders = features.get("user_total_orders", 0)
    refund_rate = features.get("user_refund_rate", 0)
    profile.total_refunds = int(round(refund_rate * total_orders))
    profile.refund_rate = refund_rate
    profile.avg_order_amount = features.get("user_avg_order_amount", 0)
    # 旅游版复用画像字段: address_count 槽位存"历史去重乘客数"兜底 0 (无实时查询)
    profile.address_count = int(features.get("trip_distinct_passenger_count", 0))
