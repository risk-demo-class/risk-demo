...
def _validate_rule_risk_combo(risk_level: str, action: str) -> None:
    """风控语义校验: 风险等级"极高"必须配动作"拒绝".

    原因: 决策引擎 (app/engine/decision.py) 对命中"极高"规则执行一票否决,
    最终决策强制为"拒绝" (final_score 抬到 >= RISK_VETO_MIN_SCORE).
    如果配置成"极高 + 标记/通过", 规则实际行为仍是拒绝, 与页面显示的动作不一致,
    会让运营误判. 这里直接拒绝这种配置, 保证整个业务流语义一致.

    注: 分值 ↔ 等级/动作 不强制一致 (如 R005 80分/高/人工审核 是业务自定义),
    最终决策由 final_score 计算得出, 不会出错; 前端输入分值时自动填充默认组合.
    """
    if risk_level == "极高" and action != "拒绝":
        raise HTTPException(
            status_code=400,
            detail="风险等级为'极高'时动作必须为'拒绝' (一票否决语义, 避免配置误导)",
        )
...
@rule_router.get("/features")
async def api_rule_features():
    """规则编辑器用: 返回 25 维特征元数据 (字段名/中文名/类型), 供前端构建条件表单.
    注意: 必须注册在 /{rule_id} 之前, 否则会被当成 rule_id 匹配."""
...
async def api_create_rule(data: RuleCreate, db: AsyncSession = Depends(get_db_async)):
    ...
    _validate_rule_risk_combo(data.risk_level, data.action)
...
async def api_update_rule(rule_id: str, data: RuleUpdate, db: AsyncSession = Depends(get_db_async)):
    ...
    # 合并更新后校验语义 (极高 ⇒ 拒绝)
    _validate_rule_risk_combo(rule.risk_level, rule.action)
