...
class RiskRule(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("下单", "支付", "售后申请", "物流投诉", "通用",
             "预订下单", "退改申请", "签证申请", name="rule_event_type_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("下单", "支付", "售后申请", "物流投诉",
             "预订下单", "退改申请", "签证申请", name="event_type_enum"),
...
class RiskCase(Base):
    ...
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("下单", "支付", "售后申请", "物流投诉",
             "预订下单", "退改申请", "签证申请", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        Enum("用户", "地址", "手机号", "护照号", "签证号", "设备指纹", name="blacklist_type_enum"),
...
class RiskUserProfile(Base):
    ...
    address_count: Mapped[int] = mapped_column(Integer, default=0, comment="地址数量(旅游版映射: 历史去重乘客数)")
