...
def _check_mysql_alive() -> tuple[str, str]:
    """检查 MySQL 是否在 localhost:3307 跑 (Docker 映射). 用 socket 探测, 不依赖 pymysql."""
    import pymysql
    from app.config import settings as _  # 加载 .env 配置
    password = os.getenv("DB_PASSWORD", _.DB_PASSWORD)
    user = os.getenv("DB_USER", _.DB_USER)
    db = os.getenv("DB_NAME", _.DB_NAME)
    try:
        conn = pymysql.connect(
            host="localhost", port=3307, user=user, password=password,
        ...
        return "WARN", (
            f"MySQL 连不上 (localhost:3307 {user}@{db}): {type(e).__name__}\n"
            f"         业务功能会失败. 检查: MySQL 启动了? 密码对了? 端口 3307 通?"
        )
...
def _check_db_initialized() -> tuple[str, str]:
    ...
    try:
        ...
        password = os.getenv("DB_PASSWORD", _s.DB_PASSWORD)
        user = os.getenv("DB_USER", _s.DB_USER)
        db = os.getenv("DB_NAME", _s.DB_NAME)
        conn = pymysql.connect(
            host="localhost", port=3307, user=user, password=password,
            database=db, connect_timeout=3,
        )
