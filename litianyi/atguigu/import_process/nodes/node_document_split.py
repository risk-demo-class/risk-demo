...
from atguigu.config.config import chunk_config
...
class NodeDocumentSplit(NodeBase):
    ...
    def _split_long_section(self, section: Dict[str, str]) -> List[Dict[str, str]]:
        ...
        # 4. 计算标题后正文允许的可用长度： 总长度 - 标题的长度
        available_len = self.chunk_config.max_length - len(prefix)
        if available_len <= 0:
        ...
        splitter = RecursiveCharacterTextSplitter(
            ...
            chunk_overlap=self.chunk_config.window_overlap,
            separators=["\n\n", "\n", "。", "！", "？", "；", ".", "!", "?", ";", " "]
        )
        ...
        for idx, chunk in enumerate(splitter.split_text(body), start=1):
            ...
            # 子章节信息封装
            sub_sections.append({
                "title": f"{title} - {idx}",      #长切后的分段名（章节名称-序号）
                "content": full_text,             #同章节内的内容
                "parent_title": title,            #父标题--章节名称
                "part": idx,                      #第几块
                "file_title": section.get("file_title")   #文档名称
            })
    ...
    def _merge_short_sections(self, sections: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        for sec in  sections:
            ...
            is_current_short = len(current_chunk["content"]) < self.chunk_config.min_length
