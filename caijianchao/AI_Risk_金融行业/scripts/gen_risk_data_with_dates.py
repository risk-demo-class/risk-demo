"""
金融风控系统 - 带日期范围的模拟风控评估数据生成 (异步, 金融风控版)
支持指定"近 N 天"或"起止日期"造数据，让仪表盘趋势图有跨天数据

【P4-L4 2026-08-12 重写】从电商版移植为金融风控版:
  表: order_info/order_detail/postsale/logistics_complaints_record
      → transaction_order (交易/转账/取现) + loan_info (贷款申请)
  事件: 下单/支付/售后申请/物流投诉 → 交易/转账/取现/贷款申请

【P4-L3 2026-08-08 第三轮】支持 --balance-pos / --target-pos-ratio 控制正负例比例:
  - --balance-pos: 80% 概率从 RISK 高风险账户挑样本 (跟 gen_risk_data.py 行为一致)
  - --target-pos-ratio: 目标正例比例 (0.0-1.0), 自动循环造数据直到达标 (最多 10 轮)

【P4-L3 2026-08-08 第五轮】--force-pos-ratio:
  强制造"已知能触发规则"的高风险事件 (RISK 账户的贷款/大额交易/可疑交易)
  + 跑完 process_event 后判断 decision, 不是 (拒绝/人工审核) 就 retry 换事件
...
用法:
    # 默认: 近 7 天, 每天随机 1~30 条 (没 RISK 账户时正例 ≈ 2%)
    ...
    # 清空后重建 + 强制 30% 正例 (需先跑 gen_risky_users.py 造 RISK 账户)
...
# 【P4-L4 2026-08-12】RISK 高风险账户前缀 (跟 gen_risky_users.py 对齐)
RISKY_ACCOUNT_PREFIX = "RISK"
...
# ============================================================
# 【P4-L3 第三轮】--balance-pos 配套: 优先从 RISK 账户挑
# 【P4-L4 2026-08-12】表改为 transaction_order / loan_info
# ============================================================
async def _pick_transaction_for_balance(db, balance_pos: bool):
    """从交易池挑一条; balance_pos=True 时 80% 概率挑 RISK 账户交易."""
    if balance_pos:
        if random.random() < 0.8:
            r = await db.execute(text("""
                SELECT t.txn_id, t.account_id
                FROM transaction_order t
                JOIN account_info a ON a.account_id = t.account_id
                WHERE t.account_id LIKE :prefix AND a.account_status != '冻结'
                ORDER BY RAND() LIMIT 1
            """), {"prefix": f"{RISKY_ACCOUNT_PREFIX}%"})
            row = r.first()
            if row:
                return (row.txn_id, row.account_id)
    r = await db.execute(text("""
        SELECT t.txn_id, t.account_id
        FROM transaction_order t
        JOIN account_info a ON a.account_id = t.account_id
        WHERE a.account_status != '冻结'
        ORDER BY RAND() LIMIT 1
    """))
    row = r.first()
    return (row.txn_id, row.account_id) if row else None


async def _pick_loan_for_balance(db, balance_pos: bool):
    """从贷款池挑一条; balance_pos=True 时 80% 概率挑 RISK 账户贷款."""
    if balance_pos:
        if random.random() < 0.8:
            r = await db.execute(text("""
                SELECT l.loan_id, l.account_id
                FROM loan_info l
                JOIN account_info a ON a.account_id = l.account_id
                WHERE l.account_id LIKE :prefix AND a.account_status != '冻结'
                ORDER BY RAND() LIMIT 1
            """), {"prefix": f"{RISKY_ACCOUNT_PREFIX}%"})
            row = r.first()
            if row:
                return (row.loan_id, row.account_id)
    r = await db.execute(text("""
        SELECT l.loan_id, l.account_id
        FROM loan_info l
        JOIN account_info a ON a.account_id = l.account_id
    ...
    """))
    row = r.first()
    return (row.loan_id, row.account_id) if row else None


# ============================================================
# 【P4-L3 第五轮】--force-pos-ratio 配套: 强制造"高风险事件"路径
# 区别于 --balance-pos (只挑 RISK 账户, 但业务规则不保证命中),
# --force-pos-ratio 选"已知能触发高风险规则"的事件:
#   RISK 账户的贷款 (信贷审批规则) / 大额交易 (交易反欺诈规则) / 可疑交易 (反洗钱规则)
# 跑完 process_event 后判断 decision, 如果还是"通过/标记"就 retry (换其他高风险事件)
# ============================================================
async def _pick_forced_loan(db):
    """强制正例路径: RISK 账户的贷款 (必触发信贷审批/逾期规则)."""
    r = await db.execute(text("""
        SELECT l.loan_id, l.account_id
        FROM loan_info l
        JOIN account_info a ON a.account_id = l.account_id
...
async def _pick_forced_high_amount_txn(db):
    """强制正例路径: RISK 账户的大额交易 (txn_amount >= 50000, 触发交易反欺诈规则)."""
    r = await db.execute(text("""
        SELECT t.txn_id, t.account_id
        FROM transaction_order t
        JOIN account_info a ON a.account_id = t.account_id
        WHERE t.account_id LIKE :prefix AND t.txn_amount >= 50000 AND a.account_status != '冻结'
        ORDER BY RAND() LIMIT 1
    """), {"prefix": f"{RISKY_ACCOUNT_PREFIX}%"})
    row = r.first()
    return (row.txn_id, row.account_id) if row else None
...
async def _pick_forced_suspicious_txn(db):
    """强制正例路径: RISK 账户的交易 (可疑交易, 触发反洗钱/行为规则)."""
    r = await db.execute(text("""
        SELECT t.txn_id, t.account_id
        FROM transaction_order t
        JOIN account_info a ON a.account_id = t.account_id
...
async def generate_risk_data_with_dates(
    ...
    【P4-L3 第三轮】--balance-pos 跟 --target-pos-ratio:
        - balance_pos=True: 80% 概率从 RISK 高风险账户挑样本 (拉高正例比例)
    ...
    【P4-L3 第五轮】--force-pos-ratio:
        强制每条按概率走"高风险事件"路径 (RISK 贷款/大额交易/可疑交易), retry 最多 3 次
    """
    async with AsyncSessionLocal() as db:
        ...
        if target_pos_ratio is not None and not balance_pos:
            ...
            balance_pos = True
        if balance_pos:
            print(f"⚠️  --balance-pos 模式: 80% 概率挑 RISK 高风险账户")
        ...
        if force_pos_ratio is not None:
            print(f"[FORCE-POS] --force-pos-ratio 模式: 强制 {force_pos_ratio*100:.0f}% 走高风险事件路径 (RISK 贷款/大额交易/可疑交易 + retry)")

        print(f"日期范围: {start_dt.date()} ~ {end_dt.date()}")
        print(f"每天数据量: {daily_rule}")

        # 1. 拿所有可用的 (txn, account) 和 (loan, account) 配对
        all_txns_result = await db.execute(text("""
            SELECT txn_id, account_id
            FROM transaction_order
        """))
        all_txns = all_txns_result.all()

        all_loans_result = await db.execute(text("""
            SELECT loan_id, account_id
            FROM loan_info
        """))
        all_loans = all_loans_result.all()

        if not all_txns and not all_loans:
            print("错误: 数据库里没有交易/贷款数据, 请先跑 init_db.py")
            return

        print(f"可用交易: {len(all_txns)} 条, 贷款: {len(all_loans)} 条\n")

        total_days = (end_dt.date() - start_dt.date()).days + 1
        grand_total = 0
        grand_success = 0
        grand_reject = 0
        grand_positive = 0  # 正例计数 (target_pos_ratio 用)

        # 【P4-L3 第四轮 修复】单轮模式跑完所有天, 不再 break
        for day_offset in range(total_days):
            ...
            for i in range(day_count):
                ...
                if use_force_pos:
                    pickers = [
                        ("贷款申请", _pick_forced_loan, "loan"),
                        ("交易", _pick_forced_high_amount_txn, "txn_high"),
                        ("取现", _pick_forced_suspicious_txn, "txn"),
                    ]
                ...
                for try_idx in range(max_tries):
                    ...
                    if use_force_pos:
                        ...
                        if picked is None:
                            # 【P4-L4 2026-08-12 修复】RISK 账户没数据时回退普通池, 避免丢条数
                            picked = await _pick_transaction_for_balance(db, False)
                            if picked is None:
                                continue  # 连普通池都没有, 试下一个 picker
                            txn_id, account_id = picked
                            request = RiskCheckRequest(
                                event_type=random.choice(["交易", "交易", "交易", "转账", "取现"]),
                                source_id=txn_id,
                                user_id=account_id,
                                account_id=account_id,
                            )
                        elif ptype == "loan":
                            loan_id, account_id = picked
                            request = RiskCheckRequest(
                                event_type="贷款申请",
                                source_id=loan_id,
                                user_id=account_id,
                        ...
                        # 普通路径: --balance-pos 80% 概率从 RISK 账户挑, 60% 概率用贷款
                        if balance_pos:
                            loan_odds = 0.6
                        else:
                            loan_odds = 0.3
                        use_loan = random.random() < loan_odds
                        if use_loan and all_loans:
                            picked = await _pick_loan_for_balance(db, balance_pos)
                            if picked:
                                loan_id, account_id = picked
                                request = RiskCheckRequest(
                                    event_type="贷款申请",
                                ...
                                picked = await _pick_transaction_for_balance(db, balance_pos)
                                if not picked:
                                    break
                                txn_id, account_id = picked
                                request = RiskCheckRequest(
                    ...
                    try:
                        ...
                        await db.execute(
                            text("UPDATE risk_user_profile SET update_time = :t WHERE account_id = :uid"),
                        ...
                        grand_success += 1
                        success_done = True
                        # 【P4-L3 第五轮】force 模式: 如果这条没触发正例, 删除已插入的记录再 retry
                        if use_force_pos and new_event and new_assess and new_assess.decision not in ("拒绝", "人工审核"):
                            # 先删子表再删主表, 避免外键约束
                            await db.execute(
                                text("DELETE FROM risk_feature WHERE event_id = :eid"),
                                {"eid": new_event.event_id},
                            )
                            await db.execute(
                                text("DELETE FROM risk_case WHERE assessment_id = :aid"),
                                {"aid": new_assess.assessment_id},
                            )
                            await db.execute(
                                text("DELETE FROM risk_assessment WHERE assessment_id = :aid"),
                                {"aid": new_assess.assessment_id},
                            )
                            await db.execute(
                                text("DELETE FROM risk_event WHERE event_id = :eid"),
                                {"eid": new_event.event_id},
                            )
                            await db.commit()
                            day_success -= 1
                            grand_success -= 1
                            continue  # 换下一个 picker 再试
                        break
                    except Exception as e:
                        await db.rollback()
                        day_reject += 1
                ...
                # 【P4-L3 第五轮】force-pos retry 兜底: 3 次都失败/无正例则跳过
                # (success_done=True 且 day_positive 没++ 即伪正例, 不计入)
            ...
            if target_pos_ratio is not None and day_success > 0:
                ...
                if current_pos_ratio < target_pos_ratio * 0.7:
                    print(f"  ⚠️  今日正例比例 {current_pos_ratio*100:.1f}% < 目标 {target_pos_ratio*100:.0f}% 的 70%, 检查 RISK 账户是否存在 (先跑 scripts/gen_risky_users.py)")
        print(f"\n{'=' * 50}")
        ...
        if overall_pos < 15 and grand_success > 0:
            print(f"⚠️  正例比例仅 {overall_pos:.1f}%, 训练可能假收敛. 建议: 先跑 scripts/gen_risky_users.py 再 --balance-pos --target-pos-ratio 0.30 重造")
