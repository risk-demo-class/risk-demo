"""
金融风控系统 - 模拟风控评估数据生成 (异步)
从现有业务数据中随机选取交易/贷款/账户，调用风控引擎生成评估记录
用于填充仪表盘和案件管理页面的初始数据
...
P4-L3 2026-08-08 新增 --balance-pos:
    ...
    --balance-pos 启用时: 优先挑 RISK00X 系列高风险账户, 把正例比例拉到 25%~35%.
"""
import argparse
import asyncio
import os
...
# P4-L3: --balance-pos 用 RISK 前缀的预置高风险账户 (gen_risky_users.py 造过)
RISKY_ACCOUNT_PREFIX = "RISK"


async def _pick_transaction(db, balance_pos: bool) -> tuple | None:
    """挑一条交易; balance_pos=True 时优先从 RISK 高风险账户里选.

    返回: (txn_id, account_id) 元组或 None
    """
    if balance_pos:
        # 80% 概率从高风险账户里挑 (RISK001~RISK005), 触发更多高风险评估
...
async def _pick_loan(db, balance_pos: bool) -> tuple | None:
    """挑一条贷款申请; balance_pos=True 时优先从 RISK 高风险账户里选."""
    if balance_pos:
        if random.random() < 0.8:
            r = await db.execute(text("""
...
async def generate_risk_data(count: int = 30, balance_pos: bool = False, target_pos_ratio: float | None = None):
    """
    随机从现有交易/贷款中选取, 对每条执行风控检查 (异步).

    Args:
        count: 评估条数
        balance_pos: True 时优先从 RISK 高风险账户挑样本, 拉高正例比例
        target_pos_ratio: 目标正例比例 (0.0-1.0), 设置后自动循环造数据直到达标
            解决 --balance-pos 后正例仍然 < 5% 的问题 (RISK 账户交易数有限)
    """
    ...
    for round_idx in range(max_rounds):
        async with AsyncSessionLocal() as db:
            if balance_pos and round_idx == 0:
                print(f"⚠️  --balance-pos 模式: 80% 概率挑 RISK 高风险账户, 拉高训练正例比例")
            success = 0
            pos_count = 0
            for i in range(count):
                # balance_pos 时更倾向用贷款申请 (高风险账户)
                loan_odds = 0.6 if balance_pos else 0.3
                use_loan = random.random() < loan_odds
                if use_loan:
                    picked = await _pick_loan(db, balance_pos)
                    if not picked:
                        picked = await _pick_transaction(db, balance_pos)
                    if not picked:
                        # 【P4-L3 第二轮】target 模式下, 持续造数据直到达标
                        if target_pos_ratio is not None:
                            continue
                        print(f"  [{i+1}/{count}] 没有可用贷款/交易, 跳过")
                        continue
                ...
                try:
                    ...
                    if target_pos_ratio is None:
                        print(f"  [{i+1}/{count}] {tag} 账户={account_id}, "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="造风控评估数据. --balance-pos 优先挑 RISK 高风险账户, 拉高训练正例比例."
    )
    parser.add_argument("--count", type=int, default=30, help="评估条数 (默认 30)")
    parser.add_argument(
        "--balance-pos", action="store_true",
        help="优先挑 RISK00X 高风险账户, 让训练时正例比例 >= 20% (XGBoost 推荐值)",
    )
    parser.add_argument(
