...
# 固定 40 维特征顺序 (训练和推理都用这个顺序, 防 dict 顺序不一致导致特征错位)
# 改这个列表前必须同步: feature.py::compute_all_features / tests/test_xgboost.py
...
FEATURE_COLUMNS: list[str] = [
    ...
    "txn_count_30d_to_new",
    "counterparty_reg_days",
    # 信贷特征 (8 个)
    "credit_inquiry_3m",
    "credit_usage_rate",
    "duration_months",
...
assert len(FEATURE_COLUMNS) == 40, f"特征数量必须是 40, 当前 {len(FEATURE_COLUMNS)}"
...
if __name__ == "__main__":
    ...
    # 2. 特征向量化 (35 维 dict → numpy 数组)
    print("\n[2] 35 维特征 → numpy 数组 (_features_to_array):")
    print(f"  FEATURE_COLUMNS 长度 = {len(FEATURE_COLUMNS)}  (必须是 35, 跟 feature.py 对齐)")
    sample = {
        # 账户特征 (15 个, acct_*)
        "acct_txn_count_7d": 5, "acct_txn_count_30d": 25, "acct_txn_amount_7d": 15000,
        "acct_txn_amount_30d": 80000, "acct_avg_txn_amount_7d": 3000, "acct_max_txn_amount_30d": 25000,
        "acct_night_txn_count_30d": 2, "acct_failed_txn_count_7d": 0, "acct_device_count_30d": 2,
        "acct_quick_in_out_ratio_30d": 0.3, "acct_counterparty_count_30d": 15,
        "acct_overseas_txn_count_30d": 0, "acct_cash_txn_ratio_30d": 0.1,
        "acct_high_risk_country_count_30d": 0, "acct_new_counterparty_ratio_30d": 0.2,
        # 信贷特征 (10 个, credit_*)
        "credit_inquiry_count_30d": 1, "credit_inquiry_count_90d": 3,
        "credit_card_utilization": 0.4, "credit_multi_lending_count": 1,
        "credit_overdue_count_30d": 0, "credit_overdue_days_max": 0,
        "credit_loan_income_ratio": 0.3, "credit_active_loan_count": 2,
        "credit_total_loan_amount": 50000, "credit_repayment_ability": 2.5,
        # 行为特征 (10 个, beh_*)
        "beh_dormant_days": 5, "beh_info_change_count_30d": 0,
        "beh_password_change_count_30d": 0, "beh_device_env_risk_score": 0.2,
        "beh_sensitive_op_interval_min": 120, "beh_login_failure_count_7d": 0,
        "beh_ip_change_count_30d": 3, "beh_address_change_count_30d": 0,
        "beh_night_operation_count_30d": 1, "beh_high_risk_operation_count_30d": 0,
    }
    arr = _features_to_array(sample)
    print(f"  shape           = {arr.shape}  (期望 (1, 35))")
    ...
    # 让 acct_txn_count_7d (FEATURE_COLUMNS[0]) 强相关 y, 方便看特征重要性
    ...
    print("\n" + "=" * 60)
    print("总结: 35 维特征对齐 / 模型加载 / predict 兜底 / 概率阈值, 4 个核心能力演示完成")
