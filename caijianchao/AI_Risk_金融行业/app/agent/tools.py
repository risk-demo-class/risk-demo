...
async def _query_user_profile_impl(*, db: AsyncSession, user_id: str) -> str:
    """查询用户风险画像. 没画像时实时算 35 个金融特征 (不写库)."""
    profile = await get_user_profile(db, user_id)
    if not profile:
        features = await compute_all_features(db, account_id=user_id)
        return json.dumps({
            "user_id": user_id,
            "has_profile": False,
...
async def _biz_user_txns(db: AsyncSession, account_id: str, txn_id: str, limit: int) -> list[Any]:
    """账户的交易记录 (含聚合总金额)."""
    stmt = (
        select(
            TransactionOrder.txn_id,
            TransactionOrder.txn_time,
            TransactionOrder.txn_type_code,
            TransactionOrder.txn_amount,
            TransactionOrder.txn_status,
            TransactionOrder.counterparty_account,
            TransactionOrder.counterparty_name,
        )
        .where(TransactionOrder.account_id == account_id)
        .order_by(TransactionOrder.txn_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_loans(db: AsyncSession, account_id: str, txn_id: str, limit: int) -> list[Any]:
    """账户的贷款记录."""
    stmt = (
        select(
            LoanInfo.loan_id,
            ...
            LoanInfo.loan_term,
            LoanInfo.annual_rate,
            LoanInfo.loan_purpose,
            LoanInfo.approve_time,
            LoanInfo.due_time,
            LoanInfo.repay_status,
            LoanInfo.overdue_days,
        )
        .where(LoanInfo.account_id == account_id)
        .order_by(LoanInfo.approve_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_credit_cards(db: AsyncSession, account_id: str, txn_id: str, limit: int) -> list[Any]:
    """账户的信用卡信息."""
    stmt = (
        select(
            CreditCardInfo.card_id,
            CreditCardInfo.total_limit,
            CreditCardInfo.used_limit,
            CreditCardInfo.update_time,
        )
        .where(CreditCardInfo.account_id == account_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_txn_detail(db: AsyncSession, account_id: str, txn_id: str, limit: int) -> list[Any]:
    """交易详情."""
    stmt = (
        select(
            TransactionOrder.txn_id,
            TransactionOrder.account_id,
            TransactionOrder.counterparty_account,
            TransactionOrder.counterparty_name,
            TransactionOrder.txn_type_code,
            TransactionOrder.channel_code,
            TransactionOrder.txn_amount,
            TransactionOrder.txn_time,
            TransactionOrder.txn_status,
        ...
        .where(TransactionOrder.txn_id == txn_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_recent_txns(db: AsyncSession, account_id: str, txn_id: str, limit: int) -> list[Any]:
    """系统最近的交易 (不限账户, 管理员视角)."""
    stmt = (
        select(
            TransactionOrder.txn_id,
...
async def _biz_high_value_txns(db: AsyncSession, account_id: str, txn_id: str, limit: int) -> list[Any]:
    """大额交易 (单笔金额 >= 50000).
    ...
    stmt = (
        ...
        .where(TransactionOrder.txn_amount >= 50000)
        .order_by(TransactionOrder.txn_amount.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
# 业务查询 dispatch table
_BIZ_QUERY_HANDLERS = {
    "user_txns": _biz_user_txns,
    "user_loans": _biz_user_loans,
    "user_credit_cards": _biz_user_credit_cards,
    "txn_detail": _biz_txn_detail,
    "recent_txns": _biz_recent_txns,
    "high_value_txns": _biz_high_value_txns,
}


async def _query_business_data_impl(
    *, db: AsyncSession, query_type: str, account_id: str, txn_id: str, limit: int,
) -> str:
    ...
    rows = await handler(db, account_id, txn_id, limit)
    return json.dumps(
...
@tool(
    description=(
        "对指定账户和事件执行实时风险检查。"
        "参数: user_id (账户ID, 如 'ACC001')、event_type (事件类型, 可选值: 交易/转账/取现/贷款申请/账户变更/登录/反洗钱预警)、source_id (关联业务ID, 如交易流水号)。"
...
@tool(
    description=(
        "查询账户的风险画像信息。"
        "参数: user_id (账户ID, 如 'ACC001')。"
        "返回: 账户的风险评分、交易统计、贷款逾期率、登录IP数等画像数据; 没画像时实时算 14 个 account 特征。"
    )
)
async def query_user_profile(user_id: str) -> str:
    return await _safe_call("账户画像查询", _query_user_profile_impl, user_id=user_id)


@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 身份证/手机号/账户/IP/设备ID/国家地区)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: user_txns/user_loans/user_credit_cards/txn_detail/recent_txns/high_value_txns)、account_id (部分需要)、txn_id (部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
async def query_business_data(
    query_type: str, account_id: str = "", txn_id: str = "", limit: int = 10,
) -> str:
    return await _safe_call("业务数据查询", _query_business_data_impl,
                            query_type=query_type, account_id=account_id, txn_id=txn_id, limit=limit)
...
if __name__ == "__main__":
    ...
    impls = [
        ...
        _biz_user_txns, _biz_user_loans, _biz_user_credit_cards,
        _biz_txn_detail, _biz_recent_txns, _biz_high_value_txns,
