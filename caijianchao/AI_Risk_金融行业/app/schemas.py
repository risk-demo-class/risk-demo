...
class RiskCheckRequest(BaseModel):
    """风险检查请求"""
    event_type: Literal["交易", "转账", "取现", "贷款申请", "账户变更", "登录", "反洗钱预警"]
    source_id: str = Field(description="关联业务ID (txn_id / loan_id 等)")
    user_id: str
...
class RiskCheckResponse(BaseModel):
    ...
    # 【P1-S9】被哪种黑名单撞了: None=没撞黑, "身份证"/"手机号"/"账户"/"IP"/"设备ID"/"国家地区" 之一.
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["交易反欺诈", "信贷审批", "反洗钱", "行为异常"]
    event_type: Literal["交易", "转账", "取现", "贷款申请", "账户变更", "登录", "反洗钱预警", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["交易反欺诈", "信贷审批", "反洗钱", "行为异常"]] = None
    event_type: Optional[Literal["交易", "转账", "取现", "贷款申请", "账户变更", "登录", "反洗钱预警", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["身份证", "手机号", "账户", "IP", "设备ID", "国家地区"]
    blacklist_value: str
...
class UserProfileResponse(BaseModel):
    ...
    risk_level: str = "低"
    txn_cnt_30d: int = 0
    txn_amount_30d: float = 0
    avg_txn_amount: float = 0
    daily_fail_count: int = 0
    device_account_count: int = 0
    credit_inquiry_3m: int = 0
    credit_usage_rate: float = 0
    unsettled_lender_count: int = 0
    loan_to_income_ratio: float = 0
    has_overdue_30d: int = 0
    last_txn_days: int = 9999
    device_risk_env: str = "正常"
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        event_type="交易", source_id="txn_demo_001", user_id="ACC001",
        account_id="ACC001", loan_id=None,
        event_data={"amount": 5000, "txn_type": "转账"},
    )
    ...
    hits = [
        RuleHitInfo(
            rule_id="R002", rule_name="单笔极端高额交易",
            rule_category="交易反欺诈", risk_level="极高",
            risk_score=95, action="拒绝",
            description="单笔交易金额≥10000元, 一票否决",
        ),
        RuleHitInfo(
            rule_id="R005", rule_name="异常大额转账",
            rule_category="交易反欺诈", risk_level="高",
    ...
    resp = RiskCheckResponse(
        ...
        features={"acct_txn_count_30d": 25, "acct_txn_amount_30d": 150000},
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="ACC001", event_type="交易", event_source_id="txn_demo_001",
