...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("交易反欺诈", "信贷审批", "反洗钱", "行为异常",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("交易", "转账", "取现", "贷款申请", "账户变更", "登录", "反洗钱预警", "反洗钱", "账户行为", "信贷审批", "通用", name="rule_event_type_enum"),
    ...
    action: Mapped[str] = mapped_column(
        Enum("通过", "标记", "人工审核", "拒绝", "标记关注", "人工复核", "一票否决", "拦截", name="rule_action_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("交易", "转账", "取现", "贷款申请", "账户变更", "登录", "反洗钱预警", "账户行为", "信贷审批", name="event_type_enum"),
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("账户", "交易", "贷款", "设备", "IP", "操作日志", "信贷", "行为", name="feature_entity_type_enum"),
    ...
    feature_value_str: Mapped[Optional[str]] = mapped_column(String(200), comment="特征值(字符串, 与 feature_value 二选一)")
...
class RiskCase(Base):
    ...
    # --- 业务回溯字段 (重做检查时使用) ---
    source_id: Mapped[Optional[str]] = mapped_column(String(50), comment="原始业务ID(交易/贷款/报告ID)")
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("交易", "转账", "取现", "贷款申请", "账户变更", "登录", "反洗钱预警", "账户行为", "信贷审批", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        Enum("身份证", "手机号", "账户", "IP", "设备ID", "国家地区", name="blacklist_type_enum"),
...
class RiskUserProfile(Base):
    """
    用户风险画像表 (宽表)
    字段与 risk_finance.sql 中的 risk_user_profile 表完全对齐
    """
    __tablename__ = "risk_user_profile"

    account_id: Mapped[str] = mapped_column(String(50), primary_key=True, comment="账户ID")
    register_days: Mapped[int] = mapped_column(Integer, default=0, comment="注册天数")
    kyc_level: Mapped[Optional[str]] = mapped_column(String(20), default="未认证", comment="KYC等级")
    account_type: Mapped[Optional[str]] = mapped_column(String(10), default="个人", comment="账户类型")
    ...
    # 交易统计特征
    txn_cnt_7d: Mapped[int] = mapped_column(Integer, default=0, comment="近7天交易笔数")
    txn_cnt_30d: Mapped[int] = mapped_column(Integer, default=0, comment="近30天交易笔数")
    txn_amount_30d: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=0, comment="近30天交易总金额")
    avg_txn_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=0, comment="历史平均交易金额")
    max_single_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=0, comment="历史最大单笔交易金额")
    total_in_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=0, comment="历史累计入账总额")
    total_out_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=0, comment="历史累计出账总额")
    daily_fail_count: Mapped[int] = mapped_column(Integer, default=0, comment="当日交易失败次数")
    night_txn_cnt_7d: Mapped[int] = mapped_column(Integer, default=0, comment="近7天凌晨(0-6点)交易笔数")
    high_freq_cnt_30d: Mapped[int] = mapped_column(Integer, default=0, comment="近30天快进快出次数")
    distinct_counterparty_30d: Mapped[int] = mapped_column(Integer, default=0, comment="近30天不同交易对手数")
    distinct_device_30d: Mapped[int] = mapped_column(Integer, default=0, comment="近30天不同设备数")
    device_account_count: Mapped[int] = mapped_column(Integer, default=0, comment="同一设备关联账户总数")
    
    # 信贷特征
    credit_inquiry_3m: Mapped[int] = mapped_column(Integer, default=0, comment="近3个月征信查询次数")
    credit_usage_rate: Mapped[Decimal] = mapped_column(Numeric(5, 2), default=0, comment="信用卡使用率(%)")
    unsettled_lender_count: Mapped[int] = mapped_column(Integer, default=0, comment="未结清贷款机构数")
    loan_to_income_ratio: Mapped[Decimal] = mapped_column(Numeric(10, 2), default=0, comment="贷款总额/月收入倍数")
    has_overdue_30d: Mapped[int] = mapped_column(Integer, default=0, comment="是否有逾期≥30天记录(0/1)")
    
    # 行为特征
    last_txn_time: Mapped[Optional[datetime]] = mapped_column(DateTime, comment="最后一次交易时间")
    last_txn_days: Mapped[int] = mapped_column(Integer, default=9999, comment="距上次交易天数")
    modify_info_7d: Mapped[int] = mapped_column(Integer, default=0, comment="近7天修改关键信息次数")
    address_change_3m: Mapped[int] = mapped_column(Integer, default=0, comment="近3个月地址变更次数")
    operation_to_txn_minutes: Mapped[Optional[int]] = mapped_column(Integer, comment="敏感操作到交易间隔分钟数")
    is_new_user: Mapped[int] = mapped_column(Integer, default=0, comment="是否新用户(注册<30天)")
    is_high_net_worth: Mapped[int] = mapped_column(Integer, default=0, comment="是否高净值(累计入账>=100万)")
    is_sleeping: Mapped[int] = mapped_column(Integer, default=0, comment="是否沉睡账户(上次交易≥90天)")
    device_risk_env: Mapped[Optional[str]] = mapped_column(String(20), default="正常", comment="设备环境风险")
    
    # 综合评估
    risk_level: Mapped[Optional[str]] = mapped_column(String(10), default="低", comment="综合风险等级")
    risk_score: Mapped[int] = mapped_column(Integer, default=0, comment="综合风险评分(0-1000)")
    update_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, onupdate=datetime.now, comment="更新时间",
    )
