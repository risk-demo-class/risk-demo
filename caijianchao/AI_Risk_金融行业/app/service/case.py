...
async def get_case_list(
    ...
    items = [
        CaseItem(
            ...
            # r[1] = final_score, r[2] = risk_level
            final_score=r[1],
            risk_level=r[2],
...
async def get_case_detail(db: AsyncSession, case_id: str) -> CaseDetailResponse | None:
    ...
    profile = (await db.execute(
        select(RiskUserProfile).where(RiskUserProfile.account_id == case.user_id)
    )).scalar_one_or_none()
    user_profile = None
    if profile:
        user_profile = {
            ...
            "txn_cnt_30d": profile.txn_cnt_30d,
            "txn_amount_30d": float(profile.txn_amount_30d) if profile.txn_amount_30d else 0,
            "avg_txn_amount": float(profile.avg_txn_amount) if profile.avg_txn_amount else 0,
            "daily_fail_count": profile.daily_fail_count,
            "device_account_count": profile.device_account_count,
            "credit_inquiry_3m": profile.credit_inquiry_3m,
            "credit_usage_rate": float(profile.credit_usage_rate) if profile.credit_usage_rate else 0,
            "unsettled_lender_count": profile.unsettled_lender_count,
            "loan_to_income_ratio": float(profile.loan_to_income_ratio) if profile.loan_to_income_ratio else 0,
            "has_overdue_30d": profile.has_overdue_30d,
            "last_txn_days": profile.last_txn_days,
            "device_risk_env": profile.device_risk_env or "正常",
        }
...
async def get_user_profile(db: AsyncSession, account_id: str) -> UserProfileResponse | None:
    profile = (await db.execute(
...
async def list_assessments(
    ...
    筛选维度 (可选):
      ...
      - risk_level: 低 / 中 / 高 / 极高
      - event_type: 交易 / 转账 / 取现 / 贷款申请 / 账户变更 / 登录 / 反洗钱预警 (JOIN risk_event)
      - user_id: 精确匹配
    ...
    class _StatDB:
        ...
        async def execute(self, stmt):
            ...
            if self.call_count == 1:
                ...
                return _R([("交易反欺诈", 12), ("信贷审批", 8), ("反洗钱", 5)])
            else:
                return _R([])
