...
class TestGenRiskyUsersLogic:
    ...
    def test_five_risk_modes_defined(self):
        """5 种金融风险模式必须定义: 高频交易/大额转账/多IP登录/贷款逾期/可疑交易."""
        ...
        expected = ["高频交易", "大额转账", "多IP登录", "贷款逾期", "可疑交易"]
    ...
    def test_count_1_generates_only_first_mode(self):
        """--count 1 只生成 1 个账户 (模式 0: 高频交易, RISK001)."""
        count = 1
        account_ids = [f"RISK{i:03d}" for i in range(1, count + 1)]
        mode_idx = (count - 1) % 5
        assert account_ids == ["RISK001"]
        assert mode_idx == 0  # 高频交易模式
    ...
    def test_count_5_generates_one_set(self):
        ...
        assert account_ids == ["RISK001", "RISK002", "RISK003", "RISK004", "RISK005"]
