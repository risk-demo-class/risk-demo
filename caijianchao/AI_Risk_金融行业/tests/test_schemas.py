...
class TestRiskCheckRequest:
    ...
    def test_valid_request(self):
        ...
        assert req.event_type == "交易"
        assert req.user_id == "ACC001"
        # 可选字段默认 None
        assert req.account_id is None
        assert req.loan_id is None
        assert req.event_data is None

    def test_all_7_event_types(self):
        """7 个合法 event_type 都能过"""
        for et in ["交易", "转账", "取现", "贷款申请", "账户变更", "登录", "反洗钱预警"]:
    ...
    def test_missing_required_field(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RiskCheckRequest(event_type="交易", user_id="u")  # 没传 source_id
...
class TestRuleCreate:
    ...
    def test_rule_score_out_of_range(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RuleCreate(
                rule_id="R1", rule_name="t", rule_category="交易反欺诈", event_type="通用",
