...
class TestScoreCalculation:
    ...
    def test_single_hit(self):
        hit = RuleHitResult(_MockRule("R1", "测试", "交易反欺诈", "高", 70, "人工审核"))
        assert calculate_final_score([hit]) == 70

    def test_multiple_hits_bonus(self):
        """多规则命中: max + BONUS × extra_count"""
        hits = [
            RuleHitResult(_MockRule("R1", "测试1", "交易反欺诈", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R2", "测试2", "行为异常", "中", 40, "标记")),
            RuleHitResult(_MockRule("R3", "测试3", "反洗钱", "低", 20, "通过")),
        ]
...
class TestCaseDeduplication:
    ...
    场景: 同一账户的"交易"事件被前端重试 3 次, 每次都触发"人工审核"
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="交易", source_id="txn_001", user_id="ACC001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="交易", source_id="txn_002", user_id="ACC001",
        )
        ...
        assert db.added[0].event_type == "交易"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="交易", source_id="txn_003", user_id="ACC001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "交易反欺诈", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        ...
        request = RiskCheckRequest(
            event_type="交易", source_id="txn_reject_001", user_id="ACC001",
        )
        ctx = SimpleNamespace(request=request, user_id="U001")
        # R002 风格: risk_level="极高" risk_score=95
        hit = RuleHitResult(_MockRule("R002", "单笔极端高额交易",
                                      "交易反欺诈", "极高", 95, "拒绝"))
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="交易", source_id="txn_review_001", user_id="ACC002",
        )
        ctx = SimpleNamespace(request=request, user_id="U002")
        hit = RuleHitResult(_MockRule("R005", "高折扣率交易",
                                      "交易反欺诈", "高", 65, "人工审核"))
...
class TestCaseCategoryAggregation:
    ...
    async def test_majority_category_wins(self):
        """3 条行为异常 + 1 条交易反欺诈 → 分类 = 行为异常 (命中数最多)"""
        from app.engine import decision
        from app.schemas import RiskCheckRequest
        from types import SimpleNamespace
        ...
        ctx = SimpleNamespace(request=request, user_id="1001")
        # priority 高的 R007 (行为异常) 排第一
        hits = [
            RuleHitResult(_MockRule("R007", "30天30笔交易", "行为异常", "极高", 90, "拒绝")),
            RuleHitResult(_MockRule("R006", "7天10笔", "行为异常", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R008", "金额异常", "行为异常", "高", 65, "人工审核")),
            RuleHitResult(_MockRule("R001", "5000+", "交易反欺诈", "高", 70, "人工审核")),
        ]
        ...
        # 3 条行为异常 vs 1 条交易反欺诈 → 取多的
        assert case.case_category == "行为异常", (
            f"应取命中数最多的 '行为异常', 实际 {case.case_category}"
        )
    ...
    async def test_tie_first_one_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="交易", source_id="txn_y", user_id="ACC001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # 1 条交易反欺诈 + 1 条行为异常 → 平局
        hits = [
            RuleHitResult(_MockRule("R1", "a", "交易反欺诈", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R2", "b", "行为异常", "高", 70, "人工审核")),
        ]
        ...
        assert db.added[0].case_category in ("交易反欺诈", "行为异常")
