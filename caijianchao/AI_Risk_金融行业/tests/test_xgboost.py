...
Coverage:
  1. Feature column order stability (35 dim)
...
class TestFeatureColumns:
    ...
    def test_feature_count_is_35(self):
        """Must be 35-dim features (align with 35 keys computed by feature.py)"""
        assert len(ml_model.FEATURE_COLUMNS) == 35

    def test_feature_prefixes(self):
        """15 acct_* + 10 credit_* + 10 beh_* (align with feature.py naming)"""
        acct = [c for c in ml_model.FEATURE_COLUMNS if c.startswith("acct_")]
        credit = [c for c in ml_model.FEATURE_COLUMNS if c.startswith("credit_")]
        beh = [c for c in ml_model.FEATURE_COLUMNS if c.startswith("beh_")]
        assert len(acct) == 15, f"acct features should be 15, got {len(acct)}"
        assert len(credit) == 10, f"credit features should be 10, got {len(credit)}"
        assert len(beh) == 10, f"beh features should be 10, got {len(beh)}"

    def test_features_to_array(self):
        """dict -> 1x35 ndarray, missing filled with 0, string -> float"""
        features = {
            "acct_txn_count_7d": 10,         # 0
            "acct_txn_count_30d": 3,         # 1
            # intentionally omit 32 middle keys -> 0
            "beh_high_risk_operation_count_30d": 0,  # 34 (last in FEATURE_COLUMNS)
        }
        arr = ml_model._features_to_array(features)
        assert arr.shape == (1, 35)
        assert arr.dtype == np.float32
        assert arr[0, 0] == 10.0
        assert arr[0, 1] == 3.0
        assert arr[0, 2] == 0.0     # missing filled
        assert arr[0, 34] == 0.0    # explicit 0

    def test_feature_columns_align_with_feature_module(self):
        """
        FEATURE_COLUMNS must align 1:1 with the 35 keys actually computed by feature.py.
        ...
        """
        # 15 acct + 10 credit + 10 beh, aligned with feature.py's 3 feat_funcs dict order
        expected = [
            # --- 15 acct features (compute_account_features) ---
            "acct_txn_count_7d",
            "acct_txn_count_30d",
            "acct_txn_amount_7d",
            "acct_txn_amount_30d",
            "acct_avg_txn_amount_7d",
            "acct_max_txn_amount_30d",
            "acct_night_txn_count_30d",
            "acct_failed_txn_count_7d",
            "acct_device_count_30d",
            "acct_quick_in_out_ratio_30d",
            "acct_counterparty_count_30d",
            "acct_overseas_txn_count_30d",
            "acct_cash_txn_ratio_30d",
            "acct_high_risk_country_count_30d",
            "acct_new_counterparty_ratio_30d",
            # --- 10 credit features (compute_credit_features) ---
            "credit_inquiry_count_30d",
            "credit_inquiry_count_90d",
            "credit_card_utilization",
            "credit_multi_lending_count",
            "credit_overdue_count_30d",
            "credit_overdue_days_max",
            "credit_loan_income_ratio",
            "credit_active_loan_count",
            "credit_total_loan_amount",
            "credit_repayment_ability",
            # --- 10 beh features (compute_behavior_features) ---
            "beh_dormant_days",
            "beh_info_change_count_30d",
            "beh_password_change_count_30d",
            "beh_device_env_risk_score",
            "beh_sensitive_op_interval_min",
            "beh_login_failure_count_7d",
            "beh_ip_change_count_30d",
            "beh_address_change_count_30d",
            "beh_night_operation_count_30d",
            "beh_high_risk_operation_count_30d",
        ]
        assert ml_model.FEATURE_COLUMNS == expected, (
            "FEATURE_COLUMNS not aligned with the 35 keys computed by feature.py. "
...
class TestModelLoad:
    ...
    def test_load_real_model(self, capsys):
        ...
        # 2. 准备一组"正常账户"特征 (低值, 期望 → 通过/标记)
        normal_features = {
            # 账户特征 (15个)
            "acct_txn_count_7d": 5,
            "acct_txn_count_30d": 20,
            "acct_txn_amount_7d": 5000.0,
            "acct_txn_amount_30d": 20000.0,
            "acct_avg_txn_amount_7d": 1000.0,
            "acct_max_txn_amount_30d": 3000.0,
            "acct_night_txn_count_30d": 0,
            "acct_failed_txn_count_7d": 0,
            "acct_device_count_30d": 1,
            "acct_quick_in_out_ratio_30d": 0.1,
            "acct_counterparty_count_30d": 5,
            "acct_overseas_txn_count_30d": 0,
            "acct_cash_txn_ratio_30d": 0.0,
            "acct_high_risk_country_count_30d": 0,
            "acct_new_counterparty_ratio_30d": 0.1,
            # 信贷特征 (10个)
            "credit_inquiry_count_30d": 0,
            "credit_inquiry_count_90d": 1,
            "credit_card_utilization": 0.2,
            "credit_multi_lending_count": 0,
            "credit_overdue_count_30d": 0,
            "credit_overdue_days_max": 0,
            "credit_loan_income_ratio": 0.1,
            "credit_active_loan_count": 0,
            "credit_total_loan_amount": 0,
            "credit_repayment_ability": 0.9,
            # 行为特征 (10个)
            "beh_dormant_days": 30,
            "beh_info_change_count_30d": 0,
            "beh_password_change_count_30d": 0,
            "beh_device_env_risk_score": 0.1,
            "beh_sensitive_op_interval_min": 1440,
            "beh_login_failure_count_7d": 0,
            "beh_ip_change_count_30d": 1,
            "beh_address_change_count_30d": 0,
            "beh_night_operation_count_30d": 0,
            "beh_high_risk_operation_count_30d": 0,
        }
        # 3. 准备一组"高风险账户"特征 (强信号, 期望 → 人工审核/拒绝)
        risky_features = dict(normal_features)
        risky_features.update({
            # 高风险账户特征
            "acct_txn_count_7d": 150,
            "acct_txn_count_30d": 500,
            "acct_txn_amount_30d": 800000.0,
            "acct_night_txn_count_30d": 80,
            "acct_failed_txn_count_7d": 30,
            "acct_device_count_30d": 8,
            "acct_quick_in_out_ratio_30d": 0.95,
            "acct_overseas_txn_count_30d": 25,
            "acct_cash_txn_ratio_30d": 0.6,
            "acct_high_risk_country_count_30d": 10,
            # 高风险信贷特征
            "credit_inquiry_count_30d": 15,
            "credit_inquiry_count_90d": 40,
            "credit_card_utilization": 0.95,
            "credit_overdue_count_30d": 5,
            "credit_overdue_days_max": 90,
            "credit_loan_income_ratio": 2.5,
            "credit_active_loan_count": 8,
            "credit_repayment_ability": 0.2,
            # 高风险行为特征
            "beh_dormant_days": 0,
            "beh_info_change_count_30d": 10,
            "beh_password_change_count_30d": 5,
            "beh_device_env_risk_score": 0.9,
            "beh_sensitive_op_interval_min": 5,
            "beh_login_failure_count_7d": 20,
            "beh_ip_change_count_30d": 15,
            "beh_night_operation_count_30d": 40,
            "beh_high_risk_operation_count_30d": 25,
        })
...
class TestPredict:
    ...
    def test_predict_normal_user(self):
        """Normal account (low 35-dim features) -> reject probability low -> decision 'pass'"""
        result = ml_model.predict({
            "acct_txn_count_7d": 5,
            "acct_txn_count_30d": 20,
            "acct_txn_amount_30d": 5000.0,
        })
        assert 0.0 <= result.score <= 1.0
        # Normal account should be decided as "pass"
        # Note: missing 32-dim features keep model stable, but the sample's features
        # are all low, so should not be "reject"
        assert result.decision in ("\u901a\u8fc7", "\u6807\u8bb0")  # "pass" or "mark"

    def test_predict_risky_user(self):
        """高风险账户: 35 维完整特征, 跟训练数据分布对齐.
        用全 35 维特征 + 把"高风险信号"叠加在关键特征上, 让模型能在训练分布内识别."""
        from app.engine.ml_model import FEATURE_COLUMNS
        # 高风险账户: 35 维全部填, 高交易频率/大额/夜间/多设备/高逾期
        risky = {
            # 账户特征 (15个) - 高频大额交易 + 多设备 + 境外交易
            "acct_txn_count_7d": 150,
            "acct_txn_count_30d": 500,
            "acct_txn_amount_7d": 200000.0,
            "acct_txn_amount_30d": 800000.0,
            "acct_avg_txn_amount_7d": 1333.0,
            "acct_max_txn_amount_30d": 50000.0,
            "acct_night_txn_count_30d": 80,
            "acct_failed_txn_count_7d": 30,
            "acct_device_count_30d": 8,
            "acct_quick_in_out_ratio_30d": 0.95,
            "acct_counterparty_count_30d": 50,
            "acct_overseas_txn_count_30d": 25,
            "acct_cash_txn_ratio_30d": 0.6,
            "acct_high_risk_country_count_30d": 10,
            "acct_new_counterparty_ratio_30d": 0.8,
            # 信贷特征 (10个) - 高逾期 + 高负债
            "credit_inquiry_count_30d": 15,
            "credit_inquiry_count_90d": 40,
            "credit_card_utilization": 0.95,
            "credit_multi_lending_count": 10,
            "credit_overdue_count_30d": 5,
            "credit_overdue_days_max": 90,
            "credit_loan_income_ratio": 2.5,
            "credit_active_loan_count": 8,
            "credit_total_loan_amount": 500000.0,
            "credit_repayment_ability": 0.2,
            # 行为特征 (10个) - 频繁信息变更 + 夜间操作 + 高风险操作
            "beh_dormant_days": 0,
            "beh_info_change_count_30d": 10,
            "beh_password_change_count_30d": 5,
            "beh_device_env_risk_score": 0.9,
            ...
            "beh_login_failure_count_7d": 20,
            "beh_ip_change_count_30d": 15,
            "beh_address_change_count_30d": 8,
            "beh_night_operation_count_30d": 40,
            "beh_high_risk_operation_count_30d": 25,
        }
        # 验证 35 维都填了 (否则回退到训练分布外)
        ...
        assert result.score >= normal_result.score, (
            f"高风险账户 score={result.score} 应 >= 普通账户 score={normal_result.score}。"
            f"  如果 fail: 1) 训练数据可能不平衡 (用 --balance-pos 重造), "
            f"2) 特征工程 35 维可能没区分度, 3) 验证 best_iter 是否太低 (假收敛)"
        )
    ...
    def test_predict_fallback_when_not_loaded(self):
        ...
        try:
            ...
            result = ml_model.predict({"acct_txn_count_7d": 1})
