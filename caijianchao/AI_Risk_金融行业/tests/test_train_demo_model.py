...
class TestTrainDemoModelStructure:
    ...
    def test_has_7_pattern_generators(self):
        ...
        for name in ["_gen_high_overdue_rate", "_gen_high_complaint", "_gen_high_amount",
    ...
    def test_uses_correct_feature_order(self):
        """35 维特征顺序必须跟 FEATURE_COLUMNS 一致."""
        src = SCRIPT.read_text(encoding="utf-8")
...
class TestSyntheticDatasetGeneration:
    ...
    def test_gen_synthetic_dataset_default_size(self):
        ...
        assert X.shape == (2000, 35), f"X.shape 错: {X.shape}"
    ...
    def test_gen_synthetic_dataset_feature_order_matches_columns(self):
        ...
        注: 只测单种模式 (高逾期率) 的特征范围, 6 种模式轮换时整体范围更大.
        """
        import sys
        for p in [str(ROOT)]:
            if p not in sys.path:
                sys.path.insert(0, p)
        from scripts.train_demo_model import _gen_high_overdue_rate
        import random
        rng = random.Random(42)
        # 拿 10 个高逾期率样本
        X = np.array([_gen_high_overdue_rate(rng) for _ in range(10)])
        # 第 1 列 (idx=0) 应该是 acct_txn_count_7d (uniform 5-30)
        assert (X[:, 0] >= 5).all() and (X[:, 0] <= 30).all(), (
            f"acct_txn_count_7d 应该是 5-30, 实际范围: {X[:, 0].min()}-{X[:, 0].max()}"
        )
        # 第 20 列 (idx=19) 应该是 credit_overdue_count_30d (uniform 5-20)
        assert (X[:, 19] >= 5).all() and (X[:, 19] <= 20).all(), (
            f"credit_overdue_count_30d 应该是 5-20, 实际: {X[:, 19].min()}-{X[:, 19].max()}"
        )

    def test_normal_user_low_risk_features(self):
        """正常账户模式: 逾期率/设备数/夜间操作 都应偏低."""
        import sys
        for p in [str(ROOT)]:
            if p not in sys.path:
                sys.path.insert(0, p)
        ...
        # 负例: credit_card_utilization (col 17) <= 0.4
        assert (X[:, 17] <= 0.4).all(), f"正常账户 credit_card_utilization 应 <= 0.4, 实际最大: {X[:, 17].max()}"
        # 负例: acct_device_count_30d (col 8) <= 3
        assert (X[:, 8] <= 3).all(), f"正常账户 device_count 应 <= 3, 实际最大: {X[:, 8].max()}"
        # 负例: beh_night_operation_count_30d (col 33) <= 5
        assert (X[:, 33] <= 5).all(), f"正常账户 night_operation 应 <= 5, 实际最大: {X[:, 33].max()}"
...
class TestTrainedModelQuality:
    ...
    def test_trained_model_loads(self, tmp_path):
        ...
        # 推理 1 个样本 (正常账户)
        from scripts.train_demo_model import _gen_normal_user, _gen_high_overdue_rate
        import random
        rng = random.Random(123)
        X_normal = _gen_normal_user(rng).reshape(1, -1)
        X_risk = _gen_high_overdue_rate(rng).reshape(1, -1)
        ...
        # 正常账户 < 0.5, RISK 账户 > 0.5
        assert prob_normal < 0.5, f"正常账户应 < 0.5, 实际: {prob_normal}"
        assert prob_risk > 0.5, f"RISK 账户应 > 0.5, 实际: {prob_risk}"
