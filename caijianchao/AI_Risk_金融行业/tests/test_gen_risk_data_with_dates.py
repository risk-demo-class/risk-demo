...
class TestGenRiskDataForcedPosLogic:
    ...
    def test_force_pos_pickers_defined(self):
        """3 个高风险 picker 必须存在: 贷款逾期 / 可疑交易 / 大额转账."""
        script = SCRIPT_PATH.read_text(encoding="utf-8")
        # 3 个 _pick_forced_* 函数
        assert "_pick_forced_loan_overdue" in script, "必须有 _pick_forced_loan_overdue (RISK 贷款逾期)"
        assert "_pick_forced_suspicious_txn" in script, "必须有 _pick_forced_suspicious_txn (可疑交易)"
        assert "_pick_forced_large_transfer" in script, "必须有 _pick_forced_large_transfer (大额转账)"
