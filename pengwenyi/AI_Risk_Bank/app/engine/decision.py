...
7 步流水线:
  1. 校验 + 准备上下文 (Context)
  2. 创建事件记录 (risk_event)
  3-4. 计算 + 保存特征快照 (48 维, risk_feature)
  5. 加载 + 匹配规则
...
def check_veto(hits: list[RuleHitResult]) -> bool:
    ...
    当前银行版"极高"级别规则: R001 (异地大额转账) / R008 (多卡归集) / R030 (黑卡拦截).
    """
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    """步骤 1b: 请求 → Context, 补全事件关联的业务参数.

    银行 4 大场景:
      - 转账: source_id=txn_id, from_card/to_card/device_id/ip 从 request 或 event_data 补
      - 登录: source_id=login_id, device_id/ip
      - 贷款申请: source_id=loan_id, device_id/ip (金额等走 event_data)
      - 信用卡: source_id=card_id, device_id/ip (金额走 event_data)
    """
    ...
    return _RiskCheckContext(
        ...
        from_card=request.from_card or ed.get("from_card"),
        to_card=request.to_card or ed.get("to_card"),
        device_id=request.device_id or ed.get("device_id"),
        ip=request.ip or ed.get("ip"),
    )
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算 48 个风控特征 (用户/交易/登录/贷款/信用卡 5 类) """
    return await compute_all_features(
        db,
        user_id=ctx.user_id,
        event_type=ctx.request.event_type,
        source_id=ctx.source_id,
        from_card=ctx.from_card,
        to_card=ctx.to_card,
...
def _classify_feature_entity(feature_name: str) -> tuple[str, str]:
    """特征名前缀 → 实体类型. user_* → 用户, txn_* → 交易, login_* → 登录,
    loan_* → 贷款, card_* → 交易 (信用卡消费也属交易, 与 risk_feature.entity_type 枚举对齐),
    其他 (device/ip 相关) → 设备"""
    if feature_name.startswith("user_"):
        return "用户", feature_name
    if feature_name.startswith("txn_") or feature_name.startswith("card_"):
        return "交易", feature_name
    if feature_name.startswith("login_"):
        return "登录", feature_name
    if feature_name.startswith("loan_"):
        return "贷款", feature_name
...
def _save_feature_snapshot(
    ...
    """步骤 4: 把 48 个特征落库到 risk_feature (审计回溯).

    entity_id 填充规则 (按 entity_type):
      - 用户特征 → ctx.user_id
      - 交易/登录/贷款/信用卡特征 → ctx.source_id
      - 设备特征 → ctx.device_id (缺失用 user_id 顶替)
    """
    for fname, fval in features.items():
        entity_type, _ = _classify_feature_entity(fname)
        if entity_type == "用户":
            entity_id = ctx.user_id
        elif entity_type in ("交易", "登录", "贷款", "信用卡"):
...
async def _update_user_profile(
    ...
    risk_user_profile 表结构跟电商版一致 (纪律约束: 9 张表 schema 不动),
    字段按银行语义映射: total_orders→交易笔数, total_refunds→贷款申请数,
    avg_order_amount→平均交易金额, address_count→持卡数, complaint_count→近7天登录失败.
    """
    ...
    # 银行语义映射 (表结构不动, 复用字段)
    profile.total_orders = int(features.get("user_total_txn_count", 0))      # 交易笔数
    profile.total_refunds = int(features.get("user_total_loan_count", 0))    # 贷款申请数
    profile.refund_rate = features.get("user_credit_utilization", 0)         # 信用卡额度使用率
    profile.avg_order_amount = features.get("user_avg_txn_amount", 0)        # 平均交易金额
    profile.address_count = int(features.get("user_card_count", 0))          # 持卡数
    profile.complaint_count = int(features.get("user_failed_login_7d", 0))   # 近7天登录失败
...
def _build_response(
    ...
    features: 48 特征全返回, 但 RISK_FEATURES_FULL_RETURN=False 时脱敏 (P3-M6).
    真生产要按角色判断, 留 TODO.
    """
    triggered_rules = [
        RuleHitInfo(
