...
【重要约定】
  - 特征顺序固定 (FEATURE_COLUMNS), 跟 feature.py 算出的 48 个 key 一一对应
...
# 固定 48 维特征顺序 (训练和推理都用这个顺序, 防 dict 顺序不一致导致特征错位)
# 改这个列表前必须同步: feature.py::compute_user_features / compute_txn_features /
# compute_login_features / compute_loan_features / compute_card_features
...
assert len(FEATURE_COLUMNS) == 48, f"特征数量必须是 48, 当前 {len(FEATURE_COLUMNS)}"
...
def _features_to_array(features: dict) -> np.ndarray:
    """48 维 dict → numpy 数组 (1×48). 缺失用 0.0 兜底; None/字符串统一 float."""
    row = []
...
def predict(features: dict) -> MlResult:
    """推理入口: 48 维特征 → P(拒绝) + ML 维度决策. 模型没加载 → 兜底 score=0 通过."""
...
if __name__ == "__main__":
    ...
    # 2. 特征向量化 (48 维 dict → numpy 数组)
    print("\n[2] 48 维特征 → numpy 数组 (_features_to_array):")
    print(f"  FEATURE_COLUMNS 长度 = {len(FEATURE_COLUMNS)}  (必须是 48, 跟 feature.py 对齐)")
    sample = {
        # 用户维度 15
        "user_credit_score": 720, "user_kyc_level": 3, "user_register_days": 365,
        "user_total_txn_amount": 80000, "user_total_txn_count": 45,
        "user_avg_txn_amount": 1777.78, "user_max_txn_amount": 30000,
        "user_total_loan_count": 1, "user_debt_ratio": 0.3, "user_monthly_income": 20000,
        "user_card_count": 2, "user_credit_utilization": 0.4,
        "user_failed_login_7d": 0, "user_device_count": 1, "user_city_count": 1,
        # 转账维度 9
        "txn_amount": 10000, "txn_is_night": 0, "txn_1h_count": 1,
        "txn_1h_amount": 10000, "txn_city_match": 0, "txn_geo_risk": 0,
        "txn_to_card_black": 0, "txn_1h_into_count": 1, "txn_device_user_count": 1,
        # 登录维度 7
        "login_success": 1, "login_is_night": 0, "login_1h_count": 1,
        "login_city_match": 0, "login_geo_risk": 0, "login_device_new": 0,
        "login_device_user_count": 1,
        # 贷款维度 9
        "loan_amount": 50000, "loan_term_months": 12, "loan_debt_ratio": 0.3,
        "loan_monthly_income": 20000, "loan_month_count": 1, "loan_3m_count": 1,
        "loan_apply_gap_days": 200, "loan_amount_income_ratio": 2.5, "loan_geo_risk": 0,
        # 信用卡维度 8
        "card_amount": 3000, "card_is_night": 0, "card_credit_limit": 50000,
        "card_utilization": 0.2, "card_7d_txn_count": 5, "card_7d_txn_amount": 12000,
        "card_geo_count_7d": 1, "card_device_new": 0,
    }
    arr = _features_to_array(sample)
    print(f"  shape           = {arr.shape}  (期望 (1, 48))")
    ...
    X_demo = np.random.uniform(0, 1, size=(300, 48)).astype(np.float32)
    y_demo = (np.random.rand(300) > 0.66).astype(np.int32)  # 约 100 正
    # 让 user_credit_score (FEATURE_COLUMNS[0]) 强相关 y, 方便看特征重要性
    ...
    print("\n" + "=" * 60)
    print("总结: 48 维特征对齐 / 模型加载 / predict 兜底 / 概率阈值, 4 个核心能力演示完成")
    print("训练数据准备: python scripts/gen_risk_data.py --days 30 --per-day 100")
