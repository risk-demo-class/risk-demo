...
- 审计日志 (RiskActionLog) / 告警 (RiskAlert)

【纪律约束】字段结构(列名/类型/约束)与原电商版 1:1 对齐, 仅枚举值域按银行场景初始化
"""
import json
...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("交易风险", "账户风险", "信贷风险", "设备风险", "网络风险",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类(银行)",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("转账", "登录", "贷款申请", "信用卡", "通用", name="rule_event_type_enum"),
        nullable=False, server_default="通用", comment="适用事件类型(银行)",
    )
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("转账", "登录", "贷款申请", "信用卡", name="event_type_enum"),
        nullable=False, comment="事件类型(银行 4 大场景)",
    )
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("用户", "交易", "设备", "IP", "贷款", "登录", name="feature_entity_type_enum"),
        nullable=False, comment="实体类型(银行)",
    )
...
class RiskCase(Base):
    ...
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("转账", "登录", "贷款申请", "信用卡", name="case_event_type_enum"),
        comment="触发案件的事件类型(银行)",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
    )
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        ...
        nullable=False, comment="黑名单类型(结构保持; 银行专属类型走 blacklist_extra 表)",
    )
