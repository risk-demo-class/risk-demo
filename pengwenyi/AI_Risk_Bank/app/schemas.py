...
class RiskCheckRequest(BaseModel):
    """银行风控检查请求 (4 大场景: 转账/登录/贷款申请/信用卡)

    请求体字段说明:
      - 转账: source_id=txn_id, 可选 from_card/to_card/device_id/ip
      - 登录: source_id=login_id, 可选 device_id/ip
      - 贷款申请: source_id=loan_id, 可选 device_id/ip
      - 信用卡: source_id=card_id, 可选 device_id/ip
    """
    event_type: Literal["转账", "登录", "贷款申请", "信用卡"]
    source_id: str = Field(description="关联业务ID (txn_id / login_id / loan_id / card_id)")
    user_id: str
    # 以下字段可由 event.py 自动补全, 也可前端显式传入 (决策用)
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["交易风险", "账户风险", "信贷风险", "设备风险", "网络风险"]
    event_type: Literal["转账", "登录", "贷款申请", "信用卡", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["交易风险", "账户风险", "信贷风险", "设备风险", "网络风险"]] = None
    event_type: Optional[Literal["转账", "登录", "贷款申请", "信用卡", "通用"]] = None
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        event_type="转账", source_id="txn_demo_001", user_id="U0001",
        from_card="card_001", to_card="card_002", device_id="dev_001",
        ip="1.2.3.4",
        event_data={"amount": 50000, "geo": "广东-深圳"},
    )
    ...
    hits = [
        RuleHitInfo(
            rule_id="R001", rule_name="异地大额转账",
            rule_category="交易风险", risk_level="极高",
            risk_score=95, action="拒绝",
            description="登录城市≠常用城市 且 转账>5万, 一票否决",
        ),
        RuleHitInfo(
            rule_id="R025", rule_name="IP 代理/秒拨",
            rule_category="网络风险", risk_level="中",
            risk_score=40, action="标记",
        ),
    ]
    resp = RiskCheckResponse(
        ...
        features={"user_credit_score": 620, "txn_amount": 80000},
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="U0001", event_type="转账", event_source_id="txn_demo_001",
        final_score=95, risk_level="极高", decision="拒绝",
        rule_count=2, triggered_rules=hits,
        create_time=datetime.now(),
        ml_score=0.92, ml_decision="拒绝",
        event_data=json.dumps({"amount": 80000}, ensure_ascii=False),
    )
    print("\n[3] AssessmentDetailResponse (P3-S9 评估历史详情):")
