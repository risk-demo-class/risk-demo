...
# ============================================================
# 业务数据查询 (字典派发 6 种 query_type, 银行版: 交易/贷款/登录/银行卡)
# ============================================================

async def _biz_user_cards(db: AsyncSession, user_id: str, biz_id: str, limit: int) -> list[Any]:
    """用户的银行卡列表."""
    stmt = (
        select(
            BankCard.card_id,
        ...
        .order_by(BankCard.card_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_transactions(db: AsyncSession, user_id: str, biz_id: str, limit: int) -> list[Any]:
    """用户的交易记录 (通过卡归属反查: 付款卡或收款卡属于该用户)."""
    subq = select(BankCard.card_id).where(BankCard.user_id == user_id)
    stmt = (
        select(
            Transaction.txn_id,
        ...
        .where(Transaction.from_card.in_(subq) | Transaction.to_card.in_(subq))
        .order_by(Transaction.txn_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_loans(db: AsyncSession, user_id: str, biz_id: str, limit: int) -> list[Any]:
    """用户的贷款申请记录."""
    stmt = (
        select(
...
async def _biz_user_logins(db: AsyncSession, user_id: str, biz_id: str, limit: int) -> list[Any]:
    """用户的登录记录."""
    stmt = (
        select(
            LoginLog.login_id,
...
async def _biz_recent_transactions(db: AsyncSession, user_id: str, biz_id: str, limit: int) -> list[Any]:
    """系统最近的交易 (不限用户, 管理员视角)."""
    stmt = (
        select(
            Transaction.txn_id,
...
async def _biz_high_value_transactions(db: AsyncSession, user_id: str, biz_id: str, limit: int) -> list[Any]:
    """大额交易 (单笔 >= 50000, 对应 R001 异地大额/洗钱监测场景)."""
    stmt = (
        select(
            Transaction.txn_id,
            Transaction.from_card,
        ...
        .where(Transaction.amount >= 50000)
...
_BIZ_QUERY_HANDLERS = {
    ...
    "high_value_transactions": _biz_high_value_transactions,
}


async def _query_business_data_impl(
    *, db: AsyncSession, query_type: str, user_id: str, biz_id: str, limit: int,
) -> str:
    """业务数据查询统一入口 (字典派发 6 种 query_type, 银行版)."""
    handler = _BIZ_QUERY_HANDLERS.get(query_type)
    if not handler:
        return f"不支持的查询类型: {query_type}, 可选: {', '.join(_BIZ_QUERY_HANDLERS)}"
    rows = await handler(db, user_id, biz_id, limit)
    return json.dumps(
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查 (银行 4 大场景)。"
        "参数: user_id (用户ID, 如 'U1001' / 'RISK001')、event_type (事件类型, 可选值: 转账/登录/贷款申请/信用卡)、source_id (关联业务ID: txn_id / login_id / loan_id / card_id)。"
...
@tool(
    description=(
        "查询用户的风险画像信息。"
        "参数: user_id (用户ID, 如 'U1001')。"
        "返回: 用户的风险评分、信用分、负债率、交易/贷款/登录统计等画像数据; 没画像时实时算 user 特征。"
    )
)
...
@tool(
    description=(
        "查询银行业务数据, 用于数据分析和风控辅助判断。"
        "参数: query_type (查询类型, 可选值: user_cards/user_transactions/user_loans/user_logins/recent_transactions/high_value_transactions)、user_id (部分需要)、biz_id (业务ID, 部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
async def query_business_data(
    query_type: str, user_id: str = "", biz_id: str = "", limit: int = 10,
) -> str:
    return await _safe_call("业务数据查询", _query_business_data_impl,
                            query_type=query_type, user_id=user_id, biz_id=biz_id, limit=limit)
...
if __name__ == "__main__":
    ...
    print(f"  _BIZ_QUERY_HANDLERS = {list(_BIZ_QUERY_HANDLERS.keys())}  (6 个 query_type, 银行版)")
    ...
    impls = [
        ...
        _biz_user_cards, _biz_user_transactions, _biz_user_loans,
        _biz_user_logins, _biz_recent_transactions, _biz_high_value_transactions,
