...
class TestDeadCodeRemoved:
    ...
    def test_ensure_source_belongs_to_user_kept(self):
        """防越权的核心函数 (业务记录归属校验), 保留."""
        assert hasattr(validator_module, "ensure_source_belongs_to_user")
...
class TestValidatorPublicApi:
    ...
    def test_validator_demo_block_mentions_bank_ensure(self):
        """__main__ demo 块打印 'ensure_*' (银行版, 不含已删死代码)."""
        src = inspect.getsource(validator_module)
        assert "ensure_*" in src, (
            "__main__ demo 块应描述 ensure_* 函数 (ensure_order_exists / ensure_postsale_exists 已删)"
        )
        assert "银行版" in src, "demo 块应标注银行版"
        assert "ensure_order_exists" not in src, "ensure_order_exists 死代码应彻底删除"
        assert "ensure_postsale_exists" not in src, "ensure_postsale_exists 死代码应彻底删除"
