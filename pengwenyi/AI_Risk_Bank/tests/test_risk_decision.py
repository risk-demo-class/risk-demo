...
class TestVetoHardVetoAfterFusion:
    ...
    反例: R030 (黑卡) 命中 → rule_score=95, XGBoost 给 P=0.1 → 10
    ...
    def test_veto_with_low_ml_still_reject(self, monkeypatch):
        ...
        # 模拟一票否决规则: R030 (极高, 95 分)
        veto_hit = RuleHitResult(_MockRule(
            "R030", "黑卡拦截", "交易风险", "极高", 95, "拒绝",
        ))
        ...
        final_score, risk_level, decision_str, ml_score, ml_decision = decision._calculate_decision(
            [veto_hit],
            {"txn_to_card_black": 1, "txn_amount": 12000},  # 触发 R030
        )
...
class TestCaseDeduplication:
    ...
    场景: 同一笔转账的"转账"事件被前端重试 3 次, 每次都触发"人工审核"
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="txn_001", user_id="1001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="txn_002", user_id="1001",
        )
        ...
        assert db.added[0].source_id == "txn_002"
        assert db.added[0].event_type == "转账"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="txn_003", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "交易风险", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="txn_reject_001", user_id="U001",
        )
        ctx = SimpleNamespace(request=request, user_id="U001")
        # R030 风格: risk_level="极高" risk_score=95 (黑卡拦截一票否决)
        hit = RuleHitResult(_MockRule("R030", "黑卡拦截",
                                      "交易风险", "极高", 95, "拒绝"))
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="txn_review_001", user_id="U002",
        )
        ctx = SimpleNamespace(request=request, user_id="U002")
        hit = RuleHitResult(_MockRule("R002", "凌晨密集操作",
                                      "交易风险", "高", 68, "人工审核"))
...
class TestCaseCategoryAggregation:
    ...
    async def test_majority_category_wins(self):
        """3 条交易风险 + 1 条网络风险 → 分类 = 交易风险 (命中数最多)"""
        from app.engine import decision
        from app.schemas import RiskCheckRequest
        from types import SimpleNamespace

        request = RiskCheckRequest(
            event_type="转账", source_id="txn_x", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # priority 高的 R001 (交易风险) 排第一
        rules = [
            RuleHitResult(_MockRule("R001", "异地大额转账", "交易风险", "极高", 92, "拒绝")),
            RuleHitResult(_MockRule("R002", "凌晨密集操作", "交易风险", "高", 68, "人工审核")),
            RuleHitResult(_MockRule("R003", "深夜异常登录", "交易风险", "高", 65, "人工审核")),
            RuleHitResult(_MockRule("R025", "IP代理", "网络风险", "中", 40, "标记")),
        ]
        ...
        # 3 条交易风险 vs 1 条网络风险 → 取多的
        assert case.case_category == "交易风险", (
            f"应取命中数最多的 '交易风险', 实际 {case.case_category}"
        )
    ...
    async def test_tie_first_one_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="txn_y", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # 1 条交易风险 + 1 条网络风险 → 平局
        rules = [
            RuleHitResult(_MockRule("R1", "a", "交易风险", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R2", "b", "网络风险", "高", 70, "人工审核")),
        ]
        ...
        assert db.added[0].case_category in ("交易风险", "网络风险")
