"""
【银行版】测试 - 黑名单 2 种类型 (用户 / IP) 拦截

银行版 _check_all_blacklists 只查:
  1. risk_blacklist "用户" (check_blacklist)
  2. request.ip 命中 blacklist_extra (type='IP') → 返回 "IP"
(电商版的"地址/手机号"黑名单在银行版不存在, 银行专属黑名单走 BlacklistExtra)
"""
...
def _make_request(**overrides) -> RiskCheckRequest:
    """构造一个 RiskCheckRequest, 默认值是"转账/txn_001/1001/10.0.0.1"."""
    base = dict(
        event_type="转账",
        source_id="txn_001",
        user_id="1001",
...
class TestCheckAllBlacklists:
    """测试 _check_all_blacklists: 用户 → IP 顺序查, 短路返回."""
    ...
    async def test_ip_blacklist_hit(self):
        """用户不撞, IP 撞 blacklist_extra → 返回 'IP'"""
        request = _make_request()
        db = _mock_db()
        # 第 1 次 execute 是 check_blacklist (被 patch), 第 2 次是 IP 计数查询 → scalar()=1
    ...
    @pytest.mark.asyncio
    async def test_no_ip_skips_ip_check(self):
        """ip=None → 跳过 IP 查询, 只查用户"""
...
class TestBlacklistReject:
    ...
    def test_blocked_by_2_types(self):
        """2 种类型都能正确填 (用户/IP)"""
        for btype in ("用户", "IP"):
