...
class TestTrainDataValidation:
    ...
    def test_warns_when_pos_ratio_below_15pct(self, caplog):
        ...
        # 警告里应有 "gen_risk_data" 或 "--target-pos-ratio" 提示
        assert any("gen_risk_data" in r.message or "--target-pos-ratio" in r.message for r in caplog.records), (
            "正例比例警告应给出修复建议 (gen_risk_data.py --balance-pos --target-pos-ratio)"
        )
