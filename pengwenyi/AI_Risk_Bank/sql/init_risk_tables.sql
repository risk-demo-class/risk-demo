-- ============================================
-- 银行风控系统 - 风控表 DDL 初始化脚本 (9 张)
-- 纪律约束: 9 张风控表字段结构(列名/类型/约束)与原电商版 1:1 对齐,
-- 仅在新库建表时按银行值域初始化枚举(非 ALTER)
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('交易风险','账户风险','信贷风险','设备风险','网络风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('转账','登录','贷款申请','信用卡','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('转账','登录','贷款申请','信用卡') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','交易','设备','IP','贷款','登录') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `source_id` VARCHAR(50) DEFAULT NULL COMMENT '原始业务ID(交易/登录/贷款/卡ID), 重做检查时用',
    `event_type` ENUM('转账','登录','贷款申请','信用卡') DEFAULT NULL COMMENT '触发案件的事件类型',
...
-- 6. 风控黑名单表 (结构保持: 用户/地址/手机号 3 类型不动)
-- 银行专属黑名单(黑卡/设备指纹/身份证)走 blacklist_extra + 特征规则
