...
总表数: 8 业务 + 9 风控 = 17 张 (risk_bank 库)
"""
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_bank_tables.sql", "创建 8 张银行业务表"),
    ("init_bank_data.sql", "导入银行业务打样数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (银行值域)"),
...
用法:
  ...
  python scripts/init_db.py --db risk_bank_test  # 初始化测试库
        """,
    )
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 17 张表重建 + 业务数据 + 12 条规则全部就绪")
    else:
