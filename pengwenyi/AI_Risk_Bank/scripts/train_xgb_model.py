...
步骤:
  ...
  7. 额外输出 app/engine/xgb_metrics.json (val_auc/特征重要性TOP10/假收敛检查)
     → 供模型报告页 /model 与 README 使用
...
# 【训练数据严格化】显式 WHERE ml_score IS NULL: 训练只取"无 ml 痕迹"数据
#   原因: gen_risk_data.py 造评估数据后统一置 ml_score=NULL (避免"未训练模型"垃圾值),
...
def _load_data_from_mysql() -> tuple[np.ndarray, np.ndarray]:
    ...
    try:
        with conn.cursor() as cur:
            # 1.1 拉评估 (LIMIT 用 settings.XGB_TRAIN_DATA_LIMIT 配置)
            ...
            if len(rows) < settings.XGB_TRAIN_DATA_LIMIT * 0.8:
                logger.warning(
                    ...
                    "建议先 init_db.py --reset --yes 清空, 再 gen_risky_users.py + gen_risk_data.py 重新造",
            ...
            if span_days is not None and span_days < 1 and db_total > 100:
                logger.warning(
                    ...
                    "建议: gen_risk_data.py --days 30 --per-day 100 --clean",
            ...
            logger.info("拉取特征: %d 个 event 各 48 维", len(event_to_features))
            # 2. 拼训练矩阵: 银行 4 场景特征是子集 (转账=15用户+9交易, 登录=15+8,
            #    贷款=15+9, 信用卡=15+8), 缺失维度按 FEATURE_COLUMNS 补 0
            X_list, y_list = [], []
            missing = 0
            ...
            # 【数据质量校验】防止"看起来很多但全是噪声"
            # 真实案例: 5465 条评估里只有 103 个正例 (1.9%), 模型 pos 比例永远 < 3%
            if len(y) > 0 and pos_ratio < 0.15:
                logger.warning(
                    "⚠️  正例比例仅 %.1f%% < 推荐 15%% (DB 总正例: %d / %d = %.1f%%). "
                    "模型学不到 '正例模式', 会假收敛. "
                    "建议: gen_risky_users.py --count 30 + gen_risk_data.py --days 30 --per-day 100",
...
def main() -> None:
    ...
    if "val_accuracy" in metrics:
        ...
        if warnings:
            ...
            logger.warning("        2) 检查 feature.py 48 维是否真的有区分度 (y=1 跟 y=0 特征分布差多少)")
    ...
    if model is not None:
        ...
        for rank, (feat, score) in enumerate(top[:10], 1):
            ...
            top10.append({"rank": rank, "feature": feat, "gain": round(float(score), 2),
                          "ratio": round(100 * score / total_gain, 2)})
        logger.info("=" * 60)
        logger.info("Top 3 特征解释了 %.1f%% 的模型决策", 100 * sum(v for _, v in top[:3]) / total_gain)
        logger.info("→ 业务含义: 重点优化 TOP 5 特征的计算准确性 (P5 优化方向)")

    # 5. 【模型报告】输出 xgb_metrics.json (供 /model 页 + README 使用)
    try:
        import json
        from datetime import datetime as _dt
        ...
        if metrics["best_iteration"] < settings.XGB_MIN_BEST_ITER:
            warnings.append(f"best_iter={metrics['best_iteration']} < {settings.XGB_MIN_BEST_ITER}")
        if metrics.get("val_auc", 0) < settings.XGB_MIN_VAL_AUC:
            warnings.append(f"val_auc={metrics.get('val_auc', 0):.3f} < {settings.XGB_MIN_VAL_AUC}")
        if metrics.get("val_f1", 0) < settings.XGB_MIN_VAL_F1:
            warnings.append(f"val_f1={metrics.get('val_f1', 0):.3f} < {settings.XGB_MIN_VAL_F1}")
        metrics_json = {
            "model": "XGBoost 银行风控双轨模型 (规则 + ML)",
            "trained_at": _dt.now().isoformat(timespec="seconds"),
            "n_train": metrics["n_train"],
            "n_pos": metrics["n_pos"],
            "pos_ratio": round(metrics["pos_ratio"], 4),
            ...
            "val_auc": round(metrics.get("val_auc", 0), 4),
            "val_f1": round(metrics.get("val_f1", 0), 4),
            "val_accuracy": round(metrics.get("val_accuracy", 0), 4),
            "best_f1_threshold": round(metrics.get("best_f1_threshold", 0.5), 4),
            "scale_pos_weight": round(metrics["scale_pos_weight"], 2),
            "feature_importance_top10": top10 if model is not None else [],
            "fake_convergence_check": {
                "ok": not warnings,
                "issues": warnings,
                "min_best_iter": settings.XGB_MIN_BEST_ITER,
                "min_val_auc": settings.XGB_MIN_VAL_AUC,
                "min_val_f1": settings.XGB_MIN_VAL_F1,
            },
            "model_path": metrics["model_path"],
        }
        metrics_path = PROJECT_ROOT / "app" / "engine" / "xgb_metrics.json"
        metrics_path.write_text(json.dumps(metrics_json, ensure_ascii=False, indent=2), encoding="utf-8")
        logger.info("模型报告已写入: %s", metrics_path)
        if warnings:
            logger.warning("[假收敛嫌疑] %s", " | ".join(warnings))
        else:
            logger.info("[假收敛检查] OK: 无假收敛嫌疑 (best_iter>=%d, val_auc>=%.2f, val_f1>=%.2f)",
                        settings.XGB_MIN_BEST_ITER, settings.XGB_MIN_VAL_AUC, settings.XGB_MIN_VAL_F1)
    except Exception as e:  # metrics JSON 失败不影响训练主流程
        logger.warning("输出 xgb_metrics.json 失败: %s", e)
