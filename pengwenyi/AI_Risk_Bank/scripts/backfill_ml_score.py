"""
银行风控系统 - XGBoost ml_score 字段回填脚本

【目的】
  gen_risk_data.py 造评估数据后统一置 ml_score=NULL (避免"未训练模型"推理垃圾值).
  训完基础模型后, 用本脚本回填 ml_score 字段:
    - 用训好的 XGBoost 推理 risk_assessment (从 risk_event.event_data 重建上下文)
        ...
        for i, (aid, eid, uid, dec) in enumerate(rows, 1):
            try:
                async with SessionLocal() as db:
                    # 从 risk_event.event_data 重建上下文 (from_card/to_card/device_id/ip/amount)
                    ev_row = (await db.execute(
                        __import__("sqlalchemy").text("""
                            SELECT event_source_id, event_type, event_data
                            FROM risk_event
                            WHERE event_id = :eid
                        """),
                        {"eid": eid},
                    ...
                    if not ev_row:
                        continue
                    ev_type = ev_row.event_type or "转账"
                    ed = {}
                    if ev_row.event_data:
                        try:
                            ed = json.loads(ev_row.event_data) if isinstance(ev_row.event_data, str) else dict(ev_row.event_data)
                        except Exception:
                            ed = {}
                    # 算 48 维特征 (银行版签名: 按 event_type 派发)
                    features = await compute_all_features(
                        db, uid,
                        event_type=ev_type,
                        source_id=ev_row.event_source_id,
                        from_card=ed.get("from_card"),
                        to_card=ed.get("to_card"),
