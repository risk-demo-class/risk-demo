...
cd ..                       # 回到项目根
python -m app              # 或: uvicorn app.main:app --host 0.0.0.0 --port 8010
```

浏览器打开 **http://127.0.0.1:8010/** 即可使用控制台。

### 7.3 开发期热更新（可选）

若需修改前端并实时预览，可单独启动 Vite dev server（`web/vite.config.ts` 已配置 `/api` 代理到 8010）：
...
| 页面 | 路由 key | 调用的核心接口 |
|------|----------|---------------|
| 风控仪表盘 | `dashboard` | `GET /api/dashboard`（统计概览：今日评估/高风险/通过率/待处理 + 7 天趋势 + 风险等级分布 + 决策动作分布 + Top 规则） |
| 规则管理 | `rules` | `GET /api/rules`、`POST /api/rules`、`PUT /api/rules/{id}`、`DELETE /api/rules/{id}` |
| 案件管理 | `cases` | `GET /api/cases`（含 `active_only` / `all` 等状态筛选） |
| 评估历史 | `assessments` | `GET /api/assessments` |
| 风险检查 | `risk` | `POST /api/risk`（提交 `event_type` + `event_data`，返回决策/评分/命中规则） |
| AI 风控助手 | `agent` | `POST /api/agent/chat` |
| 黑名单 | `blacklist` | `GET /api/blacklist`、`POST /api/blacklist`、`DELETE /api/blacklist/{id}` |

接口层完整路径定义见 `app/routers/` 下对应的 `dashboard.py`、`rule.py`、`case.py`、`assessment.py`、`risk.py`、`agent.py`、`blacklist.py` 等。

> 说明：前端页面默认统一中文展示。规则管理中 `event_type` 以中文显示（转账/贷款申请/登录/卡片交易/还款/通用）；案件管理列表默认按"全部状态"展示。
...
3. **特征回填**：`process_event` 已将三大特征族计算结果通过 `compute_all_features()` 安全合并回事件特征扩展位，可直接落库。
