...
async def _risk_tables_ready(db) -> bool:
    """检查风控核心表是否存在; 教育库刚建业务表时允许调度器跳过。"""
    global _missing_table_warned
    rows = (await db.execute(text("""
        SELECT COUNT(*)
        FROM INFORMATION_SCHEMA.TABLES
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME IN ('risk_case', 'risk_assessment', 'risk_alert')
    """))).scalar() or 0
    ready = rows == 3
    if not ready and not _missing_table_warned:
        logger.warning("[scheduled] 风控核心表未初始化, 跳过本轮调度; 请执行 scripts/init_education_db.py")
...
async def _run_once() -> dict:
    ...
    async with AsyncSessionLocal() as db:
        if not await _risk_tables_ready(db):
            result["errors"].append("risk_tables_not_ready")
            return result
        try:
