...
def _to_comparable_pair(actual: Any, value: Any) -> tuple[Any, Any]:
    """尽量按数字比较, 不能转数字时按原始值比较."""
...
def _compare(actual: Any, op: str, value: Any) -> bool:
    """执行 1 次具体比较运算. 支持数字、布尔和字符串字段."""
    left, right = _to_comparable_pair(actual, value)
    if op == ">":    return left > right
        ...
        return any(left == _to_comparable_pair(actual, item)[1] for item in value)
    elif op == "not_in":
        return all(left != _to_comparable_pair(actual, item)[1] for item in value)
    elif op == "between":
