-- ============================================
-- 旅游风控系统 - 风控核心表 DDL 初始化脚本
-- 结构完全复用电商基线, 仅业务枚举值按行业调整
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('预订欺诈','支付风险','账户风险','签证欺诈','退改签滥用','乘客风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('预订','支付','签证申请','退改签','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('预订','支付','签证申请','退改签') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','订单','乘客') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `source_id` VARCHAR(50) DEFAULT NULL COMMENT '原始业务ID(订单/签证申请ID), 重做检查时用',
    `event_type` ENUM('预订','支付','签证申请','退改签') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','护照号','身份证号','手机号','设备指纹','IP','签证号') NOT NULL COMMENT '黑名单类型',
...
CREATE TABLE IF NOT EXISTS `risk_user_profile` (
    ...
    `total_refunds` INT DEFAULT 0 COMMENT '退改签次数',
    `refund_rate` DECIMAL(5,4) DEFAULT 0 COMMENT '退改率',
    `avg_order_amount` DECIMAL(10,2) DEFAULT 0 COMMENT '平均订单金额',
    `address_count` INT DEFAULT 0 COMMENT '目的地国家数',
    `complaint_count` INT DEFAULT 0 COMMENT '拒签次数',
