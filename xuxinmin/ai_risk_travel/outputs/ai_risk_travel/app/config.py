...
旅游风控版: 业务事件类型 / 规则分类 / 黑名单类型全部按行业定义.
"""
...
class Settings(BaseSettings):
    ...
    # ---- 旅游行业业务枚举 (业务层自定义, 风控核心表结构不变) ----
    # 业务事件类型 (≥3 种, 与 app/schemas.py / 风控表 ENUM 保持一致)
    BUSINESS_EVENT_TYPES: tuple[str, ...] = ("预订", "支付", "签证申请", "退改签")
    # 规则分类
    BUSINESS_RULE_CATEGORIES: tuple[str, ...] = (
        "预订欺诈", "支付风险", "账户风险", "签证欺诈", "退改签滥用", "乘客风险",
    )
    # 黑名单类型 (设备指纹 / 护照号 / 身份证号 等按行业定)
    BUSINESS_BLACKLIST_TYPES: tuple[str, ...] = (
