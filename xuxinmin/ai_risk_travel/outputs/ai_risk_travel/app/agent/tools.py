...
def _row_to_dict(row) -> dict:
    """SQLAlchemy 2.0 的 Row / dict → dict.  datetime → ISO 字符串, Decimal/numpy 数值 → float."""
...
async def _biz_order_detail(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    return [
        ...
        {"passengers": [_row_to_dict(p) for p in passengers]},
        {"flights": [_row_to_dict(f) for f in flights]},
        {"hotels": [_row_to_dict(h) for h in hotels]},
    ]
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (用户ID, 如 'U0001')、event_type (事件类型, 可选值: 预订/支付/签证申请/退改签)、source_id (关联业务ID, 如订单号或签证申请ID)。"
...
@tool(
    description=(
        ...
        "返回: 用户的风险评分、订单统计、退改率、目的地国家数等画像数据; 没画像时实时算 14 个 user 特征。"
    )
)
