"""
旅游风控系统 - 业务实体校验器
集中处理"用户/订单/签证申请"等业务实体的存在性、一致性校验.
所有校验失败都抛 HTTPException, 由 FastAPI 统一返回 4xx 响应.

核心改动 (行业业务):
  _EVENT_SOURCE_VALIDATORS 派发表按旅游事件类型重写:
    - 预订 / 支付 / 退改签 → source_id 必须是 order_info.order_id
    - 签证申请            → source_id 必须是 visa_application.visa_id
"""
import asyncio
import logging
...
_EVENT_SOURCE_VALIDATORS = {
    ("预订", "支付", "退改签"): (OrderInfo, "order_id", None, "订单", 400),
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Service Validator — ensure_* + 旅游事件类型字典派发表")
    print("=" * 60)
    ...
    for evt_types, (model, field, _, label, status) in _EVENT_SOURCE_VALIDATORS.items():
        ...
        print(f"  {evt_str:<14} {model.__name__:<16} {field:<12} {label:<6} {status}")
    ...
    async def demo_ensure_exists():
        ...
        try:
            await ensure_exists(_FakeDB(1), UserInfo, "user_id", "U001", entity_label="用户")
            print("  [OK]   user_id='U001' 存在 → 不抛异常")
        except HTTPException as e:
            print(f"  [FAIL] {e.detail}")
        try:
            await ensure_exists(_FakeDB(0), UserInfo, "user_id", "U999", entity_label="用户")
            print("  [FAIL] 不该到这里")
        except HTTPException as e:
    ...
    print("\n[2] ensure_source_matches_event_type 行为:")
    ...
    async def demo_event_dispatch():
        ...
        req_bad = RiskCheckRequest(event_type="预订", source_id="VSA001", user_id="U001")
        try:
            await ensure_source_matches_event_type(_FlexDB(0, None), req_bad)
            print("  [FAIL] 不该到这里")
        except HTTPException as e:
            print(f"  [400]  source_id='VSA001' 在 OrderInfo 找不到 → {e.detail[:60]}...")

    asyncio.run(demo_event_dispatch())

    print("\n[3] ensure_order_belongs_to_user 防越权:")
    ...
    async def demo_ownership():
        class _OrderOwnerU002:
            async def execute(self, stmt):
                class _R:
                    ...
                    def scalar_one_or_none(self): return "U002"
                return _R()
        try:
        ...
        class _OrderOwnerU001:
            async def execute(self, stmt):
                class _R:
                    def scalar(self): return 1
                    def scalar_one_or_none(self): return "U001"
                return _R()
        try:
    ...
    print("\n" + "=" * 60)
    print("总结: ensure_* 覆盖 '实体存在 + 类型匹配 + 归属' 3 维度校验")
