"""
旅游风控系统 - 风控核心表 ORM (9 张)
结构完全复用电商基线 (不许改列/关系), 仅业务枚举值按行业调整:
  - 事件类型: 预订 / 支付 / 签证申请 / 退改签
  - 规则分类: 预订欺诈 / 支付风险 / 账户风险 / 签证欺诈 / 退改签滥用 / 乘客风险
  - 黑名单类型: 用户 / 护照号 / 身份证号 / 手机号 / 设备指纹 / IP / 签证号
  - 特征实体: 用户 / 订单 / 乘客
- 规则配置 (RiskRule)
...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("预订欺诈", "支付风险", "账户风险", "签证欺诈", "退改签滥用", "乘客风险",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("预订", "支付", "签证申请", "退改签", "通用", name="rule_event_type_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("预订", "支付", "签证申请", "退改签", name="event_type_enum"),
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("用户", "订单", "乘客", name="feature_entity_type_enum"),
...
class RiskCase(Base):
    ...
    # --- 业务回溯字段 (重做检查时使用) ---
    source_id: Mapped[Optional[str]] = mapped_column(String(50), comment="原始业务ID(订单/签证申请ID)")
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("预订", "支付", "签证申请", "退改签", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        Enum("用户", "护照号", "身份证号", "手机号", "设备指纹", "IP", "签证号",
...
class RiskUserProfile(Base):
    ...
    total_refunds: Mapped[int] = mapped_column(Integer, default=0, comment="退改签次数")
    ...
    address_count: Mapped[int] = mapped_column(Integer, default=0, comment="目的地国家数")
    complaint_count: Mapped[int] = mapped_column(Integer, default=0, comment="拒签次数")
