"""Pydantic 请求/响应模型: API 端点的数据校验和序列化 (旅游风控版)"""
...
class RiskCheckRequest(BaseModel):
    """风险检查请求"""
    event_type: Literal["预订", "支付", "签证申请", "退改签"]
    source_id: str = Field(description="关联业务ID (order_id / visa_id 等)")
    user_id: str
...
class RiskCheckResponse(BaseModel):
    ...
    # 【P1-S9】被哪种黑名单撞了: None=没撞黑, 用户/护照号/身份证号/手机号/设备指纹/IP 等.
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["预订欺诈", "支付风险", "账户风险", "签证欺诈", "退改签滥用", "乘客风险"]
    event_type: Literal["预订", "支付", "签证申请", "退改签", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["预订欺诈", "支付风险", "账户风险", "签证欺诈", "退改签滥用", "乘客风险"]] = None
    event_type: Optional[Literal["预订", "支付", "签证申请", "退改签", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["用户", "护照号", "身份证号", "手机号", "设备指纹", "IP", "签证号"]
    blacklist_value: str
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        ...
        event_data={"amount": 5000, "dest_country": "日本"},
    )
    ...
    hits = [
        RuleHitInfo(
            rule_id="R030", rule_name="黑护照拦截",
            rule_category="乘客风险", risk_level="极高",
            risk_score=100, action="拒绝",
            description="乘客证件号在黑名单, 一票否决",
        ),
        RuleHitInfo(
            rule_id="R005", rule_name="大额跨境游",
            rule_category="预订欺诈", risk_level="高",
