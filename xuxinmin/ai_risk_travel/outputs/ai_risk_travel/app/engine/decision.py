"""
旅游风控系统 - 风控决策引擎: 串联 7 步流水线, 事件 → 特征 → 规则 → 评分 → 决策 → 持久化.
...
def check_veto(hits: list[RuleHitResult]) -> bool:
    ...
    当前项目"极高"级别规则: R001 (拒签拦截) / R008 (黄牛囤票) / R030 (黑护照拦截).
    """
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则:
      - "预订" / "支付" / "退改签" 事件: source_id 就是 order_id
      - "签证申请" 事件: source_id 是 visa_id, 不关联订单
    """
    order_id = request.order_id
    if not order_id and request.event_type in ("预订", "支付", "退改签"):
        order_id = request.source_id
    return _RiskCheckContext(
        request=request,
        user_id=request.user_id,
        order_id=order_id,
        receive_id=None,  # 旅游行业无收货地址
    )


async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 旅游行业没有收货地址, receive_id 恒为空, 保留函数签名兼容流水线."""
    return
...
def _save_feature_snapshot(
    ...
    entity_id 填充规则 (按 entity_type):
      ...
      - 乘客特征 → ctx.order_id (缺失用 user_id 顶替)
    """
