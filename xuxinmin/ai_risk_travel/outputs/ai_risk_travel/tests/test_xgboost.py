...
class TestFeatureColumns:
    ...
    def test_feature_prefixes(self):
        """14 user_* + 7 order_* + 4 pax_* (align with feature.py naming)"""
        ...
        pax = [c for c in ml_model.FEATURE_COLUMNS if c.startswith("pax_")]
        assert len(user) == 14, f"user features should be 14, got {len(user)}"
        assert len(order) == 7, f"order features should be 7, got {len(order)}"
        assert len(pax) == 4, f"pax features should be 4, got {len(pax)}"
    ...
    def test_features_to_array(self):
        ...
        features = {
            ...
            "pax_avg_age": 0,                # 24 (last in FEATURE_COLUMNS)
        }
    ...
    def test_feature_columns_align_with_feature_module(self):
        ...
        """
        # 14 user + 7 order + 4 pax, aligned with feature.py's 3 feat_funcs dict order
        expected = [
            ...
            "user_address_count",
            "user_visa_reject_count_90d",
            "user_visa_countries_30d",
            "user_real_name_status",
            "user_account_age_days",
            # --- 7 order features (compute_order_features) ---
            "order_total_amount",
            "order_passenger_count",
            "order_trip_days",
            "order_is_night",
            "order_is_urgent",
            "order_dest_country_count",
            "order_flight_count",
            # --- 4 pax features (compute_passenger_features) ---
            "pax_id_consistency",
            "pax_blacklist_hits",
            "pax_foreign_nationality_count",
            "pax_avg_age",
        ]
...
class TestModelLoad:
    ...
    def test_load_real_model(self, capsys):
        ...
        normal_features = {
            ...
            "user_orders_30d": 1,
            "user_orders_7d": 0,
            "user_total_amount": 20000.0,
            "user_avg_order_amount": 4000.0,
            "user_max_order_amount": 8000.0,
            "user_refund_count": 0,
            "user_refund_rate": 0.0,
            "user_complaint_count": 0,
            "user_address_count": 1,
            "user_visa_reject_count_90d": 0,
            ...
            "order_is_night": 0,
            "order_is_urgent": 0,
            "order_dest_country_count": 1,
            "order_flight_count": 1,
            "pax_id_consistency": 1.0,
            "pax_blacklist_hits": 0,
            "pax_foreign_nationality_count": 0,
            "pax_avg_age": 30,
        }
        ...
        risky_features.update({
            ...
            "user_orders_30d": 20,
            "user_total_amount": 300000.0,
            "user_refund_count": 15,
            "user_refund_rate": 0.6,
            "user_complaint_count": 5,
            "user_visa_reject_count_90d": 3,
            "order_total_amount": 60000.0,
            "pax_blacklist_hits": 1,
        })
...
class TestPredict:
    ...
    def test_predict_risky_user(self):
        ...
        # 高风险用户: 25 维全部填, 高拒签/大额/深夜紧急/证件不一致
        risky = {
            # 用户画像 (高拒签 + 高频退订 + 新号)
            "user_total_orders": 30,
            "user_orders_30d": 15,
            "user_orders_7d": 8,
            "user_total_amount": 300000,       # 30w 累计
            "user_avg_order_amount": 10000,
            "user_max_order_amount": 60000,    # 6w 单笔
            "user_refund_count": 12,           # 高退订
            "user_refund_rate": 0.4,
            "user_complaint_count": 4,         # 拒签 4 次
            "user_address_count": 6,           # 多目的地国家
            "user_visa_reject_count_90d": 3,   # 近 90 天拒签 3 次
            "user_visa_countries_30d": 4,      # 30 天 4 国签证
            "user_real_name_status": 0,        # 未实名
            "user_account_age_days": 3,        # 新号
            # 订单特征 (大额 + 深夜 + 紧急 + 多航班)
            "order_total_amount": 60000,
            "order_passenger_count": 5,
            "order_trip_days": 3,
            "order_is_night": 1,               # 凌晨下单
            "order_is_urgent": 1,              # 紧急行程
            "order_dest_country_count": 4,
            "order_flight_count": 5,           # 黄牛囤票
            # 乘客特征 (证件不一致 + 撞黑)
            "pax_id_consistency": 0.0,
            "pax_blacklist_hits": 1,
            "pax_foreign_nationality_count": 2,
            "pax_avg_age": 35,
        }
