"""
TravelRisk 旅游风控中心 - 一键数据库初始化脚本（异步）
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 9 张旅游业务表"),
    ("init_business_data.sql", "导入业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 8 条旅游风控规则 (TR001-TR008)"),
]
...
async def main():
    parser = argparse.ArgumentParser(
        description="TravelRisk - 一键数据库初始化（异步）. 默认 --reset 重置整个数据库",
...
用法:
  ...
  python scripts/init_db.py --db travel_risk_test # 初始化测试库
        """,
    )
    ...
    print("=" * 60)
    print("TravelRisk 旅游风控中心 - 数据库初始化（异步）")
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 9 张旅游业务表 + 9 张风控表 + 8 条规则全部就绪")
    else:
