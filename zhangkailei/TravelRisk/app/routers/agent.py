...
async def api_agent_chat(data: AgentChatRequest, _admin: str = Depends(require_admin)):
    try:
...
async def api_clear_session(session_id: str, _admin: str = Depends(require_admin)):
