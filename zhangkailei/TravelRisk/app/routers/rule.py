...
async def api_create_rule(data: RuleCreate, db: AsyncSession = Depends(get_db_async),
    ...
    if existing:
        ...
        rule = RiskRule(
            ...
            rule_category=data.rule_category, event_type=data.event_type,
            rule_condition=json.dumps(data.rule_condition, ensure_ascii=False),
            risk_level=data.risk_level, risk_score=data.risk_score,
            action=data.action, priority=data.priority, description=data.description,
        )
        db.add(rule)
    await record_action(
