...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则:
      - source_id 指向机票、酒店、签证或支付实体
      - 旅游适配层负责把它们统一解析为 order_id
    """
...
async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    ...
    旅游适配层已经完成关联载体补全，此函数保留流水线兼容性。
    """
    # 旅游适配层已在 process_event 中把设备等关联载体写入 receive_id。
    return
...
async def _update_user_profile(
    ...
    profile.total_orders = int(features.get("user_order_count_30d", 0))
    profile.total_refunds = int(features.get("user_visa_reject_count_90d", 0))
    profile.refund_rate = features.get("user_cancel_rate_90d", 0)
    count = max(features.get("user_order_count_30d", 0), 1)
    profile.avg_order_amount = round(features.get("user_total_amount_30d", 0) / count, 2)
    profile.address_count = int(features.get("user_device_count_7d", 0))
    profile.complaint_count = int(features.get("user_visa_reject_count_90d", 0))
...
def _build_response(
    ...
    if decision == "拒绝":
        message = "触发高危旅游规则，系统拒绝"
    elif decision == "人工审核":
        message = "风险较高，已进入人工审核"
    elif decision == "标记":
        message = "订单放行并标记观察"
    else:
        message = "风险检查通过"
