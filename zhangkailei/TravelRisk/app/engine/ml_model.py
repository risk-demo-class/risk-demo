...
FEATURE_COLUMNS: list[str] = [
    ...
    "user_order_count_30d",
    "user_country_count_30d",
    "user_visa_apply_count_30d",
    "user_visa_reject_count_90d",
    "user_cancel_rate_90d",
    "user_total_amount_30d",
    "user_device_count_7d",
    "user_passenger_match_rate",
    "order_total_amount",
    ...
    "order_is_night",
    "order_same_flight_count_1h",
    "order_payment_user_mismatch",
    "order_non_refundable_ratio",
    "order_destination_risk_score",
    "addr_passport_blacklisted",
    "addr_duplicate_identity_users",
    "addr_nationality_mismatch",
    "addr_device_shared_users_7d",
    "addr_ip_booking_count_1h",
]
...
def load_model(model_path: Optional[str] = None) -> bool:
    ...
    try:
        ...
        model_features = _MODEL.feature_names
        if model_features != FEATURE_COLUMNS:
            expected = ", ".join(FEATURE_COLUMNS[:3])
            actual = ", ".join((model_features or [])[:3]) or "无特征名"
            logger.warning(
                "模型特征与旅游风控特征不兼容: "
                f"expected={len(FEATURE_COLUMNS)}维[{expected}, ...], "
                f"actual={len(model_features or [])}维[{actual}, ...]。请重新训练旅游模型"
            )
            _MODEL = None
            _LOADED = False
            return False
...
def predict(features: dict) -> MlResult:
    ...
    try:
        ...
        # 推理失败必须退回纯规则，不能用 ML=0 参与融合拉低规则分。
        return MlResult(score=0.0, decision="通过", is_loaded=False)
