...
# TravelRisk 嵌套在原项目中开发时，如果还没创建自己的 .env，只复用父项目的
# MySQL 连接凭据；DB_NAME 仍使用本项目默认的 travel_risk，避免误写原业务库。
if not PROJECT_ENV_FILE.exists():
    parent_env = dotenv_values(PROJECT_ROOT.parent / ".env")
    for key in ("DB_HOST", "DB_PORT", "DB_USER", "DB_PASSWORD"):
        value = parent_env.get(key)
        if value:
            os.environ.setdefault(key, str(value))
    # PyCharm 常会把父项目 .env 全量注入进程，导致 DB_NAME=ecs 以环境变量形式
    # 覆盖本项目默认值。旅游项目无独立 .env 时必须隔离数据库名，避免串库。
    os.environ["DB_NAME"] = os.environ.get("TRAVEL_DB_NAME", "travel_risk")
...
class Settings(BaseSettings):
    ...
    # ---- 旅游行业配置 ----
    TRAVEL_HIGH_RISK_COUNTRIES: list[str] = ["高风险国A", "高风险国B", "高风险国C"]
    TRAVEL_BLACKLIST_TYPES: list[str] = ["PASSPORT", "VISA", "DEVICE", "IP"]
