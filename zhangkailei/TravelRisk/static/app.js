...
// 1. 25 维旅游特征 key↔中文标签（与 ml_model.FEATURE_COLUMNS 对齐）
const FEATURE_LABELS = {
    "user_account_age_days": "账号注册天数",
    "user_real_name_status": "实名认证状态",
    "user_order_count_30d": "30天旅游订单数",
    "user_country_count_30d": "30天目的地国家数",
    "user_visa_apply_count_30d": "30天签证申请数",
    "user_visa_reject_count_90d": "90天拒签次数",
    "user_cancel_rate_90d": "90天订单取消率",
    "user_total_amount_30d": "30天累计预订金额",
    "user_device_count_7d": "7天使用设备数",
    "user_passenger_match_rate": "历史乘客匹配率",
    "order_total_amount":        "订单总金额",
    "order_passenger_count": "乘客数量",
    "order_days_to_depart": "距离出发天数",
    "order_trip_days": "行程天数",
    "order_is_cross_border": "是否跨境",
    "order_is_night":            "是否凌晨下单",
    "order_same_flight_count_1h": "一小时同航班票数",
    "order_payment_user_mismatch": "支付主体是否异常",
    "order_non_refundable_ratio": "不可退款金额占比",
    "order_destination_risk_score": "目的地风险分",
    "addr_passport_blacklisted": "证件是否命中黑名单",
    "addr_duplicate_identity_users": "同证件关联用户数",
    "addr_nationality_mismatch": "国籍资料是否冲突",
    "addr_device_shared_users_7d": "7天设备关联用户数",
    "addr_ip_booking_count_1h": "一小时同IP预订数",
};
...
const RISK_EVENT_THRESHOLDS = {
    "机票预订": {pass: 30, mark: 60, review: 85},
    "酒店预订": {pass: 35, mark: 60, review: 85},
    "签证申请": {pass: 25, mark: 55, review: 85},
    "订单支付": {pass: 25, mark: 55, review: 85},
...
function renderCondNode(cond, parent, isRoot) {
    if (cond && (cond.and || cond.or)) {
        ...
        footer.querySelector('.cond-add-leaf').addEventListener('click', () => {
            for (let i = 0; i < 3; i++) {
                children.push({field: 'user_order_count_30d', op: '>=', value: 0});
            }
        ...
        footer.querySelector('.cond-add-group').addEventListener('click', () => {
            ...
            children.push({and: [{field: 'user_order_count_30d', op: '>=', value: 0}]});
            buildConditionUI(collectCondition(parent), parent.id);
        });
