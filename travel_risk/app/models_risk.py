...
- 操作审计 (RiskActionLog) + 告警 (RiskAlert)
"""
import json
...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("下单欺诈", "支付风险", "账户风险", "退改滥用", "出行人风险", "供应商风险", "综合风险",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("注册", "下单", "支付", "退改申请", "理赔申请", "投诉", "通用", name="rule_event_type_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("注册", "下单", "支付", "退改申请", "理赔申请", "投诉", "通用", name="event_type_enum"),
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("用户", "订单", "出行人", name="feature_entity_type_enum"),
...
class RiskCase(Base):
    ...
    source_id: Mapped[Optional[str]] = mapped_column(String(50), comment="原始业务ID(订单/退改/理赔/投诉ID)")
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("注册", "下单", "支付", "退改申请", "理赔申请", "投诉", "通用", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        Enum("用户", "手机号", "设备", name="blacklist_type_enum"),
...
class RiskUserProfile(Base):
    ...
    total_bookings: Mapped[int] = mapped_column(Integer, default=0, comment="总旅游订单数")
    total_refunds: Mapped[int] = mapped_column(Integer, default=0, comment="退改次数")
    refund_rate: Mapped[Decimal] = mapped_column(Numeric(5, 4), default=0, comment="退改率")
    avg_order_amount: Mapped[Decimal] = mapped_column(Numeric(10, 2), default=0, comment="平均订单金额")
    traveler_count: Mapped[int] = mapped_column(Integer, default=0, comment="累计出行人数")
    complaint_count: Mapped[int] = mapped_column(Integer, default=0, comment="投诉次数")
    claim_count: Mapped[int] = mapped_column(Integer, default=0, comment="理赔次数")
...
class RiskAlert(Base):
    """告警记录表: 风控系统自身健康监控 (规则命中率突降/案件积压/撞黑异常)"""
