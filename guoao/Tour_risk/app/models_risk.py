"""
物流风控系统 - 风控表 ORM (9 张, 与电商基线一致, 只改枚举)
风控系统自建表, 跟业务表分开管理
- 规则配置 (RiskRule)
...
- 审计 / 告警 (RiskActionLog / RiskAlert)

枚举按物流行业调整:
  - 事件类型: 寄件 / 跨境申报 / 代收货款 / 实名认证
  - 规则分类: 危险品防控 / 寄递行为 / 代收货款 / 跨境申报 / 账户风险 / 地址风险
  - 特征实体: 用户 / 运单 / 地址
  - 行业扩展黑名单 (身份证号/寄件网点) 放业务表 blacklist_extra
"""
import json
...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("危险品防控", "寄递行为", "代收货款", "跨境申报", "账户风险", "地址风险",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("寄件", "跨境申报", "代收货款", "实名认证", "通用", name="rule_event_type_enum"),
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("寄件", "跨境申报", "代收货款", "实名认证", name="event_type_enum"),
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("用户", "运单", "地址", name="feature_entity_type_enum"),
...
class RiskCase(Base):
    ...
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("寄件", "跨境申报", "代收货款", "实名认证", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
