"""
风控决策引擎 (物流版): 串联 7 步流水线, 事件 → 特征 → 规则 → 评分 → 决策 → 持久化.
...
7 步流水线:
  1. 校验 + 准备上下文 (Context)
  2. 创建事件记录 (risk_event)
  3-4. 计算 + 保存特征快照 (25 维物流特征, risk_feature)
  5. 加载 + 匹配规则
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    """步骤 1b: 请求 → Context, 顺便补全 shipment_id.

    业务规则:
      - "寄件" / "跨境申报" / "代收货款" 事件: source_id 就是 shipment_id (运单号)
      - "实名认证" 事件: source_id 是 user_id, 没有运单
    """
    shipment_id = request.shipment_id
    if not shipment_id and request.event_type in ("寄件", "跨境申报", "代收货款"):
        shipment_id = request.source_id
...
async def _enrich_address_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 从运单里补全 address_id (收件地址 ID).

    退出条件 (任一满足就不补): 已有 address_id / 没有 shipment_id (实名认证场景).
    """
    if ctx.address_id or not ctx.shipment_id:
        return
    row = (await db.execute(
        select(Shipment.address_id).where(Shipment.shipment_id == ctx.shipment_id)
    )).first()
    if row:
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算 25 个物流风控特征 (用户/运单/地址 3 类)"""
    return await compute_all_features(
        db,
        user_id=ctx.user_id,
        shipment_id=ctx.shipment_id,  # 可能 None (实名认证场景只算用户+地址)
        address_id=ctx.address_id,
    )


def _classify_feature_entity(feature_name: str) -> tuple[str, str]:
    """特征名前缀 → 实体类型. user_* → 用户, shipment_* → 运单, 其他 → 地址"""
...
def _save_feature_snapshot(
    ...
    entity_id 填充规则 (按 entity_type):
      - 用户特征 → ctx.user_id
      - 运单特征 → ctx.shipment_id (缺失用 source_id 顶替)
    ...
    for fname, fval in features.items():
        ...
        if entity_type == "用户":
            ...
            entity_id = ctx.shipment_id or ctx.request.source_id
        else:  # 地址
...
async def _update_user_profile(
    ...
    字段语义按物流行业映射: 总订单→总寄件数, 退款→COD 拒收, 平均金额→平均申报价值.
    """
    ...
    profile.total_orders = int(features.get("user_shipment_count", 0))
    profile.total_refunds = int(features.get("user_cod_count", 0))
    profile.refund_rate = features.get("user_cod_reject_rate", 0)
    ...
    profile.complaint_count = int(features.get("user_undeclared_count", 0))
