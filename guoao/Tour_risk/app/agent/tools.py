...
# ============================================================
# 业务数据查询 (物流版, 字典派发 6 种 query_type)
# ============================================================

async def _biz_user_shipments(db: AsyncSession, user_id: str, shipment_id: str, limit: int) -> list[Any]:
    """用户的所有运单 (含重量/申报价值/COD 金额/状态)."""
    stmt = (
        select(
...
async def _biz_user_addresses(db: AsyncSession, user_id: str, shipment_id: str, limit: int) -> list[Any]:
    """用户的收件地址."""
    stmt = (
        select(
            Address.address_id,
...
async def _biz_shipment_items(db: AsyncSession, user_id: str, shipment_id: str, limit: int) -> list[Any]:
    """运单的物品明细行."""
    stmt = (
        select(
            ...
            ShipmentItem.item_category,
            ShipmentItem.quantity,
            ShipmentItem.unit_weight,
...
async def _biz_recent_shipments(db: AsyncSession, user_id: str, shipment_id: str, limit: int) -> list[Any]:
...
async def _biz_high_value_shipments(db: AsyncSession, user_id: str, shipment_id: str, limit: int) -> list[Any]:
    """高价值/高金额运单 (申报价值 >= 5000 或 COD 金额 >= 3000)."""
    stmt = (
        select(
            Shipment.shipment_id,
            Shipment.user_id,
        ...
        .where(
            (Shipment.declared_value >= 5000) | (Shipment.cod_amount >= 3000)
        )
        .order_by(Shipment.create_time.desc())
        .limit(limit)
    )
...
async def _biz_shipment_detail(db: AsyncSession, user_id: str, shipment_id: str, limit: int) -> list[Any]:
    """单个运单详情 (含收件人/地址/申报信息)."""
    stmt = (
        select(
            Shipment.shipment_id,
            ...
            Shipment.origin_province,
            Shipment.dest_province,
            Shipment.dest_city,
            Shipment.weight_kg,
            Shipment.declared_value,
            Shipment.cod_amount,
            Shipment.is_dangerous_declared,
            Shipment.status,
            Shipment.create_time,
        )
...
_BIZ_QUERY_HANDLERS = {
    ...
    "high_value_shipments": _biz_high_value_shipments,
    "shipment_detail": _biz_shipment_detail,
}


async def _query_business_data_impl(
    *, db: AsyncSession, query_type: str, user_id: str, shipment_id: str, limit: int,
) -> str:
    ...
    rows = await handler(db, user_id, shipment_id, limit)
    return json.dumps(
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: user_shipments/user_addresses/shipment_items/recent_shipments/high_value_shipments/shipment_detail)、user_id (部分需要)、shipment_id (运单号, 部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
async def query_business_data(
    query_type: str, user_id: str = "", shipment_id: str = "", limit: int = 10,
) -> str:
    return await _safe_call("业务数据查询", _query_business_data_impl,
                            query_type=query_type, user_id=user_id, shipment_id=shipment_id, limit=limit)
