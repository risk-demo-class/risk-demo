"""Pydantic 请求/响应模型: API 端点的数据校验和序列化 (物流版)"""
...
class RiskCheckRequest(BaseModel):
    """风险检查请求 (物流版: 事件类型为 寄件/跨境申报/代收货款/实名认证)"""
    event_type: Literal["寄件", "跨境申报", "代收货款", "实名认证"]
    source_id: str = Field(description="关联业务ID (shipment_id 运单号 / user_id)")
    user_id: str
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["危险品防控", "寄递行为", "代收货款", "跨境申报", "账户风险", "地址风险"]
    event_type: Literal["寄件", "跨境申报", "代收货款", "实名认证", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["危险品防控", "寄递行为", "代收货款", "跨境申报", "账户风险", "地址风险"]] = None
    event_type: Optional[Literal["寄件", "跨境申报", "代收货款", "实名认证", "通用"]] = None
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        event_type="寄件", source_id="SH2026080001", user_id="U0001",
        shipment_id="SH2026080001", address_id="AD_U0001_0",
        event_data={"weight_kg": 5, "declared_value": 800},
    )
    ...
    hits = [
        RuleHitInfo(
            rule_id="R001", rule_name="危险品瞒报",
            rule_category="危险品防控", risk_level="极高",
            risk_score=95, action="拒绝",
            description="运单含危险品但未如实申报, 一票否决",
        ),
        RuleHitInfo(
            rule_id="R005", rule_name="大额代收货款",
            rule_category="代收货款", risk_level="高",
    ...
    resp = RiskCheckResponse(
        ...
        features={"user_shipment_count": 3, "shipment_declared_value": 15000},
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="U0001", event_type="寄件", event_source_id="SH2026080001",
