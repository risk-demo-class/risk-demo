"""
业务实体校验器 (物流版): 集中处理"用户/运单"等业务实体的存在性、一致性校验.
...
from app.models import Shipment, UserInfo
...
async def ensure_shipment_belongs_to_user(
    ...
    owner = (await db.execute(
        select(Shipment.user_id).where(Shipment.shipment_id == shipment_id).limit(1)
    )).scalar_one_or_none()
    if not owner:
    ...
    if owner != user_id:
        ...
        raise HTTPException(
            status_code=403,
            detail=f"运单 {shipment_id} 属于用户 {owner}, 与请求用户 {user_id} 不一致",
        )


# source_id 与 event_type 匹配的校验规则 (字典派发, 加新 event_type 只加 1 行)
_EVENT_SOURCE_VALIDATORS = {
    ("寄件", "跨境申报", "代收货款"): (Shipment, "shipment_id", None, "运单", 400),
    ("实名认证",): (UserInfo, "user_id", None, "用户", 400),
}
...
async def validate_risk_check_request(
    ...
    if request.event_type in ("寄件", "跨境申报", "代收货款"):
        shipment_id = request.shipment_id or request.source_id
        await ensure_shipment_belongs_to_user(db, shipment_id, request.user_id)


# ============================================================
# Demo: 字典派发表 + 完整校验链 (mock DB)
# 跑法: python app/service/validator.py
# ============================================================
if __name__ == "__main__":
    print("=" * 60)
    print("Service Validator (物流版) — 校验派发表")
    print("=" * 60)
    print(f"  寄件/跨境申报/代收货款 → Shipment.shipment_id (运单存在 + 归属校验)")
    print(f"  实名认证              → UserInfo.user_id")
