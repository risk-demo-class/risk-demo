...
async def execute_sql_file(conn, filepath, description, db_name):
    """执行单个 SQL 脚本文件 (异步)"""
    ...
    # 把 SQL 里硬编码的 USE ecs; 替换成目标库 (init_business 系列没有 USE, 风控系列有)
    sql_content = sql_content.replace("USE ecs;", f"USE `{db_name}`;")
    ...
    try:
        for idx, (filename, desc) in enumerate(SQL_FILES, start=2):
            ...
            if not os.path.exists(filepath):
                ...
                continue
            print(f"\n[步骤 {idx}/5]", end="")
            errors = await execute_sql_file(conn, filepath, desc, args.db)
            total_errors += errors
    finally:
        await conn.ensure_closed()
