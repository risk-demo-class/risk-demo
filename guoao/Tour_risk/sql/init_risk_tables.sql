-- ============================================
-- 物流风控系统 - 风控表 DDL 初始化脚本
-- 9 张风控表 (与电商基线一致, 枚举按物流行业调整)
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('危险品防控','寄递行为','代收货款','跨境申报','账户风险','地址风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('寄件','跨境申报','代收货款','实名认证','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('寄件','跨境申报','代收货款','实名认证') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','运单','地址') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('寄件','跨境申报','代收货款','实名认证') DEFAULT NULL COMMENT '触发案件的事件类型',
