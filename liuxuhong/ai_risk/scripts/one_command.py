"""
教育风控系统 - 一条龙命令 (P4-L4 2026-08-08)
从零开始, 一键完成所有准备 + 训练, 最后启动 Web 服务.

【工作流 6 步】
  1. 重置数据库 (init_db.py --reset --yes, 含 16 条规则)
  ...
  6. 造 50 条今日业务数据 (gen_risk_data.py --count 50 --balance-pos)
...
def step_6_business_today() -> None:
    ...
    _run_subprocess(
        "6/6 造 50 条今日业务数据 (gen_risk_data.py --count 50 --balance-pos)",
        [sys.executable, os.path.join(SCRIPTS, "gen_risk_data.py"),
         "--count", "50", "--balance-pos"],
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="教育风控系统 - 一条龙命令 (6 步: 重置 → RISK → 训练数据 → 训练 → 回填 → 启动)",
...
示例:
  ...
  python scripts/one_command.py --only-start   # 只跑步骤 6 (造今日业务数据)
    ...
    print("  python scripts/gen_train_dataset.py --reset   # 重造强标注训练集")
    print("=" * 70)
