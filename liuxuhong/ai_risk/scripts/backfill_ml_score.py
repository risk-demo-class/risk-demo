        ...
        for i, (aid, eid, uid, dec) in enumerate(rows, 1):
            try:
                async with SessionLocal() as db:
                    # 从 risk_event 读 event_source_id + event_type (教育版无 order_id 列)
                    # 课程报名: source_id 就是 order_id, 可算 order 特征
                    # 退费/认证/打赏: 只算 user 特征 (order_id=None)
                    ...
                    if not ev_row:
                        continue
                    # 算 25 维特征
                    order_id = ev_row.event_source_id if ev_row.event_type == "课程报名" else None
