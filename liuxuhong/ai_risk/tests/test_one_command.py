...
class TestOneCommandStructure:
    ...
    def test_steps_call_real_scripts(self):
        ...
        assert "gen_risk_data.py" in src, "step 6 调 gen_risk_data.py (今日业务数据)"
    ...
    def test_step_6_uses_balance_pos(self):
        """步骤 6 用 --balance-pos (优先 RISK 用户, 造出可看的高风险评估)"""
        src = SCRIPT.read_text(encoding="utf-8")
        m = re.search(r"def step_6_business_today.*?(?=\ndef )", src, re.DOTALL)
        assert m, "找不到 step_6_business_today"
        assert "--balance-pos" in m.group(0), "步骤 6 必须加 --balance-pos (造高风险评估)"
