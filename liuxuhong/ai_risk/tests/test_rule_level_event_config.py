...
class TestRiskEventThresholds:
    ...
    def test_get_event_thresholds_with_fallback(self):
        ...
        th = settings.get_event_thresholds("退费申请")
        assert th["pass"] == 40  # 退费更严
        # fallback: 通用阈值
    ...
    def test_退费_比_报名_严(self):
        """退费阈值应该 >= 报名 (薅羊毛防风险), 打赏最敏感."""
        th_报名 = settings.RISK_EVENT_THRESHOLDS["课程报名"]
        th_退费 = settings.RISK_EVENT_THRESHOLDS["退费申请"]
        th_打赏 = settings.RISK_EVENT_THRESHOLDS["直播打赏"]
        # 退费 pass 阈值应该 >= 报名 pass (更严格)
        assert th_退费["pass"] >= th_报名["pass"], (
            "退费应该比报名严 (pass 阈值更高)"
        )
        # 打赏 pass 阈值应该 <= 报名 pass (打赏更敏感, 洗钱风险)
        assert th_打赏["pass"] <= th_报名["pass"], (
            "打赏应该比报名敏感 (pass 阈值更低)"
        )
...
class TestScoreToLevelWithEventType:
    ...
    def test_score_to_level_with_event_type(self):
        ...
        # 退费: pass=40, mark=70, review=85
        assert _score_to_level(39, "退费申请") == "低"
        assert _score_to_level(40, "退费申请") == "中"
        assert _score_to_level(69, "退费申请") == "中"
        assert _score_to_level(70, "退费申请") == "高"
        assert _score_to_level(85, "退费申请") == "极高"

    def test_score_to_decision_with_event_type(self):
        from app.engine.decision import _score_to_decision
        # 直播打赏: pass=25, mark=55, review=75
        assert _score_to_decision(24, "直播打赏") == "通过"
        assert _score_to_decision(25, "直播打赏") == "标记"
        assert _score_to_decision(75, "直播打赏") == "拒绝"
...
class TestFrontendRuleBuilder:
    ...
    def test_has_risk_event_thresholds(self, appjs):
        ...
        for et in ["课程报名", "退费申请", "学历认证", "直播打赏", "通用"]:
