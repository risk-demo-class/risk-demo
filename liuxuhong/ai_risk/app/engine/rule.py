...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        ("基础 ==",         {"field": "addr_is_new_device", "op": "==", "value": 1},     {"addr_is_new_device": 1},     True),
        ("基础 !=",         {"field": "user_total_enrollments", "op": "!=", "value": 0}, {"user_total_enrollments": 5}, True),
        ("AND 嵌套 (R002)", {"and": [
            {"field": "order_total_amount", "op": ">=", "value": 500},
            {"field": "user_avg_completion_rate", "op": "<=", "value": 0.01},
        ]}, {"order_total_amount": 2000, "user_avg_completion_rate": 0.0}, True),
        ("OR 嵌套",         {"or": [
            {"field": "user_total_enrollments", "op": ">=", "value": 50},
            {"field": "user_total_enrollments", "op": "<", "value": 1},
        ]}, {"user_total_enrollments": 0}, True),
        ("AND 失败",        {"and": [
            {"field": "x", "op": ">", "value": 5},
            {"field": "y", "op": ">", "value": 5},
        ]}, {"x": 10, "y": 1}, False),
    ...
    # match_rules 完整流程 — 用 mock 规则 (教育行业)
    print("\n[2] match_rules 端到端: 模拟 3 条教育规则对 1 份特征求值")
    ...
    class _MockRule:
        ...
        def __init__(self, rid, cond_dict, score, level, action, priority):
            ...
            self.is_enabled = 1
            self.description = f"Demo 规则 {rid}: {cond_dict}"
        @property
    ...
    rules = [
        ...
        _MockRule("R002", {"field": "user_enrollments_7d", "op": ">=", "value": 3}, 92, "极高", "拒绝", 100),
        _MockRule("R005", {"field": "user_refund_rate", "op": ">=", "value": 0.5}, 50, "中", "标记", 70),
    ]
    # 高风险用户特征: 7天内3门课 + 高退费率
    feats = {"order_total_amount": 15000, "user_enrollments_7d": 4, "user_refund_rate": 0.6}
    ...
    print("\n" + "=" * 60)
    print("结论: 规则引擎框架完全复用, 只需更换业务规则 JSON 条件表达式")
