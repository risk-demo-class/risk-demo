...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则 (教育行业):
      - "课程报名" 事件: source_id 就是 order_id
      - "退费申请" 事件: source_id 是 refund_id, order_id 已由 event.py 补全
      - "学历认证" / "直播打赏" 事件: 简化处理
    """
    order_id = request.order_id
    if not order_id and request.event_type in ("课程报名",):
...
async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 补全 receive_id (地址/设备 ID).

    教育场景没有"收货地址"概念 (设备指纹替代), receive_id 恒为 None;
    保留此函数仅为兼容 pipeline 结构, 直接跳过 DB 查询.
    """
    # 教育行业无 receive_id 字段 (order_info 表没有 receive_id 列), 跳过
    return
