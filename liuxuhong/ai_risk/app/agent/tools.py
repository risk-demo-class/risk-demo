...
async def _blacklist_add_impl(
    *, db: AsyncSession, blacklist_type: Literal["用户","学号","身份证","设备指纹","直播账号"], value: str, reason: str,
) -> str:
...
async def _manage_blacklist_impl(
    *, db: AsyncSession, action: str, blacklist_type: Literal["用户","学号","身份证","设备指纹","直播账号"], value: str, reason: str,
) -> str:
...
# ============================================================
# 业务数据查询 (字典派发 6 种 query_type, 教育行业)
# ============================================================
...
async def _biz_user_refunds(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            ...
            RefundRequest.study_minutes_before_refund,
            RefundRequest.refund_amount,
            RefundRequest.status,
            RefundRequest.create_time,
        )
        .where(RefundRequest.user_id == user_id)
        .order_by(RefundRequest.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())


async def _biz_user_devices(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户的设备指纹."""
    stmt = (
        select(
...
async def _biz_user_learning_progress(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户的学习进度."""
    stmt = (
        select(
            LearningProgress.progress_id,
            LearningProgress.course_id,
            LearningProgress.order_id,
            LearningProgress.total_minutes,
            LearningProgress.completion_rate,
            LearningProgress.last_active_at,
        )
        .where(LearningProgress.user_id == user_id)
        .order_by(LearningProgress.last_active_at.desc().nulls_last())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_course_list(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """课程列表."""
    stmt = (
        select(
            Course.course_id,
...
_BIZ_QUERY_HANDLERS = {
    ...
    "user_devices": _biz_user_devices,
    "user_learning_progress": _biz_user_learning_progress,
    "recent_orders": _biz_recent_orders,
    "high_value_orders": _biz_high_value_orders,
    "course_list": _biz_course_list,
}
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (用户ID)、event_type (事件类型: 课程报名/退费申请/学历认证/直播打赏)、source_id (关联业务ID, 如订单号)。"
...
@tool(
    description=(
        "查询用户的风险画像信息。"
        "参数: user_id (用户ID)。"
        "返回: 用户的风险评分、报名统计、退费率、设备数等画像数据; 没画像时实时算 14 个 user 特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/学号/身份证/设备指纹/直播账号)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型: user_orders/user_refunds/user_devices/user_learning_progress/recent_orders/high_value_orders/course_list)、user_id (部分需要)、order_id (部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
...
if __name__ == "__main__":
    ...
    print(f"  _BIZ_QUERY_HANDLERS = {list(_BIZ_QUERY_HANDLERS.keys())}  (7 个 query_type)")
    ...
    impls = [
        ...
        _biz_user_orders, _biz_user_refunds, _biz_user_devices,
        _biz_user_learning_progress, _biz_recent_orders, _biz_high_value_orders, _biz_course_list,
        _query_business_data_impl,
    ]
    print(f"  共 {len(impls)} 个 _impl (4 个黑名单 + 7 个业务查询 + 4 个统计)")
