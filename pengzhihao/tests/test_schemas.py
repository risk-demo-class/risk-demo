...
class TestRiskCheckRequest:
    ...
    def test_valid_request(self):
        ...
        assert req.event_type == "寄件受理"
        assert req.user_id == "U001"
        # 公开契约严格只有四个字段
        assert set(type(req).model_fields) == {"event_type", "source_id", "user_id", "event_data"}
        assert req.event_data is None

    def test_all_4_event_types(self):
        """4 个合法 event_type 都能过"""
        for et in ["寄件受理", "安检验视", "跨境申报", "代收货款"]:
    ...
    def test_legacy_adapter_fields_are_not_public(self):
        with pytest.raises(ValidationError):
            RiskCheckRequest(
                event_type="寄件受理", source_id="SHP0001", user_id="U001",
                order_id="SHP0001",
            )
    ...
    def test_missing_required_field(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RiskCheckRequest(event_type="寄件受理", user_id="u")  # 没传 source_id
...
class TestRuleCreate:
    ...
    def test_rule_score_out_of_range(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RuleCreate(
                rule_id="R1", rule_name="t", rule_category="行为异常", event_type="通用",
