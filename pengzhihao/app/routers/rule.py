...
async def api_list_rules(
    ...
    core_category = LOGISTICS_RULE_CATEGORY_TO_CORE.get(category, category) if category else None
    if core_category:
        count_stmt = count_stmt.where(RiskRule.rule_category == core_category)
    total = int((await db.execute(count_stmt)).scalar() or 0)

    # 1 次 paged SELECT
    stmt = select(RiskRule)
    if core_category:
        stmt = stmt.where(RiskRule.rule_category == core_category)
    stmt = (
...
async def api_create_rule(data: RuleCreate, db: AsyncSession = Depends(get_db_async)):
    ...
    rule = RiskRule(
        ...
        rule_category=LOGISTICS_RULE_CATEGORY_TO_CORE[data.rule_category],
        event_type=LOGISTICS_EVENT_TO_CORE.get(data.event_type, data.event_type),
...
async def api_update_rule(rule_id: str, data: RuleUpdate, db: AsyncSession = Depends(get_db_async)):
    ...
    for key, value in data.model_dump(exclude_unset=True).items():
        ...
        elif key == "rule_category" and value is not None:
            value = LOGISTICS_RULE_CATEGORY_TO_CORE[value]
        elif key == "event_type" and value is not None:
            value = LOGISTICS_EVENT_TO_CORE.get(value, value)
...
def _rule_to_response(rule: RiskRule) -> RuleResponse:
    return RuleResponse(
        ...
        rule_category=CORE_RULE_CATEGORY_TO_LOGISTICS.get(rule.rule_category, rule.rule_category),
        event_type=CORE_EVENT_TO_LOGISTICS.get(rule.event_type, rule.event_type),
...
if __name__ == "__main__":
    ...
    example = {
        ...
        "rule_category": "跨境申报",
        "event_type": "跨境申报",
        "rule_condition": {"field": "order_total_amount", "op": ">=", "value": 10000},
        "risk_level": "高",
        "risk_score": 50,
        "action": "人工审核",
        "priority": 70,
        "description": "跨境运单申报价值异常时进入人工审核",
    }
    import json
