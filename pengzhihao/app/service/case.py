...
from app.config import (
    CORE_BLACKLIST_LABEL_TO_TYPE,
    CORE_BLACKLIST_TYPE_TO_LABEL,
    CORE_EVENT_TO_LOGISTICS,
    CORE_RULE_CATEGORY_TO_LOGISTICS,
    LOGISTICS_EVENT_TO_CORE,
    LOGISTICS_RULE_CATEGORY_TO_CORE,
)
...
async def add_blacklist(db: AsyncSession, request: BlacklistCreate) -> BlacklistResponse:
    core_type = CORE_BLACKLIST_LABEL_TO_TYPE[request.blacklist_type]
    ...
    if existing:
        ...
        return BlacklistResponse(
            ...
            blacklist_type=CORE_BLACKLIST_TYPE_TO_LABEL.get(existing.blacklist_type, existing.blacklist_type),
    ...
    return BlacklistResponse(
        ...
        blacklist_type=CORE_BLACKLIST_TYPE_TO_LABEL.get(bl.blacklist_type, bl.blacklist_type),
        blacklist_value=bl.blacklist_value,
        reason=bl.reason,
...
async def get_blacklist(
    ...
    return total, [
        BlacklistResponse(
            ...
            blacklist_type=CORE_BLACKLIST_TYPE_TO_LABEL.get(b.blacklist_type, b.blacklist_type),
...
async def get_case_list(
    ...
    if category:
        filters.append(
            RiskCase.case_category == LOGISTICS_RULE_CATEGORY_TO_CORE.get(category, category)
        )
    ...
    items = [
        CaseItem(
            ...
            case_category=CORE_RULE_CATEGORY_TO_LOGISTICS.get(r[0].case_category, r[0].case_category),
            ...
            event_type=CORE_EVENT_TO_LOGISTICS.get(r[0].event_type, r[0].event_type),
        )
        for r in rows
    ]
...
async def get_case_detail(db: AsyncSession, case_id: str) -> CaseDetailResponse | None:
    ...
    if assessment and assessment.rule_results:
        try:
            triggered_rules = [RuleHitInfo(
                **{**r, "rule_category": CORE_RULE_CATEGORY_TO_LOGISTICS.get(
                    r.get("rule_category"), r.get("rule_category")
                )}
            ) for r in json.loads(assessment.rule_results)]
    ...
    return CaseDetailResponse(
        ...
        case_category=CORE_RULE_CATEGORY_TO_LOGISTICS.get(case.case_category, case.case_category),
        ...
        event_type=CORE_EVENT_TO_LOGISTICS.get(case.event_type, case.event_type),
    )
...
async def get_case_statistics(db: AsyncSession) -> CaseStatistics:
    ...
    by_category = {
        CORE_RULE_CATEGORY_TO_LOGISTICS.get(cat, cat): cnt
        for cat, cnt in category_rows if cat
    }
...
async def list_assessments(
    ...
    筛选维度 (可选):
      ...
      - risk_level: 低 / 中 / 高 / 极高
      - event_type: 寄件受理 / 安检验视 / 跨境申报 / 代收货款 (JOIN risk_event)
      - user_id: 寄件人 ID 精确匹配
    ...
    core_event_type = LOGISTICS_EVENT_TO_CORE.get(event_type, event_type) if event_type else None
    if core_event_type:
        count_stmt = count_stmt.where(RiskEvent.event_type == core_event_type)
    for f in a_filters:
        count_stmt = count_stmt.where(f)
    total = int((await db.execute(count_stmt)).scalar() or 0)
    ...
    if core_event_type:
        paged_stmt = paged_stmt.where(RiskEvent.event_type == core_event_type)
    paged_stmt = (
    ...
    items = [
        AssessmentItem(
            ...
            user_id=r[0].user_id,
            event_type=CORE_EVENT_TO_LOGISTICS.get(r[1], r[1]) if r[1] else "未知",
...
async def get_assessment_detail(
    ...
    return AssessmentDetailResponse(
        ...
        event_id=a.event_id,
        user_id=a.user_id,
        event_type=CORE_EVENT_TO_LOGISTICS.get(ev.event_type, ev.event_type) if ev else "未知",
    ...
    class _StatDB:
        ...
        async def execute(self, stmt):
            ...
            if self.call_count == 1:
                ...
                return _R([("物流风险", 12), ("售后滥用", 8), ("账户风险", 5)])
            else:
                return _R([])
