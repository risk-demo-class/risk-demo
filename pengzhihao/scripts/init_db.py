...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 5 张物流业务表"),
    ("init_business_data.sql", "导入物流业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 10 条物流风控规则 (L001-L010)"),
]
...
async def execute_sql_file(conn, filepath, description):
    ...
    async with conn.cursor() as cur:
        for stmt in statements:
            # 核心 DDL 按任务要求保持原样，其中仍含 ``USE ecs``。
            # 初始化器已连接到用户指定数据库，必须忽略脚本内的库切换。
...
async def main():
    parser = argparse.ArgumentParser(
        description="物流风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置独立演示数据库",
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 14 张表重建 + 物流业务数据 + 10 条规则全部就绪")
    else:
