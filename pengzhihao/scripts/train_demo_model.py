"""
物流风控系统 - XGBoost 合成演示模型训练

【目的】
  不依赖 DB，纯 numpy 合成 2000 个物流运单样本，训练 25 维兼容特征模型。
  训完保存到 app/engine/xgb_model.json, run_app.py 启动时直接加载.

【为什么需要这个脚本】
  正式交付模型由 train_xgb_model.py 使用 shipment.risk_label 训练；本脚本用于无数据库演示。
...
def _gen_high_refund_rate(rng: random.Random) -> np.ndarray:
    """模式 1：COD 高拒收率（兼容键 user_refund_*）。"""
    return np.array([
...
def _gen_high_complaint(rng: random.Random) -> np.ndarray:
    """模式 2：实名核验异常（兼容键 user_complaint_count）。"""
    return np.array([
        rng.uniform(10, 40),
        rng.uniform(2, 10),
        rng.uniform(1, 4),
