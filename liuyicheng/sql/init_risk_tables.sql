...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('转账欺诈','信用卡风险','信贷风险','账户安全','设备风险','IP风险') NOT NULL COMMENT '风险场景分类',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','事件','上下文') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `source_id` VARCHAR(50) DEFAULT NULL COMMENT '原始银行业务ID, 重做检查时用',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','设备指纹','IP','银行卡号','身份证号') NOT NULL COMMENT '黑名单类型',
...
CREATE TABLE IF NOT EXISTS `risk_user_profile` (
    ...
    `total_orders` INT DEFAULT 0 COMMENT '近30天交易数(兼容列)',
    `total_refunds` INT DEFAULT 0 COMMENT '近30天贷款申请数(兼容列)',
    `refund_rate` DECIMAL(5,4) DEFAULT 0 COMMENT '高风险事件率(兼容列)',
    `avg_order_amount` DECIMAL(10,2) DEFAULT 0 COMMENT '近30天平均交易额(兼容列)',
    `address_count` INT DEFAULT 0 COMMENT '近30天设备数(兼容列)',
    `complaint_count` INT DEFAULT 0 COMMENT '近24小时失败登录数(兼容列)',
