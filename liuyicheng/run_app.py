...
def _check_env_file() -> tuple[str, str]:
    ...
    if not os.path.exists(".env"):
        return "WARN", (
            ".env 不存在, 复制 .env.example 为 .env 并填写数据库配置；"
            "LLM_API_KEY 仅 Agent 需要"
        )
    return "OK", ".env 存在"
...
def _check_mysql_alive() -> tuple[str, str]:
    ...
    try:
        ...
        return "WARN", (
            f"MySQL 连不上 ({settings.DB_HOST}:{settings.DB_PORT}/{settings.DB_NAME}): {type(e).__name__}\n"
            "         业务功能会失败，请检查服务、密码与端口。"
        )
