...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("转账欺诈", "信用卡风险", "信贷风险", "账户安全", "设备风险", "IP风险",
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("用户", "事件", "上下文", name="feature_entity_type_enum"),
...
class RiskCase(Base):
    ...
    # --- 业务回溯字段 (重做检查时使用) ---
    source_id: Mapped[Optional[str]] = mapped_column(String(50), comment="原始银行业务ID")
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        Enum("用户", "设备指纹", "IP", "银行卡号", "身份证号", name="blacklist_type_enum"),
...
class RiskUserProfile(Base):
    ...
    # 核心表结构保持基线兼容；列名不改，银行语义见注释与 profile_data。
    total_orders: Mapped[int] = mapped_column(Integer, default=0, comment="近30天交易数(兼容列)")
    total_refunds: Mapped[int] = mapped_column(Integer, default=0, comment="近30天贷款申请数(兼容列)")
    refund_rate: Mapped[Decimal] = mapped_column(Numeric(5, 4), default=0, comment="高风险事件率(兼容列)")
    avg_order_amount: Mapped[Decimal] = mapped_column(Numeric(10, 2), default=0, comment="近30天平均交易额(兼容列)")
    address_count: Mapped[int] = mapped_column(Integer, default=0, comment="近30天设备数(兼容列)")
    complaint_count: Mapped[int] = mapped_column(Integer, default=0, comment="近24小时失败登录数(兼容列)")
