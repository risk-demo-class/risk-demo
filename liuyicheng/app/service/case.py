...
async def add_blacklist(db: AsyncSession, request: BlacklistCreate) -> BlacklistResponse:
    # 唯一键包含 type/value，已软删的记录必须复活，不能再次 INSERT。
    ...
    if existing:
        before_value = {
            ...
            "deleted_at": str(existing.deleted_at) if existing.deleted_at else None,
        }
        ...
        await record_action(
            ...
            remark=f"更新或恢复黑名单 {request.blacklist_type}={request.blacklist_value}",
        )
...
async def get_blacklist(
    ...
    total = int((await db.execute(
        select(func.count()).select_from(RiskBlacklist).where(*filters)
    )).scalar() or 0)
...
async def get_case_detail(db: AsyncSession, case_id: str) -> CaseDetailResponse | None:
    ...
    if profile:
        user_profile = {
            ...
            "transaction_count_30d": profile.total_orders,
            "loan_application_count_30d": profile.total_refunds,
            # DECIMAL → float (JSON 不支持 Decimal 类型)
            "high_risk_rate": float(profile.refund_rate),
            "avg_transaction_amount_30d": float(profile.avg_order_amount),
            "device_count_30d": profile.address_count,
            "failed_login_count_24h": profile.complaint_count,
...
async def list_assessments(
    ...
    筛选维度 (可选):
      ...
      - risk_level: 低 / 中 / 高 / 极高
      - event_type: 信用卡 / 贷款 / 转账 / 登录 (JOIN risk_event)
      - user_id: 精确匹配
    ...
    # 1 次 paged SELECT (带回事件类型和业务ID，供评估历史一键复查)
    paged_stmt = (
        select(RiskAssessment, RiskEvent.event_type, RiskEvent.event_source_id)
        .outerjoin(RiskEvent, RiskAssessment.event_id == RiskEvent.event_id)
        .where(*a_filters)
    )
    if event_type:
    ...
    class _StatDB:
        ...
        async def execute(self, stmt):
            ...
            if self.call_count == 1:
                ...
                return _R([("转账欺诈", 12), ("信用卡风险", 8), ("账户安全", 5)])
            else:
                return _R([])
