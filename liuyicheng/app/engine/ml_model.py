...
FEATURE_COLUMNS: list[str] = [
    ...
    "user_transaction_amount_30d",
    "user_failed_login_count_24h",
    "user_loan_application_count_30d",
    "user_avg_debt_ratio",
    ...
    "event_hour",
    "event_is_new_device",
    "event_device_age_days",
    "event_ip_is_proxy",
    "event_ip_is_tor",
    "event_geo_mismatch",
    "event_velocity_1h",
    "event_distinct_source_cards_1h",
    "context_card_credit_utilization",
    "context_loan_amount_income_ratio",
    "context_login_failure_streak",
    "context_shared_device_user_count",
    "context_to_card_blacklisted",
]
...
def unload_model() -> None:
    """清空进程内模型；训练样本生成必须用纯规则标签，避免旧模型数据泄漏。"""
...
def train_and_save(
    ...
    if pos_ratio < settings.XGB_MIN_POS_RATIO:
        logger.warning(
            ...
            "  建议: 调整银行样例风险模式或规则分布后，重新运行 gen_train_dataset.py",
...
# 不在模块 import 时自动加载模型：
# - FastAPI 由 scripts/main.py 的 lifespan 显式 load_model()；
# - gen_train_dataset.py 必须保持模型未加载，用纯规则决策生成无泄漏标签；
# - 训练脚本只需要 train_and_save()，也不应预加载旧模型。
...
if __name__ == "__main__":
    ...
    if not is_model_loaded():
        print("  (load_model() 加载失败，业务会自动退化为纯规则模式)")
    ...
    print("训练数据准备: python scripts/gen_train_dataset.py --repeats 12 --seed 42 --limit 1000")
