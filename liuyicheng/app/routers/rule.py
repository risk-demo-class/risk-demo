...
async def api_create_rule(data: RuleCreate, db: AsyncSession = Depends(get_db_async)):
    ...
    if existing:
        existing.rule_name = data.rule_name
        existing.rule_category = data.rule_category
        existing.event_type = data.event_type
        existing.rule_condition = json.dumps(data.rule_condition, ensure_ascii=False)
        ...
        await record_action(
            ...
            target_type="rule", target_id=existing.rule_id,
            after_value={"restored": True, "rule_name": existing.rule_name},
            remark="恢复已软删除规则",
        )
        await db.commit()
...
if __name__ == "__main__":
    ...
    example = {
        ...
        "rule_category": "转账欺诈",
        "event_type": "转账",
        "rule_condition": {"field": "event_amount", "op": ">=", "value": 50000},
        "risk_level": "高",
        "risk_score": 50,
        "action": "人工审核",
        "priority": 70,
        "description": "单笔转账≥50000 进入人工审核",
    }
    import json
