 ...
 *   - 25 个特征 key ↔ 中文标签映射 (FEATURE_LABELS)
...
// 1. 25 特征 key↔中文标签 (与 ml_model.py FEATURE_COLUMNS 严格对齐)
const FEATURE_LABELS = {
    "user_credit_score": "客户信用分",
    "user_account_age_days": "账户存续天数",
    "user_kyc_level": "KYC等级",
    "user_card_count": "持卡数",
    "user_transaction_count_30d": "30天交易数",
    "user_transaction_amount_30d": "30天交易金额",
    "user_failed_login_count_24h": "24小时失败登录数",
    "user_loan_application_count_30d": "30天贷款申请数",
    "user_avg_debt_ratio": "平均负债率",
    "user_loan_institution_count_30d": "30天贷款机构数",
    "event_amount": "事件金额",
    "event_is_night": "是否凌晨事件",
    "event_hour": "事件小时",
    "event_is_new_device": "是否新设备",
    "event_device_age_days": "设备存续天数",
    "event_ip_is_proxy": "IP是否代理",
    "event_ip_is_tor": "IP是否Tor",
    "event_geo_mismatch": "是否异地",
    "event_velocity_1h": "1小时事件频次",
    "event_distinct_source_cards_1h": "1小时来源卡数",
    "context_card_credit_utilization": "信用卡额度使用率",
    "context_loan_amount_income_ratio": "贷款收入比",
    "context_login_failure_streak": "连续登录失败数",
    "context_shared_device_user_count": "设备关联用户数",
    "context_to_card_blacklisted": "收款卡是否命中外部黑名单",
};
...
const RISK_EVENT_THRESHOLDS = {
    "信用卡":   {pass: 25, mark: 55, review: 80},
    "贷款":     {pass: 30, mark: 60, review: 85},
    "转账":     {pass: 20, mark: 50, review: 75},
...
function renderCondNode(cond, parent, isRoot) {
    if (cond && (cond.and || cond.or)) {
        ...
        footer.querySelector('.cond-add-leaf').addEventListener('click', () => {
            ...
            children.push({field: 'user_transaction_count_30d', op: '>=', value: 0});
            buildConditionUI(window._CURRENT_COND, 'condBuilder');
        });
        footer.querySelector('.cond-add-group').addEventListener('click', () => {
            // [P4-L5 2026-08-10] + 分组默认带 1 个 AND 子组 (嵌套场景), 用户可继续往里加
            // [P4-L5 2026-08-11] 整体重渲同 leaf
            children.push({and: [{field: 'user_transaction_count_30d', op: '>=', value: 0}]});
            buildConditionUI(window._CURRENT_COND, 'condBuilder');
        });
