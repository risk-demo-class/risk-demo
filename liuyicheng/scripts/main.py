...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # 先停止仍可能使用数据库的后台任务，再在同一个事件循环中释放连接池。
        # 否则 aiomysql 连接可能拖到 GC 阶段才析构，此时事件循环已经关闭。
...
app = FastAPI(
    title="银行智能风控系统",
    description="覆盖信用卡、贷款、转账、登录的规则引擎 + XGBoost + AI Agent 风控系统",
    version="1.0.0",
    lifespan=lifespan,
)
