...
def _load_data_from_mysql(sample_limit: int) -> tuple[np.ndarray, np.ndarray]:
    """
    从 MySQL 拉数据, 返回 (X, y).
    X: (N, 25) float32
    y: (N,) int (0/1)
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(SQL_FETCH_ASSESSMENTS, (sample_limit,))
            rows = cur.fetchall()
            logger.info("拉取评估: %d 条 (LIMIT=%d)", len(rows), sample_limit)
            # 统计总评估和可训练评估；只有 ml_score=NULL 才能避免旧模型标签泄漏。
            cur.execute("SELECT COUNT(*) FROM risk_assessment")
            db_total = cur.fetchone()[0]
            cur.execute(
                "SELECT COUNT(*) FROM risk_assessment "
                "WHERE decision IN ('通过', '标记', '人工审核', '拒绝') AND ml_score IS NULL"
            )
            db_trainable = cur.fetchone()[0]
            cur.execute(
                "SELECT COUNT(*) FROM risk_assessment "
                "WHERE decision IN ('人工审核', '拒绝') AND ml_score IS NULL"
            )
            db_trainable_pos = cur.fetchone()[0]
            # 教学数据会在同一天生成，时间跨度短是正常现象，不作为失败条件。
            if 0 < len(rows) < sample_limit:
                raise RuntimeError(
                    f"要求训练 {sample_limit} 条，但只能加载 {len(rows)} 条完整候选"
                    f"（可训练 {db_trainable} 条，DB总数 {db_total} 条）。"
                    f"请先运行 gen_train_dataset.py --samples {sample_limit} 生成同等数量"
                )
            if not rows:
                if db_total > 0 and db_trainable == 0:
                    raise RuntimeError(
                        f"数据库有 {db_total} 条评估，但全部包含 ml_score，不能用于无泄漏训练。"
                        f"请用 gen_train_dataset.py --samples {sample_limit} 再生成纯规则样本，然后重新训练"
                    )
                raise RuntimeError(
                    "没有可训练评估。请先运行 gen_train_dataset.py "
                    f"--repeats 12 --seed 42 --samples {sample_limit}"
                )
            # event_id → label 映射
            ...
            for eid, label in event_to_label.items():
                ...
                y_list.append(label)
            if missing:
                raise RuntimeError(f"有 {missing} 条样本的25维特征不完整，已停止训练以保证数量一致")
            ...
            if len(y) > 0 and pos_ratio < 0.15:
                logger.warning(
                    "⚠️  正例比例仅 %.1f%% < 推荐 15%% (可训练正例: %d / %d = %.1f%%). "
                    "模型学不到 '正例模式', 会假收敛. "
                    "建议调整银行样例中的风险事件比例或规则分布后重新生成训练样本",
                    100 * pos_ratio, db_trainable_pos, db_trainable,
                    100 * db_trainable_pos / max(1, db_trainable),
                )
            elif len(y) > 0 and pos_ratio < 0.25:
                logger.warning(
                    "⚠️  正例比例 %.1f%% 偏低 (< 25%% 推荐)，可调整风险样例比例后重造",
                    100 * pos_ratio,
                )
            return X, y
    finally:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="使用指定数量的纯规则评估训练银行XGBoost模型")
    parser.add_argument(
        "--samples", "--limit", dest="samples", type=int,
        default=settings.XGB_TRAIN_DATA_LIMIT,
        help="训练读取的最近样本数；--limit为兼容别名",
    )
    ...
    logger.info("=" * 60)
    # 1. 拉数据
    X, y = _load_data_from_mysql(args.samples)
    n = len(X)
    if n < 50:
    ...
    if "val_accuracy" in metrics:
        ...
        if warnings:
            ...
            logger.warning("  建议: 1) 调整银行样例风险模式或规则分布后重新生成训练样本")
