"""
银行智能风控系统 - 一键数据库初始化脚本 (异步)
按顺序执行所有 SQL 脚本: 创建数据库 → 业务表 → 业务数据 → 风控表 → 风控规则

总表数: 8 张银行业务表 + 9 张可复用风控表 = 17 张。
业务数据和表结构会按场景 B 重建，不兼容旧电商业务表。
"""
...
# MySQL 对 DROP ... IF EXISTS 仍通过驱动发 Warning；这些是幂等初始化的预期提示。
warnings.filterwarnings("ignore", message="Unknown table.*")
warnings.filterwarnings("ignore", message="Can't drop database.*")
warnings.filterwarnings("ignore", message="Integer display width.*")
...
SQL_FILES = [
    ...
    ("init_business_data.sql", "导入不少于 100 条银行样例数据"),
    ("init_risk_tables.sql", "创建 9 张通用风控表"),
    ("init_risk_data.sql", "导入 12 条银行风控规则"),
]

# 默认连接配置统一读取 .env，命令行参数仍可覆盖。
...
async def main():
    parser = argparse.ArgumentParser(
        description="银行智能风控系统 - 一键数据库初始化 (异步). 默认保留已有数据库",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
用法:
  python scripts/init_db.py                       # 默认: 保留数据库并幂等导入
  python scripts/init_db.py --keep-data           # 与默认行为相同
  python scripts/init_db.py --reset --yes        # 重置 + 自动确认 (CI/脚本用)
  python scripts/init_db.py --db bank_risk_test  # 初始化测试库
        """,
    )
    ...
    # 默认保留数据库；只有显式 --reset 才执行删库。
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--reset", dest="reset", action="store_true", default=False,
        help="先 DROP DATABASE 再 CREATE（会删除目标库全部数据）",
    )
    group.add_argument(
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 17 张表重建 + 187 条初始业务数据 + 12 条规则全部就绪")
    else:
