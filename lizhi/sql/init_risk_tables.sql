...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('订单欺诈','支付风险','账户风险','售后滥用','地址风险','物流风险','黄牛囤票','签证风险','退改欺诈','营销作弊') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('下单','支付','售后申请','物流投诉','预订下单','支付成功','出票确认','退改签申请','出行核销','签证申请','索赔投诉','评价发布','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('下单','支付','售后申请','物流投诉','预订下单','支付成功','出票确认','退改签申请','出行核销','签证申请','索赔投诉','评价发布') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('下单','支付','售后申请','物流投诉','预订下单','支付成功','出票确认','退改签申请','出行核销','签证申请','索赔投诉','评价发布') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('用户','地址','手机号','设备指纹','IP','护照号','签证号','身份证号') NOT NULL COMMENT '黑名单类型',
