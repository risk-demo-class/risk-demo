"""
测试 - 特征工程冗余调用优化 (P5, 旅游行业版)
【P5 优化】_feat_user_total_orders 在 compute_user_features 内被
2 个派生函数 (avg_order_amount / refund_rate) 各调 1 次, 冗余.
修复: 预计算 1 次 total_orders, 传给 2 个派生函数复用.
验证:
...
class TestPreComputedTotalOrders:
    ...
    async def test_zero_total_orders_returns_zero(self):
        """total_orders=0 时, 2 个派生特征都返回 0, 不再查更多 SQL"""
        db = MagicMock()
        call_log = []
...
class TestComputeUserFeaturesOptimization:
    ...
    async def test_total_orders_called_only_once(self):
        ...
        with pytest.MonkeyPatch.context() as mp:
            ...
            # 11 个独立特征 (旅游版 user 族): orders_30d / total_amount / max_order_amount
            # cancel_count / refund_count / night_order_count / same_flight_1h_count
            # visa_apply_count / visa_reject_count / visa_multi_country_30d / account_age_days
            for fn_name in [
                "_feat_user_orders_30d",
                "_feat_user_total_amount",
                "_feat_user_max_order_amount",
                "_feat_user_cancel_count",
                "_feat_user_refund_count",
                "_feat_user_night_order_count",
                "_feat_user_same_flight_1h_count",
                "_feat_user_visa_apply_count",
                "_feat_user_visa_reject_count",
                "_feat_user_visa_multi_country_30d",
        ...
        assert "user_avg_order_amount" in features
        assert "user_refund_rate" in features
