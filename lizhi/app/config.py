...
class Settings(BaseSettings):
    ...
    # ---- 旅游行业业务配置 (任务 2: 数据层, 对应场景 A) ----
    # 业务事件类型: risk_event.event_type / risk_rule.event_type / risk_case.event_type
    # 的枚举值来源 (需同步到 sql/init_risk_tables.sql 的 ENUM 列表, 否则 MySQL 拒绝入库)
    BUSINESS_EVENT_TYPES: list[str] = [
        "预订下单", "支付成功", "出票确认", "退改签申请",
        "出行核销", "签证申请", "索赔投诉", "评价发布",
    ]
    # 业务事件类型 → 事件码 (与 1-业务说明.md 的事件类型一一对应)
    BUSINESS_EVENT_TYPE_CODES: dict[str, str] = {
        "预订下单": "ORDER_CREATE",
    ...
    # 业务黑名单类型: risk_blacklist.blacklist_type / blacklist_extra.type 的取值来源
    # (任务书 A.1 BlacklistExtra: 设备指纹 / IP / 护照号 / 签证号 / 身份证号)
    BLACKLIST_TYPES: list[str] = [
        "用户", "设备指纹", "IP", "护照号", "签证号", "身份证号",
    ]
