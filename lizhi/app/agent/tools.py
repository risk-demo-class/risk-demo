...
# ============================================================
# 业务数据查询 (字典派发 7 种 query_type, 旅游行业)
# ============================================================
...
async def _biz_user_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            OrderInfo.order_id, OrderInfo.order_type, OrderInfo.total_amount,
            OrderInfo.dest_country, OrderInfo.depart_date, OrderInfo.order_status,
...
async def _biz_user_refunds(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户的退改记录 (订单状态=已退改)."""
    stmt = (
        select(
            OrderInfo.order_id, OrderInfo.order_type, OrderInfo.total_amount,
            OrderInfo.dest_country, OrderInfo.depart_date, OrderInfo.create_time,
        )
        .where(OrderInfo.user_id == user_id, OrderInfo.order_status == "已退改")
        .order_by(OrderInfo.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_user_destinations(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """用户去过的目的地国家 (含出行次数)."""
    stmt = (
        select(OrderInfo.dest_country, func.count().label("trip_count"))
        .where(OrderInfo.user_id == user_id)
        .group_by(OrderInfo.dest_country)
...
async def _biz_order_passengers(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """订单的乘客明细."""
    stmt = (
        select(
            PassengerInfo.passenger_id, PassengerInfo.name, PassengerInfo.id_type,
            PassengerInfo.id_number, PassengerInfo.nationality, PassengerInfo.age,
        )
        .where(PassengerInfo.order_id == order_id)
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_user_visas(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            VisaApplication.visa_id, VisaApplication.dest_country, VisaApplication.visa_type,
            VisaApplication.reject_history, VisaApplication.status, VisaApplication.submit_time,
        )
...
async def _biz_recent_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            ...
            OrderInfo.total_amount, OrderInfo.dest_country, OrderInfo.order_status,
            OrderInfo.create_time,
        )
        .order_by(OrderInfo.create_time.desc())
        .limit(limit)
...
async def _biz_high_value_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """高价值订单 (单笔 >= 5000 元, 出境/跟团游大单)."""
    stmt = (
        select(
            OrderInfo.order_id, OrderInfo.user_id, OrderInfo.order_type,
            OrderInfo.total_amount, OrderInfo.dest_country, OrderInfo.create_time,
        )
        .where(OrderInfo.total_amount >= 5000)
...
# 业务查询 dispatch table (7 种 query_type)
_BIZ_QUERY_HANDLERS = {
    "user_orders": _biz_user_orders,
    "user_refunds": _biz_user_refunds,
    "user_destinations": _biz_user_destinations,
    "order_passengers": _biz_order_passengers,
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (用户ID, 如 'U0001')、event_type (事件类型, 可选值: 预订下单/支付成功/出票确认/退改签申请/出行核销/签证申请/索赔投诉/评价发布)、source_id (关联业务ID, 订单号或签证申请ID)。"
...
@tool(
    description=(
        "查询用户的风险画像信息。"
        "参数: user_id (用户ID, 如 'U0001')。"
        "返回: 用户的风险评分、订单统计、退改率、目的地数等画像数据; 没画像时实时算 14 个 user 特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/护照号/设备指纹/IP/签证号/身份证号)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: user_orders/user_refunds/user_destinations/order_passengers/user_visas/recent_orders/high_value_orders)、user_id (部分需要)、order_id (部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
...
if __name__ == "__main__":
    ...
    impls = [
        ...
        _biz_user_orders, _biz_user_refunds, _biz_user_destinations,
        _biz_order_passengers, _biz_user_visas, _biz_recent_orders, _biz_high_value_orders,
        _query_business_data_impl,
    ]
    print(f"  共 {len(impls)} 个 _impl (其中 4 个黑名单 + 7 个业务查询 + 4 个统计)")
