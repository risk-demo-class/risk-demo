"""
业务实体校验器: 集中处理"用户/订单/签证申请"等旅游业务实体的存在性、一致性校验.
...
# 旅游行业: 订单类事件 source_id=order_id; 签证申请 source_id=visa_id
_EVENT_SOURCE_VALIDATORS = {
    ("预订下单", "支付成功", "出票确认", "退改签申请", "出行核销", "索赔投诉", "评价发布"): (
        OrderInfo, "order_id", None, "订单", 400,
    ),
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Service Validator — 4 个 ensure_* + 字典派发表 + 完整校验链 (旅游行业)")
    print("=" * 60)
    ...
    print("  event_type='预订下单' 但用 visa_id 当 source_id → 报错 (派发错模型)")
    ...
    async def demo_event_dispatch():
        ...
        from app.schemas import RiskCheckRequest
        req_ok = RiskCheckRequest(event_type="预订下单", source_id="ORD001", user_id="U001")
        # count=1 (存在), row 也有 (单条订单)
        try:
            await ensure_source_matches_event_type(_FlexDB(1, SimpleNamespace(order_id="ORD001")), req_ok)
            print("  [OK]   event_type=预订下单 + source_id=ORD001 → OrderInfo 存在, 通过")
        except HTTPException as e:
            print(f"  [FAIL] {e.detail}")

        # 错误配对: 用 visa_id 当 source_id 但 event_type=预订下单 → 走 OrderInfo 但查不到
        req_bad = RiskCheckRequest(event_type="预订下单", source_id="V001", user_id="U001")
        try:
            await ensure_source_matches_event_type(_FlexDB(0, None), req_bad)
            print("  [FAIL] 不该到这里")
        except HTTPException as e:
            print(f"  [400]  source_id='V001' 在 OrderInfo 找不到 → {e.detail[:60]}...  (status={e.status_code})")
