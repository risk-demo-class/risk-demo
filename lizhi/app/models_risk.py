...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        # 【任务3 旅游行业】仅扩展枚举值列表 (不改表结构): 新增黄牛/签证/退改/营销 4 类
    ...
    event_type: Mapped[str] = mapped_column(
        # 【任务2 旅游行业】仅扩展枚举值列表 (不改表结构): 保留电商 4 型 + 新增旅游 8 型
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        # 【任务2 旅游行业】仅扩展枚举值列表 (不改表结构): 新增 设备指纹/IP/护照号/签证号/身份证号
        Enum("用户", "地址", "手机号",
             "设备指纹", "IP", "护照号", "签证号", "身份证号",
