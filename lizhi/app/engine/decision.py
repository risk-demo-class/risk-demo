...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    旅游行业业务规则:
      - 订单类事件 (预订下单/支付成功/出票确认/退改签申请/出行核销/索赔投诉/评价发布):
        source_id 就是 order_id
      - "签证申请" 事件: source_id 是 visa_id, 无订单关联 (只算用户+目的地特征)
    """
    order_id = request.order_id
    if not order_id and request.event_type in (
        "预订下单", "支付成功", "出票确认", "退改签申请",
        "出行核销", "索赔投诉", "评价发布",
    ):
...
async def _enrich_order_context(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 补全订单上下文 (旅游行业无 receive_id 概念, 保留步骤位置).

    order_id 已在 _build_context 从 source_id 推断; 签证申请事件无订单, 跳过.
    """
    return
...
async def _update_user_profile(
    ...
    # 退改率直接取特征 (已算好的比值, 防 0 除)
    profile.refund_rate = features.get("user_refund_rate", 0)
    profile.avg_order_amount = features.get("user_avg_order_amount", 0)
    # 旅游行业: "地址"族映射为目的地维度, 投诉无对应表置 0
    profile.address_count = int(features.get("addr_dest_country_count", 0))
...
async def run_risk_check(
    ...
    await _enrich_order_context(db, ctx)
