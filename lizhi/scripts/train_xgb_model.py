...
#   原因: gen_risk_data.py 造训练数据时强制 ml_score=NULL (避免"未训练模型"垃圾值),
        ...
        with conn.cursor() as cur:
            ...
            if len(rows) < settings.XGB_TRAIN_DATA_LIMIT * 0.8:
                logger.warning(
                    ...
                    "建议先 init_db.py --reset --yes 清空, 再 gen_business_data.py --reset + gen_risk_data.py --count 300 --balance-pos --target-pos-ratio 0.30 重新造",
            ...
            if span_days is not None and span_days < 1 and db_total > 100:
                logger.warning(
                    ...
                    "建议: gen_risk_data.py --count 300 --balance-pos --target-pos-ratio 0.30 重造",
            ...
            if len(y) > 0 and pos_ratio < 0.15:
                logger.warning(
                    ...
                    "模型学不到 '正例模式', 会假收敛. "
                    "建议: gen_business_data.py --reset + gen_risk_data.py --count 300 --balance-pos --target-pos-ratio 0.30",
