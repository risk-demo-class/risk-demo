...
ALTERS = [
    ...
    ("risk_case", "source_id",
     "VARCHAR(50) DEFAULT NULL COMMENT '原始业务ID(订单ID/签证申请ID), 重做检查时用' AFTER risk_detail"),
    ("risk_case", "event_type",
     "ENUM('下单','支付','售后申请','物流投诉','预订下单','支付成功','出票确认','退改签申请','出行核销','签证申请','索赔投诉','评价发布') DEFAULT NULL COMMENT '触发案件的事件类型' AFTER source_id"),
]
