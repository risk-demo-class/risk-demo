...
【工作流 6 步】
  1. 重置数据库 (init_db.py --reset --yes, 含 8 条旅游规则)
  2. 造旅游业务数据 (gen_business_data.py --reset, 含 5 类 RISK 高风险用户)
  3. 造 300 条强标注评估数据 (gen_risk_data.py --balance-pos, ml_score=NULL)
  4. 训练 XGBoost 模型 (train_xgb_model.py)
  5. 回填 ml_score 字段 (backfill_ml_score.py, 用训好的模型推理)
  6. 造 50 条今日业务数据 (gen_risk_data.py --count 50)
...
def step_2_risk_users() -> None:
    """步骤 2: 造旅游业务数据 (含 5 类 RISK 高风险用户, 订单 >=100)."""
    _run_subprocess(
        "2/6 造旅游业务数据 (gen_business_data.py --reset)",
        [sys.executable, os.path.join(SCRIPTS, "gen_business_data.py"), "--reset"],
    )


def step_3_train_dataset() -> None:
    """步骤 3: 造 300 条强标注评估数据 (balance-pos 拉高正例, ml_score=NULL)."""
    _run_subprocess(
        "3/6 造 300 条评估数据 (gen_risk_data.py --count 300 --balance-pos --target-pos-ratio 0.30)",
        [sys.executable, os.path.join(SCRIPTS, "gen_risk_data.py"),
         "--count", "300", "--balance-pos", "--target-pos-ratio", "0.30"],
    )
...
def step_6_business_today() -> None:
    """步骤 6: 造 50 条今日评估数据 (仪表盘/案件/评估立刻能看)."""
    _run_subprocess(
        "6/6 造 50 条今日评估数据 (gen_risk_data.py --count 50)",
        [sys.executable, os.path.join(SCRIPTS, "gen_risk_data.py"), "--count", "50"],
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="旅游风控系统 - 一条龙命令 (6 步: 重置 → 业务数据 → 评估数据 → 训练 → 回填 → 今日数据)",
...
示例:
  ...
  python scripts/one_command.py --only-start   # 只跑步骤 6 (造今日业务数据)
    ...
    print("  python scripts/gen_risk_data.py --count 100 --balance-pos --target-pos-ratio 0.30")
    print("=" * 70)
