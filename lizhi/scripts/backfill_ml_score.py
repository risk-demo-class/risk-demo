"""
旅游风控系统 - XGBoost ml_score 字段回填脚本 (P4-L4 2026-08-08)

【目的】
  gen_risk_data.py 造训练数据时强制 ml_score=NULL (避免"未训练模型"推理垃圾值).
        ...
        for i, (aid, eid, uid, dec) in enumerate(rows, 1):
            try:
                async with SessionLocal() as db:
                    # 找该 event 的 source_id / event_type (risk_event 无 order_id 列)
                    ...
                    if not ev_row:
                        continue
                    # 旅游行业: 订单类事件 source_id 即 order_id; 签证申请无订单
                    order_id = ev_row.event_source_id if ev_row.event_type != "签证申请" else None
                    # 算 25 维特征
                    features = await compute_all_features(
                        db, uid, order_id=order_id, receive_id=None,
                    )
                    # 推理
                    ml = predict(features)
                    if ml is None:
