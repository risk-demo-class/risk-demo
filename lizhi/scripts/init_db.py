...
总表数: 7 旅游业务 + 9 风控 = 16 张 (2026-08-07 含 P4 2 张: risk_action_log + risk_alert)
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 7 张旅游业务表"),
    ("init_business_data.sql", "导入业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 8 条旅游业务规则 (R001-R008)"),
]
...
DEFAULT_PASSWORD = _load_env_password()
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 16 张表重建 (7 旅游业务 + 9 风控) + 业务数据 + 8 条规则全部就绪")
    else:
