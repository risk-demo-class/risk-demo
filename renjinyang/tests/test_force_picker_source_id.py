...
class TestForcePickerSourceId:
    ...
    def test_tourism_source_ids_are_strings(self):
        """旅游订单、签证主键均为字符串，和 validator 的 source_id 约定一致。"""
        sql = SQL_PATH.read_text(encoding="utf-8")
        assert re.search(r"`order_id`\s+VARCHAR\(50\)", sql, re.IGNORECASE)
        assert re.search(r"`visa_id`\s+VARCHAR\(50\)", sql, re.IGNORECASE)
