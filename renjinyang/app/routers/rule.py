...
if __name__ == "__main__":
    ...
    for route in rule_router.routes:
        if hasattr(route, "methods") and hasattr(route, "path"):  # type: ignore[attr-defined]
            methods = "|".join(sorted(route.methods - {"HEAD", "OPTIONS"}))  # type: ignore[attr-defined]
            status = f" (status={route.status_code})" if hasattr(route, "status_code") and route.status_code else ""  # type: ignore[attr-defined]
            print(f"  {methods:<10} {route.path}{status}")  # type: ignore[attr-defined]
