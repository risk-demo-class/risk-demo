...
# TODO(任务 3): 业务查询工具的 SQL 仍是电商口径，将在旅游特征与工具任务中重写。
# 当前任务只保证旅游 ORM 替换后本模块可以正常 import。
...
async def _risk_check_impl(
    *, db: AsyncSession, user_id: str, event_type: str, source_id: str,
    ...
    request = RiskCheckRequest(event_type=event_type, source_id=source_id, user_id=user_id)  # type: ignore[arg-type]
...
async def _blacklist_add_impl(
    *, db: AsyncSession, blacklist_type: str, value: str, reason: str,
    ...
    result = await add_blacklist(db, BlacklistCreate(
        blacklist_type=blacklist_type, blacklist_value=value, reason=reason,  # type: ignore[arg-type]
    ))
    await db.commit()
...
async def _analyze_risk_trend_impl(*, db: AsyncSession, days: int) -> str:
    ...
    level_dist = dict((await db.execute(  # type: ignore[call-overload]
    ...
    decision_dist = dict((await db.execute(  # type: ignore[call-overload]
