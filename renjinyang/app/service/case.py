...
async def add_blacklist(db: AsyncSession, request: BlacklistCreate) -> BlacklistResponse:
    ...
    if existing:
        ...
        await record_action(
            ...
            target_type="blacklist", target_id=str(existing.blacklist_id),
...
async def get_assessment_detail(
    ...
    return AssessmentDetailResponse(
        ...
        create_time=a.create_time or datetime.now(),
...
if __name__ == "__main__":
    ...
    async def demo_active_only():
        db = _FakeDB()
        await get_case_list(db, active_only=True, page=1, page_size=20)  # type: ignore[arg-type]
        db2 = _FakeDB()
        await get_case_list(db2, active_only=False, page=1, page_size=20)  # type: ignore[arg-type]
        db3 = _FakeDB()
        await get_case_list(db3, status="已通过", page=1, page_size=20)  # type: ignore[arg-type]
    ...
    async def demo_stats():
        stats = await get_case_statistics(_StatDB())  # type: ignore[arg-type]
...
async def auto_close_timeout_cases(db: AsyncSession, hours: int | None = None) -> int:
    ...
    for c in cases:
        ...
        await record_action(
            ...
            remark=f"超时自动关闭, create_time={c.create_time.isoformat() if c.create_time else 'N/A'}",
        )
