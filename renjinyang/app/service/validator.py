"""
业务实体校验器: 集中处理"用户/旅游订单/签证"等业务实体的存在性、一致性校验.
...
_EVENT_SOURCE_VALIDATORS = {
    ("机票预订", "酒店预订", "跟团游报名", "退改申请"):
        (OrderInfo, "order_id", None, "旅游订单", 400),
...
if __name__ == "__main__":
    ...
    async def demo_ensure_exists():
        # 存在 → 正常返回
        try:
            await ensure_exists(_FakeDB(found=1), UserInfo, "user_id", "U001", entity_label="用户")  # type: ignore[arg-type]
        ...
        # 不存在 → 抛 404
        try:
            await ensure_exists(_FakeDB(found=0), UserInfo, "user_id", "U999", entity_label="用户")  # type: ignore[arg-type]
            print("  [FAIL] 不该到这里")
        except HTTPException as e:
    ...
    print("  event_type='机票预订' 但用 visa_id 当 source_id → 报错 (派发错模型)")
    ...
    async def demo_event_dispatch():
        ...
        from app.schemas import RiskCheckRequest
        req_ok = RiskCheckRequest(event_type="机票预订", source_id="ORD001", user_id="U001")
        # count=1 (存在), row 也有 (单条订单)
        try:
            await ensure_source_matches_event_type(_FlexDB(1, SimpleNamespace(order_id="ORD001")), req_ok)  # type: ignore[arg-type]
            print("  [OK]   event_type=机票预订 + source_id=ORD001 → OrderInfo 存在, 通过")
        except HTTPException as e:
            print(f"  [FAIL] {e.detail}")

        # 错误配对: 用 visa_id 当 source_id 但 event_type=机票预订 → 走 OrderInfo 但查不到
        req_bad = RiskCheckRequest(event_type="机票预订", source_id="VISA001", user_id="U001")
        try:
            await ensure_source_matches_event_type(_FlexDB(0, None), req_bad)  # type: ignore[arg-type]
            print("  [FAIL] 不该到这里")
        except HTTPException as e:
            print(f"  [400]  source_id='VISA001' 在 OrderInfo 找不到 → {e.detail[:60]}...  (status={e.status_code})")
    ...
    async def demo_ownership():
        ...
        class _OrderOwnerU002:
            async def execute(self, stmt):
                ...
                return _R()
        try:
            await ensure_order_belongs_to_user(_OrderOwnerU002(), "ORD999", "U001")  # type: ignore[arg-type]
            print("  [FAIL] 不该到这里")
        except HTTPException as e:
        ...
        class _OrderOwnerU001:
            async def execute(self, stmt):
                ...
                return _R()
        try:
            await ensure_order_belongs_to_user(_OrderOwnerU001(), "ORD001", "U001")  # type: ignore[arg-type]
...
async def validate_risk_check_request(
    ...
    # 3. 订单类旅游事件: 校验订单归属 (防绕过)
    if request.event_type in ("机票预订", "酒店预订", "跟团游报名", "退改申请"):
