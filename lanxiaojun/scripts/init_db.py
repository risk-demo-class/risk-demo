"""
教育风控系统 - 一键数据库初始化脚本 (异步)
按顺序执行所有 SQL 脚本: 创建数据库 → 业务表 → 业务数据 → 风控表 → 风控规则

总表数: 10 教育业务 + 9 风控 = 19 张 (含 P4 2 张: risk_action_log + risk_alert)
...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 10 张教育业务表"),
    ("init_business_data.sql", "导入教育业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (含 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 30 条预置教育风控规则 (ER001-ER030)"),
]
...
async def main():
    parser = argparse.ArgumentParser(
        description="教育风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
用法:
  python scripts/init_db.py                       # 默认: --reset (删库重建)
  python scripts/init_db.py --keep-data          # 保留: 只补表结构
  python scripts/init_db.py --reset --yes        # 重置 + 自动确认 (CI)
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    group.add_argument(
        ...
        help="默认: 先 DROP DATABASE 再 CREATE (重置整个库)",
    )
    group.add_argument(
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 19 张表重建 + 业务数据 + 30 条教育规则全部就绪")
    else:
