...
def _decision_to_label(decision: str) -> int:
    ...
    业务 4 档 → 机器学习 2 分类
        ...
        人工审核/拒绝 → 1 (高风险, 模型拦截)
    """
    # 【2026-08-12】标记也视为正例 (特征不再极端但仍被规则识别为风险)
    return 1 if decision in ("标记", "人工审核", "拒绝") else 0
...
def _load_data_from_mysql() -> tuple[np.ndarray, np.ndarray]:
    ...
    try:
        with conn.cursor() as cur:
            ...
            logger.info("拉取特征: %d 个 event 各 20 维 (教育版)", len(event_to_features))
            # 2. 拼训练矩阵 (只保留 20 维特征齐全的样本)
            X_list, y_list = [], []
            missing = 0
