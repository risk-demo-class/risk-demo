"""
教育风控系统 - 带日期范围的模拟风控评估数据生成 (异步, 教育版)
...
# 事件类型池 (教育版: 英文枚举)
EVENT_TYPES_POOL = ["purchase", "purchase", "purchase", "course_watch", "course_watch"]
...
async def _pick_order_for_balance(db, balance_pos: bool):
    """从订单池挑一条; balance_pos=True 时 80% 概率挑 RISK 用户订单 (教育版)."""
    if balance_pos:
...
async def _pick_refund_for_balance(db, balance_pos: bool):
    """从退费申请表挑一条; balance_pos=True 时 80% 概率挑 RISK 用户退费 (教育版)."""
    if balance_pos:
        if random.random() < 0.8:
            r = await db.execute(text("""
...
async def _pick_complaint_for_balance(db, balance_pos: bool):
    """从投诉表挑一条; balance_pos=True 时 80% 概率挑 RISK 用户投诉 (教育版)."""
    if balance_pos:
        if random.random() < 0.8:
            r = await db.execute(text("""
            ...
            row = r.first()
            if row:
                return (row.complaint_id, row.user_id)
    r = await db.execute(text("""
        SELECT complaint_id, user_id
        FROM complaint
        ORDER BY RAND() LIMIT 1
    """))
    row = r.first()
    return (row.complaint_id, row.user_id) if row else None
...
async def _pick_forced_refund(db):
    """强制正例路径: 选 RISK 用户的退费 (必触发高退费率规则)."""
    r = await db.execute(text("""
        SELECT refund_id, user_id
        FROM refund_request
        WHERE user_id LIKE :prefix
...
async def _pick_forced_complaint(db):
    """强制正例路径: RISK 用户的投诉 (必触发投诉规则)."""
    r = await db.execute(text("""
        SELECT complaint_id, user_id
        FROM complaint
        WHERE user_id LIKE :prefix
...
async def _pick_forced_high_amount_order(db):
    """强制正例路径: RISK 用户的高额订单 (可能触发高额订单规则)."""
    r = await db.execute(text("""
        SELECT order_id, user_id
        FROM order_info
        WHERE user_id LIKE :prefix
...
async def generate_risk_data_with_dates(
    ...
    async with AsyncSessionLocal() as db:
        ...
        if force_pos_ratio is not None:
            print(f"[FORCE-POS] --force-pos-ratio 模式: 强制 {force_pos_ratio*100:.0f}% 走高风险事件路径 (RISK 退费/投诉/高额订单 + retry)")
        ...
        all_refunds_result = await db.execute(text("""
            SELECT refund_id, user_id FROM refund_request
        """))
        all_refunds = all_refunds_result.all()

        all_complaints_result = await db.execute(text("""
            SELECT complaint_id, user_id FROM complaint
        """))
        all_complaints = all_complaints_result.all()

        if not all_orders:
            print("错误: 数据库里没有订单数据, 请先跑 init_db.py")
            return

        print(f"可用订单: {len(all_orders)} 条, 退费: {len(all_refunds)} 条, 投诉: {len(all_complaints)} 条\n")
        ...
        for day_offset in range(total_days):
            ...
            for i in range(day_count):
                ...
                if use_force_pos:
                    pickers = [
                        ("refund_apply", _pick_forced_refund, "refund"),
                        ("complaint", _pick_forced_complaint, "complaint"),
                        ("purchase", _pick_forced_high_amount_order, "order"),
                    ]
                ...
                for try_idx in range(max_tries):
                    ...
                    if use_force_pos:
                        ...
                        source_id, user_id = picked
                        if ptype == "order":
                            request = RiskCheckRequest(
                                event_type=et, source_id=source_id, user_id=user_id,
                                order_id=source_id,
                            )
                        else:
                            request = RiskCheckRequest(
                        ...
                        # 普通路径: --balance-pos 80% 概率从 RISK 用户挑, 按概率选退费/投诉/订单
                        if balance_pos:
                            odds_table = {"refund": 0.4, "complaint": 0.2, "order": 0.4}
                        else:
                            odds_table = {"refund": 0.2, "complaint": 0.1, "order": 0.7}

                        pick_roll = random.random()
                        if pick_roll < odds_table["refund"] and all_refunds:
                            picked = await _pick_refund_for_balance(db, balance_pos)
                            if picked:
                                source_id, user_id = picked
                                request = RiskCheckRequest(
                                ...
                                continue
                        elif pick_roll < odds_table["refund"] + odds_table["complaint"] and all_complaints:
                            picked = await _pick_complaint_for_balance(db, balance_pos)
                            if picked:
                                source_id, user_id = picked
                                request = RiskCheckRequest(
                            ...
                            if not picked:
                                continue
                            source_id, user_id = picked
                            event_type = random.choice(EVENT_TYPES_POOL)
                            kwargs = {
                            ...
                            request = RiskCheckRequest(**kwargs)
                    ...
                    try:
                        ...
                        if new_assess:
                            ...
                            # 【2026-08-12 调整】标记/人工审核/拒绝都算正例 (特征不再极端但仍被规则识别)
                            if new_assess.decision in ("标记", "人工审核", "拒绝"):
                                day_positive += 1
                                grand_positive += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="带日期范围的造数据脚本 (教育版, 每天条数: --per-day 固定, 或随机 1~--max-per-day)"
    )
    ...
    parser.add_argument("--force-pos-ratio", type=float, default=None,
                        help="【P4-L3 第五轮】强制每条按此概率走'高风险事件'路径 (RISK 用户退费/投诉/高额订单), "
