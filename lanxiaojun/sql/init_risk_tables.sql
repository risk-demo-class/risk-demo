-- ============================================
-- 教育风控系统 - 风控表 DDL 初始化脚本
-- 创建 9 张风控表（含 P4 系统管理表）
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('注册风险','账号风险','营销作弊','刷课行为','退费欺诈','支付风险','内容安全','师资风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('register','login','real_name_auth','purchase','coupon_claim','course_watch','refund_apply','complaint','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
    ...
    PRIMARY KEY (`rule_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='风控规则配置表(教育版)';

-- 2. 风控事件审计表 (教育版枚举)
CREATE TABLE IF NOT EXISTS `risk_event` (
    `event_id` VARCHAR(50) NOT NULL COMMENT '事件ID',
    `event_type` ENUM('register','login','real_name_auth','purchase','coupon_claim','course_watch','refund_apply','complaint') NOT NULL COMMENT '事件类型',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='风控事件审计表(教育版)';

-- 3. 风控特征快照表 (教育版 entity_type)
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','课程','学习') NOT NULL COMMENT '实体类型',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='风控特征快照表(教育版)';

-- 4. 风控评估结果表 (结构不变, ml_score/ml_decision 保留)
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `source_id` VARCHAR(50) DEFAULT NULL COMMENT '原始业务ID(订单/退费/投诉ID)',
    `event_type` ENUM('register','login','real_name_auth','purchase','coupon_claim','course_watch','refund_apply','complaint') DEFAULT NULL COMMENT '触发案件的事件类型',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='风控案件表(教育版)';

-- 6. 风控黑名单表 (教育版 6 种类型)
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    `blacklist_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '黑名单ID',
    `blacklist_type` ENUM('用户','手机号','学号','身份证','设备指纹','支付账号') NOT NULL COMMENT '黑名单类型',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='风控黑名单表(教育版)';
...
CREATE TABLE IF NOT EXISTS `risk_user_profile` (
    ...
    `total_enrollments` INT DEFAULT 0 COMMENT '历史报名数',
    `total_refunds` INT DEFAULT 0 COMMENT '退费次数',
    `refund_rate` DECIMAL(5,4) DEFAULT 0 COMMENT '退费率',
    `avg_order_amount` DECIMAL(10,2) DEFAULT 0 COMMENT '平均订单金额',
    `device_count` INT DEFAULT 0 COMMENT '关联设备数',
    ...
    `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户风险画像表(教育版)';

-- ============================================
-- 系统管理表 (P4-L1 / P4-L2, 2026-08-07 新增)
-- 教育版保留不变
-- ============================================
