...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("用户", "课程", "学习", name="feature_entity_type_enum"),
...
class RiskUserProfile(Base):
    ...
    total_enrollments: Mapped[int] = mapped_column(Integer, default=0, comment="历史报名数")
    total_refunds: Mapped[int] = mapped_column(Integer, default=0, comment="退费次数")
    refund_rate: Mapped[Decimal] = mapped_column(Numeric(5, 4), default=0, comment="退费率")
    avg_order_amount: Mapped[Decimal] = mapped_column(Numeric(10, 2), default=0, comment="平均订单金额")
    device_count: Mapped[int] = mapped_column(Integer, default=0, comment="关联设备数")
