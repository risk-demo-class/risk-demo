...
【重要约定】
  - 特征顺序固定 (FEATURE_COLUMNS), 跟 feature.py 算出的 20 个 key 一一对应
...
# 固定 20 维教育特征顺序 (训练和推理都用这个顺序, 防 dict 顺序不一致导致特征错位)
...
FEATURE_COLUMNS: list[str] = [
    ...
    "course_total_hours",
    "course_teacher_rating",
    "order_amount",
    "order_pay_hour",
    "user_multi_course_count",
    "user_payment_account_count",
    "behavior_today_minutes",
    "behavior_active_courses",
    "behavior_night_session",
    "behavior_device_count",
]

assert len(FEATURE_COLUMNS) == 20, f"特征数量必须是 20 (教育版), 当前 {len(FEATURE_COLUMNS)}"
