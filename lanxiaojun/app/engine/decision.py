...
7 步流水线:
  1. 校验 + 准备上下文 (Context)
  2. 创建事件记录 (risk_event)
  3-4. 计算 + 保存特征快照 (20 维, risk_feature)
  5. 加载 + 匹配规则
...
class _RiskCheckContext:
    ...
    order_id: Optional[str] = None  # purchase/refund_apply 场景使用
    event_id: str = ""


def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    """步骤 1b: 请求 → Context, 补全 course_id / device_id.

    业务规则:
      - "purchase" 事件: source_id 就是 order_id, 可从 event_data 拿 course_id
      - "refund_apply" 事件: source_id 是 refund_id
      - "course_watch" 事件: source_id 是 order_id, 需要 course_id
      - "register/login/real_name_auth/coupon_claim" 不需要额外补全
    """
    order_id = request.order_id or (request.source_id if request.event_type in ("purchase", "course_watch") else None)
...
async def _enrich_course_info(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 从订单补全 course_id (如果未传).

    如果请求没有 course_id, 从 OrderInfo 关联查询.
    只有 purchase/course_watch 事件需要补课程信息.
    """
    if ctx.course_id or not ctx.order_id:
        return
    if ctx.request.event_type not in ("purchase", "course_watch"):
        return
    row = (await db.execute(
        select(OrderInfo.course_id).where(OrderInfo.order_id == ctx.order_id)
    )).first()
    if row:
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算 20 个教育风控特征"""
    return await compute_all_features(
        db,
        user_id=ctx.user_id,
        order_id=ctx.order_id,
        receive_id=None,  # 教育不涉及地址
    )


def _classify_feature_entity(feature_name: str) -> tuple[str, str]:
    """特征名前缀 → 实体类型. user_* → 用户, course_*/order_* → 课程, behavior_* → 学习"""
    if feature_name.startswith("user_"):
        return "用户", feature_name
    if feature_name.startswith("course_") or feature_name.startswith("order_"):
        return "课程", feature_name
    if feature_name.startswith("behavior_"):
        return "学习", feature_name
    return "用户", feature_name  # 兜底
...
def _save_feature_snapshot(
    ...
    """步骤 4: 把 20 个教育特征落库到 risk_feature (审计回溯).

    entity_id 填充规则 (按 entity_type):
      - 用户特征 → ctx.user_id
      - 课程特征 → ctx.course_id or ctx.request.source_id
      - 学习特征 → ctx.device_id or ctx.user_id
    """
    for fname, fval in features.items():
        ...
        if entity_type == "用户":
            ...
            entity_id = ctx.course_id or ctx.request.source_id
        else:  # 学习
...
async def _update_user_profile(
    ...
    10 个字段从 features 取 (复用刚才算好的 20 特征, 不再查 DB).
    """
    ...
    profile.total_enrollments = int(features.get("user_total_enrollments", 0))
    profile.total_refunds = int(features.get("user_total_refunds", 0))
    # 退费率直接从特征取
    profile.refund_rate = features.get("user_refund_rate", 0)
    profile.complaint_count = int(features.get("user_complaint_count", 0))
    profile.device_count = int(features.get("behavior_device_count", 0))
