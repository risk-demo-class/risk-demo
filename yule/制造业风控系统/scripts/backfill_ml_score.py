"""
制造业风控系统 - XGBoost ml_score 字段回填脚本 (P4-L4 2026-08-08)
...
async def backfill_ml_score(limit: int | None = None, dry_run: bool = False):
    ...
    # 3. 逐条推理回填 (制造业: 从 risk_event 反查 order_id / product_sn)
    print(f"\n[3] 推理 + 回填...")
        ...
        for i, (aid, eid, uid, dec) in enumerate(rows, 1):
            try:
                async with SessionLocal() as db:
                    # 从 risk_event 拿事件类型 + 源业务ID + 事件快照
                    ev_row = (await db.execute(
                        select(RiskEvent.event_type, RiskEvent.event_source_id, RiskEvent.event_data)
                        .where(RiskEvent.event_id == eid)
                    )).first()
                    if not ev_row:
                        continue
                    event_type = ev_row.event_type
                    source_id = ev_row.event_source_id
                    event_data = json.loads(ev_row.event_data) if ev_row.event_data else {}

                    order_id = None
                    product_sn = event_data.get("product_sn")
                    if event_type == "经销商订货":
                        order_id = source_id
                    elif event_type == "设备保修":
                        wr = (await db.execute(
                            select(WarrantyRecord.order_id, WarrantyRecord.product_sn)
                            .where(WarrantyRecord.warranty_id == source_id)
                        )).first()
                        if wr:
                            order_id = wr.order_id
                            product_sn = product_sn or wr.product_sn
                    elif event_type == "跨区串货举报":
                        cr = (await db.execute(
                    ...
                    features = await compute_all_features(
                        db, uid, order_id=order_id, product_sn=product_sn,
                    )
                    ml = predict(features)
                    if ml is None:
                        continue
