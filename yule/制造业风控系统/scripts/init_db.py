...
SQL_FILES = [
    ...
    ("init_risk_data.sql", "导入 16 条预置风控规则 (R001-R030)"),
]

# 默认连接配置 (与 .env 一致, 支持环境变量覆盖)
DEFAULT_HOST = os.getenv("DB_HOST", "localhost")
DEFAULT_PORT = int(os.getenv("DB_PORT", "3306"))
DEFAULT_USER = os.getenv("DB_USER", "root")
DEFAULT_PASSWORD = os.getenv("DB_PASSWORD", "123456")
DEFAULT_DB = os.getenv("DB_NAME", "mfg_risk")
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 16 张表重建 + 业务数据 + 16 条规则全部就绪")
    else:
