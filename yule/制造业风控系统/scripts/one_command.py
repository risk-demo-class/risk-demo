"""
制造业风控系统 - 一条龙命令 (P4-L4 2026-08-08)
从零开始, 一键完成所有准备 + 训练, 最后启动 Web 服务.

【工作流 6 步】
  1. 重置数据库 (init_db.py --reset --yes, 含 16 条规则)
  2. 造 30 个 RISK 高风险经销商 (gen_risky_users.py --count 30)
  ...
  6. 造 50 条今日业务评估数据 (gen_risk_data_with_dates.py --days 1 --per-day 50 --live)
...
def step_2_risk_users() -> None:
    """步骤 2: 造 30 个 RISK 高风险经销商 (8 模式轮换)."""
    _run_subprocess(
...
def main() -> None:
    ...
    description="制造业风控系统 - 一条龙命令 (6 步: 重置 → RISK → 训练数据 → 训练 → 回填 → 启动)",
