...
class TestGenTrainDatasetScript:
    ...
    def test_script_picks_risk_and_normal_users(self):
        """脚本应分别选 RISK 高风险经销商和普通经销商"""
        src = GEN_PY.read_text(encoding="utf-8")
        assert "_pick_risk_dealers" in src, "应有 _pick_risk_dealers 函数"
        assert "_pick_normal_dealers" in src, "应有 _pick_normal_dealers 函数"
        assert "RISK" in src, "应区分 RISK 经销商"
