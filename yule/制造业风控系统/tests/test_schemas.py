...
class TestRiskCheckRequest:
    ...
    def test_valid_request(self):
        ...
        req = RiskCheckRequest(
            event_type="经销商订货",
            source_id="ORD_D001_001",
            user_id="D001",
        )
        assert req.event_type == "经销商订货"
        assert req.user_id == "D001"
        # 可选字段默认 None
        assert req.order_id is None
        assert req.receive_id is None
        assert req.event_data is None

    def test_all_3_event_types(self):
        """3 个制造业合法 event_type 都能过"""
        for et in ["经销商订货", "设备保修", "跨区串货举报"]:
    ...
    def test_missing_required_field(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RiskCheckRequest(event_type="经销商订货", user_id="u")  # 没传 source_id
...
class TestRuleCreate:
    ...
    def test_rule_score_out_of_range(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            RuleCreate(
                rule_id="R1", rule_name="t", rule_category="订货风险", event_type="通用",
