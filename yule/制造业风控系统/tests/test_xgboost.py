...
class TestFeatureColumns:
    ...
    def test_feature_prefixes(self):
        """12 user_* + 10 order_* + 3 addr_* (align with feature.py naming)"""
        ...
        assert len(user) == 12, f"user features should be 12, got {len(user)}"
        assert len(order) == 10, f"order features should be 10, got {len(order)}"
    ...
    def test_features_to_array(self):
        ...
        features = {
            ...
            "addr_ship_is_new": 0,           # 24 (last in FEATURE_COLUMNS)
        }
    ...
    def test_feature_columns_align_with_feature_module(self):
        ...
        """
        # 12 user + 10 order + 3 addr, aligned with feature.py's 3 compute_* functions
        expected = [
            # --- 12 user features (compute_user_features) ---
            "user_total_orders",
            "user_orders_30d",
            "user_orders_7d",
            "user_total_amount",
            ...
            "user_warranty_count",
            "user_warranty_rate",
            "user_repair_cost_total",
            "user_out_warranty_count",
            "user_cross_report_count",
            "user_contract_age_days",
            # --- 10 order features (compute_order_features) ---
            "order_total_amount",
            "order_quantity",
            "order_unit_price",
            "order_is_night",
            "order_is_cross_region",
            "order_contract_expired",
            "order_warranty_30d",
            "order_warranty_90d",
            "order_repair_cost_ratio",
            "order_cross_report_count",
            # --- 3 addr features (compute_address_features) ---
            "addr_ship_region_count",
            "addr_dealer_region_count",
            "addr_ship_is_new",
        ]
...
class TestModelLoad:
    ...
    def test_load_real_model(self, capsys):
        ...
        # 2. 准备一组"正常经销商"特征 (低值, 期望 → 通过/标记)
        normal_features = {
            "user_total_orders": 5,
            "user_orders_30d": 1,
            "user_orders_7d": 0,
            "user_total_amount": 50000.0,
            "user_avg_order_amount": 10000.0,
            "user_max_order_amount": 20000.0,
            "user_warranty_count": 0,
            "user_warranty_rate": 0.0,
            "user_repair_cost_total": 0,
            "user_out_warranty_count": 0,
            "user_cross_report_count": 0,
            "user_contract_age_days": 300,
            "order_total_amount": 100000.0,
            "order_quantity": 10,
            "order_unit_price": 10000.0,
            "order_is_night": 0,
            "order_is_cross_region": 0,
            "order_contract_expired": 0,
            "order_warranty_30d": 0,
            "order_warranty_90d": 0,
            "order_repair_cost_ratio": 0.0,
            "order_cross_report_count": 0,
            "addr_ship_region_count": 1,
            "addr_dealer_region_count": 1,
            "addr_ship_is_new": 0,
        }
        # 3. 准备一组"高风险经销商"特征 (强信号, 期望 → 人工审核/拒绝)
        risky_features = dict(normal_features)
        risky_features.update({
            "user_total_orders": 50,
            "user_orders_30d": 20,
            "user_warranty_count": 40,
            "user_warranty_rate": 0.8,
            "user_repair_cost_total": 500000.0,
            "user_cross_report_count": 5,
            "order_warranty_90d": 4,
            "order_repair_cost_ratio": 0.8,
            "order_cross_report_count": 2,
        })
        ...
        print(f"  正常经销商 → P(拒绝)={normal_res.score:.4f}, decision={normal_res.decision}")

        risky_res = ml_model.predict(risky_features)
        print(f"  高风险经销商 → P(拒绝)={risky_res.score:.4f}, decision={risky_res.decision}")
...
class TestPredict:
    ...
    def test_predict_normal_user(self):
        """Normal dealer (full 25-dim features, low values) -> reject prob low -> pass/mark"""
        ...
        normal.update({
            ...
            "user_orders_30d": 1,
            "user_orders_7d": 0,
            "user_total_amount": 50000,
            "user_avg_order_amount": 10000,
            "user_max_order_amount": 20000,
            "user_contract_age_days": 300,
            "order_total_amount": 100000,
            "order_quantity": 10,
            "order_unit_price": 10000,
            "addr_ship_region_count": 1,
            "addr_dealer_region_count": 1,
        })
        result = ml_model.predict(normal)
        assert 0.0 <= result.score <= 1.0
        # 正常经销商应判为 通过/标记, 不应是 拒绝
    ...
    def test_predict_risky_user(self):
        ...
        from app.engine.ml_model import FEATURE_COLUMNS
        # 高风险经销商: 25 维全部填, 高保修率/串货/大额囤货/套保/维修费异常
        risky = {
            # 经销商画像 (高保修 + 高维修费 + 串货)
            "user_total_orders": 200,
            "user_orders_30d": 100,
            "user_orders_7d": 50,
            "user_total_amount": 8000000,     # 800w 累计
            "user_avg_order_amount": 40000,
            "user_max_order_amount": 500000,  # 50w 单笔
            "user_warranty_count": 180,       # 90% 保修率
            "user_warranty_rate": 0.9,
            "user_repair_cost_total": 700000,  # 累计维修费很高
            "user_out_warranty_count": 30,     # 保修期外报修多
            "user_cross_report_count": 30,     # 串货举报多
            "user_contract_age_days": 500,
            # 订单特征 (大额囤货 + 跨区 + 资质过期)
            "order_total_amount": 500000,
            "order_quantity": 150,             # 大额囤货
            "order_unit_price": 3300,
            "order_is_night": 1,               # 凌晨下单
            "order_is_cross_region": 1,        # 跨区发货
            "order_contract_expired": 1,       # 资质过期
            "order_warranty_30d": 3,
            "order_warranty_90d": 5,           # 套保
            "order_repair_cost_ratio": 0.85,   # 维修费虚高
            "order_cross_report_count": 3,
            # 区域特征
            "addr_ship_region_count": 8,
            "addr_dealer_region_count": 3,
            "addr_ship_is_new": 1,
        }
