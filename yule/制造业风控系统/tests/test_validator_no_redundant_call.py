...
class TestProcessEventCallsValidatorOnce:
    ...
    async def test_process_event_calls_validate_risk_check_request_once(self, monkeypatch):
        ...
        request = RiskCheckRequest(
            event_type="经销商订货", source_id="ORD001", user_id="D001", order_id="ORD001"
        )
...
class TestValidatorPublicApi:
    ...
    def test_validator_demo_block_mentions_3_ensure(self):
        """__main__ demo 块打印 '3 个 ensure_*' (制造业 3 个校验函数)."""
        src = inspect.getsource(validator_module)
        assert "3 个 ensure_*" in src, (
            "__main__ demo 块描述应为 '3 个 ensure_*' (制造业校验函数)"
        )
