...
class TestScoreCalculation:
    ...
    def test_single_hit(self):
        hit = RuleHitResult(_MockRule("R1", "测试", "订货风险", "高", 70, "人工审核"))
        assert calculate_final_score([hit]) == 70

    def test_multiple_hits_bonus(self):
        """多规则命中: max + BONUS × extra_count"""
        hits = [
            RuleHitResult(_MockRule("R1", "测试1", "订货风险", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R2", "测试2", "保修风险", "中", 40, "标记")),
...
class TestCaseDeduplication:
    ...
    场景: 同一订单的"经销商订货"事件被前端重试 3 次, 每次都触发"人工审核"
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="经销商订货", source_id="order_001", user_id="1001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="经销商订货", source_id="order_002", user_id="1001",
        )
        ...
        assert db.added[0].event_type == "经销商订货"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="经销商订货", source_id="order_003", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "订货风险", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        ...
        request = RiskCheckRequest(
            event_type="经销商订货", source_id="order_reject_001", user_id="U001",
        )
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="经销商订货", source_id="order_review_001", user_id="U002",
        )
...
class TestCaseCategoryAggregation:
    ...
    async def test_majority_category_wins(self):
        """3 条保修风险 + 1 条订货风险 → 分类 = 保修风险 (命中数最多)"""
        from app.engine import decision
        from app.schemas import RiskCheckRequest
        from types import SimpleNamespace

        request = RiskCheckRequest(
            event_type="设备保修", source_id="order_x", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # priority 高的 R007 (保修风险) 排第一
        rules = [
            RuleHitResult(_MockRule("R007", "30天30单", "保修风险", "极高", 90, "拒绝")),
            RuleHitResult(_MockRule("R006", "7天10单", "保修风险", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R008", "金额异常", "保修风险", "高", 65, "人工审核")),
            RuleHitResult(_MockRule("R001", "5000+", "订货风险", "高", 70, "人工审核")),
        ]
        ...
        # 3 条保修风险 vs 1 条订货风险 → 取多的
        assert case.case_category == "保修风险", (
            f"应取命中数最多的 '保修风险', 实际 {case.case_category}"
        )
    ...
    async def test_tie_first_one_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="经销商订货", source_id="order_y", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # 1 条订货风险 + 1 条串货风险 → 平局
        rules = [
            RuleHitResult(_MockRule("R1", "a", "订货风险", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("R2", "b", "串货风险", "高", 70, "人工审核")),
        ]
        ...
        assert db.added[0].case_category in ("订货风险", "串货风险")
