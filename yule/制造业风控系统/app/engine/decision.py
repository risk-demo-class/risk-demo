"""
制造业风控系统 - 风控决策引擎: 串联 7 步流水线, 事件 → 特征 → 规则 → 评分 → 决策 → 持久化.
...
def check_veto(hits: list[RuleHitResult]) -> bool:
    ...
    当前项目"极高"级别规则: R001 (串货举报>=2) / R008 (90 天同 SN 保修>=2).
    """
...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则 (制造业):
      ...
      - "设备保修" 事件: source_id 是 warranty_id, order_id/product_sn 从 event_data 或保修单补全
      - "跨区串货举报" 事件: source_id 是 report_id
    """
...
async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 补全地址 ID (制造业无收货地址概念, 保留为空操作).

    制造业业务表没有 receive_id 字段, 区域维度特征直接用订单 ship_to_region 计算.
    """
    return
...
async def _compute_features(db: AsyncSession, ctx: _RiskCheckContext) -> dict:
    """步骤 3: 调 feature.py 计算 25 个风控特征 (经销商用户/订货保修事件/区域 3 类)"""
    return await compute_all_features(
        db,
        user_id=ctx.user_id,
        order_id=ctx.order_id,        # 可能 None (跨区举报场景只算用户+区域)
        receive_id=ctx.receive_id,
        product_sn=ctx.product_sn,
    )
...
async def _update_user_profile(
    ...
    profile.refund_rate = (
        round(features.get("user_warranty_count", 0) / total_orders, 4)
        if total_orders > 0 else 0
    )
    profile.avg_order_amount = features.get("user_avg_order_amount", 0)
    profile.address_count = int(features.get("addr_ship_region_count", 0))
    profile.complaint_count = int(features.get("user_cross_report_count", 0))
