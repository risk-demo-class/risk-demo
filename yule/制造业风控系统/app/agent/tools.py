"""
制造业风控系统 - AI Agent 工具集 (8 个 LangChain @tool).
...
async def _query_user_profile_impl(*, db: AsyncSession, user_id: str) -> str:
    """查询经销商风险画像. 没画像时实时算 13 个 user 特征 (不写库)."""
...
async def _blacklist_add_impl(
    *, db: AsyncSession, blacklist_type: Literal["用户","经销商","设备SN","维修工","手机号","地址"], value: str, reason: str,
) -> str:
...
async def _manage_blacklist_impl(
    *, db: AsyncSession, action: str, blacklist_type: Literal["用户","经销商","设备SN","维修工","手机号","地址"], value: str, reason: str,
) -> str:
...
async def _biz_user_warranties(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """经销商的设备保修记录 (warranty → order)."""
    stmt = (
        select(
            WarrantyRecord.warranty_id,
            WarrantyRecord.product_sn,
            WarrantyRecord.order_id,
            WarrantyRecord.issue_date,
            WarrantyRecord.issue_type,
            WarrantyRecord.repair_cost,
...
async def _biz_dealer_regions(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """经销商的历史发货区域 (DISTINCT)."""
    stmt = (
        select(OrderInfo.ship_to_region, func.count().label("order_count"))
        .where(OrderInfo.dealer_id == user_id)
        .group_by(OrderInfo.ship_to_region)
...
async def _biz_order_info(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """单张订货订单详情 (含产品信息)."""
    stmt = (
        select(
            OrderInfo.order_id,
...
async def _biz_bulk_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    """大额囤货订单 (单笔订货数量 >= 50 台, 管理员视角)."""
    stmt = (
        select(
            OrderInfo.order_id,
            OrderInfo.dealer_id,
        ...
        .where(OrderInfo.quantity >= 50)
        .order_by(OrderInfo.quantity.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
_BIZ_QUERY_HANDLERS = {
    ...
    "user_warranties": _biz_user_warranties,
    "dealer_regions": _biz_dealer_regions,
    "order_info": _biz_order_info,
    "recent_orders": _biz_recent_orders,
    "bulk_orders": _biz_bulk_orders,
}
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (经销商账号, 如 'D001')、event_type (事件类型, 可选值: 经销商订货/设备保修/跨区串货举报)、source_id (关联业务ID, 订单号/保修单号/举报ID)。"
...
@tool(
    description=(
        "查询用户的风险画像信息。"
        "参数: user_id (经销商账号, 如 'D001')。"
        "返回: 经销商的风险评分、订货统计、保修率、串货举报数等画像数据; 没画像时实时算 13 个 user 特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/经销商/设备SN/维修工/手机号/地址)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        ...
        "参数: query_type (查询类型, 可选值: user_orders/user_warranties/dealer_regions/order_info/recent_orders/bulk_orders)、user_id (部分需要)、order_id (部分需要)、limit (返回数量, 默认10)。"
        "返回: 业务数据 (JSON 字符串)。"
    )
)
...
if __name__ == "__main__":
    ...
    impls = [
        ...
        _biz_user_orders, _biz_user_warranties, _biz_dealer_regions,
        _biz_order_info, _biz_recent_orders, _biz_bulk_orders,
