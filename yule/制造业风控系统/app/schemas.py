"""Pydantic 请求/响应模型: 制造业风控 API 端点的数据校验和序列化"""
...
class RiskCheckRequest(BaseModel):
    """风险检查请求"""
    event_type: Literal["经销商订货", "设备保修", "跨区串货举报"]
    source_id: str = Field(description="关联业务ID (order_id / warranty_id / report_id)")
    user_id: str
    order_id: Optional[str] = None
    receive_id: Optional[str] = None  # 制造业无收货地址概念, 保留字段兼容旧接口
...
class RiskCheckResponse(BaseModel):
    ...
    # 【P1-S9】被哪种黑名单撞了: None=没撞黑, "用户"/"经销商"/"设备SN"/"维修工" 四选一.
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["订货风险", "保修风险", "串货风险", "资质风险", "维修风险", "账户风险"]
    event_type: Literal["经销商订货", "设备保修", "跨区串货举报", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["订货风险", "保修风险", "串货风险", "资质风险", "维修风险", "账户风险"]] = None
    event_type: Optional[Literal["经销商订货", "设备保修", "跨区串货举报", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["用户", "经销商", "设备SN", "维修工", "手机号", "地址"]
    blacklist_value: str
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        event_type="经销商订货", source_id="ord_demo_001", user_id="D001",
        order_id="ord_demo_001",
        event_data={"quantity": 120, "total_amount": 3000000},
    )
    ...
    resp = RiskCheckResponse(
        ...
        user_id="D001", final_score=95, risk_level="极高", decision="拒绝",
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="D001", event_type="经销商订货", event_source_id="ord_demo_001",
