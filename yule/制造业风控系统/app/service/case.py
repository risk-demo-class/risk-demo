...
async def get_user_profile(db: AsyncSession, user_id: str) -> UserProfileResponse | None:
    ...
    return UserProfileResponse(
        ...
        risk_level=profile.risk_level,
        total_orders=profile.total_orders,
        # 制造业画像: total_refunds 列存保修次数, refund_rate 列存保修率
        warranty_count=profile.total_refunds,
        warranty_rate=float(profile.refund_rate),
        avg_order_amount=float(profile.avg_order_amount),
        # address_count 列存发货区域数, complaint_count 列存串货举报数
        region_count=profile.address_count,
        cross_report_count=profile.complaint_count,
        assessment_count=profile.assessment_count,
...
async def list_assessments(
    ...
    筛选维度 (可选):
      ...
      - risk_level: 低 / 中 / 高 / 极高
      - event_type: 经销商订货 / 设备保修 / 跨区串货举报 (JOIN risk_event)
      - user_id: 精确匹配
