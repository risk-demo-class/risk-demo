...
class RiskCheckRequest(BaseModel):
    """风险检查请求 (银行版)"""
    event_type: Literal["转账", "贷款申请", "登录"]
    source_id: str = Field(description="关联业务ID (txn_id / loan_id / login_id)")
    user_id: str
    order_id: Optional[str] = Field(default=None, description="银行场景=txn_id (引擎兼容)")
    receive_id: Optional[str] = Field(default=None, description="银行场景=设备ID/IP (引擎兼容)")
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["转账风险", "信贷风险", "账户风险", "设备风险", "IP风险", "交易风险"]
    event_type: Literal["转账", "贷款申请", "登录", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["转账风险", "信贷风险", "账户风险", "设备风险", "IP风险", "交易风险"]] = None
    event_type: Optional[Literal["转账", "贷款申请", "登录", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    """添加黑名单请求"""
    blacklist_type: Literal["用户", "设备指纹", "IP", "银行卡号", "身份证号"]
    blacklist_value: str
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        ...
        order_id="TXN_demo_001", receive_id="DEV_A001",
        event_data={"amount": 50000, "channel": "网银"},
    )
