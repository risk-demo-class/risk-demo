...
FEATURE_COLUMNS: list[str] = [
    # 用户维度 (14)
    "user_credit_score",          # 征信分
    "user_account_age_days",      # 账户注册天数
    "user_kyc_level",             # KYC等级(数值)
    "user_txns_30d",              # 近30天交易数
    "user_txns_7d",               # 近7天交易数
    "user_total_transfer_amount", # 历史总交易金额
    "user_avg_transfer_amount",   # 平均交易金额
    "user_max_transfer_amount",   # 最大单笔交易
    "user_loan_count",            # 贷款申请次数
    "user_login_count_7d",        # 近7天登录次数
    "user_device_count",          # 关联设备数
    "user_card_count",            # 绑卡数
    "user_ip_region_count",       # IP不同省份数
    "user_debt_ratio",            # 负债率
    # 交易维度 (8)
    "order_txn_amount",           # 本次交易金额
    "order_txn_is_night",         # 是否夜间交易(0-5点)
    "order_txn_channel",          # 交易渠道(数值)
    "order_txn_hourly_count",     # 1小时内交易数
    "order_txn_cross_city",       # 是否跨城市
    "order_txn_hour",             # 交易小时(0-23)
    "order_txn_days_since_last",  # 距上次交易天数
    "order_txn_to_card_new",      # 收款卡是否新卡
    # IP/设备维度 (3)
    "addr_ip_is_proxy",           # IP是否代理
    "addr_ip_login_freq_7d",      # 该IP近7天登录频次
    "addr_ip_is_new",             # IP是否新发现
]
...
def _features_to_array(features: dict) -> np.ndarray:
    ...
    【P6 优化 2026-08-11】预分配 numpy 数组避免 Python 列表 + 二次转换.
    """
    row = np.zeros(len(FEATURE_COLUMNS), dtype=np.float32)
    get = features.get
    for i, col in enumerate(FEATURE_COLUMNS):
        try:
            row[i] = float(get(col, 0.0))
...
if __name__ == "__main__":
    ...
    sample = {
        "user_credit_score": 720, "user_account_age_days": 365, "user_kyc_level": 3,
        "user_txns_30d": 5, "user_txns_7d": 2,
        "user_total_transfer_amount": 150000, "user_avg_transfer_amount": 30000,
        "user_max_transfer_amount": 80000,
        "user_loan_count": 0, "user_login_count_7d": 14, "user_device_count": 1,
        "user_card_count": 2, "user_ip_region_count": 1, "user_debt_ratio": 0.15,
        "order_txn_amount": 50000, "order_txn_is_night": 0, "order_txn_channel": 2,
        "order_txn_hourly_count": 1, "order_txn_cross_city": 0, "order_txn_hour": 10,
        "order_txn_days_since_last": 3, "order_txn_to_card_new": 0,
        "addr_ip_is_proxy": 0, "addr_ip_login_freq_7d": 3, "addr_ip_is_new": 0,
    }
