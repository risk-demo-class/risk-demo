...
def match_rules(
    ...
    【P6 优化 2026-08-11】局部变量引用避免重复属性查找.
    """
    ...
    for rule in rules:
        try:
            ...
            log_warn("规则 %s 条件解析失败: %s", rule.rule_id, e)
            continue
        if eval_cond(condition, features):
            log_info("规则命中: %s (%s), 分值=%d", rule.rule_id, rule.rule_name, rule.risk_score)
...
def _compare(actual: float, op: str, value: Any) -> bool:
    ...
    【P6 优化 2026-08-11】dict 派发替代 if-elif 链, 减少 Python 字节码.
    """
    fv = float(value)
    dispatch = {
        ">":       lambda: actual > fv,
        ...
        "!=":      lambda: actual != fv,
        # 列表运算: value 是 [v1, v2, ...]
        "in":      lambda: actual in [float(v) for v in value],
        "not_in":  lambda: actual not in [float(v) for v in value],
        "between": lambda: float(value[0]) <= actual <= float(value[1]),
    }
