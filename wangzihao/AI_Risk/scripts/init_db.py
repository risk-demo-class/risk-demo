...
# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 6 张制造业业务表"),
    ("init_business_data.sql", "导入制造业业务测试数据"),
    ("init_risk_tables.sql", "创建 9 张风控表 (7 原 + 2 P4 系统管理表)"),
    ("init_risk_data.sql", "导入 7 条制造业风控规则 (R001-R007)"),
]
...
async def execute_sql_file(conn, filepath, description, target_db=None):
    """执行单个 SQL 脚本文件 (异步)"""
    ...
    async with conn.cursor() as cur:
        for stmt in statements:
            try:
                # SQL 历史文件可能含 ``USE ecs``。命令行 --db 应始终优先，
                # 避免初始化测试库时把后续表或数据误写进默认库。
                if target_db and stmt.lstrip().upper().startswith("USE "):
                    escaped_db = target_db.replace("`", "``")
                    stmt = f"USE `{escaped_db}`"
...
async def main():
    parser = argparse.ArgumentParser(
        description="制造业设备经销商风控系统 - 一键数据库初始化 (异步). 默认 --reset 重置整个数据库",
...
用法:
  ...
  python scripts/init_db.py --db ecs_test        # 初始化测试库
    ...
    try:
        for idx, (filename, desc) in enumerate(SQL_FILES, start=2):
            ...
            if not os.path.exists(filepath):
                ...
                continue
            print(f"\n[步骤 {idx}/5]", end="")
            errors = await execute_sql_file(conn, filepath, desc, target_db=args.db)
            total_errors += errors
    finally:
        await conn.ensure_closed()
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 15 张表重建（6 业务 + 9 风控）+ 初始化数据全部就绪")
    else:
