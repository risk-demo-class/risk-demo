...
class TestValidatorPublicApi:
    ...
    def test_manufacturing_event_mapping_is_centralized(self):
        from app.industry_mapping import EVENT_MAPPINGS

        assert {mapping.source_entity for mapping in EVENT_MAPPINGS.values()} == {
            "purchase_order", "warranty_claim", "cross_region_report",
        }

    def test_validator_contains_only_manufacturing_source_models(self):
        """入口校验已迁移到制造业来源，不应重新出现旧电商 ORM。"""
        src = inspect.getsource(validator_module)
        for expected in ("PurchaseOrder", "WarrantyClaim", "CrossRegionReport", "Device", "Dealer"):
            assert expected in src
        for removed in ("OrderInfo", "PostSale", "ReceiveInfo", "LogisticsComplaintsRecord"):
