...
class TestPredict:
    ...
    def test_predict_risky_user(self):
        """制造业新经销商大额采购的概率应高于正常长期经销商采购。"""
        from app.engine.ml_model import FEATURE_COLUMNS
        risky = {col: 0.0 for col in FEATURE_COLUMNS}
        risky.update({
            "user_total_orders": 3,
            "user_total_amount": 3_300_000,
            "user_avg_order_amount": 1_100_000,
            "user_max_order_amount": 1_150_000,
            "user_cancel_count": 16,  # 合作天数 < 30
            "user_address_count": 4,
            "order_total_amount": 1_120_000,
            "order_item_count": 2,
            "order_sku_count": 9,
            "order_pay_interval_sec": 30 * 24 * 3600,
            "order_category_count": 2,
            "addr_total_count": 4,
        })
        ...
        normal.update({
            "user_total_orders": 3,
            "user_total_amount": 600_000,
            "user_avg_order_amount": 200_000,
            "user_max_order_amount": 250_000,
            "user_cancel_count": 1900,
            "user_address_count": 4,
            "order_total_amount": 200_000,
            "order_item_count": 2,
            "order_sku_count": 6,
            "order_pay_interval_sec": 30 * 24 * 3600,
            "order_category_count": 2,
...
class TestTrainScriptUnpacking:
    """训练脚本必须在 XGBoost 前执行 dealer 分组切分。"""

    def test_train_script_unpacks_metrics_first_then_model(self):
        """不得再调用内部随机拆分的 train_and_save。"""
        script = Path("scripts/train_xgb_model.py").read_text(encoding="utf-8")
        assert "group_split(dataset)" in script
        assert "GroupShuffleSplit" in script
        assert "xgb.train(" in script
        assert "train_and_save(" not in script
