...
AI Agent 工具集 (8 个 LangChain @tool).
  ...
  风控决策工具 (RISK_TOOLS, 4): 风险检查 / 案件查询 / 经销商画像 / 黑名单管理
...
from app.agent.explanation import explain_persisted_evidence
...
async def _query_cases_impl(
    *, db: AsyncSession, status: str, page: int,
    ...
    return json.dumps({
        "cases": [
            {**item.model_dump(mode="json"), "dealer_id": item.user_id}
...
async def _query_user_profile_impl(*, db: AsyncSession, user_id: str) -> str:
    ...
    if not profile:
        ...
        return json.dumps({
            ...
            "user_id": user_id,
            "has_profile": False,
            "computed_dealer_features": features,
        }, ensure_ascii=False, indent=2)
    profile_data = profile.model_dump(mode="json")
    return json.dumps({
        **profile_data,
        "dealer_id": profile.user_id,
        "manufacturing_meaning": {
            "total_orders": "历史采购单数量",
            "total_refunds": "高金额保修申请数量（兼容字段）",
            "refund_rate": "高金额保修占比（兼容字段）",
            "avg_order_amount": "平均采购金额",
            "address_count": "活跃区域数量",
            "complaint_count": "串货报告数量",
        },
    }, ensure_ascii=False, indent=2)
...
async def _manage_blacklist_impl(
    *, db: AsyncSession, action: str, blacklist_type: str, value: str, reason: str,
    ...
    if action == "add":
        return await impl(
            db=db, blacklist_type=blacklist_type, value=value, reason=reason,
        )
...
# ============================================================
# 制造业业务与证据查询（保持单一通用 Tool，通过 dispatch 扩展）
# ============================================================

async def _biz_dealer_info(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    return list((await db.execute(
        select(Dealer.dealer_id, Dealer.name, Dealer.level, Dealer.region,
               Dealer.authorized_at, Dealer.contract_end)
        .where(Dealer.dealer_id == user_id).limit(1)
    )).all())


async def _biz_dealer_purchase_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    stmt = select(
        PurchaseOrder.po_id, PurchaseOrder.dealer_id, PurchaseOrder.total_amount,
        PurchaseOrder.items, PurchaseOrder.ship_to, PurchaseOrder.payment_term,
        PurchaseOrder.create_time,
    )
    if order_id:
        stmt = stmt.where(PurchaseOrder.po_id == order_id)
    elif user_id:
        stmt = stmt.where(PurchaseOrder.dealer_id == user_id)
    return list((await db.execute(stmt.order_by(PurchaseOrder.create_time.desc()).limit(limit))).all())


async def _biz_device_history(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    claim_counts = (
        select(WarrantyClaim.device_id, func.count().label("warranty_claim_count"))
        .group_by(WarrantyClaim.device_id).subquery()
    )
    report_counts = (
        select(CrossRegionReport.device_id, func.count().label("cross_region_report_count"))
        .group_by(CrossRegionReport.device_id).subquery()
    )
    stmt = (
        select(Device.device_id, Device.sn, Device.model, Device.batch_no,
               Device.factory_at, Device.warranty_end, Device.dealer_id,
               func.coalesce(claim_counts.c.warranty_claim_count, 0).label("warranty_claim_count"),
               func.coalesce(report_counts.c.cross_region_report_count, 0).label("cross_region_report_count"))
        .outerjoin(claim_counts, claim_counts.c.device_id == Device.device_id)
        .outerjoin(report_counts, report_counts.c.device_id == Device.device_id)
    )
    if order_id:
        stmt = stmt.where((Device.device_id == order_id) | (Device.sn == order_id))
    elif user_id:
        stmt = stmt.where(Device.dealer_id == user_id)
    return list((await db.execute(stmt.order_by(Device.factory_at.desc()).limit(limit))).all())


async def _biz_warranty_claims(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    stmt = select(
        WarrantyClaim.claim_id, WarrantyClaim.device_id, Device.sn,
        WarrantyClaim.dealer_id, WarrantyClaim.fault_desc,
        WarrantyClaim.claim_amount, WarrantyClaim.photos, WarrantyClaim.create_time,
    ).join(Device, Device.device_id == WarrantyClaim.device_id)
    if order_id:
        stmt = stmt.where((WarrantyClaim.claim_id == order_id) | (WarrantyClaim.device_id == order_id) | (Device.sn == order_id))
    elif user_id:
        stmt = stmt.where(WarrantyClaim.dealer_id == user_id)
    return list((await db.execute(stmt.order_by(WarrantyClaim.create_time.desc()).limit(limit))).all())


async def _biz_cross_region_reports(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    stmt = select(
        CrossRegionReport.report_id, CrossRegionReport.device_id, Device.sn,
        Device.dealer_id, CrossRegionReport.expected_region,
        CrossRegionReport.actual_region, CrossRegionReport.reporter_id,
        CrossRegionReport.create_time,
    ).join(Device, Device.device_id == CrossRegionReport.device_id)
    if order_id:
        stmt = stmt.where((CrossRegionReport.report_id == order_id) | (CrossRegionReport.device_id == order_id) | (Device.sn == order_id))
    elif user_id:
        stmt = stmt.where(Device.dealer_id == user_id)
    return list((await db.execute(stmt.order_by(CrossRegionReport.create_time.desc()).limit(limit))).all())


async def _biz_high_value_purchase_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    stmt = select(
        PurchaseOrder.po_id, PurchaseOrder.dealer_id, Dealer.name,
        PurchaseOrder.total_amount, PurchaseOrder.ship_to,
        PurchaseOrder.payment_term, PurchaseOrder.create_time,
    ).join(Dealer, Dealer.dealer_id == PurchaseOrder.dealer_id).where(
        PurchaseOrder.total_amount >= 1_000_000
    )
    if user_id:
        stmt = stmt.where(PurchaseOrder.dealer_id == user_id)
    return list((await db.execute(stmt.order_by(PurchaseOrder.total_amount.desc()).limit(limit))).all())


async def _biz_blacklist_extra(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    stmt = select(BlacklistExtra.entry_id, BlacklistExtra.type, BlacklistExtra.value,
                  BlacklistExtra.reason, BlacklistExtra.expire_at)
    if order_id:
        stmt = stmt.where(BlacklistExtra.value == order_id)
    return list((await db.execute(stmt.limit(limit))).all())


async def _biz_risk_assessment_evidence(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[dict]:
    """Read persisted risk evidence; never recompute scores or decisions."""
    stmt = (
    ...
    if order_id:
        stmt = stmt.where(
            (RiskAssessment.assessment_id == order_id)
            | (RiskEvent.event_id == order_id)
            | (RiskEvent.event_source_id == order_id)
        )
    elif user_id:
        stmt = stmt.where(RiskAssessment.user_id == user_id)
    rows = (await db.execute(stmt.limit(limit))).all()
    evidence = []
    for assessment, event in rows:
        feature_rows = (await db.execute(
            select(RiskFeature.feature_name, RiskFeature.feature_value,
                   RiskFeature.entity_type)
            .where(RiskFeature.event_id == event.event_id)
            .order_by(RiskFeature.feature_id)
        )).all()
        try:
            rules = json.loads(assessment.rule_results or "[]")
        except (json.JSONDecodeError, TypeError):
            rules = []
        rule_ids = [rule.get("rule_id") for rule in rules if rule.get("rule_id")]
        if rule_ids:
            definitions = (await db.execute(
                select(RiskRule.rule_id, RiskRule.rule_condition)
                .where(RiskRule.rule_id.in_(rule_ids))
            )).all()
            conditions = {}
            for definition in definitions:
                try:
                    conditions[definition.rule_id] = json.loads(definition.rule_condition)
                except (json.JSONDecodeError, TypeError):
                    conditions[definition.rule_id] = None
            rules = [
                {**rule, "condition": conditions.get(rule.get("rule_id"))}
                for rule in rules
            ]
        item = {
            "assessment_id": assessment.assessment_id,
            "event_id": event.event_id,
            "event_type_internal_code": event.event_type,
            "source_id": event.event_source_id,
            "dealer_id": assessment.user_id,
            ...
            "features": [
                {"name": row.feature_name, "value": float(row.feature_value), "entity_type": row.entity_type}
                for row in feature_rows
            ],
        }
        item["agent_explanation"] = explain_persisted_evidence(item)
...
_BIZ_QUERY_HANDLERS = {
    ...
    "dealer_purchase_orders": _biz_dealer_purchase_orders,
    "device_history": _biz_device_history,
    "warranty_claims": _biz_warranty_claims,
    "cross_region_reports": _biz_cross_region_reports,
    "high_value_purchase_orders": _biz_high_value_purchase_orders,
    "blacklist_extra": _biz_blacklist_extra,
    "risk_assessment_evidence": _biz_risk_assessment_evidence,
}
...
@tool(
    description=(
        "对指定经销商和制造业事件执行实时风险检查。"
        "参数: user_id 是兼容字段，填写 dealer_id；event_type 可使用经销商采购提交/采购付款/设备保修申请/串货举报或内部码；source_id 对应采购单、保修申请或串货报告ID。"
        "返回数据库真实评估结果，包括事件ID、25维特征、命中规则、ML概率和最终决策。"
        "内部严格走 process_event: 校验→关联补全→黑名单→run_risk_check。"
    )
)
...
@tool(
    description=(
        "查询经销商风险画像。参数 user_id 填 dealer_id。"
        "返回经销商风险评分、采购、保修、串货和区域指标；内部 user_* 兼容键对外按经销商语义解释。"
    )
)
async def query_user_profile(user_id: str) -> str:
    return await _safe_call("经销商画像查询", _query_user_profile_impl, user_id=user_id)


@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (add/remove/check/list)；经销商黑名单内部使用 blacklist_type=用户，区域/联系方式继续使用兼容类型。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
...
@tool(
    description=(
        "查询制造业业务数据或持久化风险证据。"
        "query_type 可选: dealer_info/dealer_purchase_orders/device_history/warranty_claims/cross_region_reports/high_value_purchase_orders/blacklist_extra/risk_assessment_evidence。"
        "user_id 填 dealer_id；order_id 是兼容参数，可填写 po_id/device_id/SN/claim_id/report_id/assessment_id/event_id/source_id。"
    )
)
...
if __name__ == "__main__":
    ...
    tools_list = [
        ("risk_check",            "对经销商和制造业事件执行实时风控检查"),
        ("query_cases",           "查询案件列表 (按 status)"),
        ("query_user_profile",    "查询经销商风险画像"),
        ...
        ("query_business_data",   "制造业业务/风险证据查询 (8 query_type)"),
    ]
    ...
    print(f"  _BIZ_QUERY_HANDLERS = {list(_BIZ_QUERY_HANDLERS.keys())}  (8 个 query_type)")
    ...
    impls = [
        ...
        _biz_dealer_info, _biz_dealer_purchase_orders, _biz_device_history,
        _biz_warranty_claims, _biz_cross_region_reports,
        _biz_high_value_purchase_orders, _biz_blacklist_extra,
        _biz_risk_assessment_evidence,
