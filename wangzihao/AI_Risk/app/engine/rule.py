...
数据库中存的是前端定义的规则 json类型   读取到python中需要json.loads

支持的运算符: >, >=, <, <=, ==, !=, in, not_in, between, and, or
"""
import json
import logging
from typing import Any  #any即为各种类型都可以

from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from app.models import RiskRule #SQLAlchemy ORM 类
...
def match_rules(
    ...
    for rule in rules:
        try:
            condition = rule.condition_dict  # @property 自动 json.loads 拿规则的条件字典
        except (json.JSONDecodeError, TypeError) as e:
            # 条件 JSON 坏了, 跳过这条, 不影响其他规则
            logger.warning("规则 %s 条件解析失败: %s", rule.rule_id, e)
            continue
        if evaluate_condition(condition, features):#条件字典和 特征 做判断
