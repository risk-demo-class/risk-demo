...
from app.industry_mapping import normalize_event_type
...
class RiskCheckRequest(BaseModel):
    """风险检查请求；兼容字段 user_id 在制造业中表示 dealer_id。"""
    event_type: Literal["下单", "支付", "售后申请", "物流投诉"]
    source_id: str = Field(description="制造业来源ID (po_id / claim_id / report_id)")
    user_id: str = Field(description="兼容字段；制造业语义为 dealer_id")
    ...
    @classmethod
    def normalize_manufacturing_event_type(cls, value: object) -> object:
