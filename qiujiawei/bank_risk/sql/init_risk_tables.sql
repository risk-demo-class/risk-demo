-- ============================================
-- 银行风控系统 - 风控表 DDL 初始化脚本
-- 在 bank_risk 数据库中创建 9 张风控表
-- 跟 AI_Risk/sql/init_risk_tables.sql 结构一致, 但 Enum 值改为银行场景:
--   rule_category: 信用卡欺诈, 贷款风险, 转账风险, 登录风险, 账户风险, 设备风险
--   event_type:    信用卡, 贷款, 转账, 登录, 通用
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('信用卡欺诈','贷款风险','转账风险','登录风险','账户风险','设备风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('信用卡','贷款','转账','登录','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('信用卡','贷款','转账','登录') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','交易','设备','贷款','登录') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_assessment` (
    ...
    -- XGBoost 双轨融合字段: ml_score ∈ [0,1] = P(拒绝), ml_decision = ML 单维度决策
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    -- 业务回溯字段: decision.py 写入, "重做检查" 按钮回查用
    `source_id` VARCHAR(50) DEFAULT NULL COMMENT '原始业务ID(交易/贷款/登录ID), 重做检查时用',
    `event_type` ENUM('信用卡','贷款','转账','登录') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('user','card','device','ip','id_card') NOT NULL COMMENT '黑名单类型',
    ...
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    -- 软删: NULL=未删, 有值=删除时间 (撞黑检查/列表查都加 WHERE deleted_at IS NULL)
    `deleted_at` DATETIME DEFAULT NULL COMMENT '软删除时间(NULL=未删)',
...
CREATE TABLE IF NOT EXISTS `risk_user_profile` (
    ...
    `total_txns` INT DEFAULT 0 COMMENT '总交易数',
    `total_loans` INT DEFAULT 0 COMMENT '贷款申请数',
    `avg_txn_amount` DECIMAL(15,2) DEFAULT 0 COMMENT '平均交易金额',
