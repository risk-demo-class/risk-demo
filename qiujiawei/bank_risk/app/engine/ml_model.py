...
def load_model(model_path: Optional[str] = None) -> bool:
    ...
    if model_path:
        ...
        # 项目根 = ml_model.py 所在目录的 3 级父 (bank_risk/app/engine/ml_model.py → bank_risk/)
...
def train_and_save(
    ...
    # warmup 防假收敛: 前 N 轮不让早停, 给模型充分训练时间
    ...
    if len(eval_set) > 1:
        ...
        # 最佳 F1 阈值扫描: 固定 0.5 阈值在不平衡数据上 F1 暴跌
...
# 跑法: python bank_risk/app/engine/ml_model.py
# ============================================================
if __name__ == "__main__":
    import os
    print("=" * 60)
    print("XGBoost 推理 — 模型生命周期 + 降级兜底 (银行风控)")
    print("=" * 60)
    ...
    sample = {
        "user_txn_count_30d": 15, "user_txn_amount_30d": 25000,
        "user_avg_txn_amount": 1666.67, "user_max_txn_amount": 5000,
        "user_loan_count_6m": 1, "user_login_count_7d": 5,
        "user_login_fail_count_7d": 0, "user_device_count": 2,
        "user_credit_score": 750, "user_kyc_level": 2,
        "txn_amount": 3000, "txn_channel": 2, "txn_is_night": 0,
        "txn_device_age_days": 30, "txn_amount_to_limit_ratio": 0.3,
        "txn_hour_freq": 1, "txn_to_same_card_1h": 0, "txn_geo_changed": 0,
        "dev_multi_user_count": 1, "dev_risk_score": 10,
        "ip_is_proxy": 0, "ip_is_tor": 0, "ip_is_new": 0,
        "login_geo_is_new": 0, "login_fail_rate_7d": 0.0,
    }
    ...
    print("\n" + "=" * 60)
    print("总结: 25 维银行特征对齐 / 模型加载 / predict 兜底 / 概率阈值, 4 个核心能力演示完成")
