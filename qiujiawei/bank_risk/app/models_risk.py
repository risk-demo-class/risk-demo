...
跟 AI_Risk/app/models_risk.py 结构一致, 但 Enum 值改为银行场景:
  - rule_category_enum: 信用卡欺诈, 贷款风险, 转账风险, 登录风险, 账户风险, 设备风险
  - rule_event_type_enum: 信用卡, 贷款, 转账, 登录, 通用
  - risk_level_enum: 低, 中, 高, 极高
  - rule_action_enum: 通过, 标记, 人工审核, 拒绝
  - case_status_enum: 待审核, 审核中, 已通过, 已拒绝, 已关闭
"""
import json
...
class RiskRule(Base):
    ...
    rule_category: Mapped[str] = mapped_column(
        Enum("信用卡欺诈", "贷款风险", "转账风险", "登录风险", "账户风险", "设备风险",
             name="rule_category_enum"),
        nullable=False, comment="风险场景分类",
    )
    event_type: Mapped[str] = mapped_column(
        Enum("信用卡", "贷款", "转账", "登录", "通用", name="rule_event_type_enum"),
    ...
    # 软删除: NULL=未删, 有值=删除时间. 所有 SELECT 加 WHERE deleted_at IS NULL
...
class RiskEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        Enum("信用卡", "贷款", "转账", "登录", name="event_type_enum"),
...
class RiskFeature(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        Enum("用户", "账户", "设备", name="feature_entity_type_enum"),
...
class RiskCase(Base):
    ...
    # --- 业务回溯字段 (重做检查时使用) ---
    source_id: Mapped[Optional[str]] = mapped_column(String(50), comment="原始业务ID(交易/贷款/登录ID)")
    event_type: Mapped[Optional[str]] = mapped_column(
        Enum("信用卡", "贷款", "转账", "登录", name="case_event_type_enum"),
        comment="触发案件的事件类型",
    )
    create_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime, server_default=func.now(), default=datetime.now, comment="创建时间",
...
class RiskBlacklist(Base):
    ...
    blacklist_type: Mapped[str] = mapped_column(
        Enum("user", "card", "device", "ip", "id_card", name="blacklist_type_enum"),
    ...
    # 软删除: NULL=未删, 有值=删除时间. 撞黑检查/列表查询都加 WHERE deleted_at IS NULL
    deleted_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="软删除时间(NULL=未删)",
    )
...
class RiskUserProfile(Base):
    ...
    total_txns: Mapped[int] = mapped_column(Integer, default=0, comment="总交易数")
    total_loans: Mapped[int] = mapped_column(Integer, default=0, comment="贷款申请数")
    avg_txn_amount: Mapped[Decimal] = mapped_column(Numeric(15, 2), default=0, comment="平均交易金额")
    device_count: Mapped[int] = mapped_column(Integer, default=0, comment="设备数量")
    login_fail_count: Mapped[int] = mapped_column(Integer, default=0, comment="登录失败次数")
