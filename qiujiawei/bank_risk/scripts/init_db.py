...
# bank_risk/scripts/init_db.py → 上两级 = bank_risk/
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SQL_DIR = os.path.join(BASE_DIR, "sql")

# SQL 脚本执行顺序
SQL_FILES = [
    ("init_business_tables.sql", "创建 8 张业务表"),
    ("init_risk_tables.sql", "创建 9 张风控表"),
    ("init_risk_data.sql", "导入 8 条预置风控规则"),
]
...
async def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
用法:
  python -m bank_risk.scripts.init_db                       # 默认: --reset (删库重建)
  python -m bank_risk.scripts.init_db --keep-data          # 保留: 只补表结构
  python -m bank_risk.scripts.init_db --reset --yes        # 重置 + 自动确认 (CI/脚本用)
  python -m bank_risk.scripts.init_db --db bank_risk_test  # 初始化测试库
        """,
    )
    ...
    if total_errors == 0:
        ...
        if args.reset:
            print("数据库已重置: 17 张表重建 + 8 条规则全部就绪")
    else:
