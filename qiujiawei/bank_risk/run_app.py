...
启动前自动做 6 步自检:
  ...
  3. MySQL 是否在跑 (localhost:3306, 数据库 bank_risk)
  4. 数据库是否已初始化 (risk_rule 表能否查询且有数据)
  5. 8001 端口是否被占
...
# bank_risk/run_app.py → 上两级 = 项目根 (让 bank_risk 包可被 import)
...
def _check_env_file() -> tuple[str, str]:
    """检查 .env 文件. 返回 (级别, 消息)."""
    # config.py 用 env_file=".env" (相对 CWD), 优先查 CWD, 再查 bank_risk 目录
    bank_risk_dir = os.path.dirname(os.path.abspath(__file__))
    if os.path.exists(".env") or os.path.exists(os.path.join(bank_risk_dir, ".env")):
        return "OK", ".env 存在"
    return "WARN", (
        ".env 不存在, 复制 bank_risk/.env.example 为 .env 并填真实值 "
        "(LLM_API_KEY / MYSQL 密码)"
    )
...
def _check_mysql_alive() -> tuple[str, str]:
    """检查 MySQL 是否在 localhost:3306 跑, 数据库 bank_risk 可连."""
    import pymysql
    from bank_risk.app.config import settings as _  # 加载 .env 配置
    password = os.getenv("DB_PASSWORD", "123321")
    user = os.getenv("DB_USER", "root")
    db = os.getenv("DB_NAME", "bank_risk")
    try:
...
def _check_db_initialized() -> tuple[str, str]:
    ...
    try:
        ...
        try:
            ...
            return "WARN", "risk_rule 表存在但无数据, 跑: python -m bank_risk.scripts.init_db --yes"
        finally:
            conn.close()
    except Exception as e:
        err = str(e)
        if "1146" in err or "doesn't exist" in err.lower():
            return "WARN", (
                ...
                "python -m bank_risk.scripts.init_db --yes"
            )
        return "WARN", f"数据库状态未知: {type(e).__name__}: {err[:80]}"


def _check_port_available(port: int = 8001) -> tuple[str, str]:
    """检查 8001 端口是否被占."""
...
def _check_xgb_model() -> tuple[str, str]:
    ...
    try:
        ...
        # 相对路径以 bank_risk/ 目录为基准 (run_app.py 所在目录)
        bank_risk_dir = os.path.dirname(os.path.abspath(__file__))
        if not os.path.isabs(path):
            path = os.path.join(bank_risk_dir, path)
...
if __name__ == "__main__":
    ...
    # 2. 加载日志配置 + 启动 uvicorn
    from bank_risk.app.logging_config import LOGGING_CONFIG  # noqa: E402
    import uvicorn  # noqa: E402
    from bank_risk.scripts.main import app  # noqa: E402
