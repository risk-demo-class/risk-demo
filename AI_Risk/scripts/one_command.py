"""
银行工商风控系统 - 一条龙命令 (P4-L4 2026-08-08)
...
【工作流 6 步】
  ...
  6. 造 50 条今日业务数据 (gen_risk_data_with_dates.py --days 1 --per-day 50)
...
def step_0_gen_business_sql() -> None:
    """步骤 0: 生成业务数据 SQL (50 用户, 确保足够普通用户 + 关联数据)."""
    _run_subprocess(
        "0/7 生成业务数据 SQL (gen_business_data.py --count 50)",
        [sys.executable, os.path.join(SCRIPTS, "gen_business_data.py"), "--count", "50"],
    )
...
def step_1_reset_db() -> None:
    ...
    _run_subprocess(
        "1/7 重置数据库 (init_db.py --reset --yes)",
...
def step_2_risk_users() -> None:
    ...
    _run_subprocess(
        "2/7 造 30 个 RISK 用户 (gen_risky_users.py --count 30)",
...
def step_3_train_dataset() -> None:
    ...
    _run_subprocess(
        "3/7 造 1500 条强标注训练数据 (gen_train_dataset.py --reset)",
...
def step_4_train_xgboost() -> None:
    ...
    _run_subprocess(
        "4/7 训练 XGBoost 模型 (train_xgb_model.py)",
...
def step_5_backfill_ml_score() -> None:
    ...
    _run_subprocess(
        "5/7 回填 ml_score 字段 (backfill_ml_score.py)",
...
def step_6_business_today() -> None:
    ...
    _run_subprocess(
        "6/7 造 50 条今日业务数据 (gen_risk_data_with_dates.py --days 1 --per-day 50)",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="银行工商风控系统 - 一条龙命令 (6 步: 重置 → RISK → 训练数据 → 训练 → 回填 → 启动)",
  ...
  python scripts/one_command.py --only-start   # 只跑步骤 6 (造今日业务数据)
    ...
    try:
        if args.only_start:
            ...
            # 步骤 0: 生成业务数据 SQL (确保足够普通用户)
            if not args.skip_init:
                step_0_gen_business_sql()
                step_1_reset_db()
                step_2_risk_users()
            else:
                print("[跳过] 0+1+2: 业务 SQL + DB + RISK 用户已就绪")
    ...
    print("  浏览器访问 http://localhost:8080            # 仪表盘/案件/评估/规则/黑名单/风险检查")
    print()
    print("造数据脚本:")
    print("  python scripts/gen_risk_data.py --balance-pos --target-pos-ratio 0.30 --count 200")
    print("  python scripts/gen_risk_data_with_dates.py --days 1 --per-day 100 ")
    print("=" * 70)
