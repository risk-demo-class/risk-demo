"""
银行工商风控系统 - 模拟风控评估数据生成 (异步)
从现有业务数据中随机选取交易/贷款申请/用户，调用风控引擎生成评估记录
用于填充仪表盘和案件管理页面的初始数据
银行工商风控系统 - 适配 banking 表 structure
...
async def _pick_txn(db, balance_pos: bool) -> tuple | None:
    """挑一条交易(txn); balance_pos=True 时优先从 RISK 高风险用户里选.

    返回: (txn_id, user_id, from_card) 元组或 None
    """
    if balance_pos:
        # 80% 概率从高风险用户里挑 (RISK001~RISK005), 触发更多高风险评估
        if random.random() < 0.8:
            r = await db.execute(text("""
                SELECT txn_id, user_id, from_card
                FROM transaction
            ...
            row = r.first()
            if row:
                return (row.txn_id, row.user_id, row.from_card)
    r = await db.execute(text("""
        SELECT txn_id, user_id, from_card
        FROM transaction
        ORDER BY RAND() LIMIT 1
    """))
    row = r.first()
    return (row.txn_id, row.user_id, row.from_card) if row else None


async def _pick_loan(db, balance_pos: bool) -> tuple | None:
    """挑一条贷款申请; balance_pos=True 时优先从 RISK 高风险用户里选."""
    if balance_pos:
        if random.random() < 0.8:
            r = await db.execute(text("""
            ...
            row = r.first()
            if row:
                return (row.application_id, row.user_id)
    r = await db.execute(text("""
        SELECT application_id, user_id
        FROM loan_application
        ORDER BY RAND() LIMIT 1
    """))
    row = r.first()
    return (row.application_id, row.user_id) if row else None
            ...
            for i in range(count):
                ...
                if use_loan:
                    ...
                    if not picked:
                        ...
                        continue
                    txn_id, user_id, from_card = picked
                    event_type = random.choice(["转账/支付", "转账/支付", "转账/支付", "转账/支付", "转账/支付"])
                    request = RiskCheckRequest(
                        event_type=event_type,
                        source_id=txn_id,
                        user_id=user_id,
                        txn_id=txn_id,
                        receive_id=from_card,
                    )
                    tag = "交易"
                try:
