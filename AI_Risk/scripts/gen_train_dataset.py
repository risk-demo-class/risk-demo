"""
银行工商风控系统 - 训练数据集生成 (P4-L4 2026-08-08)
...
【为什么需要这个脚本】
  ...
  现在严格控制:
    - 30 RISK 用户 → 25 条高风险事件 (贷款申请) → 99% 拒绝/审核
    - 30 普通用户 → 25 条正常事件 (转账/支付) → 99% 通过
    - 正例比例 ≈ 45-50%
...
async def _pick_loan_for_user(db, user_id: str) -> tuple | None:
    """挑该用户的一条售后记录 (application_id, user_id)."""
    r = await db.execute(text("""
        SELECT p.application_id, p.user_id
        FROM loan_application p
        WHERE p.user_id = :uid
...
async def _pick_txn_for_user(db, user_id: str) -> tuple | None:
    """挑该用户的一条订单 (txn_id, user_id, from_card)."""
    r = await db.execute(text("""
...
async def gen_train_dataset(
    ...
    async with AsyncSessionLocal() as db:
        ...
        failed = 0
        plan = []
        # RISK 用户 → 贷款申请 (99% 触发 R004 高退款率等)
        for uid in risk_users:
            for _ in range(per_user):
                plan.append((uid, "贷款申请"))
        # 普通用户 → 转账/支付 (99% 不触规则)
        for uid in normal_users:
            for _ in range(per_user):
                plan.append((uid, "转账/支付"))
        ...
        for idx, (uid, event_type) in enumerate(plan, 1):
            try:
                if event_type == "贷款申请":
                    picked = await _pick_loan_for_user(db, uid)
                    if not picked:
                        # 贷款不够, fallback 到转账/支付
                        picked = await _pick_txn_for_user(db, uid)
                        if not picked:
                            failed += 1
                            continue
                        txn_id, user_id, from_card = picked
                        request = RiskCheckRequest(
                            event_type="转账/支付", source_id=txn_id, user_id=user_id,
                            order_id=txn_id, receive_id=from_card,
                        )
                    else:
                        loan_id, user_id = picked
                        request = RiskCheckRequest(
                            event_type="贷款申请", source_id=loan_id, user_id=user_id,
                        )
                else:  # 转账/支付
                    picked = await _pick_txn_for_user(db, uid)
                    if not picked:
