"""
物流行业风控系统 - 训练数据集生成 (P4-L4 2026-08-08)
...
【为什么需要这个脚本】
  之前 gen_risk_data.py 随机挑运单 → 跑规则 → 写到 risk_assessment:
    - 数据随机, 标签跟用户身份弱相关
    - 正例比例 < 3% (RISK 用户少), 模型假收敛
    - ml_score 字段是 XGBoost 推理结果, 但当时模型未训 → 0.00141 这种垃圾值
  现在严格控制:
    - 30 RISK 用户 → 25 条高风险事件 (取消运单) → 拒绝/审核
    - 30 普通用户 → 25 条正常事件 (寄件) → 通过
    - 正例比例 ≈ 45-50%
...
async def _pick_postsale_for_user(db, user_id: str) -> tuple | None:
    """挑该用户的一张已取消运单 (shipment_id, user_id, dest_address_id)."""
    r = await db.execute(text("""
        SELECT shipment_id, sender_id, dest_address_id
        FROM shipment
        WHERE sender_id = :uid AND status = '已取消'
...
async def _pick_order_for_user(db, user_id: str) -> tuple | None:
    """挑该用户的一张运单 (shipment_id, user_id, dest_address_id)."""
    r = await db.execute(text("""
        SELECT shipment_id, sender_id, dest_address_id
        FROM shipment
        WHERE sender_id = :uid
...
async def gen_train_dataset(
    ...
    async with AsyncSessionLocal() as db:
        ...
        failed = 0
        plan = []
        # RISK 用户 → 取消运单 (触发取消率/投诉规则)
        for uid in risk_users:
            for _ in range(per_user):
                plan.append((uid, "shipment_cancel"))
        # 普通用户 → 普通寄件 (低概率触发规则)
        for uid in normal_users:
            for _ in range(per_user):
                plan.append((uid, "shipment_create"))
        ...
        for idx, (uid, event_type) in enumerate(plan, 1):
            try:
                if event_type == "shipment_cancel":
                    picked = await _pick_postsale_for_user(db, uid)
                    if not picked:
                        # 已取消运单不够, fallback 到寄件
                        ...
                        request = RiskCheckRequest(
                            event_type="shipment_create", source_id=shipment_id, user_id=user_id,
                            order_id=shipment_id, receive_id=receive_id,
                        )
                    else:
                        shipment_id, user_id, receive_id = picked
                        request = RiskCheckRequest(
                            event_type="shipment_cancel", source_id=shipment_id, user_id=user_id,
                            order_id=shipment_id, receive_id=receive_id,
                        )
                else:  # shipment_create
                    picked = await _pick_order_for_user(db, uid)
                    if not picked:
                        failed += 1
                        continue
