...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则:
      - "开户" / "登录" 事件: source_id 就是 txn_id
      - "转账" 事件: source_id 是 txn_id, txn_id 要从 request 传
      - "贷款申请" 事件: source_id 是 application_id
    """
...
async def _update_user_profile(
    ...
    profile.complaint_count = int(features.get("user_dispute_count", 0))
