...
def _build_context(request: RiskCheckRequest) -> _RiskCheckContext:
    ...
    业务规则:
      - shipment_create / shipment_cancel / shipment_receive 事件:
        source_id 就是运单号 shipment_id
      - id_verification 事件: 无运单关联
    """
    order_id = request.order_id
    if not order_id and request.event_type in ("shipment_create", "shipment_cancel", "shipment_receive"):
...
async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    """步骤 1c: 从运单里补全 dest_address_id (地址 ID).

    退出条件 (任一满足就不补): 已有 receive_id / 没有 shipment_id (id_verification 场景).
    """
    if ctx.receive_id or not ctx.order_id:
        return
    row = (await db.execute(
        select(Shipment.dest_address_id).where(Shipment.shipment_id == ctx.order_id)
    )).first()
    if row:
        ctx.receive_id = row.dest_address_id
