...
async def _biz_user_postsales(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        select(
            ShipmentComplaint.complaint_id,
            ShipmentComplaint.shipment_id,
            ShipmentComplaint.complaint_type,
            ShipmentComplaint.content,
            ShipmentComplaint.create_time,
        )
        .where(ShipmentComplaint.user_id == user_id)
        .order_by(ShipmentComplaint.create_time.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
async def _biz_high_value_orders(db: AsyncSession, user_id: str, order_id: str, limit: int) -> list[Any]:
    ...
    stmt = (
        ...
        .where(Shipment.declared_value >= 1000)
        .order_by(Shipment.declared_value.desc())
        .limit(limit)
    )
    return list((await db.execute(stmt)).all())
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (用户ID, 如 'USR_10000')、event_type (事件类型, 可选值: shipment_create/shipment_cancel/shipment_receive/id_verification)、source_id (关联业务ID, 如运单号)。"
