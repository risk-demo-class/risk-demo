...
class RiskCheckRequest(BaseModel):
    """风险检查请求"""
    event_type: Literal["shipment_create", "shipment_cancel", "shipment_receive", "id_verification"]
    source_id: str = Field(description="关联业务ID (shipment_id / user_id 等)")
    user_id: str
...
class RuleCreate(BaseModel):
    ...
    rule_category: Literal["dangerous_goods", "cod_fraud", "cross_border", "identity_risk", "address_risk",
                           "危险品瞒报", "代收货款欺诈", "跨境申报", "实名核验", "地址风险"]
    event_type: Literal["shipment_create", "shipment_cancel", "shipment_receive", "id_verification", "通用"] = "通用"
    rule_condition: dict
...
class RuleUpdate(BaseModel):
    ...
    rule_category: Optional[Literal["dangerous_goods", "cod_fraud", "cross_border", "identity_risk", "address_risk",
                                   "危险品瞒报", "代收货款欺诈", "跨境申报", "实名核验", "地址风险"]] = None
    event_type: Optional[Literal["shipment_create", "shipment_cancel", "shipment_receive", "id_verification", "通用"]] = None
...
class BlacklistCreate(BaseModel):
    ...
    blacklist_type: Literal["用户", "地址", "手机号",
                            "sender", "receiver", "phone", "address", "id_card"]
    blacklist_value: str
...
if __name__ == "__main__":
    ...
    req = RiskCheckRequest(
        event_type="shipment_create", source_id="ship_demo_001", user_id="U0001",
        order_id="ship_demo_001", receive_id="addr_001",
    ...
    hits = [
        RuleHitInfo(
            rule_id="L_R002", rule_name="单笔极端高额申报运单",
            rule_category="危险品瞒报", risk_level="极高",
            risk_score=95, action="拒绝",
            description="单笔申报价值≥12000元, 一票否决",
        ),
        RuleHitInfo(
            rule_id="L_R006", rule_name="用户7天内高频寄件",
            rule_category="跨境申报", risk_level="高",
    ...
    detail = AssessmentDetailResponse(
        assessment_id="ast_demo_xxx", event_id="evt_demo_xxx",
        user_id="U0001", event_type="shipment_create", event_source_id="ship_demo_001",
