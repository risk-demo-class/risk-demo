-- ============================================
-- 银行工商风控系统 - 风控表 DDL 初始化脚本
-- 在 BankRisk 数据库中创建 7 张新增风控表
-- ============================================
...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('登录场景','转账/支付场景','贷款场景','信用卡场景','设备/网络环境','账户行为') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('注册/开户','登录','转账/支付','贷款申请','绑卡/换卡','大额/可疑交易上报','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('注册/开户','登录','转账/支付','贷款申请','绑卡/换卡','大额/可疑交易上报') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_feature` (
    ...
    `entity_type` ENUM('用户','交易','卡') NOT NULL COMMENT '实体类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('注册/开户','登录','转账/支付','贷款申请','绑卡/换卡','大额/可疑交易上报') DEFAULT NULL COMMENT '触发案件的事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_blacklist` (
    ...
    `blacklist_type` ENUM('身份证','卡号','IP','设备指纹','手机号','企业信用代码') NOT NULL COMMENT '黑名单类型',
