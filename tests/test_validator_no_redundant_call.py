...
class TestDeadCodeRemoved:
    ...
    def test_ensure_shipment_belongs_to_sender_kept(self):
        """防越权的核心函数 (物流版), 保留."""
        assert hasattr(validator_module, "ensure_shipment_belongs_to_sender"), (
            "物流版核心校验: ensure_shipment_belongs_to_sender 必须保留"
        )
...
class TestProcessEventCallsValidatorOnce:
    ...
    async def test_process_event_calls_validate_risk_check_request_once(self, monkeypatch):
        ...
        request = RiskCheckRequest(
            event_type="寄件下单", source_id="SHIP001", user_id="U001", order_id="SHIP001"
        )
...
class TestValidatorPublicApi:
    ...
    def test_validator_demo_block_mentions_4_steps(self):
        """__main__ demo 块"校验链"包含 4 步 (与 validator 逻辑一致)."""
        src = inspect.getsource(validator_module)
        # [2] 校验链 有 4 步, 用标记验证
        assert "校验链 validate_risk_check_request" in src, "应有 validate_risk_check_request 校验链说明"
        # ④ 个步骤标记均出现 (①②③④ 序号的中文数字断言)
        for idx in ("①", "②", "③", "④"):
            assert idx in src, f"校验链说明缺少步骤 {idx}"
