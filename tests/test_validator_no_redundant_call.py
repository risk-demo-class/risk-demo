...
class TestDeadCodeRemoved:
    ...
    def test_ensure_order_belongs_to_dealer_kept(self):
        """防越权的核心函数 (制造业经销商归属), 保留."""
        assert hasattr(validator_module, "ensure_order_belongs_to_dealer")
        assert hasattr(validator_module, "ensure_warranty_belongs_to_dealer")
        assert hasattr(validator_module, "ensure_report_belongs_to_dealer")
...
class TestProcessEventCallsValidatorOnce:
    ...
    async def test_process_event_calls_validate_risk_check_request_once(self, monkeypatch):
        ...
        request = RiskCheckRequest(
            event_type="经销商订货", source_id="ORD001", user_id="U001", order_id="ORD001"
        )
...
class TestValidatorPublicApi:
    ...
    def test_validator_demo_block_mentions_manufacturing_checks(self):
        """__main__ demo 块打印制造业 3 类归属校验."""
        src = inspect.getsource(validator_module)
        assert "3 个归属校验" in src or "3 维度校验" in src, (
            "demo 块应说明制造业 3 类归属校验 (订货单/保修工单/串货举报)"
        )
        assert "ensure_postsale_exists" not in src, "旧电商死代码不应残留"
