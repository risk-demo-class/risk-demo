...
class TestGenRiskDataForcedPosLogic:
    ...
    def test_force_pos_pickers_defined(self):
        """3 个高风险 picker 必须存在: 投诉/物流投诉/大额运单 (物流版)."""
        script = SCRIPT_PATH.read_text(encoding="utf-8")
        # 3 个 _pick_forced_* 函数 (物流版命名, 保留 _pick_forced_logistics_complaint)
        assert "_pick_forced_complaint_record" in script, "必须有 _pick_forced_complaint_record (RISK 投诉记录)"
        assert "_pick_forced_logistics_complaint" in script, "必须有 _pick_forced_logistics_complaint (物流投诉)"
        assert "_pick_forced_order_for_high_amount" in script, "必须有 _pick_forced_order_for_high_amount (大额运单)"
    ...
    def test_force_pos_retry_logic(self):
        ...
        # pickers 列表 3 个高风险 (物流版: 投诉 + 物流投诉)
        assert "_pick_forced_complaint_record" in script and "_pick_forced_logistics_complaint" in script, (
...
class TestGenRiskDataWithDatesLogic:
    ...
    def test_balance_pos_uses_risky_user_prefix(self):
        """--balance-pos 模式必须用 RISK 前缀查 RISK 高风险用户 (物流版)."""
        script = SCRIPT_PATH.read_text(encoding="utf-8")
        ...
        # _pick_order_for_balance / _pick_complaint_for_balance 函数必须存在 (物流版)
        assert "_pick_order_for_balance" in script, "必须有 _pick_order_for_balance 函数 (→ 运单)"
        assert "_pick_complaint_for_balance" in script, "必须有 _pick_complaint_for_balance 函数 (→ 投诉申诉)"
