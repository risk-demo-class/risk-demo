...
class TestGenRiskDataWithDatesCLI:
    ...
    def test_script_uses_manufacturing_pickers(self):
        """必须复用 mfg_pickers (5 种制造业事件), 不残留旧电商 picker."""
        script = SCRIPT_PATH.read_text(encoding="utf-8")
        assert "pick_random_event" in script, "必须用 pick_random_event 选制造业事件"
        assert "postsale" not in script.lower(), "旧电商 postsale picker 不应残留"
        assert "物流投诉" not in script, "旧电商物流投诉 picker 不应残留"


class TestGenRiskDataPickerLogic:
    """制造业事件 picker (mfg_pickers.py) 覆盖 5 种事件."""

    def test_five_event_pickers_defined(self):
        """5 种事件 picker 必须存在: 订货 / 采购 / 保修 / 维修 / 串货举报."""
        script = PICKER_PATH.read_text(encoding="utf-8")
        for fn in ("pick_order", "pick_warranty", "pick_report", "pick_random_event"):
            assert fn in script, f"picker 模块必须有 {fn}"
        for et in ("经销商订货", "采购订单", "保修申请", "售后维修", "串货举报"):
            assert et in script, f"picker 必须覆盖事件 {et}"

    def test_report_source_id_is_raw(self):
        """串货举报 source_id 必须是 str(report_id), 不带前缀 (validator int 强转)."""
        script = PICKER_PATH.read_text(encoding="utf-8")
        assert "str(row.report_id)" in script
        assert "COMP_" not in script, "不应残留 COMP_ 前缀拼法"
...
class TestGenRiskDataWithDatesLogic:
    ...
    def test_balance_pos_uses_risky_user_prefix(self):
        """--balance-pos 模式必须用 RISK 前缀查 RISK 高风险经销商 (mfg_pickers.py)."""
        script = PICKER_PATH.read_text(encoding="utf-8")
        assert 'RISKY_USER_PREFIX = "RISK"' in script or "RISKY_USER_PREFIX='RISK'" in script
        assert "dealer_id LIKE :prefix" in script or "user_id LIKE :prefix" in script
        # 带日期脚本必须转发 balance_pos
        gen_script = SCRIPT_PATH.read_text(encoding="utf-8")
        assert "balance_pos" in gen_script
