...
def _make_request(**overrides) -> RiskCheckRequest:
    """构造一个物流版 RiskCheckRequest, 默认值"寄件下单/U001/SHIP001/101"."""
    base = dict(
        event_type="寄件下单",
        source_id="SHIP001",
        user_id="U001",
...
class TestCheckAllBlacklists:
    ...
    async def test_phone_blacklist_hit(self):
        """用户/地址不撞, 手机号撞 → 返回 '手机号' (查 Address.contact_phone)"""
        request = _make_request()
        ...
        db.execute = AsyncMock(return_value=MagicMock(
            first=MagicMock(return_value=_row(contact_phone="13800000000",
    ...
    async def test_no_receive_id_skips_addr_and_phone(self):
        """receive_id=None → 跳过地址+手机号 (运单号仍会查, 但 return_value 明确, 不会报错)"""
        ...
        db = _mock_db()
        db.execute = AsyncMock(return_value=MagicMock(
            first=MagicMock(return_value=_row(waybill_no="SF100"))))

        with patch.object(event_module, "check_blacklist", side_effect=fake_check):
            result = await event_module._check_all_blacklists(db=db, request=request)
        assert result is None
        # 只查了用户 + (可能)运单号, 绝对没查地址/手机号
        assert "地址" not in call_log
        assert "手机号" not in call_log
        assert "用户" in call_log

    @pytest.mark.asyncio
    async def test_receive_id_no_phone_skips_phone(self):
        """address 查不到 contact_phone 或 row 为 None → 跳过手机号"""
        request = _make_request()
        call_log = []
        ...
        db = _mock_db()
        # first 两次: Address 查询 None → 跳过手机号; Shipment 查询运单
        def _side_effect(*a, **kw):
            m = MagicMock()
            m.first.side_effect = [None, _row(waybill_no="SF1000")]
            return m
...
class TestBlacklistReject:
    ...
    def test_blocked_by_3_types(self):
        """3 种基础类型 + 运单号都能正确填 blocked_by."""
        for btype in ("用户", "地址", "手机号", "运单号"):
