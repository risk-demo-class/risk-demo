    ...
    async def test_returns_assessment_items_with_event_type(self):
        ...
        class _FakeDB:
            ...
            async def execute(self, stmt):
                ...
                return _FakeResult([(fake_a1, "寄件下单"), (fake_a2, "跨境申报")])  # paged
        ...
        # event_type 从 JOIN 拿
        assert result.items[0].event_type == "寄件下单"
        assert result.items[1].event_type == "跨境申报"
        # ml_score 浮点
...
class TestGetAssessmentDetail:
    ...
    async def test_detail_with_triggered_rules(self):
        ...
        fake_a = SimpleNamespace(
            ...
            rule_count=2, rule_results=(
                '[{"rule_id":"L002","rule_name":"30天30单高频寄件","rule_category":"寄件欺诈",'
        ...
        fake_ev = SimpleNamespace(
            event_id="evt1", event_type="寄件下单", event_source_id="SHIP999123",
        ...
        assert result.event_source_id == "SHIP999123"
        assert result.event_data == '{"amount": 15000}'
        assert len(result.triggered_rules) == 1
        assert result.triggered_rules[0].rule_id == "L002"
    ...
    async def test_detail_corrupt_rule_results_fallback_empty(self):
        ...
        fake_ev = SimpleNamespace(
            event_id="evt1", event_type="寄件下单", event_source_id="SHIP1", event_data=None,
        )
