...
class TestRuleHitResultToDict:
    ...
    def test_to_dict_all_fields(self):
        """所有字段都正确转换"""
        rule = _mock_rule("L1", {}, score=80, level="高", action="人工审核",
                          name="大额夜间寄件", category="寄件欺诈")
        hit = RuleHitResult(rule)
        d = hit.to_dict()
        assert d["rule_id"] == "L1"
        assert d["rule_name"] == "大额夜间寄件"
        assert d["rule_category"] == "寄件欺诈"
        assert d["risk_level"] == "高"
        assert d["risk_score"] == 80
        assert d["action"] == "人工审核"
        assert d["description"] == "描述-L1"
