...
class TestGenRiskyUsersLogic:
    ...
    def test_five_risk_modes_defined(self):
        """5 种制造业风险模式必须定义 (委托 gen_business_data.build_risk_sql)."""
        source = SCRIPT_PATH.read_text(encoding="utf-8")
        for mode in ["高保修率", "30天高频订货", "套保", "跨区串货", "新经销商大单"]:
            assert mode in source, f"gen_risky_users 应覆盖模式: {mode}"
        # 委托实现: build_risk_sql 必须存在
        sys.path.insert(0, str(ROOT))
        from scripts.gen_business_data import build_risk_sql
        assert callable(build_risk_sql)
        # 5 个用户 → 覆盖 5 种模式 (5 条批量 INSERT)
        stmts = build_risk_sql(5)
        assert stmts, "build_risk_sql(5) 应生成 RISK 数据 SQL"
