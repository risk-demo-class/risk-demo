...
class TestScoreCalculation:
    ...
    def test_single_hit(self):
        hit = RuleHitResult(_MockRule("L1", "测试", "寄件欺诈", "高", 70, "人工审核"))
        assert calculate_final_score([hit]) == 70

    def test_multiple_hits_bonus(self):
        """多规则命中: max + BONUS × extra_count"""
        hits = [
            RuleHitResult(_MockRule("L1", "测试1", "寄件欺诈", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("L2", "测试2", "实名风险", "中", 40, "标记")),
            RuleHitResult(_MockRule("L3", "测试3", "综合风险", "低", 20, "通过")),
        ]
...
class TestVetoHardVetoAfterFusion:
    ...
    def test_veto_with_low_ml_still_reject(self, monkeypatch):
        ...
        # 模拟一票否决规则: L002 (极高, 95 分)
        veto_hit = RuleHitResult(_MockRule(
            "L002", "30天30单高频寄件", "寄件欺诈", "极高", 95, "拒绝",
        ))
    ...
    def test_veto_with_high_ml_also_reject(self, monkeypatch):
        ...
        veto_hit = RuleHitResult(_MockRule(
            "L005", "单票 5 万 + 保价缺失", "危险品瞒报", "极高", 90, "拒绝",
        ))
...
class TestCaseDeduplication:
    """
    同一 (source_id, event_type) 不应建多个未结案.
    场景: 同一运单的"寄件下单"事件被前端重试 3 次, 每次都触发"人工审核"
    """
    @pytest.mark.asyncio
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="寄件下单", source_id="SHIP_001", user_id="1001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="寄件下单", source_id="SHIP_002", user_id="1001",
        )
        ...
        assert db.added[0].source_id == "SHIP_002"
        assert db.added[0].event_type == "寄件下单"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="寄件下单", source_id="SHIP_003", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("L1", "测试", "寄件欺诈", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        """decision="拒绝" → case_status="已拒绝" 自动关案."""
        from app.engine import decision
        from app.schemas import RiskCheckRequest
        from types import SimpleNamespace

        request = RiskCheckRequest(
            event_type="寄件下单", source_id="SHIP_REJ_001", user_id="U001",
        )
        ctx = SimpleNamespace(request=request, user_id="U001")
        # L002 风格: risk_level="极高" risk_score=95
        hit = RuleHitResult(_MockRule("L002", "30天30单高频寄件",
                                      "寄件欺诈", "极高", 95, "拒绝"))
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="寄件下单", source_id="SHIP_REV_001", user_id="U002",
        )
        ctx = SimpleNamespace(request=request, user_id="U002")
        hit = RuleHitResult(_MockRule("L003", "大额夜间寄件",
                                      "寄件欺诈", "高", 65, "人工审核"))
...
class TestCaseCategoryAggregation:
    """
    案件分类按命中规则 category 聚合, 取命中数最多的.
    """

    @pytest.mark.asyncio
    async def test_majority_category_wins(self):
        """3 条到付拒收 + 1 条寄件欺诈 → 分类 = 到付拒收"""
        from app.engine import decision
        from app.schemas import RiskCheckRequest
        from types import SimpleNamespace

        request = RiskCheckRequest(
            event_type="到付签收", source_id="SHIP_X", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        rules = [
            RuleHitResult(_MockRule("L022", "拒收率 80%+", "到付拒收", "极高", 90, "拒绝")),
            RuleHitResult(_MockRule("L023", "7 天 5 次拒收", "到付拒收", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("L024", "新地址拒收", "到付拒收", "高", 65, "人工审核")),
            RuleHitResult(_MockRule("L001", "单笔超 3 万", "寄件欺诈", "高", 70, "人工审核")),
        ]
        ...
        # decision="拒绝" → 1 个 case + 1 条 AUTO_REJECT_CASE 审计
        case = next((x for x in db.added if x.__class__.__name__ == "RiskCase"), None)
        log = next((x for x in db.added if x.__class__.__name__ == "RiskActionLog"), None)
        assert case is not None and log is not None
        assert case.case_category == "到付拒收", (
            f"应取命中数最多的 '到付拒收', 实际 {case.case_category}"
        )

    @pytest.mark.asyncio
    async def test_tie_first_one_wins(self):
        """2+2 平局 → 取 first occurrence"""
        from app.engine import decision
        from app.schemas import RiskCheckRequest
        from types import SimpleNamespace

        request = RiskCheckRequest(
            event_type="寄件下单", source_id="SHIP_Y", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        rules = [
            RuleHitResult(_MockRule("L1", "a", "寄件欺诈", "高", 70, "人工审核")),
            RuleHitResult(_MockRule("L2", "b", "地址风险", "高", 70, "人工审核")),
        ]
        ...
        # 平局时, Counter.most_common 返回先出现的
        assert db.added[0].case_category in ("寄件欺诈", "地址风险")
