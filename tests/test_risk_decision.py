...
class TestCaseDeduplication:
    ...
    场景: 同一订单的"医保结算"事件被前端重试 3 次, 每次都触发"人工审核"
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="医保结算", source_id="order_001", user_id="1001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="医保结算", source_id="order_002", user_id="1001",
        )
        ...
        assert db.added[0].event_type == "医保结算"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="医保结算", source_id="order_003", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "医保欺诈", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        ...
        request = RiskCheckRequest(
            event_type="医保结算", source_id="order_reject_001", user_id="U001",
        )
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="医保结算", source_id="order_review_001", user_id="U002",
        )
...
class TestCaseCategoryAggregation:
    ...
    async def test_majority_category_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="处方审核", source_id="order_x", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        ...
        rules = [
            ...
            RuleHitResult(_MockRule("R001", "5000+", "医保欺诈", "高", 70, "人工审核")),
        ]
    ...
    async def test_tie_first_one_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="医保结算", source_id="order_y", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # 1 条订单欺诈 + 1 条地址风险 → 平局
        rules = [
            RuleHitResult(_MockRule("R1", "a", "医保欺诈", "高", 70, "人工审核")),
        ...
        assert db.added[0].case_category in ("医保欺诈", "地址风险")
