...
class TestScoreCalculation:
    ...
    def test_single_hit(self):
        hit = RuleHitResult(_MockRule("R1", "测试", "欺诈风险", "高", 70, "人工审核"))
...
class TestCaseDeduplication:
    ...
    场景: 同一贷款申请的"贷款申请"事件被前端重试 3 次, 每次都触发"人工审核"
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="贷款申请", source_id="LN_001", user_id="C00001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="C00001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="贷款申请", source_id="LN_002", user_id="C00001",
        )
        ...
        assert db.added[0].source_id == "LN_002"
        assert db.added[0].event_type == "贷款申请"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="贷款申请", source_id="LN_003", user_id="C00001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "欺诈风险", "高", 70, "人工审核"))
    ...
    async def test_auto_reject_creates_closed_case(self):
        ...
        request = RiskCheckRequest(
            event_type="贷款申请", source_id="LN_REJ_001", user_id="C00002",
        )
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="贷款申请", source_id="LN_REV_001", user_id="C00003",
        )
...
class TestCaseCategoryAggregation:
    ...
    async def test_majority_category_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="放款", source_id="LN_X", user_id="C00001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
    ...
    async def test_tie_first_one_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="贷款申请", source_id="LN_Y", user_id="C00001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # 1 条订单欺诈 + 1 条地址风险 → 平局
        rules = [
            RuleHitResult(_MockRule("R1", "a", "欺诈风险", "高", 70, "人工审核")),
        ...
        assert db.added[0].case_category in ("欺诈风险", "地址风险")
