...
class TestBaseLayout:
    ...
    def test_sidebar_html_structure(self, base_html):
        """侧边栏必含 6 个 nav-link (数据看台/待审案件/历史案件/风控规则/风控检查/黑名单), 无 AI 助手独立页"""
        nav_block = re.search(
        ...
        # 6 个 nav-link
        assert nav_html.count('class="nav-link') == 6, \
            f"侧边栏应有 6 个 nav-link, 实际 {nav_html.count('class=\"nav-link')}"
        # 顺序: 数据看台 → 待审案件 → 历史案件 → 风控规则 → 风控检查 → 黑名单
        nav_items = re.findall(
            r'<a class="nav-link[^"]*" href="([^"]+)">.*?</a>', nav_html, re.DOTALL,
        )
        assert nav_items == ["/", "/cases", "/assessments", "/rules", "/risk-check", "/blacklist"], (
            f"侧边栏顺序/链接错误: {nav_items}"
        )
        # 文案
        for label in ["数据看台", "待审案件", "历史案件", "风控规则", "风控检查", "黑名单"]:
            assert label in nav_html, f"侧边栏应含文案: {label}"
        assert "/chat" not in nav_html, "AI 助手已集成到数据看台, 侧边栏不应再有 /chat"
