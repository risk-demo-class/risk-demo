...
class TestTrainDemoModelStructure:
    ...
    def test_has_7_pattern_generators(self):
        ...
        for name in ["_gen_high_warranty_rate", "_gen_high_freq_order", "_gen_warranty_fraud",
...
class TestSyntheticDatasetGeneration:
    ...
    def test_gen_synthetic_dataset_feature_order_matches_columns(self):
        """合成数据特征顺序必须跟 FEATURE_COLUMNS 一致 (测高保修率模式)."""
        import sys
        for p in [str(ROOT)]:
            if p not in sys.path:
                sys.path.insert(0, p)
        from scripts.train_demo_model import _gen_high_warranty_rate
        import random
        rng = random.Random(42)
        X = np.array([_gen_high_warranty_rate(rng) for _ in range(10)])
        # 第 1 列 (idx=0) 应该是 user_total_orders (uniform 5-30)
        assert (X[:, 0] >= 5).all() and (X[:, 0] <= 15).all(), (
            f"user_total_orders 应该是 5-15, 实际范围: {X[:, 0].min()}-{X[:, 0].max()}"
        )
        # 第 9 列 (idx=8) 应该是 user_warranty_rate (uniform 0.8-1.0)
        assert (X[:, 8] >= 0.8).all() and (X[:, 8] <= 1.0).all(), (
            f"user_warranty_rate 应该是 0.8-1.0, 实际: {X[:, 8].min()}-{X[:, 8].max()}"
        )

    def test_normal_user_low_risk_features(self):
        """正常经销商模式: 保修率/被举报数/大额订单 都应偏低."""
        import sys
        for p in [str(ROOT)]:
            if p not in sys.path:
                sys.path.insert(0, p)
        from scripts.train_demo_model import _gen_normal_dealer
        import random
        rng = random.Random(42)
        X = np.array([_gen_normal_dealer(rng) for _ in range(100)])
        # 负例: user_warranty_rate (col 8) <= 0.15
        assert (X[:, 8] <= 0.15).all(), f"正常经销商 warranty_rate 应 <= 0.15, 实际最大: {X[:, 8].max()}"
        # 负例: user_report_count (col 11) == 0
        assert (X[:, 11] == 0).all(), f"正常经销商 report_count 应 = 0, 实际最大: {X[:, 11].max()}"
        # 负例: user_max_order_amount (col 5) <= 200000
        assert (X[:, 5] <= 200000).all(), f"正常经销商 max_order 应 <= 200000, 实际最大: {X[:, 5].max()}"
...
class TestTrainedModelQuality:
    ...
    def test_trained_model_loads(self, tmp_path):
        ...
        # 推理 1 个样本 (正常用户)
        from scripts.train_demo_model import _gen_normal_dealer, _gen_high_warranty_rate
        import random
        rng = random.Random(123)
        X_normal = _gen_normal_dealer(rng).reshape(1, -1)
        X_risk = _gen_high_warranty_rate(rng).reshape(1, -1)
