...
CREATE TABLE IF NOT EXISTS `risk_rule` (
    ...
    `rule_category` ENUM('交易欺诈','账户风险','信贷风险','设备风险','网络风险') NOT NULL COMMENT '风险场景分类',
    `event_type` ENUM('信用卡交易','转账','贷款申请','登录','通用') NOT NULL DEFAULT '通用' COMMENT '适用事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_event` (
    ...
    `event_type` ENUM('信用卡交易','转账','贷款申请','登录') NOT NULL COMMENT '事件类型',
...
CREATE TABLE IF NOT EXISTS `risk_case` (
    ...
    `event_type` ENUM('信用卡交易','转账','贷款申请','登录') DEFAULT NULL COMMENT '触发案件的事件类型',
