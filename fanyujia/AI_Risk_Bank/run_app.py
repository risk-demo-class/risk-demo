...
启动前自动做 6 步自检:
  ...
  3. MySQL 是否在跑（读取 DB_HOST / DB_PORT，默认 localhost:9999）
...
def _database_connection_settings() -> tuple[str, int, str, str, str]:
    """读取本机启动所需的数据库连接配置。"""
...
def _check_mysql_alive() -> tuple[str, str]:
    """检查配置的 MySQL 是否可连接。"""
    import pymysql
    host, port, user, password, db = _database_connection_settings()
    try:
        ...
        return "WARN", (
            ...
            f"         业务功能会失败。检查 MySQL 是否启动、密码和端口是否正确。"
        )
