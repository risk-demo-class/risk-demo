...
async def _biz_user_transactions(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    stmt = select(BankTransaction).where(BankTransaction.user_id == user_id).order_by(BankTransaction.txn_time.desc()).limit(limit)
    return list((await db.execute(stmt)).scalars().all())


async def _biz_user_loans(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    stmt = select(LoanApplication).where(LoanApplication.user_id == user_id).order_by(LoanApplication.apply_at.desc()).limit(limit)
    return list((await db.execute(stmt)).scalars().all())


async def _biz_user_logins(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    stmt = select(LoginLog).where(LoginLog.user_id == user_id).order_by(LoginLog.login_at.desc()).limit(limit)
    return list((await db.execute(stmt)).scalars().all())


async def _biz_user_devices(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    stmt = select(DeviceFingerprint).where(DeviceFingerprint.user_id == user_id).order_by(DeviceFingerprint.last_seen.desc()).limit(limit)
    return list((await db.execute(stmt)).scalars().all())


async def _biz_recent_transactions(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    stmt = select(BankTransaction).order_by(BankTransaction.txn_time.desc()).limit(limit)
    return list((await db.execute(stmt)).scalars().all())


async def _biz_high_value_transactions(db: AsyncSession, user_id: str, source_id: str, limit: int) -> list[Any]:
    stmt = select(BankTransaction).where(BankTransaction.amount >= 10000).order_by(BankTransaction.amount.desc()).limit(limit)
    return list((await db.execute(stmt)).scalars().all())
...
@tool(
    description=(
        "对指定用户和事件执行实时风险检查。"
        "参数: user_id (用户ID, 如 'BU00001')、event_type (事件类型, 可选值: 信用卡交易/转账/贷款申请/登录)、source_id (关联业务ID, 如交易流水号)。"
...
@tool(
    description=(
        ...
        "返回: 用户的风险评分、交易统计、异常登录率、设备与 IP 数等画像数据; 没画像时实时计算 14 个用户特征。"
    )
)
...
@tool(
    description=(
        "管理风控黑名单。"
        "参数: action (操作类型: add/remove/check/list)、blacklist_type (黑名单类型: 用户/设备指纹/IP/银行卡号/身份证号)、value (黑名单值, 添加/检查/移除时需要)、reason (加黑原因, 添加时需要)。"
        "返回: 操作结果 (文本或 JSON)。"
    )
)
