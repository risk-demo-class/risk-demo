...
def check_veto(hits: list[RuleHitResult]) -> bool:      #todo 待优化  没有参考机器模型的得分 应该是只要出现了一次极高风险 都否决
...
async def _enrich_receive_id(db: AsyncSession, ctx: _RiskCheckContext) -> None:
    ...
    """
    # 银行业务的设备 ID 已由 process_event 的补全步骤写入 request.receive_id。
    # 保留这个钩子和七步调用顺序，避免变更决策流水线结构。
    return None
...
async def _update_user_profile(
    ...
    profile.total_orders = int(features.get("user_txn_count_24h", 0))
    profile.total_refunds = int(features.get("user_loan_apply_count_30d", 0))
    profile.refund_rate = float(features.get("user_debt_ratio", 0))
    profile.avg_order_amount = features.get("user_avg_txn_amount_30d", 0)
    profile.address_count = int(features.get("user_distinct_devices_30d", 0))
    profile.complaint_count = int(features.get("user_login_fail_count_1h", 0))
