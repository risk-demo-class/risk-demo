...
class TestCheckMysqlAlive:
    ...
    def test_mysql_alive_uses_configured_database_port(self, monkeypatch):
        """启动前检查必须连接 DB_PORT 指定的本机 MySQL 端口。"""
        monkeypatch.setenv("DB_HOST", "localhost")
        monkeypatch.setenv("DB_PORT", "9999")
        monkeypatch.setenv("DB_NAME", "bank_risk")
        mock_conn = MagicMock()
        with patch("pymysql.connect", return_value=mock_conn) as connect:
            level, _ = _run._check_mysql_alive()

        assert level == "OK"
        assert connect.call_args.kwargs["host"] == "localhost"
        assert connect.call_args.kwargs["port"] == 9999
        assert connect.call_args.kwargs["database"] == "bank_risk"

    def test_mysql_alive_reads_connection_values_from_dotenv(self, tmp_path, monkeypatch):
        """本地启动时，预检查应使用 .env 中的连接参数而非忽略密码。"""
        ...
        assert connect.call_args.kwargs["port"] == 23306
        assert connect.call_args.kwargs["user"] == "tester"
        assert connect.call_args.kwargs["password"] == "from_dotenv"
        assert connect.call_args.kwargs["database"] == "dotenv_bank"
