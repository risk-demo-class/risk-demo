...
class TestBlacklistCreate:
    ...
    def test_accepts_all_bank_blacklist_types(self):
        for blacklist_type in ("用户", "设备指纹", "IP", "银行卡号", "身份证号"):
            request = BlacklistCreate(
                blacklist_type=blacklist_type,
                blacklist_value="bank_demo_value",
            )
            assert request.blacklist_type == blacklist_type

    def test_rejects_ecommerce_address_blacklist_type(self):
        with pytest.raises(ValidationError):
            BlacklistCreate(blacklist_type="地址", blacklist_value="legacy_value")
