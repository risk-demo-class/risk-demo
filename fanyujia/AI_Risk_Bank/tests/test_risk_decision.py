...
class TestCaseDeduplication:
    ...
    async def test_existing_open_case_skips_new_one(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="TXN001", user_id="1001",
        )
        ctx = SimpleNamespace(
            request=request,
            user_id="1001",
    ...
    async def test_no_existing_case_creates_new(self):
        ...
        request = RiskCheckRequest(
            event_type="转账", source_id="TXN002", user_id="1001",
        )
        ...
        assert db.added[0].source_id == "TXN002"
    ...
    async def test_pass_decision_skips_creation(self):
        ...
        request = RiskCheckRequest(
            event_type="信用卡交易", source_id="TXN003", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        hit = RuleHitResult(_MockRule("R1", "测试", "订单欺诈", "高", 70, "人工审核"))
    ...
    async def test_manual_review_keeps_pending(self):
        ...
        request = RiskCheckRequest(
            event_type="贷款申请", source_id="LOAN_REVIEW_001", user_id="U002",
        )
...
class TestCaseCategoryAggregation:
    ...
    async def test_majority_category_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="信用卡交易", source_id="TXN_X", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
    ...
    async def test_tie_first_one_wins(self):
        ...
        request = RiskCheckRequest(
            event_type="登录", source_id="LOGIN_Y", user_id="1001",
        )
        ctx = SimpleNamespace(request=request, user_id="1001")
        # 1 条订单欺诈 + 1 条地址风险 → 平局
        rules = [
